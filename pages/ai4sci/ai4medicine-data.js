/**
 * ai4medicine-data.js — 由 pipeline/build.py 于 2026-05-20 17:14:02 自动生成。
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>双潜变量设计</strong>：潜向量 \\(\\mathbf{z} = [\\mathbf{z}_T, \\mathbf{z}_G]\\) 分别编码树拓扑结构和图连接方式",
        "<strong>两阶段生成</strong>：先由 Tree Decoder 生成连接树骨架 \\(\\hat{\\mathcal{T}}\\)，再由 Graph Decoder 将子结构组装为完整分子图 \\(\\hat{G}\\)",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>形式文法推导规则</strong>：通过推导状态 \\(\\mathbf{X}_n\\)（\\(n=0,1,...,4\\)）追踪剩余价键数，自动约束后续原子的键合方式",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>稀疏注意力</strong>：基于几何邻近性的稀疏 attention，避免全原子 \\(O(N^2)\\) 复杂度，实现约 10× 加速",
        ...
        "<strong>Classifier-free guidance</strong>：训练时随机丢弃条件信号，推理时通过引导强度 \\(s\\) 增强条件遵从",
