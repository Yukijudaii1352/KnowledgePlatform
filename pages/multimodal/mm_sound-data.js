/**
 * mm_sound-data.js — 由 pipeline/build.py 于 2026-05-20 17:14:15 自动生成。
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>转导格（Transduction Lattice）</strong>：在 \\(T \\times (U+1)\\) 的格点上定义所有可能的对齐路径，水平移动对应消耗输入（输出 ∅），垂直移动对应发射标签",
        "<strong>联合网络设计</strong>：\\(h(k, t, u) = \\exp(f_t^k + g_u^k)\\)，将编码器和解码器的输出通过加法耦合后 softmax 归一化",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Next-Chunk Prediction</strong>：以 \\(C=4\\) 个 token 为一个 chunk 进行自回归预测，chunk 内部并行、chunk 间因果，兼顾建模质量与推理速度",
