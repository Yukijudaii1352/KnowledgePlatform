#!/usr/bin/env bash
# 并行精读 content/llm/llm_base.yaml 中的论文；若有失败算法，仅重试失败项。
...
YAML_FILE="${SCRIPT_DIR}/llm/llm_base.yaml"
LOG_DIR="${SCRIPT_DIR}/_batch_logs"
RUN_TS="$(date +%Y%m%d_%H%M%S)"
MAIN_LOG="${LOG_DIR}/llm_base_parallel_retry_${RUN_TS}.log"
...
默认行为:
  只处理 ${YAML_FILE}
  首轮用 --jobs 并行精读 YAML 中的算法/论文；
  若有失败项，则生成临时 YAML，仅重试失败算法，最多重试 ${MAX_RETRIES} 轮。
...
示例:
  bash ${0##*/}
  bash ${0##*/} --jobs 6 --retry-jobs 2 --max-retries 3
  JOBS=8 RETRY_JOBS=2 bash ${0##*/}
EOF
}
...
topic_log_dir_for_yaml() {
    ...
    domain_1="$(basename "$(dirname "${yaml_file}")")"
    domain_2="$(basename "${yaml_file}" .yaml)"
    printf '%s/%s/%s/logs\n' "${SCRIPT_DIR}" "${domain_1}" "${domain_2}"
}
...
run_yaml_once() {
    ...
    local jobs="$2"
    local label="$3"
    local topic_log_dir marker single_log rc summary_file fail_list_file safe_label
    ...
    single_log="${LOG_DIR}/llm_base_${safe_label}_${RUN_TS}_$$_${RANDOM}.log"
...
log "OUTPUT_DIR=${SCRIPT_DIR}/llm/llm_base"
...
if (( current_failed_count > 0 )); then
    ...
    for ((attempt=1; attempt<=MAX_RETRIES; attempt++)); do
        ...
        retry_root="$(mktemp -d)"
        retry_parent_name="$(basename "$(dirname "${YAML_FILE}")")"
