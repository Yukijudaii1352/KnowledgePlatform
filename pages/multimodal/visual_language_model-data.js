/**
 * visual_language_model-data.js — 由 pipeline/build.py 于 2026-06-15 17:41:34 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "detail": "<h3>1. 整体架构</h3>\n<p><img alt=\"Qwen-VL 训练流程\" src=\"https://ar5iv.labs.arxiv.org/html/2308.12966/assets/x3.png\" />\n<em>图：Qwen-VL 三阶段训练流程。Stage 1 冻结 LLM 进行视觉-语言对齐预训练；Stage 2 解冻全部参数进行多任务高分辨率训练；Stage 3 冻结 ViT 进行指令微调。</em></p>\n<p>Qwen-VL 由三个核心组件构成：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>组件</th>\n<th>具体实现</th>\n<th>参数量</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>视觉编码器</td>\n<td>OpenCLIP ViT-bigG/14</td>\n<td>1.9B</td>\n</tr>\n<tr>\n<td>VL 适配器</td>\n<td>位置感知单层交叉注意力</td>\n<td>0.08B</td>\n</tr>\n<tr>\n<td>语言模型</td>\n<td>Qwen-7B (Transformer Decoder)</td>\n<td>7.7B</td>\n</tr>\n<tr>\n<td><strong>总计</strong></td>\n<td></td>\n<td><strong>9.6B</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>输入处理流程</strong>：图像经 ViT-bigG 编码后产生一系列视觉特征，通过 VL 适配器压缩为固定 256 个 token，再与文本 token 拼接后送入 Qwen-7B 进行自回归生成。</p>\n<h3>2. 位置感知 VL 适配器（核心创新）</h3>\n<p>适配器的核心是一个<strong>单层交叉注意力</strong>模块，其关键设计：</p>\n<pre><code>输入：\n  - 视觉特征 V ∈ R^(L×D)    # L = (H/14)×(W/14), D = ViT hidden dim\n  - 可学习查询 Q ∈ R^(256×D)  # 固定 256 个查询向量\n\n位置编码：\n  - 2D 绝对位置编码 PE_2d，编码每个 patch 的 (row, col) 位置\n  - 分别加到 Cross-Attention 的 Query 和 Key 上\n\n计算过程：\n  Q' = Q + PE_2d(query_positions)\n  K' = Linear_K(V) + PE_2d(patch_positions)  \n  V' = Linear_V(V)\n  Output = softmax(Q'·K'^T / √d) · V'    # → R^(256×D)\n</code></pre>\n<p><strong>为什么需要 2D 位置编码？</strong>\n- 标准 ViT 的 1D 位置编码在经过交叉注意力压缩后会丢失空间位置信息\n- 2D PE 显式编码了每个 patch 的行列坐标，使压缩后的 256 个视觉 token 仍保留空间位置感知\n- 这对于细粒度定位任务（如 grounding、OCR）至关重要</p>\n<p><strong>与其他适配器的对比</strong>：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>压缩方式</th>\n<th>位置信息</th>\n<th>输出 token 数</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>LLaVA</td>\n<td>线性投影（无压缩）</td>\n<td>隐式保留</td>\n<td>576 (24×24)</td>\n</tr>\n<tr>\n<td>BLIP-2</td>\n<td>Q-Former (多层交叉注意力)</td>\n<td>无显式位置编码</td>\n<td>32</td>\n</tr>\n<tr>\n<td><strong>Qwen-VL</strong></td>\n<td><strong>单层交叉注意力 + 2D PE</strong></td>\n<td><strong>显式 2D 位置编码</strong></td>\n<td><strong>256</strong></td>\n</tr>\n<tr>\n<td>Flamingo</td>\n<td>Perceiver Resampler</td>\n<td>无显式位置编码</td>\n<td>64</td>\n</tr>\n</tbody>\n</table></div>\n<h3>3. 统一的边界框与引用表示</h3>\n<p>Qwen-VL 通过特殊标记实现了<strong>定位（grounding）与引用（referring）的统一文本表示</strong>：</p>\n<pre><code>特殊标记：\n  &lt;img&gt; ... &lt;/img&gt;    # 包裹图像 token（256个视觉 token）\n  &lt;box&gt; ... &lt;/box&gt;    # 包裹边界框坐标\n  &lt;ref&gt; ... &lt;/ref&gt;    # 包裹引用的文本描述\n\n边界框格式：\n  &lt;box&gt;(X_topleft, Y_topleft),(X_bottomright, Y_bottomright)&lt;/box&gt;\n  坐标归一化到 [0, 1000) 整数范围\n\n示例 - Grounded Captioning：\n  输入: &lt;img&gt;image_tokens&lt;/img&gt; Describe the image with grounding.\n  输出: &lt;ref&gt;A dog&lt;/ref&gt;&lt;box&gt;(120,50),(450,380)&lt;/box&gt; is playing in &lt;ref&gt;the park&lt;/ref&gt;&lt;box&gt;(0,100),(999,600)&lt;/box&gt;.\n\n示例 - Referring Expression Comprehension：\n  输入: &lt;img&gt;image_tokens&lt;/img&gt; &lt;ref&gt;the red car on the left&lt;/ref&gt;\n  输出: &lt;box&gt;(23,150),(340,420)&lt;/box&gt;\n</code></pre>\n<p>这种设计的优势：\n- <strong>双向能力</strong>：既能根据图像生成带定位的描述（grounded captioning），也能根据文本描述定位目标（referring）\n- <strong>与 LLM 原生兼容</strong>：边界框以文本 token 形式参与自回归生成，无需额外的检测头\n- <strong>灵活组合</strong>：可以在一次生成中混合使用多个 box 和 ref 标记</p>\n<h3>4. 三阶段训练策略</h3>\n<h4>Stage 1：视觉-语言对齐预训练</h4>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>配置</th>\n<th>详情</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>数据</td>\n<td>1.4B 图文对（从 5B 清洗而来），来源包括 LAION-en/zh、DataComp、CC12M、COCO 等</td>\n</tr>\n<tr>\n<td>分辨率</td>\n<td>224 × 224</td>\n</tr>\n<tr>\n<td>冻结策略</td>\n<td>冻结 LLM，训练 ViT + VL 适配器</td>\n</tr>\n<tr>\n<td>优化器</td>\n<td>AdamW, lr=2e-4, cosine schedule</td>\n</tr>\n<tr>\n<td>批大小</td>\n<td>30,720</td>\n</tr>\n<tr>\n<td>训练步数</td>\n<td>50,000</td>\n</tr>\n<tr>\n<td>目标</td>\n<td>下一个 token 预测（仅计算文本部分的 loss）</td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>数据清洗流程</strong>：从公开数据集出发，经过以下步骤：\n1. 基于 CLIP ViT-bigG 的图文相似度过滤\n2. 去重（图像级和文本级）\n3. 过滤低质量文本（过短、过多特殊字符等）\n4. 最终从 5B 筛选到 1.4B 高质量图文对</p>\n<h4>Stage 2：多任务高分辨率训练</h4>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>配置</th>\n<th>详情</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>分辨率</td>\n<td>448 × 448（从 224 提升）</td>\n</tr>\n<tr>\n<td>冻结策略</td>\n<td><strong>全部解冻</strong>（ViT + 适配器 + LLM）</td>\n</tr>\n<tr>\n<td>任务数</td>\n<td>7 类任务</td>\n</tr>\n</tbody>\n</table></div>\n<p>7 类训练任务：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>任务</th>\n<th>数据来源</th>\n<th>说明</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Image Captioning</td>\n<td>COCO, CC3M, CC12M, SBU 等</td>\n<td>图像描述生成</td>\n</tr>\n<tr>\n<td>Visual QA</td>\n<td>VQAv2, OKVQA, GQA, OCRVQA 等</td>\n<td>视觉问答</td>\n</tr>\n<tr>\n<td>Grounding</td>\n<td>GRIT-20M, RefCOCO/+/g</td>\n<td>文本→边界框</td>\n</tr>\n<tr>\n<td>Ref. Grounding</td>\n<td>RefCOCO/+/g</td>\n<td>表达式→边界框</td>\n</tr>\n<tr>\n<td>Grounded Captioning</td>\n<td>Flickr30K Entities, RefCOCO/+/g</td>\n<td>带定位的描述生成</td>\n</tr>\n<tr>\n<td>OCR</td>\n<td>SynthDoG, SROIE, DocVQA 等</td>\n<td>文字识别与理解</td>\n</tr>\n<tr>\n<td>Pure Text</td>\n<td>纯文本语料</td>\n<td>防止语言能力退化</td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>关键设计</strong>：\n- 分辨率从 224 提升到 448，使 ViT 输出从 16×16=256 patches 增加到 32×32=1024 patches，但经过适配器仍压缩为 256 token\n- 混入纯文本数据防止 LLM 语言能力退化（\"catastrophic forgetting\"）\n- 多任务联合训练使模型同时具备理解、定位、OCR 等多种能力</p>\n<h4>Stage 3：指令微调（SFT）</h4>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>配置</th>\n<th>详情</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>数据</td>\n<td>350K 多任务指令数据</td>\n</tr>\n<tr>\n<td>冻结策略</td>\n<td>冻结 ViT，训练 LLM + 适配器</td>\n</tr>\n<tr>\n<td>目标</td>\n<td>对话式指令跟随</td>\n</tr>\n</tbody>\n</table></div>\n<p>SFT 后的模型称为 <strong>Qwen-VL-Chat</strong>，具备多轮对话、指令跟随能力。</p>\n<h3>5. 关键实验结果</h3>\n<p><img alt=\"Qwen-VL 性能对比\" src=\"https://ar5iv.labs.arxiv.org/html/2308.12966/assets/x1.png\" />\n<em>图：Qwen-VL 在多个 benchmark 上与同期模型的性能对比。</em></p>\n<h4>Zero-shot Image Captioning</h4>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>模型</th>\n<th>Flickr30K CIDEr</th>\n<th>NoCaps CIDEr</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Flamingo-9B</td>\n<td>61.5</td>\n<td>-</td>\n</tr>\n<tr>\n<td>Flamingo-80B</td>\n<td>67.2</td>\n<td>-</td>\n</tr>\n<tr>\n<td><strong>Qwen-VL (9.6B)</strong></td>\n<td><strong>85.8</strong></td>\n<td><strong>121.4</strong></td>\n</tr>\n</tbody>\n</table></div>\n<h4>General VQA</h4>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>模型</th>\n<th>VQAv2</th>\n<th>GQA</th>\n<th>OKVQA</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>InstructBLIP-13B</td>\n<td>-</td>\n<td>49.5</td>\n<td>-</td>\n</tr>\n<tr>\n<td>Shikra-13B</td>\n<td>77.4</td>\n<td>-</td>\n<td>47.2</td>\n</tr>\n<tr>\n<td><strong>Qwen-VL</strong></td>\n<td><strong>79.5</strong></td>\n<td><strong>59.3</strong></td>\n<td><strong>58.6</strong></td>\n</tr>\n</tbody>\n</table></div>\n<h4>Referring Expression Comprehension (Grounding)</h4>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>模型</th>\n<th>RefCOCO val</th>\n<th>RefCOCO+ val</th>\n<th>RefCOCOg val</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Shikra-13B</td>\n<td>87.83</td>\n<td>81.60</td>\n<td>82.27</td>\n</tr>\n<tr>\n<td><strong>Qwen-VL</strong></td>\n<td><strong>89.36</strong></td>\n<td><strong>83.12</strong></td>\n<td><strong>85.58</strong></td>\n</tr>\n</tbody>\n</table></div>\n<h4>Text-oriented VQA</h4>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>模型</th>\n<th>DocVQA</th>\n<th>ChartQA</th>\n<th>TextVQA</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>mPLUG-DocOwl</td>\n<td>62.2</td>\n<td>57.4</td>\n<td>52.6</td>\n</tr>\n<tr>\n<td><strong>Qwen-VL</strong></td>\n<td><strong>65.1</strong></td>\n<td><strong>65.7</strong></td>\n<td><strong>63.8</strong></td>\n</tr>\n</tbody>\n</table></div>\n<h3>6. 消融实验关键发现</h3>\n<p><strong>分辨率的影响</strong>：从 224→448，在 DocVQA 上提升 +18.1%，在 ChartQA 上提升 +12.2%，对文档/OCR 类任务影响巨大。</p>\n<p><strong>2D 位置编码的影响</strong>：移除 2D PE 后，RefCOCO grounding 准确率下降约 2-3 个百分点，验证了位置编码对定位任务的重要性。</p>\n<p><strong>多任务训练的影响</strong>：联合训练 grounding 任务不仅提升定位能力，还对 VQA 等理解任务有正向迁移效果。</p>\n<h3>7. 设计思考与局限</h3>\n<p><strong>为什么选择 256 个查询而非更少（如 BLIP-2 的 32）？</strong>\n- 256 token 在效率和信息保留之间取得平衡\n- 对于 448×448 输入（1024 patches），256 token 提供约 4:1 的压缩比\n- 过少的 token 会丢失细粒度视觉信息，影响 OCR 和定位任务</p>\n<p><strong>局限性</strong>：\n- 仅支持单一固定分辨率（448×448），对极端长宽比图像不友好\n- 边界框精度受限于 [0, 1000) 的离散化（约 0.1% 的量化误差）\n- 9.6B 参数量在部署时仍有一定开销</p>"
    },
    {
      "id": "cogvlm",
      "num": 13,
