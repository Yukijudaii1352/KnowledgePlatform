...
#### 📝 一句话总结
OpenPose 的核心目标是：提出PAFs实现自底向上多人实时检测。
...
#### 📝 一句话总结
PoseNet 的核心目标是：基于MobileNet实现浏览器端实时推理。
...
#### 📝 一句话总结
E-3DPSM 的核心目标是：利用事件相机高时间分辨率解决动态模糊。
...
#### 📝 一句话总结
DRPose 的核心目标是：基于扩散模型的姿态细化框架提升精度。
...
#### 📝 一句话总结
SMPL 的核心目标是：基于顶点的线性模型解耦形状与姿态参数。
...
#### 📝 一句话总结
SMPLify 的核心目标是：首个从单幅图像自动拟合SMPL到2D关键点。
...
#### 📝 一句话总结
HMR 的核心目标是：端到端回归参数引入对抗训练解决数据缺失。
...
#### 📝 一句话总结
GraphCMR 的核心目标是：利用图卷积直接在网格顶点上回归。
...
#### 📝 一句话总结
SPIN 的核心目标是：将模型拟合嵌入训练循环结合回归与优化。
...
#### 📝 一句话总结
VIBE 的核心目标是：引入时间序列判别器确保视频动作平滑性。
...
#### 📝 一句话总结
SMPL-X 的核心目标是：统一建模身体手部面部提供更丰富表达力。
...
#### 📝 一句话总结
PEAR 的核心目标是：像素级监督实现100+FPS的SMPL-X回归。
...
#### 📝 一句话总结
OmniFit 的核心目标是：尺度无关稠密地标预测统一处理穿衣人体。
...
#### 📝 一句话总结
Motion VAE 的核心目标是：递归VAE学习运动潜在空间支持多模态生成。
...
#### 📝 一句话总结
Action2Motion 的核心目标是：基于动作类别引入Lie代数表示生成3D运动。
...
#### 📝 一句话总结
ACTOR 的核心目标是：Transformer与VAE结合处理变长序列生成。
...
#### 📝 一句话总结
MotionDiffuse 的核心目标是：首个基于扩散模型的文本驱动动作生成。
...
#### 📝 一句话总结
T2M-GPT 的核心目标是：结合VQ-VAE与GPT离散化Token自回归生成。
...
#### 📝 一句话总结
CMDM 的核心目标是：因果扩散Transformer支持流式无尽长度生成。
...
#### 📝 一句话总结
MACE-Dance 的核心目标是：混合专家系统BiMamba架构降低长序列开销。
...
#### 📝 一句话总结
TokenDance 的核心目标是：Token-to-Token双向Mamba架构提升效率。
...
#### 📝 一句话总结
FaceNet 的核心目标是：提出三元组损失直接学习欧氏空间映射。
...
#### 📝 一句话总结
MTCNN 的核心目标是：三级级联CNN同时完成检测回归与对齐。
...
#### 📝 一句话总结
3DDFA 的核心目标是：拟合3DMM模型解决大角度人脸对齐问题。
...
#### 📝 一句话总结
RetinaFace 的核心目标是：单阶段多任务检测器引入像素级面部监督。
...
#### 📝 一句话总结
Avatar Forcing 的核心目标是：建模用户与化身间因果交互实现实时反应。
