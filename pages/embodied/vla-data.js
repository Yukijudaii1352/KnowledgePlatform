/**
 * vla-data.js — 由 pipeline/build.py 于 2026-06-16 17:00:13 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "summary": "Code as Policies 将代码生成大模型转化为机器人策略生成器，让 LLM 直接写可执行 Python 控制逻辑，解决了传统语言规划只能调用固定技能、难以表达反馈循环和几何计算的问题。它通过语言模型程序（LMP）和递归函数生成，把感知 API、控制 API、Python 控制流和第三方库组合成可在机器人上运行的策略。",
      "keyPoints": [
        ...
        "代码表达反馈策略：用 <code>if/else</code>、<code>for/while</code>、变量、函数调用和会话状态表示闭环行为、上下文引用和多步任务。",
      ...
      "detail": "<p><img alt=\"Code as Policies 框架图\" src=\"https://ar5iv.labs.arxiv.org/html/2209.07753/assets/x1.png\" />\n<em>图：CaP 用 few-shot prompt 将自然语言命令翻译为策略代码，代码调用感知 API、控制 API，并递归生成未定义函数。</em></p>\n<p>元信息中的 <code>paper_url</code> 指向 Google Research 官方博客而非论文正文；这里同时依据该博客、项目页和论文 <code>Code as Policies: Language Model Programs for Embodied Control</code>（arXiv:2209.07753）完成精读。论文的核心问题是：SayCan 等方法把 LLM 用作高层规划器，通常输出一串已有技能，但机器人仍必须预先训练或手写这些技能；一旦指令需要“向左一点”“直到看到苹果再停”“画一个更小的三角形”这类数值、反馈或几何细节，固定技能表就很难覆盖。</p>\n<p>CaP 的做法是把策略写成程序。给定自然语言指令 <span class=\"kb-math kb-math-inline\">\\ell</span>、感知 API 集合 <span class=\"kb-math kb-math-inline\">\\mathcal{P}</span>、控制 API 集合 <span class=\"kb-math kb-math-inline\">\\mathcal{A}</span> 和 few-shot 示例 <span class=\"kb-math kb-math-inline\">E</span>，LLM 生成一段代码：</p>\n<div class=\"kb-math kb-math-display\">c \\sim p_\\theta(c \\mid \\mathrm{prompt}(\\ell, \\mathcal{P}, \\mathcal{A}, E))</div>\n<p>这段代码不是离线说明，而是在受限 Python 环境中执行的机器人策略。代码可以读取对象检测结果、计算目标坐标、根据条件分支选择动作，并在循环中反复观察环境。对机器人而言，<code>get_pos(\"red block\")</code>、<code>detect_object(\"orange\")</code>、<code>put_first_on_second(obj, target)</code> 这类 API 是 grounding；对 LLM 而言，有意义的函数名和示例让它能把语言短语映射到可执行调用。</p>\n<pre><code class=\"language-python\"># Code as Policies 层级生成伪代码\ndef generate_policy(instruction, scope, examples):\n    prompt = build_prompt(hints=scope.available_apis, examples=examples)\n    code = llm_complete(prompt + f&quot;# {instruction}\\n&quot;)\n\n    while True:\n        ast_tree = parse_python(code)\n        missing = find_called_functions_not_in_scope(ast_tree, scope)\n        if not missing:\n            break\n\n        for fn_name, signature in missing:\n            fn_prompt = build_function_prompt(fn_name, signature, scope, examples)\n            fn_code = llm_complete(fn_prompt)\n            assert passes_static_safety_checks(fn_code)\n            scope.add_function(fn_name, fn_code)\n            code = fn_code + &quot;\\n\\n&quot; + code\n\n    assert passes_static_safety_checks(code)\n    exec(code, scope.globals, scope.locals)\n</code></pre>\n<p>层级生成是论文最关键的工程机制。主 LMP 可以先写出“粗略但结构清晰”的策略，例如 <code>stack_objs_in_order(obj_names)</code>；如果这个函数还不存在，系统解析 AST 找到未定义调用，再让另一个 LMP 生成函数体。这个过程以深度优先方式重复，直到所有调用都能在当前 scope 中解析。相比一次性让模型写完整长程序，递归函数生成把复杂任务拆成更短、更局部的代码生成问题，也让后续任务能复用已生成函数。</p>\n<p>安全执行不是完全放任 <code>exec</code>。论文实现会在执行前检查生成代码，禁止 import、<code>__</code> 开头的特殊变量、<code>exec</code> 和 <code>eval</code> 等高风险构造，然后把感知/控制 API 放入 <code>globals</code>，把新变量和函数放入 <code>locals</code>。这不是完整的物理安全方案，但体现了 CaP 的定位：它负责在高层组合感知结果与控制原语，真正的碰撞检查、力控限制和动作安全仍应由底层机器人控制栈承担。</p>\n<p>CaP 与传统 LLM 规划的主要差别在于“动作参数从代码中算出来”。例如“把最左边的块向右移动 5cm”不需要预训练一个专门技能；策略代码可以先对所有块的位置排序，再把目标位置加上 <span class=\"kb-math kb-math-inline\">[0.05, 0]</span>。如果任务涉及形状绘制，代码可以用 NumPy 插值路径点；如果涉及空间包含关系，代码可以用 Shapely 处理几何对象。换言之，LLM 的世界知识负责把语言翻译成程序结构，而确定性的 Python 运算负责做精确数值推理。</p>\n<div class=\"key-point\">💡 关键：CaP 的泛化发生在“解释语言、操作结构化感知、参数化控制 API”这一层；它不是端到端学习低层动力学，也不消除对可靠感知和控制原语的依赖。</div>\n<p>局限也直接来自这个边界。若感知 API 无法描述某个属性，代码就无法稳健引用它；若控制 API 没有某类动作，CaP 也不能凭空执行。生成代码还可能出现语法、类型、逻辑和安全问题，真实机器人部署需要沙箱、仿真验证、动作约束和人工监督。尽管如此，CaP 证明了一个重要方向：对具备代码能力的 LLM 来说，程序本身可以成为比自然语言计划更强的机器人策略表示。</p>",
      "quiz": {
        "q": "Code as Policies 相比只输出自然语言技能序列的 LLM 规划器，最核心的优势是什么？",
        "options": [
          "完全不需要底层机器人控制 API",
    ...
    },
    {
      ...
      "summary": "RoboAgent 提出用语义增强扩充小规模真实机器人数据，并用语言条件化的 Multi-Task Action Chunking Transformer（MT-ACT）训练单一操控策略，解决多任务机器人学习中真实演示昂贵、场景泛化差的问题。它用 7,500 条遥操作轨迹训练出覆盖 12 种技能、38 个任务的通用 Franka 操控智能体。",
      "keyPoints": [
      ...
      "detail": "<p><img alt=\"RoboAgent 总体框架\" src=\"https://ar5iv.labs.arxiv.org/html/2309.01918/assets/x2.png\" />\n<em>图：RoboAgent 离线阶段用语义增强扩充小数据集，在线阶段用 MT-ACT 根据多视角观测和语言指令预测动作块。</em></p>\n<p>RoboAgent 的动机非常直接：通用机器人需要多任务、多物体、多场景数据，但真实机器人轨迹采集又慢又贵。论文没有选择继续盲目扩大遥操作规模，而是把问题拆成两部分：先用少量真实轨迹覆盖动作和技能，再用生成式图像增强制造视觉语义多样性；随后用一个足够表达多模态动作分布的策略模型从这些数据中学习。</p>\n<p>语义增强的关键约束是“视觉变了，动作监督不能变”。对于一条轨迹中的每帧图像，方法只替换与任务语义相关但不改变机器人运动学标签的像素区域：一种是交互物体增强，把被抓取、推动、开合的物体替换成其他外观；另一种是背景增强，把桌面纹理、厨房背景和非交互干扰物替换掉。由于动作、关节状态和夹爪命令仍来自原始轨迹，增强必须避免破坏机器人本体、末端执行器和物体接触关系。</p>\n<pre><code class=\"language-python\"># RoboAgent 训练流程伪代码\nfor traj in roboset_mt_act:  # 7,500 teleoperated trajectories\n    robot_mask = forward_kinematics_to_robot_mask(traj.joint_states)\n    eef_points = forward_kinematics_to_eef_points(traj.joint_states)\n\n    obj_mask = segment_interaction_object(traj.frames, eef_points, robot_mask)\n    obj_mask = track_mask_over_time(obj_mask, traj.frames)\n    traj_obj_aug = inpaint_with_text_prompt(traj.frames, obj_mask, prompt=&quot;new task object appearance&quot;)\n\n    bg_mask = sample_background_masks(traj.frames, exclude=[robot_mask, obj_mask])\n    traj_bg_aug = inpaint_with_text_prompt(traj.frames, bg_mask, prompt=&quot;new kitchen tabletop scene&quot;)\n\n    augmented_dataset.add(traj)\n    augmented_dataset.add(traj_obj_aug)\n    augmented_dataset.add(traj_bg_aug)\n\nfor batch in augmented_dataset:\n    img_tokens = cnn_encoder(batch.multi_view_rgbd)\n    lang_tokens = language_encoder(batch.instruction)\n    proprio = encode_robot_state(batch.joint_state, batch.eef_state)\n    z = cvae_encoder(batch.future_action_chunk)\n    action_chunk = mt_act_decoder(img_tokens, lang_tokens, proprio, z)\n    loss = action_reconstruction_loss(action_chunk, batch.future_action_chunk) + beta * kl_loss(z)\n    optimizer.step(loss)\n</code></pre>\n<p>MT-ACT 的结构来自 ACT（Action Chunking Transformer），但它面向多任务和语言条件化。输入包括四个相机视角的 RGB-D 观测、Franka 关节/末端状态、自然语言任务描述和 CVAE latent；Transformer decoder 输出未来动作块 <span class=\"kb-math kb-math-inline\">a_{t:t+H}</span>，论文超参数中 <span class=\"kb-math kb-math-inline\">H=20</span>。动作空间是 Franka 的 8 维关节/夹爪控制，数据以 5Hz 采集，因此一个动作块能覆盖一段局部连续子轨迹。</p>\n<p><img alt=\"MT-ACT 架构细节\" src=\"https://ar5iv.labs.arxiv.org/html/2309.01918/assets/x5.png\" />\n<em>图：MT-ACT 用多视角图像 token、语言 embedding、机器人本体状态和 CVAE latent 条件化 Transformer action decoder。</em></p>\n<p>可以把 MT-ACT 的学习目标理解为条件动作块重建：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}\n= \\left\\|\\hat{a}_{t:t+H} - a_{t:t+H}\\right\\|_1\n+ \\beta\\,D_{\\mathrm{KL}}\\left(q_\\phi(z \\mid a_{t:t+H}, o_t, \\ell)\\,\\|\\,p(z)\\right)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">o_t</span> 是多视角观测和机器人状态，<span class=\"kb-math kb-math-inline\">\\ell</span> 是语言指令，<span class=\"kb-math kb-math-inline\">z</span> 捕捉同一任务下不同可行动作模式。CVAE 的作用不是生成图像，而是让策略能表示多峰动作分布：例如“打开抽屉”可能有不同抓握点和拉动轨迹，直接均值回归会产生不自然动作，latent 条件化可以保留多样性。</p>\n<p>推理时，模型并不是只执行完整 20 步动作块后再重新规划，而是持续滑窗预测，并对不同时间步给出的重叠动作做 temporal aggregation。新预测和旧预测之间用时间权重平滑合成，减少行为克隆模型常见的高频抖动。动作块也降低了有效决策频率，让模型更多学习“短子轨迹”而非单步反应。</p>\n<p>与 RT-1、VIL、BC、单任务 ACT 等基线相比，RoboAgent 的差异不在于更大的预训练 VLM，而在于数据和动作表示的配合。语义增强负责把小数据集变成多样视觉分布，MT-ACT 负责用动作块学习稳定控制；如果只有增强而没有动作块，模型仍容易在长动作序列中积累误差；如果只有动作块而没有增强，模型会过拟合有限厨房场景。</p>\n<div class=\"key-point\">💡 关键：RoboAgent 的“泛化”主要来自真实轨迹保持动作物理性、语义增强扩大视觉覆盖、MT-ACT 学习语言条件化动作块这三者的耦合，而不是单独依靠一个更大的生成模型。</div>\n<p>论文限制也很清楚。增强只修改图像语义，不能创造新的接触动力学或完全不同的操作策略；被增强物体的形状、尺寸和可操作性如果与原轨迹差异过大，动作标签会失真。多任务统一策略也会出现负迁移，附录中显示某些窄任务上单活动策略可能优于全任务 universal policy。因此 RoboAgent 更像一种数据高效通用操控配方，而不是一次性解决所有机器人泛化问题。</p>",
      "quiz": {
        "q": "RoboAgent 中语义增强必须遵守的核心约束是什么？",
        "options": [
    ...
    },
    {
      ...
      "summary": "DFM-VLA 将离散流匹配引入 VLA 动作 token 生成，用 token 级概率速度场反复修正整段动作序列，解决自回归和离散扩散 VLA 中早期错误 token 难以回退的问题。它在统一离散视觉-语言-动作表示上结合 embedding-guided velocity、velocity head 与两阶段解码，在 CALVIN、LIBERO 和真实双臂任务上提升动作稳定性。",
      "keyPoints": [
        "提出 irreversible commitment 问题：自回归 VLA 逐 token 固化，离散扩散 VLA 往往只更新 mask/低置信位置，早期错误会传播到整段动作。",
        "统一离散 token 化：文本使用 Emu3 tokenizer，第三视角和腕部图像用 VQ tokenizer，每张图像压缩为 <span class=\"kb-math kb-math-inline\">25 \\times 25</span> tokens，动作使用 FAST 加 BPE，动作词表大小为 1024。",
        "只对动作模态加噪和预测：语言、图像和 proprioception 作为上下文，noised action tokens <span class=\"kb-math kb-math-inline\">x_t</span> 作为待精炼状态。",
      ...
      "detail": "<p><img alt=\"DFM-VLA 总体架构\" src=\"https://chris1220313648.github.io/DFM-VLA/assets/figure/model.png\" />\n<em>图：DFM-VLA 在语言、图像和 noised action tokens 条件下预测 clean action tokens，并通过交叉熵或 velocity head 学习速度场。</em></p>\n<p>离散 VLA 的吸引力在于它能把机器人动作接入大语言/视觉语言模型的 token 训练范式，但解码方式会带来新的控制风险。自回归模型按左到右生成动作 token，前面的错 token 一旦输出，后续 token 只能在错误上下文上继续生成；离散扩散模型虽然并行，但很多实现依赖 mask 或置信度释放，已经确定的位置也很难被重新审视。机器人控制要求整段动作轨迹协调一致，所以这种 irreversible commitment 会表现为轨迹抖动、长时任务失败和低数据场景下的错误放大。</p>\n<p>DFM-VLA 的核心改写是：动作序列不是一步从噪声变成答案，而是沿着离散概率路径逐步流向真实动作分布。对离散变量 <span class=\"kb-math kb-math-inline\">x=(x^1,\\dots,x^D)</span>，DFM 定义从源分布到目标数据分布的时间路径 <span class=\"kb-math kb-math-inline\">p_t(x)</span>。一个常见混合路径可以写成：</p>\n<div class=\"kb-math kb-math-display\">p_t(x^i \\mid x_1^i)\n= (1-\\kappa_t)\\,p(x^i) + \\kappa_t\\,\\delta_{x_1^i}(x^i),\n\\quad \\kappa_0=0,\\ \\kappa_1=1</div>\n<p>这里 <span class=\"kb-math kb-math-inline\">x_1</span> 是 clean action token sequence，<span class=\"kb-math kb-math-inline\">x_t</span> 是中间噪声状态。模型要学习的不是单个 next token，而是从 <span class=\"kb-math kb-math-inline\">x_t</span> 到 <span class=\"kb-math kb-math-inline\">x_1</span> 的迁移方向。CTMC 视角下，每个 token 的下一状态由速度场 <span class=\"kb-math kb-math-inline\">u_t</span> 决定：</p>\n<div class=\"kb-math kb-math-display\">x_{t+h}^i \\sim \\delta_{x_t^i}(\\cdot) + h\\,u_t^i(\\cdot \\mid x_t^i, x_1^i)</div>\n<pre><code class=\"language-python\"># DFM-VLA 两阶段推理伪代码\ndef dfm_vla_decode(context, steps_fine=14, steps_val=2):\n    x_t = sample_uniform_action_tokens()\n    total = steps_fine + steps_val\n\n    for k in range(total):\n        t = k / total\n        logits = transformer(context, noised_action_tokens=x_t)\n        x1_pred = sample_or_argmax_clean_actions(logits)\n\n        if k &lt; steps_fine:\n            velocity = build_velocity_field(x_t, x1_pred, mode=&quot;embedding_guided&quot;)\n            x_t = ctmc_euler_update(x_t, velocity, step_size=1 / total)\n        else:\n            x_t = argmax_clean_actions(logits)\n\n    return decode_fast_bpe_actions(x_t)\n</code></pre>\n<p>第一种速度场构造是 velocity head。backbone 先从上下文和 noised action tokens 产生隐藏状态，再由额外 head 输出 replacement velocity：</p>\n<div class=\"kb-math kb-math-display\">h_t=f_\\theta(x_t,l),\\quad\nu_t^\\theta(\\cdot \\mid x_t)=u^{\\mathrm{head}}_t(h_t)</div>\n<p>它的优点是显式预测跳转速率，和 EditFlow 中的编辑操作思想接近；DFM-VLA 因为动作块长度固定，只保留 replacement，而不需要 insertion/deletion。损失只在当前 token 与目标 token 不一致的位置施加更新压力，直觉上就是“哪里还没变对，就在那里学习往哪里跳”。</p>\n<p>第二种是论文主推的 action-embedding-guided velocity。它不让一个额外 head 从零学所有速率，而是利用动作 token embedding 空间的距离 <span class=\"kb-math kb-math-inline\">d(\\cdot,\\cdot)</span> 定义概率路径：</p>\n<div class=\"kb-math kb-math-display\">p_t(x^i \\mid x_1^i)\n= \\mathrm{softmax}\\left(-\\beta_t d(x^i,x_1^i)\\right),\n\\quad\n\\beta_t = c\\left(\\frac{t}{1-t}\\right)^\\alpha</div>\n<p>当 <span class=\"kb-math kb-math-inline\">t</span> 接近 0 时，分布较平；当 <span class=\"kb-math kb-math-inline\">t</span> 接近 1 时，概率质量集中到目标 token 附近。论文进一步用 kinetic-optimal velocity 让概率质量只朝更接近目标 token 的方向流动，因此它比单纯类别交叉熵更强调动作 token 的几何邻近关系。训练时模型预测 clean action tokens，并最小化：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\mathrm{ce}}\n= \\mathbb{E}_{t,x_1,x_t}\\left[-\\log p^\\theta_{1\\mid t}(x_1 \\mid x_t,l)\\right]</div>\n<p><img alt=\"DFM-VLA 单步精炼过程\" src=\"https://chris1220313648.github.io/DFM-VLA/assets/figure/inference_one_step.png\" />\n<em>图：单个去噪步中，模型先预测最终动作，再由速度场决定哪些 token 跳转到下一状态。</em></p>\n<p>两阶段解码解决的是“探索与锁定”的平衡。若全程随机 CTMC refine，最后可能仍有局部波动；若过早 argmax，又会回到不可逆承诺。论文在固定 16 个总步数下发现 <span class=\"kb-math kb-math-inline\">T_{\\mathrm{fine}}=14, T_{\\mathrm{val}}=2</span> 最优：大部分预算用于反复修正全序列，最后少量步数把高置信动作确定下来。</p>\n<p>DFM-VLA 的效率也来自离散统一表示。视觉、语言、proprioception 和动作 token 同处一个双向 Transformer，上下文 token 在去噪迭代中基本不变，因此可以缓存其 KV；动作 token 的缓存按变化程度自适应更新。论文报告 adaptive KV caching 将 DFM 推理速度从约 60.2 提升到 121.0，同时 CALVIN 平均长度基本保持（4.42 到 4.40），说明迭代 refine 并不必然意味着高延迟。</p>\n<div class=\"key-point\">💡 关键：DFM-VLA 不是把连续扩散头外挂到 VLM 上，而是在离散 action token 空间里学习“怎样把整段动作一起修正”的速度场；这使它能保留 token 化 VLA 的统一建模优势，同时缓解早期错误不可回退的问题。</div>",
      "quiz": {
        "q": "DFM-VLA 两阶段解码中，为什么需要在 iterative refinement 后加入 deterministic validation？",
        "options": [
    ...
    },
    {
      ...
      "summary": "π0.7 提出一种可操控的通用 VLA 训练方法，用语言、子任务、子目标图像、控制模式和质量/速度/错误等元数据共同构成 prompt，让单一机器人基础模型能从混合质量、多机器人和非机器人数据中学习。它解决了此前 VLA 往往需要任务专门微调、难以组合已学技能完成新任务的问题，展示了零样本跨本体迁移和初步组合泛化。",
      "keyPoints": [
      ...
      "detail": "<p><img alt=\"π0.7 架构总览\" src=\"https://ar5iv.labs.arxiv.org/html/2604.15483/assets/x1.png\" />\n<em>图：π0.7 用 VLM backbone、observation memory、action expert、high-level policy 和 world model 组成可操控 VLA 系统。</em></p>\n<p>元信息中的 <code>paper_url</code> 是官方博客的标题式地址；公开页面实际可访问版本为 <code>https://www.pi.website/blog/pi07</code>，并链接论文 <code>π0.7: a Steerable Generalist Robotic Foundation Model with Emergent Capabilities</code>（arXiv:2604.15483）。论文给出的主张是：机器人 foundation model 的泛化瓶颈不只是数据量，而是异构数据里的“策略意图”没有被充分条件化。若把高质量演示、失败轨迹、不同机器人、不同控制模式、人类视频和互联网多模态数据直接混在一起，模型容易学到平均行为；π0.7 用更详细的 prompt 把这些差异显式告诉模型。</p>\n<p>π0.7 的低层 VLA 仍然遵循 flow-based action expert 范式：给定历史观测 <span class=\"kb-math kb-math-inline\">o_{t-T:t}</span>、上下文 <span class=\"kb-math kb-math-inline\">C_t</span>，预测未来动作块 <span class=\"kb-math kb-math-inline\">a_{t:t+H}</span>。可以把训练目标写成：</p>\n<div class=\"kb-math kb-math-display\">\\max_\\theta\\ \\mathbb{E}_{D}\\left[\\log \\pi_\\theta(a_{t:t+H}\\mid o_{t-T:t}, C_t)\\right]</div>\n<p>其中上下文不再只是短语言指令，而是多模态结构：</p>\n<div class=\"kb-math kb-math-display\">C_t = (\\ell_{\\mathrm{task}}, \\ell_{\\mathrm{subtask}}, g_t, m_{\\mathrm{quality}}, m_{\\mathrm{speed}}, m_{\\mathrm{mistake}}, m_{\\mathrm{control}})</div>\n<p><span class=\"kb-math kb-math-inline\">\\ell_{\\mathrm{task}}</span> 描述总体任务，例如“clean the kitchen”；<span class=\"kb-math kb-math-inline\">\\ell_{\\mathrm{subtask}}</span> 描述当前阶段，例如“pick up the knife”；<span class=\"kb-math kb-math-inline\">g_t</span> 是多视角子目标图像；metadata 描述该段轨迹执行得快不快、好不好、是否出错以及使用关节控制还是末端控制。这个设计的直觉是：失败轨迹也能教模型“什么情况下会失败”，但前提是模型知道它是失败轨迹，而不是把它当成理想示范。</p>\n<pre><code class=\"language-python\"># π0.7 测试时可操控推理伪代码\ndef pi07_rollout(observation_history, task_instruction, desired_metadata):\n    memory = encode_observation_history(observation_history)\n    subtask = high_level_policy(\n        observation=observation_history,\n        task=task_instruction,\n        metadata=desired_metadata,\n        previous_subtasks=[]\n    )\n\n    while not task_done():\n        subgoal_images = world_model(\n            current_observation=observation_history,\n            subtask_instruction=subtask,\n            metadata=desired_metadata\n        )\n\n        context = {\n            &quot;task&quot;: task_instruction,\n            &quot;subtask&quot;: subtask,\n            &quot;subgoal_images&quot;: subgoal_images,\n            &quot;metadata&quot;: desired_metadata,\n            &quot;control_mode&quot;: choose_control_mode(task_instruction)\n        }\n\n        action_chunk = pi07_vla_action_expert(\n            observation_memory=memory,\n            context=context\n        )\n        execute_prefix(action_chunk)\n        observation_history = update_observations()\n        memory = update_memory(memory, observation_history)\n        subtask = high_level_policy(observation_history, task_instruction, desired_metadata)\n</code></pre>\n<p><img alt=\"π0.7 prompt 示例\" src=\"https://ar5iv.labs.arxiv.org/html/2604.15483/assets/x2.png\" />\n<em>图：同一个模型可以同时接收当前观测、子目标图像、子任务文本和 metadata；折衣任务中使用 subgoal 与质量/速度提示完成跨本体迁移。</em></p>\n<p>子任务语言解决的是长时任务分解。对“把食物放到桌上”这类任务，单句总体指令不一定告诉机器人下一步该按微波炉按钮、取盘子还是关门。π0.7 在训练中为片段标注中间语义步骤，让模型能被人类实时 coaching，也能由高层策略自动生成下一条 subtask instruction。论文中空气炸锅、倒出空气炸锅、烤贝果等长时任务没有对应机器人训练轨迹，但模型可以在人工逐步口头提示下完成，再把这些语言指导轨迹用于训练高层策略，实现自主执行。</p>\n<p>子目标图像解决的是“语言不够具体”。例如“抓住把手”没有说明手腕视角下夹爪应处于何种姿态；世界模型根据当前观测和子任务生成近未来目标图像，把目标状态以视觉方式传给 VLA。这个机制在跨本体时尤其重要：源机器人和 UR5e 的工作空间、惯量和夹爪姿态不同，生成的子目标能给目标机器人一个更贴合自身形态的视觉参照。</p>\n<p>metadata 是 π0.7 能利用混合质量数据的关键。论文把 episode speed/length、quality score、mistake label 等作为 prompt token 注入训练；部署时可以要求高质量、较快、无错误的行为。这样，低质量或失败数据不会被简单平均进“理想动作”，而是作为有条件经验存在。官方博客还强调，将 RL specialist/Recap 过程中产生的自主数据加上策略元数据蒸馏进 π0.7 后，单一 generalist 能在洗衣、浓缩咖啡、折箱等任务上接近或超过专门 RL policy 的吞吐和成功率。</p>\n<p>与 π0/π0.5 这类主要依赖短任务描述的模型相比，π0.7 的提升来自更丰富的上下文接口。论文明确说它构建在 π0.6 与 MEM 记忆系统之上，并不是把泛化归因于一个孤立的新网络层；真正的设计点是让训练样本携带足够的“意图解释”，使模型能从多机器人、多策略、多质量的数据中抽取可组合技能。</p>\n<p>实验层面，π0.7 的亮点包括：无需任务特定后训练即可完成浓缩咖啡、洗衣、扔垃圾袋、折箱、削蔬菜等 dexterous long-horizon 任务；在未见厨房/卧室环境中跟随复杂语言；在没有 UR5e 洗衣折叠数据的情况下，将源双臂平台的折衣技能迁移到 bimanual UR5e，并达到接近专家遥操作员零样本表现的成功率；对新短任务可直接 prompt，对更长的新电器任务可通过语言 coaching 学会。</p>\n<div class=\"warn-box\">⚠️ 注意：π0.7 展示的是“strong signs of compositional generalization”，不是完全解决机器人组合泛化。论文也指出新任务定义、训练数据泄漏边界和长时自主稳定性仍然是评估难点。</div>",
      "quiz": {
        "q": "π0.7 为什么要在 prompt 中加入质量、速度、错误等 episode metadata？",
        "options": [
          "为了把动作空间从连续值改成纯文本输出",
