/**
 * mm_retrieval-data.js — 由 pipeline/build.py 于 2026-06-08 12:12:05 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>输入三元组表示</strong>：将传统 VLP 的 <span class=\"kb-math kb-math-inline\">(w, v)</span> 二元输入扩展为 <span class=\"kb-math kb-math-inline\">(w, q, v)</span> 三元组，其中 <span class=\"kb-math kb-math-inline\">w</span> 为文本词嵌入、<span class=\"kb-math kb-math-inline\">q</span> 为对象标签词嵌入、<span class=\"kb-math kb-math-inline\">v</span> 为图像区域特征",
      ...
      "detail": "<h5>核心架构示意图</h5>\n<p><img alt=\"OSCAR Pipeline 总览\" src=\"https://ar5iv.labs.arxiv.org/html/2004.06165/assets/x1.png\" />\n<em>图 1：OSCAR 流程总览。模型以三元组 [word tokens, object tags, region features] 为输入，使用 Masked Token Loss 和 Contrastive Loss 进行预训练，并在 5 项理解任务和 2 项生成任务上微调。</em></p>\n<p><img alt=\"OSCAR 架构详图\" src=\"https://ar5iv.labs.arxiv.org/html/2004.06165/assets/x5.png\" />\n<em>图 3：OSCAR 架构详图。输入三元组可从两个视角理解——模态视角（Modality View）将 <span class=\"kb-math kb-math-inline\">q</span> 归入图像侧，字典视角（Dictionary View）将 <span class=\"kb-math kb-math-inline\">q</span> 归入语言侧。对象标签 <span class=\"kb-math kb-math-inline\">q</span> 同时存在于两个视角中，起到跨模态桥梁作用。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># OSCAR 预训练伪代码\n# 输入: 图文对数据集 D, 预训练好的 BERT 参数 θ_BERT, Faster R-CNN 检测器\n\nfor (image, text) in D:\n    # Step 1: 提取视觉特征\n    regions, tags = FasterRCNN(image)         # v'∈R^{K×2048}, tags=[dog, couch, ...]\n    v = LinearProject(concat(regions, positions))  # v∈R^{K×H}\n    q = BERTWordEmbed(tags)                   # 对象标签的词嵌入\n    w = BERTWordEmbed(text)                   # 文本的词嵌入\n\n    # Step 2: 构造输入三元组\n    # 字典视角: h = [w, q] (语言空间), v (视觉空间)\n    # 模态视角: w (语言模态), h' = [q, v] (图像模态)\n\n    # Step 3: Masked Token Loss (字典视角)\n    h = concat(w, q)\n    h_masked = random_mask(h, prob=0.15)      # 15% token 替换为 [MASK]\n    loss_mtl = -log P(h_i | h_\\i, v)         # 基于上下文+图像预测被掩码 token\n\n    # Step 4: Contrastive Loss (模态视角)\n    if random() &lt; 0.5:\n        q_polluted = sample_random_tags(D)    # 50% 概率替换为随机标签\n        y = 0                                  # 负样本\n    else:\n        q_polluted = q\n        y = 1                                  # 正样本\n    h_prime = concat(q_polluted, v)\n    cls_repr = Transformer([w, h_prime])[CLS]  # [CLS] 融合表示\n    loss_c = -log P(y | FC(cls_repr))          # 二分类损失\n\n    # Step 5: 联合优化\n    loss = loss_mtl + loss_c\n    optimizer.step(loss)\n</code></pre>\n<h5>动机与背景</h5>\n<p>现有视觉-语言预训练（VLP）方法（如 ViLBERT、LXMERT、UNITER 等）将图像区域特征 <span class=\"kb-math kb-math-inline\">v</span> 与文本词嵌入 <span class=\"kb-math kb-math-inline\">w</span> 简单拼接后送入 Transformer，依赖自注意力机制以\"暴力\"方式学习跨模态对齐。然而，这种方式面临两个核心挑战：</p>\n<ol>\n<li><strong>语义空间不对齐</strong>：视觉区域特征和文本词嵌入分别处于不同的语义空间，模型需要从零学习跨空间映射；</li>\n<li><strong>缺乏显式锚定</strong>：没有标注哪些区域对应哪些词汇，VLP 本质上是弱监督学习，且视觉区域通常过采样、含噪声和歧义。</li>\n</ol>\n<p>OSCAR 的关键洞察是：<strong>图像中的显著对象可以被现代目标检测器准确识别，且这些对象往往在配对文本中被提及</strong>。在 MS COCO 数据集上，49.7% 的图文对至少共享 1 个对象，22.2% 共享 2 个，12.9% 共享 3 个。因此，对象标签天然可作为视觉与语言之间的\"锚点\"。</p>\n<h5>核心机制：三元组输入与双视角</h5>\n<p>OSCAR 的核心创新在于将输入从二元组 <span class=\"kb-math kb-math-inline\">(w, v)</span> 扩展为三元组 <span class=\"kb-math kb-math-inline\">(w, q, v)</span>，其中 <span class=\"kb-math kb-math-inline\">q</span> 是对象标签的词嵌入序列。这一设计的精妙之处在于 <strong><span class=\"kb-math kb-math-inline\">q</span> 同时属于两个模态</strong>：</p>\n<ul>\n<li>从<strong>语义空间</strong>角度，<span class=\"kb-math kb-math-inline\">q</span> 是文本词汇，与 <span class=\"kb-math kb-math-inline\">w</span> 共享 BERT 的语言语义空间，二者之间的对齐可以直接利用预训练 BERT 的语言理解能力；</li>\n<li>从<strong>信息来源</strong>角度，<span class=\"kb-math kb-math-inline\">q</span> 来自图像的目标检测，与 <span class=\"kb-math kb-math-inline\">v</span> 共享视觉信息来源。</li>\n</ul>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：对象标签 <span class=\"kb-math kb-math-inline\">q</span> 像一座桥梁——它在语言空间中与文本 <span class=\"kb-math kb-math-inline\">w</span> 天然可比较（因为都是词），同时在信息来源上与视觉特征 <span class=\"kb-math kb-math-inline\">v</span> 紧密关联（因为来自同一张图）。这使得模型无需从零学习跨模态映射，而是通过 <span class=\"kb-math kb-math-inline\">q</span> 这个\"中间人\"大幅降低对齐难度。</div>\n<p>形式化地，输入三元组可从两个视角解读：</p>\n<div class=\"kb-math kb-math-display\">\\boldsymbol{x} \\triangleq [\\underbrace{\\boldsymbol{w}}_{\\text{language}}, \\underbrace{\\boldsymbol{q}, \\boldsymbol{v}}_{\\text{image}}] = [\\underbrace{\\boldsymbol{w}, \\boldsymbol{q}}_{\\text{language}}, \\underbrace{\\boldsymbol{v}}_{\\text{image}}] \\triangleq \\boldsymbol{x}&#x27;</div>\n<h5>预训练目标详解</h5>\n<p><strong>1. Masked Token Loss（字典视角）</strong></p>\n<p>从字典视角出发，将 <span class=\"kb-math kb-math-inline\">w</span> 和 <span class=\"kb-math kb-math-inline\">q</span> 视为同一语言空间中的离散 token 序列 <span class=\"kb-math kb-math-inline\">h = [w, q]</span>，对其进行 15% 的随机掩码，利用上下文 token 和图像区域特征 <span class=\"kb-math kb-math-inline\">v</span> 来预测被掩码的 token：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\text{MTL}} = -\\mathbb{E}_{(v, h) \\sim \\mathcal{D}} \\log p(h_i | h_{\\backslash i}, v)</div>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：与标准 BERT MLM 不同，这里的掩码范围包括对象标签 <span class=\"kb-math kb-math-inline\">q</span>，这意味着模型不仅需要理解语言上下文，还需要利用视觉信息来推断被掩码的对象名称，从而实现视觉接地（visual grounding）。</div>\n<p><strong>2. Contrastive Loss（模态视角）</strong></p>\n<p>从模态视角出发，将 <span class=\"kb-math kb-math-inline\">h&#x27; = [q, v]</span> 视为图像模态的表示，<span class=\"kb-math kb-math-inline\">w</span> 为语言模态。通过以 50% 概率将 <span class=\"kb-math kb-math-inline\">q</span> 替换为数据集中随机采样的标签序列来构造负样本，然后在 <span class=\"kb-math kb-math-inline\">[CLS]</span> 位置的融合表示上训练一个二分类器：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\text{C}} = -\\mathbb{E}_{(h&#x27;, w) \\sim \\mathcal{D}} \\log p(y | f(h&#x27;, w))</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">y=1</span> 表示原始匹配对，<span class=\"kb-math kb-math-inline\">y=0</span> 表示被\"污染\"的对。这一损失迫使模型学习区分正确的图文对齐与错误的对齐。</p>\n<p><strong>联合目标</strong>：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\text{Pre-training}} = \\mathcal{L}_{\\text{MTL}} + \\mathcal{L}_{\\text{C}}</div>\n<h5>视觉特征提取</h5>\n<p>OSCAR 使用 Faster R-CNN 提取每张图像 <span class=\"kb-math kb-math-inline\">K</span> 个区域的特征：\n- 区域视觉特征 <span class=\"kb-math kb-math-inline\">v&#x27; \\in \\mathbb{R}^{P}</span>（<span class=\"kb-math kb-math-inline\">P=2048</span>）\n- 区域位置编码 <span class=\"kb-math kb-math-inline\">z \\in \\mathbb{R}^{R}</span>（<span class=\"kb-math kb-math-inline\">R=4</span> 或 <span class=\"kb-math kb-math-inline\">6</span>，包含左上/右下坐标及宽高）\n- 拼接后通过线性投影 <span class=\"kb-math kb-math-inline\">\\mathbf{W}</span> 映射到与词嵌入相同的维度 <span class=\"kb-math kb-math-inline\">H</span></p>\n<p>同时，Faster R-CNN 输出高置信度的对象标签，作为 <span class=\"kb-math kb-math-inline\">q</span> 的来源。</p>\n<h5>与传统 VLP 方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>传统 VLP（ViLBERT/UNITER 等）</th>\n<th>OSCAR</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>输入形式</td>\n<td>二元组 <span class=\"kb-math kb-math-inline\">(w, v)</span></td>\n<td>三元组 <span class=\"kb-math kb-math-inline\">(w, q, v)</span></td>\n</tr>\n<tr>\n<td>对齐方式</td>\n<td>纯自注意力暴力对齐</td>\n<td>对象标签作为显式锚点</td>\n</tr>\n<tr>\n<td>预训练目标</td>\n<td>MLM + ITM（图文匹配）</td>\n<td>MTL（含标签掩码）+ Contrastive Loss</td>\n</tr>\n<tr>\n<td>负样本构造</td>\n<td>替换整张图或整段文本</td>\n<td>仅替换对象标签序列</td>\n</tr>\n<tr>\n<td>语义空间</td>\n<td>视觉和语言各自独立</td>\n<td>标签在语言空间中统一表示</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 <strong>核心优势</strong>：当移除对象标签 <span class=\"kb-math kb-math-inline\">q</span> 时，OSCAR 退化为传统 VLP 方法。<span class=\"kb-math kb-math-inline\">q</span> 的引入几乎不增加计算成本（仅多了几个文本 token），但通过在语言空间中提供显式的视觉-语言锚点，大幅提升了对齐学习效率。</div>\n<h5>下游任务适配</h5>\n<p>OSCAR 在微调阶段支持两类下游任务：\n- <strong>理解任务</strong>（VQA、GQA、NLVR2、Image-Text Retrieval）：在 <span class=\"kb-math kb-math-inline\">[CLS]</span> 表示上添加任务特定的分类头\n- <strong>生成任务</strong>（Image Captioning、NoCaps）：采用 seq2seq 微调方式生成文本</p>\n<p>在所有 6+ 项任务上，OSCAR 均取得了当时的 SOTA 结果，验证了对象标签锚点策略的通用有效性。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>Sigmoid 对比损失</strong>：将图文对匹配建模为独立的二分类问题，对 batch 内所有 <span class=\"kb-math kb-math-inline\">n^2</span> 个图文对分别计算 sigmoid 损失，无需 softmax 的全局归一化",
        "<strong>可学习偏置项 <span class=\"kb-math kb-math-inline\">b</span></strong>：在相似度计算中引入 learnable bias（初始化为 <span class=\"kb-math kb-math-inline\">-\\log(n)</span> 量级），自动平衡正负样本比例（1 正 vs <span class=\"kb-math kb-math-inline\">n-1</span> 负）",
        "<strong>两种实验设置</strong>：SigLiT（锁定预训练 ViT 图像塔，仅训练文本塔）和 SigLIP（从头训练双塔）",
        "<strong>Chunked Sigmoid Loss</strong>：分块计算损失，无需在单设备上聚合全局 batch，内存复杂度从 <span class=\"kb-math kb-math-inline\">O(B^2)</span> 降至 <span class=\"kb-math kb-math-inline\">O(B^2/K)</span>（<span class=\"kb-math kb-math-inline\">K</span> 为设备数）",
      ...
      "detail": "<h5>方法总览</h5>\n<p><img alt=\"SigLIP 框架对比图\" src=\"https://ar5iv.labs.arxiv.org/html/2303.15343v2/assets/x1.png\" />\n<em>图：Softmax 对比损失（左）vs Sigmoid 对比损失（右）。Softmax 需要在整行/列上做归一化（需要全局通信），而 Sigmoid 对每个 cell 独立计算损失。</em></p>\n<p>SigLIP 的核心思想非常直观：传统 CLIP 使用 softmax 对比损失（InfoNCE），需要对 batch 内所有样本做全局归一化。这意味着在多设备分布式训练时，必须在所有设备间同步完整的相似度矩阵。SigLIP 将其替换为 sigmoid 损失，每个图文对独立判断\"是否匹配\"，彻底消除了全局依赖。</p>\n<h5>损失函数设计</h5>\n<p><strong>Softmax 对比损失（CLIP 基线）：</strong></p>\n<p>传统的图文对比学习使用 InfoNCE 损失，对 batch 内 <span class=\"kb-math kb-math-inline\">n</span> 个图文对：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\text{softmax}} = -\\frac{1}{n}\\sum_{i=1}^{n}\\left[\\log\\frac{e^{x_i \\cdot y_i / \\tau}}{\\sum_{j=1}^{n} e^{x_i \\cdot y_j / \\tau}} + \\log\\frac{e^{x_i \\cdot y_i / \\tau}}{\\sum_{j=1}^{n} e^{x_j \\cdot y_i / \\tau}}\\right]</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">x_i, y_i</span> 分别是图像和文本的归一化嵌入，<span class=\"kb-math kb-math-inline\">\\tau</span> 是温度参数。关键问题在于分母中的求和 <strong>必须遍历 batch 内所有样本</strong>，在分布式训练中需要跨设备聚合。</p>\n<p><strong>Sigmoid 对比损失（SigLIP 提出）：</strong></p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\text{sigmoid}} = -\\frac{1}{n}\\sum_{i=1}^{n}\\sum_{j=1}^{n}\\log\\frac{1}{1 + e^{z_{ij}(-x_i \\cdot y_j \\cdot t + b)}}</div>\n<p>其中：\n- <span class=\"kb-math kb-math-inline\">z_{ij} = \\begin{cases} 1 &amp; \\text{if } i = j \\text{（正样本对）} \\\\ -1 &amp; \\text{if } i \\neq j \\text{（负样本对）} \\end{cases}</span>\n- <span class=\"kb-math kb-math-inline\">t</span> 是可学习的温度参数（对应 <span class=\"kb-math kb-math-inline\">1/\\tau</span>）\n- <span class=\"kb-math kb-math-inline\">b</span> 是可学习的偏置项</p>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：Sigmoid 损失将每个图文对视为一个独立的二分类问题——\"这张图和这段文字是否匹配？\"。正样本对（对角线）标签为 1，负样本对（非对角线）标签为 -1。每个 cell 的损失计算完全独立，不依赖同行/同列的其他值。</div>\n<h5>偏置项 <span class=\"kb-math kb-math-inline\">b</span> 的作用</h5>\n<p>偏置项 <span class=\"kb-math kb-math-inline\">b</span> 是 SigLIP 的一个精妙设计。在一个 batch 中，正样本对有 <span class=\"kb-math kb-math-inline\">n</span> 个，而负样本对有 <span class=\"kb-math kb-math-inline\">n^2 - n</span> 个，正负比例约为 <span class=\"kb-math kb-math-inline\">1 : (n-1)</span>。如果没有偏置项，sigmoid 函数在零点处输出 0.5，这意味着模型初始化时会将所有对都预测为\"匹配\"，导致训练不稳定。</p>\n<p>偏置项 <span class=\"kb-math kb-math-inline\">b</span> 初始化为 <span class=\"kb-math kb-math-inline\">-\\log(n) \\approx -10</span>（当 <span class=\"kb-math kb-math-inline\">n = 32768</span> 时），使得初始时 sigmoid 输出接近 0（即\"不匹配\"），与负样本占绝大多数的先验一致。论文实验表明，<span class=\"kb-math kb-math-inline\">b</span> 的初始化值对最终性能影响不大（在 <span class=\"kb-math kb-math-inline\">-10</span> 到 <span class=\"kb-math kb-math-inline\">-15</span> 范围内结果稳定），但训练过程中 <span class=\"kb-math kb-math-inline\">b</span> 会收敛到约 <span class=\"kb-math kb-math-inline\">-10</span> 附近。</p>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：偏置项 <span class=\"kb-math kb-math-inline\">b</span> 的角色类似于逻辑回归中的截距项，它补偿了正负样本的类别不平衡。这与 Focal Loss 中处理类别不平衡的思路异曲同工。</div>\n<h5>Chunked Sigmoid Loss 分布式实现</h5>\n<pre><code class=\"language-python\"># Chunked Sigmoid Loss 伪代码\n# 假设 K 个设备，每个设备持有 n/K 个样本\n# 设备 k 上的图像嵌入: img_emb[k], 文本嵌入: txt_emb[k]\n\ndef chunked_sigmoid_loss(img_emb, txt_emb, t, b):\n    &quot;&quot;&quot;每个设备独立计算局部损失，无需聚合全局相似度矩阵&quot;&quot;&quot;\n    local_loss = 0\n    # 本地图像 vs 所有文本（通过 all-gather 获取文本嵌入）\n    for k in range(K):\n        txt_chunk = all_gather(txt_emb)[k]  # 获取第k个设备的文本\n        # 计算局部相似度矩阵 (n/K × n/K)\n        logits = img_emb @ txt_chunk.T * t + b\n        # 构造标签：只有当 chunk_k 是本设备时对角线为正\n        labels = get_labels(k, device_id)  # 1 for pos, -1 for neg\n        # Sigmoid 损失：每个元素独立计算\n        local_loss += -log_sigmoid(labels * logits).sum()\n    return local_loss / (n * n)\n</code></pre>\n<div class=\"key-point\">💡 <strong>关键优势</strong>：Softmax 损失需要在单个设备上构建完整的 <span class=\"kb-math kb-math-inline\">n \\times n</span> 相似度矩阵来计算归一化分母，内存为 <span class=\"kb-math kb-math-inline\">O(n^2)</span>。Chunked sigmoid 损失将矩阵分成 <span class=\"kb-math kb-math-inline\">K \\times K</span> 个块，每个设备只需处理 <span class=\"kb-math kb-math-inline\">K</span> 个大小为 <span class=\"kb-math kb-math-inline\">(n/K) \\times (n/K)</span> 的块，内存降至 <span class=\"kb-math kb-math-inline\">O(n^2/K)</span>。</div>\n<h5>与 Softmax 对比损失的关键区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>Softmax (CLIP)</th>\n<th>Sigmoid (SigLIP)</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>损失类型</td>\n<td>多类交叉熵</td>\n<td>逐对二分类</td>\n</tr>\n<tr>\n<td>归一化</td>\n<td>全局（跨 batch）</td>\n<td>无（每对独立）</td>\n</tr>\n<tr>\n<td>分布式通信</td>\n<td>需要聚合全局矩阵</td>\n<td>仅需 all-gather 嵌入</td>\n</tr>\n<tr>\n<td>内存复杂度</td>\n<td><span class=\"kb-math kb-math-inline\">O(B^2)</span></td>\n<td><span class=\"kb-math kb-math-inline\">O(B^2/K)</span></td>\n</tr>\n<tr>\n<td>小 batch 性能</td>\n<td>较差</td>\n<td>显著更优</td>\n</tr>\n<tr>\n<td>超参数敏感性</td>\n<td>需按 batch 调参</td>\n<td>默认超参即可</td>\n</tr>\n<tr>\n<td>偏置项</td>\n<td>无</td>\n<td>可学习偏置 <span class=\"kb-math kb-math-inline\">b</span></td>\n</tr>\n</tbody>\n</table></div>\n<h5>实验关键发现</h5>\n<p><strong>1. Sigmoid 在小 batch 下优势显著：</strong> 在 SigLiT 设置（锁定预训练 ViT-L/16 图像塔）下，batch size 为 512 时 sigmoid 比 softmax 高 3.0%（72.5% vs 69.5%），随着 batch 增大差距缩小，在 128k 时两者基本持平。</p>\n<p><strong>2. 32k 是性价比最优 batch size：</strong> 论文系统实验了从 512 到 1M 的 batch size，发现 32k 即可达到接近最优性能（84.2% vs 84.7%@1M），而所需计算资源远小于超大 batch。</p>\n<p><strong>3. 从头训练（SigLIP）同样有效：</strong> 在 WebLI 数据集上从头训练 ViT-B/16，SigLIP 在 ImageNet zero-shot 上达到 73.4%（batch=32k, 36B examples），优于同等设置的 softmax 基线。</p>\n<p><strong>4. 噪声鲁棒性：</strong> 在人工注入标签噪声的实验中，sigmoid 损失比 softmax 更鲁棒，在 40% 噪声率下仍保持合理性能。</p>",
      "quiz": {
