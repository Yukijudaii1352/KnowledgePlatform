/**
 * embodied_rl-data.js — 由 pipeline/build.py 于 2026-05-20 17:34:40 自动生成。
  ...
  "algos": [
    {
      ...
      "keyPoints": [
        "<strong>Actor-Critic 架构</strong>：Actor 网络 \\(\\mu(s|\\theta^\\mu)\\) 输出确定性动作，Critic 网络 \\(Q(s,a|\\theta^Q)\\) 估计动作价值函数",
        "<strong>经验回放缓冲区 (Replay Buffer)</strong>：存储 \\((s_t, a_t, r_t, s_{t+1})\\) 转移元组，随机采样小批量训练，打破样本时序相关性",
        "<strong>目标网络 (Target Network)</strong>：Actor 和 Critic 各维护一个目标网络副本，通过软更新 \\(\\theta' \\leftarrow \\tau\\theta + (1-\\tau)\\theta'\\) 缓慢跟踪，稳定 TD 目标",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>最大熵目标函数</strong>：在标准 RL 目标上增加策略熵项 \\(\\alpha \\mathcal{H}(\\pi(\\cdot|s))\\)，鼓励策略在完成任务的同时尽可能随机",
        "<strong>三类函数逼近器</strong>：Soft Q 网络 \\(Q_\\theta\\)（双份）、Soft 价值网络 \\(V_\\psi\\)、随机策略网络 \\(\\pi_\\phi\\)",
        ...
        "<strong>重参数化技巧</strong>：策略采样 \\(a = f_\\phi(\\epsilon; s)\\)，使策略梯度可通过 Q 网络反向传播",
        "<strong>目标网络 EMA 更新</strong>：\\(\\bar{\\psi} \\leftarrow \\tau \\psi + (1-\\tau)\\bar{\\psi}\\)，稳定训练",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>延迟策略更新 (Delayed Policy Updates)</strong>：Critic 每更新 \\(d\\) 次（默认 \\(d=2\\)），Actor 才更新一次，确保 Critic 收敛后再指导策略",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>VGG-16 回归架构</strong>：基于 VGG-16 提取特征，接全连接层直接回归物体的 \\((x, y, z)\\) 三维坐标",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>力矩极限感知的 3D 力课程</strong>：通过雅可比矩阵和关节力矩上限计算末端执行器可承受的最大力，结合 Dirichlet 分布在 3D 力空间中采样训练力，并通过渐进式缩放因子 \\(\\alpha_g\\) 逐步增加力的强度",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Expectile 回归估计 V</strong>：用非对称 L2 损失（expectile loss）对 \\(V(s)\\) 进行回归，当 \\(\\tau \\to 1\\) 时逼近 \\(\\max_a Q(s,a)\\)，实现隐式策略改进",
        "<strong>三网络架构</strong>：V 网络（状态值函数）、Q 网络（动作值函数）、π 网络（策略），外加 Q 的目标网络 \\(\\hat{\\theta}\\)",
        "<strong>两阶段训练</strong>：第一阶段交替更新 V 和 Q（TD 学习），第二阶段通过 AWR 提取策略",
        "<strong>AWR 策略提取</strong>：以 \\(\\exp(\\beta \\cdot A(s,a))\\) 为权重的行为克隆，仅使用数据集中的动作",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>策略更新公式</strong>：\\(\\pi = \\arg\\max_\\pi \\; \\mathbb{E}_{(s,a) \\sim \\mathcal{D}} \\left[ \\lambda\\, Q(s, \\pi(s)) - (\\pi(s) - a)^2 \\right]\\)，将 Q 值最大化与行为克隆损失直接相加",
        "<strong>自适应权重归一化</strong>：\\(\\lambda = \\alpha / \\frac{1}{N}\\sum_{(s_i, a_i)}|Q(s_i, a_i)|\\)，通过 Q 值绝对值均值归一化，使 Q 项和 BC 项量级可比，唯一超参 \\(\\alpha=2.5\\)",
        "<strong>状态特征归一化</strong>：将状态归一化为均值 0、标准差 1（\\(\\epsilon=10^{-3}\\) 防除零），提升跨任务稳定性",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>Symlog 预测</strong>：对世界模型的解码器和奖励预测器使用 \\(\\operatorname{symlog}\\) 变换压缩目标尺度，使同一网络适应从 \\(10^{-1}\\) 到 \\(10^{4}\\) 量级的信号",
        "<strong>RSSM 世界模型</strong>：由序列模型（GRU）、编码器、动力学先验、解码器、奖励预测器和 continue 预测器组成，在隐空间中进行想象训练",
        "<strong>KL 平衡 + Free Bits</strong>：世界模型损失中对 KL 散度使用 \\(\\alpha=0.5\\) 的 KL 平衡和 1 nat 的 free bits，避免后验坍缩和先验过拟合",
        "<strong>Critic 离散回归</strong>：Critic 在 symlog 空间的 255 个等距桶上输出 softmax 分布，使用 twohot 编码的软标签进行分类交叉熵训练，有效处理多模态回报分布",
        "<strong>鲁棒回报归一化</strong>：使用 \\(\\lambda\\)-return 的第 5 至第 95 百分位距作为缩放因子 \\(S\\)，仅在 \\(S>1\\) 时缩小回报，避免稀疏奖励下放大噪声",
        "<strong>固定超参数</strong>：单一熵正则化系数 \\(\\eta=3\\times10^{-4}\\)、折扣因子 \\(\\gamma=0.997\\)、想象步长 \\(T=16\\) 等超参数在所有领域通用",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>隐式动作模型（Latent Action Model）</strong>：训练 VAE 从连续帧对 \\((o_t, o_{t+1})\\) 中提取连续隐式动作向量 \\(z_t\\)，使无动作标注的人类视频也能以动作条件方式训练世界模型",
        "<strong>相对动作表示 + 因果动作分块</strong>：使用 \\(a_t^{\\text{rel}} = a_t - a_{t-1}\\) 消除不同机器人形态的绝对动作偏移；因果分块确保生成第 \\(t\\) 帧时仅使用 \\(a_{1:t}\\) 而非未来动作",
