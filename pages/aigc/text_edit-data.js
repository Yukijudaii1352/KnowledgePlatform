/**
 * text_edit-data.js — 由 pipeline/build.py 于 2026-05-20 17:49:40 自动生成。
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>参数化重构</strong>：将图像恢复问题转化为 \\(\\theta^* = \\arg\\min_\\theta E(f_\\theta(z); x_0)\\)，用网络参数空间替代像素空间优化",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>编码比率 \\(r\\) 控制编辑强度</strong>：较大的 \\(r\\) 允许更强的编辑以匹配查询文本，较小的 \\(r\\) 则更贴近原图",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Shared Enhancement Attention (SEA)</strong>：在 DiT 每个 block 中引入可学习权重 \\(\\alpha_i\\)，将参考图注意力特征融合到生成路径，参数共享仅新增一个标量，几乎零额外开销",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>映射网络（Mapping Network）</strong>：8 层全连接 MLP 将输入隐码 \\(\\mathbf{z} \\in \\mathcal{Z}\\) 映射为中间隐码 \\(\\mathbf{w} \\in \\mathcal{W}\\)，解纠缠潜在空间",
        "<strong>自适应实例归一化（AdaIN）</strong>：通过学习到的仿射变换将 \\(\\mathbf{w}\\) 转化为每层的缩放/偏移参数，注入合成网络各层实现风格调制",
        "<strong>常量输入替代随机输入</strong>：生成器从学习到的 \\(4 \\times 4 \\times 512\\) 常量开始合成，不再依赖传统的随机隐码直接输入",
        "<strong>逐层噪声注入</strong>：每个卷积层后添加独立的高斯噪声，建模头发丝、毛孔等随机性细节",
        "<strong>风格混合正则化（Style Mixing）</strong>：训练时以一定概率使用两个不同的 \\(\\mathbf{w}\\) 向量分别控制不同层级，防止相邻层风格相关性过高",
        "<strong>截断技巧（Truncation Trick）</strong>：在 \\(\\mathcal{W}\\) 空间中对偏离均值过远的样本进行截断，平衡生成质量与多样性",
        "<strong>FFHQ 数据集</strong>：发布了包含 70,000 张 \\(1024 \\times 1024\\) 高质量人脸图像的新数据集",
        "<strong>解纠缠度量</strong>：提出感知路径长度（PPL）和线性可分性两个定量指标评估潜在空间的解纠缠程度"
      ],
      "detail": "<h5>核心架构示意图</h5>\n<p><img alt=\"StyleGAN 生成器架构\" src=\"https://ar5iv.labs.arxiv.org/html/1812.04948/assets/x1.png\" />\n<em>图：(a) 传统 GAN 生成器直接将隐码 z 送入网络；(b) StyleGAN 生成器通过映射网络 f 将 z 映射为 w，再通过学习到的仿射变换 A 在每层以 AdaIN 方式注入风格，同时在每层注入独立噪声 B。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># StyleGAN 生成器前向传播伪代码\ndef StyleGenerator(z, noise_inputs):\n    # 1. 映射网络：z → w\n    w = MappingNetwork(z)  # 8层MLP, z∈R^512 → w∈R^512\n\n    # 2. 截断技巧（推理时）\n    w_avg = ExponentialMovingAverage(w)  # 训练中维护 w 均值\n    w = w_avg + psi * (w - w_avg)        # ψ∈[0,1] 控制截断强度\n\n    # 3. 从学习到的常量开始合成\n    x = learned_constant  # 4×4×512\n\n    # 4. 逐层合成（4×4 → 8×8 → ... → 1024×1024）\n    for layer_idx in range(num_layers):\n        # 上采样（除第一层外）\n        if layer_idx &gt; 0:\n            x = upsample(x)\n\n        # 卷积\n        x = conv(x)\n\n        # 噪声注入：逐通道缩放的高斯噪声\n        x = x + B[layer_idx] * noise_inputs[layer_idx]\n\n        # AdaIN 风格调制\n        y_s, y_b = AffineTransform(w)  # w → (scale, bias)\n        x = AdaIN(x, y_s, y_b)\n        # AdaIN(x_i) = y_{s,i} * (x_i - μ(x_i)) / σ(x_i) + y_{b,i}\n\n    return to_rgb(x)\n</code></pre>\n<h5>动机与背景</h5>\n<p>传统 GAN 生成器（如 ProGAN）将随机隐码 \\(\\mathbf{z}\\) 直接通过输入层送入网络，这种设计存在两个根本问题：</p>\n<ol>\n<li><strong>潜在空间纠缠</strong>：输入空间 \\(\\mathcal{Z}\\) 必须服从训练数据的概率密度分布，导致不同语义属性（如性别、年龄、发色）在 \\(\\mathcal{Z}\\) 中不可避免地纠缠在一起。例如，当训练数据中长发与女性高度相关时，\\(\\mathcal{Z}\\) 空间中这两个属性就会耦合。</li>\n<li><strong>缺乏层级控制</strong>：所有语义信息通过单一输入点注入，无法对不同抽象层级的属性进行独立控制。</li>\n</ol>\n<p>StyleGAN 的核心洞察是：<strong>将\"风格\"概念引入生成器设计</strong>，借鉴风格迁移（Style Transfer）中 AdaIN 的成功经验，让每一层的特征统计量（均值和方差）携带不同层级的语义信息。</p>\n<h5>核心机制详解</h5>\n<p><strong>1. 映射网络（Mapping Network）</strong></p>\n<p>映射网络 \\(f: \\mathcal{Z} \\rightarrow \\mathcal{W}\\) 是一个 8 层全连接网络，每层 512 维，使用 Leaky ReLU 激活。其关键作用是将服从均匀/正态分布的 \\(\\mathbf{z}\\) 映射到一个<strong>不需要服从固定分布</strong>的中间空间 \\(\\mathcal{W}\\)。</p>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：\\(\\mathcal{Z}\\) 空间受制于采样分布（如正态分布），其形状是固定的超球面，必须\"弯曲\"自身来匹配训练数据分布，导致纠缠。而 \\(\\mathcal{W}\\) 空间没有这个约束，可以自由学习一个更\"展开\"的表示，使得不同变化因子对应线性子空间。</div>\n<p><strong>2. 自适应实例归一化（AdaIN）</strong></p>\n<p>每个合成层中，中间隐码 \\(\\mathbf{w}\\) 通过一个学习到的仿射变换 \\(A\\) 生成该层的风格参数 \\((\\mathbf{y}_s, \\mathbf{y}_b)\\)，然后通过 AdaIN 注入：</p>\n<p>$$\\text{AdaIN}(\\mathbf{x}_i, \\mathbf{y}) = y_{s,i} \\frac{\\mathbf{x}_i - \\mu(\\mathbf{x}_i)}{\\sigma(\\mathbf{x}_i)} + y_{b,i}$$</p>\n<p>其中 \\(\\mathbf{x}_i\\) 是第 \\(i\\) 个特征图，\\(\\mu\\) 和 \\(\\sigma\\) 分别计算其空间均值和标准差。这一机制的本质是：<strong>先通过归一化\"擦除\"上一层的风格信息，再通过缩放和偏移\"写入\"新的风格</strong>。</p>\n<p>不同分辨率层级控制不同粒度的属性：\n- <strong>粗粒度层（4×4 — 8×8）</strong>：控制姿态、脸型、眼镜等高层语义\n- <strong>中粒度层（16×16 — 32×32）</strong>：控制面部特征、发型、眼睛睁闭\n- <strong>细粒度层（64×64 — 1024×1024）</strong>：控制颜色方案（肤色、发色）和微观结构</p>\n<p><strong>3. 随机噪声注入</strong></p>\n<p>在每个卷积层之后，StyleGAN 注入独立的逐像素高斯噪声，通过可学习的逐通道缩放因子 \\(B\\) 控制噪声强度：</p>\n<p>$$\\mathbf{x}' = \\mathbf{x} + B \\cdot \\boldsymbol{\\epsilon}, \\quad \\boldsymbol{\\epsilon} \\sim \\mathcal{N}(0, \\mathbf{I})$$</p>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：噪声只影响随机性的视觉细节（如头发的精确位置、毛孔、背景纹理），而不影响高层语义（如身份、姿态）。这是因为判别器对这些随机变化施加了一致性约束——改变噪声不应改变\"这是谁\"。</div>\n<p><strong>4. 风格混合正则化（Style Mixing Regularization）</strong></p>\n<p>训练时，以一定概率使用两个隐码 \\(\\mathbf{z}_1, \\mathbf{z}_2\\) 生成对应的 \\(\\mathbf{w}_1, \\mathbf{w}_2\\)，在随机选择的交叉点之前使用 \\(\\mathbf{w}_1\\)，之后使用 \\(\\mathbf{w}_2\\)。这一正则化防止网络假设相邻层的风格是相关的，从而改善各层风格的局部化。</p>\n<p><img alt=\"风格混合效果\" src=\"https://ar5iv.labs.arxiv.org/html/1812.04948/assets/figures/Stylemix/seed888-coarse-var639.jpg\" />\n<em>图：风格混合示例——将一个源图像的粗粒度风格（姿态、脸型）与另一个源图像的细粒度风格（颜色、纹理）组合。</em></p>\n<p><strong>5. 截断技巧（Truncation Trick in \\(\\mathcal{W}\\)）</strong></p>\n<p>低概率密度区域的 \\(\\mathbf{w}\\) 往往对应训练数据中罕见的样本组合，生成质量较差。截断技巧通过将 \\(\\mathbf{w}\\) 拉向均值来提升质量：</p>\n<p>$$\\mathbf{w}' = \\bar{\\mathbf{w}} + \\psi (\\mathbf{w} - \\bar{\\mathbf{w}})$$</p>\n<p>其中 \\(\\bar{\\mathbf{w}} = \\mathbb{E}_{\\mathbf{z}}[f(\\mathbf{z})]\\) 是 \\(\\mathcal{W}\\) 空间的均值，\\(\\psi < 1\\) 控制截断强度。\\(\\psi = 0\\) 时所有图像收敛到\"平均脸\"，\\(\\psi = 1\\) 时无截断。实践中只对低分辨率层（控制高层语义的层）应用截断，高分辨率层保持不变。</p>\n<h5>解纠缠度量</h5>\n<p>StyleGAN 提出了两个定量指标来衡量潜在空间的解纠缠程度：</p>\n<p><strong>感知路径长度（Perceptual Path Length, PPL）</strong>：衡量在潜在空间中沿小步插值时，生成图像在感知上的变化是否均匀。解纠缠的空间应该具有更短的路径长度：</p>\n<p>$$l_{\\mathcal{W}} = \\mathbb{E}\\left[\\frac{1}{\\epsilon^2} d\\big(G(f(\\text{lerp}(\\mathbf{z}_1, \\mathbf{z}_2, t))),\\; G(f(\\text{lerp}(\\mathbf{z}_1, \\mathbf{z}_2, t+\\epsilon)))\\big)\\right]$$</p>\n<p>其中 \\(d(\\cdot, \\cdot)\\) 使用 VGG16 的感知距离。</p>\n<p><strong>线性可分性（Linear Separability）</strong>：训练线性 SVM 在潜在空间中分类二元属性（如男/女），分类准确度越高说明该属性在潜在空间中越接近线性子空间，即解纠缠程度越高。</p>\n<p><img alt=\"解纠缠示意\" src=\"https://ar5iv.labs.arxiv.org/html/1812.04948/assets/x2.png\" />\n<em>图：传统输入空间 Z 与映射后的中间空间 W 的解纠缠对比示意。</em></p>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>传统 GAN (ProGAN)</th>\n<th>StyleGAN</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>输入方式</td>\n<td>\\(\\mathbf{z}\\) 直接送入第一层</td>\n<td>常量输入 + \\(\\mathbf{w}\\) 通过 AdaIN 逐层注入</td>\n</tr>\n<tr>\n<td>潜在空间</td>\n<td>\\(\\mathcal{Z}\\)（受采样分布约束）</td>\n<td>\\(\\mathcal{W}\\)（自由学习，更解纠缠）</td>\n</tr>\n<tr>\n<td>随机变化</td>\n<td>完全由 \\(\\mathbf{z}\\) 控制</td>\n<td>由逐层噪声独立控制</td>\n</tr>\n<tr>\n<td>属性控制</td>\n<td>全局纠缠</td>\n<td>层级化：粗/中/细粒度分离</td>\n</tr>\n<tr>\n<td>风格混合</td>\n<td>不支持</td>\n<td>天然支持，可在任意层交换风格</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：StyleGAN 的判别器和训练损失函数（WGAN-GP / R1 正则化）与 ProGAN 相同，所有改进都集中在生成器架构上。这说明生成器的架构设计对 GAN 的生成质量有决定性影响。</div>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>ID 保持网络</strong>：提出面部注意力定位策略（Facial Attention Localization Strategy），通过平衡 \\(L_1\\) 损失 \\(\\mathcal{L}_{loc}\\) 约束交叉注意力图与面部分割掩码对齐",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>Pivotal Inversion（枢轴反演）</strong>：先用 DDIM 反演（guidance scale \\(w=1\\)）计算一条近似噪声轨迹 \\(\\{z_t^*\\}\\)，作为后续优化的初始\"枢轴\"",
        "<strong>Null-text Optimization（空文本优化）</strong>：在每个去噪时间步 \\(t\\) 独立优化无条件嵌入 \\(\\varnothing_t\\)，使 classifier-free guidance 下的重建结果精确匹配 DDIM 反演的中间状态",
        "<strong>无需模型微调</strong>：仅优化 null-text embedding（约 \\(50 \\times 768\\) 维参数），不修改 UNet 权重，保留模型原有编辑能力",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>视频模型改编辑</strong>：将编辑输入-输出对 \\(\\{c, p\\}\\) 重新解释为两帧短视频，复用预训练视频模型的时序先验",
        "<strong>时序推理 token</strong>：在输入帧与目标帧之间插入随机噪声初始化的中间潜在帧，联合去噪以引导模型\"思考\"合理的过渡轨迹",
        "<strong>两阶段推理</strong>：推理时先在\"时序推理阶段\"（前 \\(N_r\\) 步）联合去噪所有帧，再在\"编辑帧生成阶段\"（后 \\(N - N_r\\) 步）丢弃中间帧只精炼目标帧，兼顾质量与效率",
