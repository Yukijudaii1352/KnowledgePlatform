/**
 * rag-data.js — 由 pipeline/build.py 于 2026-06-16 17:00:18 自动生成。
  ...
  "algos": [
    {
      ...
      "keyPoints": [
        "<strong>双塔编码器架构</strong>：独立的 Question Encoder <span class=\"kb-math kb-math-inline\">E_Q</span> 和 Passage Encoder <span class=\"kb-math kb-math-inline\">E_P</span>，均基于 BERT-base，输出 768 维向量，相似度为点积",
        "<strong>高效负样本策略</strong>：In-batch negatives（批内共享负样本，<span class=\"kb-math kb-math-inline\">B</span> 个问题产生 <span class=\"kb-math kb-math-inline\">B^2</span> 个训练对）+ 1 个 BM25 硬负样本（高 BM25 分但不含答案的段落）",
        "<strong>NLL 损失函数</strong>：对正样本段落的负对数似然，在 <span class=\"kb-math kb-math-inline\">B + 1</span> 个候选中做 softmax",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>潜变量检索框架</strong>：将检索文档 <span class=\"kb-math kb-math-inline\">z</span> 建模为潜变量，通过边际化 <span class=\"kb-math kb-math-inline\">p(y|x) = \\sum_z p(y|z,x) \\cdot p(z|x)</span> 实现端到端训练",
        ...
        "<strong>Null 文档机制</strong>：引入空文档 <span class=\"kb-math kb-math-inline\">\\varnothing</span> 作为候选，允许模型在不需要检索时选择不依赖外部文档",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>查询增强（Query Augmentation）</strong>：用 <code>[MASK]</code> token 将查询填充至固定长度 <span class=\"kb-math kb-math-inline\">N_q=32</span>，BERT 对 <code>[MASK]</code> 的上下文化输出起到<strong>隐式查询扩展</strong>作用",
    ...
    },
    {
      ...
      "summary": "FiD 提出把多个检索段落在编码器端独立编码、在解码器端统一交叉注意力融合的开放域问答架构，解决了多文档拼接编码开销高和逐文档生成难以聚合证据的问题。",
      "keyPoints": [
        "两阶段开放域 QA：先用 BM25 或 DPR 检索支持段落，再用生成式 seq2seq reader 生成答案",
        "输入模板明确：每个段落独立构造成 <code>question: {q} title: {t} context: {p}</code>",
      ...
      "detail": "<p><img alt=\"FiD 架构图\" src=\"https://aman.ai/images/papers/FiD.jpg\" />\n<em>图：FiD 架构。问题分别与多个 passage 拼接后独立编码，decoder 对拼接后的所有 encoder 表示做交叉注意力并生成答案。</em></p>\n<pre><code class=\"language-python\"># Fusion-in-Decoder 推理/训练流程伪代码\ndef fusion_in_decoder(question, passages, encoder, decoder, answer=None):\n    encoded_blocks = []\n\n    for passage in passages:\n        x_i = (\n            &quot;question: &quot; + question\n            + &quot; title: &quot; + passage.title\n            + &quot; context: &quot; + passage.text\n        )\n        tokens_i = tokenize(x_i, max_length=250)\n        h_i = encoder(tokens_i)          # 每个 passage 独立 self-attention\n        encoded_blocks.append(h_i)\n\n    h_all = concatenate(encoded_blocks, dim=&quot;sequence&quot;)\n\n    if answer is None:\n        return decoder.generate(encoder_hidden_states=h_all)\n\n    y = tokenize(answer)\n    logits = decoder(y[:-1], encoder_hidden_states=h_all)\n    return cross_entropy(logits, y[1:])\n</code></pre>\n<p>FiD 处理的是开放域问答：给定问题 <span class=\"kb-math kb-math-inline\">q</span>，系统先从 Wikipedia 等外部语料检索 <span class=\"kb-math kb-math-inline\">K</span> 个候选段落，再生成答案 <span class=\"kb-math kb-math-inline\">y</span>。闭卷生成模型可以把知识压进参数，但需要极大的模型；抽取式 reader 可以利用检索文本，却通常只能从单个段落抽 span。FiD 的选择是保留检索系统的显式知识，同时用生成式 decoder 在答案生成时聚合多段证据。</p>\n<p>方法上，FiD 对每个段落 <span class=\"kb-math kb-math-inline\">p_i</span> 都构造一个独立输入 <span class=\"kb-math kb-math-inline\">x_i=[q;t_i;p_i]</span>，再用共享编码器得到：</p>\n<div class=\"kb-math kb-math-display\">H_i=\\mathrm{Encoder}_{\\theta}(x_i), \\qquad\nH=\\mathrm{Concat}(H_1,H_2,\\ldots,H_K).</div>\n<p>随后 decoder 以 <span class=\"kb-math kb-math-inline\">H</span> 作为统一的 cross-attention memory，按自回归方式生成答案：</p>\n<div class=\"kb-math kb-math-display\">P(y\\mid q,p_{1:K})=\\prod_t P(y_t\\mid y_{&lt;t},H).</div>\n<p>这种设计的关键取舍是把跨段落交互推迟到 decoder。若把 <span class=\"kb-math kb-math-inline\">K</span> 个 passage 直接拼接进 encoder，self-attention 复杂度约为 <span class=\"kb-math kb-math-inline\">O((Kn)^2)</span>；FiD 则对每个长度为 <span class=\"kb-math kb-math-inline\">n</span> 的 passage 独立编码，encoder 复杂度约为 <span class=\"kb-math kb-math-inline\">O(Kn^2)</span>。由于开放域 QA 的答案通常很短，decoder 对 <span class=\"kb-math kb-math-inline\">K n</span> 个 hidden states 做 cross-attention 的额外代价可控。</p>\n<p>与 RAG 的区别在融合位置。RAG 类方法通常对每个 passage 独立计算生成概率，再按检索概率边际化；这意味着每条生成路径主要看见单个 passage。FiD 的 decoder 在同一生成步可以同时关注所有 passage 表示，因此更适合处理证据分散、需要比较多个段落或需要从多个候选中排除错误信息的问题。</p>\n<p>训练上，论文初始化 T5-base 或 T5-large，冻结或单独训练检索器不是重点；reader 直接用答案的负对数似然优化。实验中 Wikipedia 被切分为不重叠的 100-word passages，训练和测试默认检索 100 个 passage，并将每个输入截断到 250 word pieces。这个设置展示了 FiD 的核心价值：reader 的性能随着可读证据数量增加仍能提升，而不是在 10 到 20 个段落后迅速饱和。</p>\n<p>FiD 对后续 RAG 系统的影响在于明确了一个简单结构原则：检索文档可以先独立编码以控制成本，再在生成端用 cross-attention 进行深融合。后来的 RAG reader、multi-passage reranker 和轻量化 FiD 变体大多沿用了这个“独立编码、解码融合”的思想，只是在压缩 encoder 输出、选择证据 token 或加速 decoder attention 上做改进。</p>",
      "quiz": {
        "q": "FiD 为什么把多个 passage 的融合放到 decoder 而不是 encoder？",
        "options": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>损失函数</strong>：InfoNCE 对比损失，带温度参数 <span class=\"kb-math kb-math-inline\">\\tau</span>",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>分块交叉注意力（CCA）</strong>：将输入序列分成固定长度的块（chunk），每个块独立检索 <span class=\"kb-math kb-math-inline\">k</span> 个最近邻，通过 CCA 层将检索信息融入中间层表示",
      ...
      "detail": "<h5>核心架构图</h5>\n<p><img alt=\"RETRO 架构总览\" src=\"https://ar5iv.labs.arxiv.org/html/2112.04426/assets/x2.png\" />\n<em>图：RETRO 架构示意。左侧为标准 Transformer 解码器（ATTN + FFW），右侧展示了 CCA 层如何将检索编码器的输出融入解码器的中间表示。编码器接收检索到的邻居块 <span class=\"kb-math kb-math-inline\">\\text{RET}(C_u)</span> 并结合前一个块的中间激活 <span class=\"kb-math kb-math-inline\">H_u</span> 进行交叉注意力。</em></p>\n<p><img alt=\"RETRO 性能缩放\" src=\"https://ar5iv.labs.arxiv.org/html/2112.04426/assets/x1.png\" />\n<em>图：RETRO 在不同模型规模和数据库规模下的性能。检索增益在所有模型规模上保持一致，且随数据库规模增大而增大。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># RETRO 训练/推理核心流程伪代码\n# 输入: 序列 X = (x_1, ..., x_n), n=2048\n# 分块: l 个块, 每块 m=64 tokens → C = (C_1, ..., C_l), l=32\n\n# === 第一步: 检索 ===\nfor u in range(1, l+1):\n    if u == 1:\n        RET(C_1) = ∅  # 第一个块无检索(保持自回归性)\n    else:\n        # 用前一个块 C_{u-1} 的 frozen BERT 嵌入作为 query\n        query = BERT_frozen(C_{u-1})  # [d] 维向量\n        # 从数据库中用 SCaNN 近似最近邻搜索 k 个邻居\n        RET(C_u) = top_k_neighbours(query, DB, k=2)\n        # 每个邻居 = [N_j (邻居块, m tokens), F_j (后续块, m tokens)]\n\n# === 第二步: 编码检索结果 ===\nfor u in range(2, l+1):\n    for j in range(1, k+1):\n        # 拼接邻居块和后续块\n        neighbour_j = concat(N_j, F_j)  # [2m] tokens\n        # 编码器: 2层Transformer, 融合当前块的因果上下文\n        E_u_j = ENCODER(neighbour_j, ca_context=H_u)  # H_u 来自解码器中间层\n\n# === 第三步: 解码器前向传播 ===\nfor layer_idx in range(num_layers):\n    H = ATTN(H)           # 因果自注意力 (所有层)\n    if layer_idx in CCA_layers:  # 从第6层开始, 每3层一次\n        H = CCA(H, E)     # 分块交叉注意力: Q=H_u, KV=E_u\n    H = FFW(H)            # 前馈网络\n\n# 输出: 下一个 token 的概率分布\nlogits = output_head(H)\nloss = cross_entropy(logits, X_shifted)\n</code></pre>\n<h5>方法详解</h5>\n<p><strong>动机与背景</strong></p>\n<p>传统语言模型通过增加参数量来存储更多世界知识，但这导致训练和推理成本急剧增长。GPT-3（175B 参数）和 Jurassic-1（178B 参数）虽然性能强大，但资源消耗巨大。RETRO 的核心洞察是：<strong>语言模型不需要将所有知识压缩到参数中，可以通过检索外部数据库来\"按需查阅\"知识</strong>。这种半参数化方法使得 7.5B 参数的 RETRO 在多个基准上达到与 175B 参数模型相当的性能。</p>\n<div class=\"key-point\">💡 关键：RETRO 的检索增益等效于约 10 倍的参数量提升——即 RETRO 7.5B ≈ 纯参数模型 ~75B 的性能。</div>\n<p><strong>核心机制一：分块检索（Chunked Retrieval）</strong></p>\n<p>输入序列 <span class=\"kb-math kb-math-inline\">X = (x_1, \\ldots, x_n)</span> 被分成 <span class=\"kb-math kb-math-inline\">l</span> 个不重叠的块，每块 <span class=\"kb-math kb-math-inline\">m</span> 个 token：</p>\n<div class=\"kb-math kb-math-display\">C_u = (x_{(u-1)m+1}, \\ldots, x_{um}), \\quad u \\in \\{1, \\ldots, l\\}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">n = 2048</span>，<span class=\"kb-math kb-math-inline\">m = 64</span>，因此 <span class=\"kb-math kb-math-inline\">l = 32</span>。对于每个块 <span class=\"kb-math kb-math-inline\">C_u</span>（<span class=\"kb-math kb-math-inline\">u \\geq 2</span>），使用<strong>前一个块</strong> <span class=\"kb-math kb-math-inline\">C_{u-1}</span> 的冻结 BERT 嵌入作为查询，从数据库中检索 <span class=\"kb-math kb-math-inline\">k</span> 个最近邻：</p>\n<div class=\"kb-math kb-math-display\">\\text{RET}(C_u) = \\{[N_1, F_1], \\ldots, [N_k, F_k]\\}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">N_j</span> 是邻居块（<span class=\"kb-math kb-math-inline\">m</span> 个 token），<span class=\"kb-math kb-math-inline\">F_j</span> 是其后续块（<span class=\"kb-math kb-math-inline\">m</span> 个 token）。使用前一个块而非当前块作为查询，是为了<strong>严格保持自回归性</strong>——避免在生成当前块时\"偷看\"自身内容。</p>\n<div class=\"warn-box\">⚠️ 注意：第一个块 <span class=\"kb-math kb-math-inline\">C_1</span> 没有检索结果（<span class=\"kb-math kb-math-inline\">\\text{RET}(C_1) = \\emptyset</span>），因为没有前驱块可用作查询。</div>\n<p><strong>核心机制二：检索数据库（Key-Value Database）</strong></p>\n<p>数据库以键值对形式存储：\n- <strong>键（Key）</strong>：每个文本块经冻结 BERT 编码后的 <span class=\"kb-math kb-math-inline\">d</span> 维嵌入向量（取 <span class=\"kb-math kb-math-inline\">[\\text{CLS}]</span> token 的最后一层隐藏状态的平均）\n- <strong>值（Value）</strong>：对应的文本块 <span class=\"kb-math kb-math-inline\">N</span> 及其后续块 <span class=\"kb-math kb-math-inline\">F</span></p>\n<p>检索使用 L2 距离度量，通过 Google 的 SCaNN 库进行近似最近邻搜索。训练时使用约 6000 亿 Token 的数据库，评估时扩展至 1.75 万亿 Token。</p>\n<div class=\"key-point\">💡 关键：包含后续块 <span class=\"kb-math kb-math-inline\">F</span> 是因为邻居块 <span class=\"kb-math kb-math-inline\">N</span> 与查询块 <span class=\"kb-math kb-math-inline\">C_{u-1}</span> 对齐，而 <span class=\"kb-math kb-math-inline\">F</span> 才与当前待预测块 <span class=\"kb-math kb-math-inline\">C_u</span> 的内容最相关。</div>\n<p><strong>核心机制三：分块交叉注意力（Chunked Cross-Attention, CCA）</strong></p>\n<p>CCA 是 RETRO 的核心创新。在标准 Transformer 层中，每一层的计算为：</p>\n<div class=\"kb-math kb-math-display\">\\text{LM}(H) = \\text{FFW}(\\text{ATTN}(H))</div>\n<p>RETRO 在部分层中插入 CCA，变为：</p>\n<div class=\"kb-math kb-math-display\">\\text{RETRO}(H, E) = \\text{FFW}(\\text{CCA}(\\text{ATTN}(H), E))</div>\n<p>CCA 的具体计算过程如下。对于第 <span class=\"kb-math kb-math-inline\">u</span> 个块，设解码器中间激活为 <span class=\"kb-math kb-math-inline\">H_u \\in \\mathbb{R}^{m \\times d}</span>，检索编码结果为 <span class=\"kb-math kb-math-inline\">E_u \\in \\mathbb{R}^{k \\times 2m \\times d}</span>：</p>\n<div class=\"kb-math kb-math-display\">\\text{CCA}(H_u, E_u) = \\text{softmax}\\!\\left(\\frac{Q_u K_u^\\top}{\\sqrt{d_k}}\\right) V_u + H_u</div>\n<p>其中：\n- <span class=\"kb-math kb-math-inline\">Q_u = H_u W_Q \\in \\mathbb{R}^{m \\times d_k}</span>（查询来自解码器）\n- <span class=\"kb-math kb-math-inline\">K_u = E_u W_K \\in \\mathbb{R}^{(k \\cdot 2m) \\times d_k}</span>（键来自编码器输出）\n- <span class=\"kb-math kb-math-inline\">V_u = E_u W_V \\in \\mathbb{R}^{(k \\cdot 2m) \\times d_v}</span>（值来自编码器输出）</p>\n<p>CCA 层从第 6 层开始，每隔 3 层插入一次（即第 6、9、12... 层），这样低层保持纯语言建模能力，高层逐步融合检索信息。</p>\n<p><strong>核心机制四：检索编码器（Retrieval Encoder）</strong></p>\n<p>编码器是一个轻量级的 2 层双向 Transformer（约 36M 参数）。它接收拼接后的邻居文本 <span class=\"kb-math kb-math-inline\">[N_j; F_j]</span>（共 <span class=\"kb-math kb-math-inline\">2m = 128</span> 个 token），并在第二层通过交叉注意力融合来自解码器的上下文信息：</p>\n<div class=\"kb-math kb-math-display\">E_u^{(1)} = \\text{BiAttn}(\\text{Embed}([N_j; F_j]))</div>\n<div class=\"kb-math kb-math-display\">E_u^{(2)} = \\text{BiAttn}(E_u^{(1)}) + \\text{CA}(E_u^{(1)}, H_u^+)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">H_u^+</span> 是解码器在 CCA 插入点之前、第 <span class=\"kb-math kb-math-inline\">u</span> 个块的最后一个 token 的中间激活（仅用最后一个 token 以保持因果性）。</p>\n<p><strong>RETROfit：改造预训练模型</strong></p>\n<p>RETRO 支持在已预训练的标准 Transformer 上进行改造：冻结原有的 ATTN 和 FFW 参数，仅训练新增的 CCA 层和编码器参数。实验表明，仅需 3% 的预训练序列数即可恢复大部分检索增益。</p>\n<p><strong>与传统方法的区别</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>kNN-LM</th>\n<th>REALM/RAG</th>\n<th>DPR</th>\n<th><strong>RETRO</strong></th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>检索粒度</td>\n<td>Token 级</td>\n<td>文档级</td>\n<td>段落级</td>\n<td><strong>块级 (64 tokens)</strong></td>\n</tr>\n<tr>\n<td>检索器训练</td>\n<td>冻结</td>\n<td>端到端</td>\n<td>端到端</td>\n<td><strong>冻结 BERT</strong></td>\n</tr>\n<tr>\n<td>融合方式</td>\n<td>插值概率</td>\n<td>拼接输入</td>\n<td>拼接输入</td>\n<td><strong>分块交叉注意力</strong></td>\n</tr>\n<tr>\n<td>数据库规模</td>\n<td>~百亿 Token</td>\n<td>~百亿 Token</td>\n<td>~百亿 Token</td>\n<td><strong>万亿 Token</strong></td>\n</tr>\n<tr>\n<td>模型规模</td>\n<td>&lt;1B</td>\n<td>&lt;1B</td>\n<td>&lt;1B</td>\n<td><strong>最大 7.5B</strong></td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 关键：RETRO 的冻结检索器 + CCA 设计使得数据库可以独立于模型更新，且支持万亿级规模的高效检索。</div>\n<h5>关键公式汇总</h5>\n<p><strong>1. 分块定义：</strong></p>\n<div class=\"kb-math kb-math-display\">C_u = (x_{(u-1)m+1}, \\ldots, x_{um}), \\quad n = l \\cdot m</div>\n<p><strong>2. 检索距离（L2）：</strong></p>\n<div class=\"kb-math kb-math-display\">d(C_{u-1}, N) = \\|\\text{BERT}(C_{u-1}) - \\text{BERT}(N)\\|_2</div>\n<p><strong>3. CCA 注意力计算：</strong></p>\n<div class=\"kb-math kb-math-display\">\\text{CCA}(H_u, E_u) = \\text{softmax}\\!\\left(\\frac{(H_u W_Q)(E_u W_K)^\\top}{\\sqrt{d_k}}\\right)(E_u W_V) + H_u</div>\n<p><strong>4. RETRO 层公式：</strong></p>\n<div class=\"kb-math kb-math-display\">\\text{RETRO}(H, E) = \\text{FFW}\\bigl(\\text{CCA}(\\text{ATTN}(H),\\; E)\\bigr)</div>\n<p><strong>5. 标准 LM 层公式（对比）：</strong></p>\n<div class=\"kb-math kb-math-display\">\\text{LM}(H) = \\text{FFW}\\bigl(\\text{ATTN}(H)\\bigr)</div>\n<p><strong>6. 泄漏感知评估（按重叠比例 <span class=\"kb-math kb-math-inline\">\\alpha</span> 过滤）：</strong></p>\n<div class=\"kb-math kb-math-display\">\\text{bpb}_{\\text{filtered}}(\\alpha) = \\frac{\\sum_{c:\\, \\text{overlap}(c) \\leq \\alpha} \\text{bits}(c)}{\\sum_{c:\\, \\text{overlap}(c) \\leq \\alpha} \\text{bytes}(c)}</div>",
      "quiz": {
    ...
    },
    {
      ...
      "summary": "E5 通过构建并过滤大规模弱监督文本对 CCPairs，再用双塔对比学习和少量有监督微调训练通用文本嵌入，使稠密向量在零样本检索与多任务 embedding 基准上显著提升。",
      "keyPoints": [
        ...
        "角色前缀：用 <code>query:</code> 与 <code>passage:</code> 打破双塔对称性，适配非对称检索任务",
        "InfoNCE 预训练：使用余弦相似度除以温度 <span class=\"kb-math kb-math-inline\">\\tau=0.01</span>，并依赖大 batch in-batch negatives",
      ...
      "detail": "<p><img alt=\"E5 数据构建与训练流程图\" src=\"https://arxiv.org/html/2212.03533v2/x1.png\" />\n<em>图：E5 的 CCPairs 数据构建、过滤、对比预训练与有监督微调整体流程。</em></p>\n<pre><code class=\"language-python\"># E5 弱监督对比预训练与监督微调伪代码\ndef build_ccpairs(raw_sources, first_stage_encoder):\n    noisy_pairs = harvest_pairs(raw_sources)          # ~1.3B pairs\n    filtered_pairs = []\n    random_pool = sample_passages(size=1_000_000)\n\n    for q, p in noisy_pairs:\n        rank = rank_positive_among_pool(first_stage_encoder, q, p, random_pool)\n        if rank &lt;= 2:\n            filtered_pairs.append((q, p))             # ~270M pairs\n    return filtered_pairs\n\n\ndef contrastive_pretrain(encoder, ccpairs, batch_size=32768, tau=0.01):\n    for queries, passages in batches(ccpairs, batch_size):\n        q_emb = mean_pool(encoder(prefix(&quot;query: &quot;, queries)))\n        p_emb = mean_pool(encoder(prefix(&quot;passage: &quot;, passages)))\n        scores = cosine_similarity_matrix(q_emb, p_emb) / tau\n        labels = arange(len(queries))                 # 对角线为正例\n        loss = cross_entropy(scores, labels)\n        update(encoder, loss)\n\n\ndef supervised_finetune(encoder, labeled_sets, ce_teacher, alpha):\n    for query, positive, hard_negatives in labeled_sets:\n        candidates = [positive] + hard_negatives\n        stu_scores = bi_encoder_scores(encoder, query, candidates)\n        ce_scores = ce_teacher.score(query, candidates)\n        loss = kl_divergence(softmax(ce_scores), softmax(stu_scores))\n        loss += alpha * contrastive_loss(stu_scores, positive_index=0)\n        update(encoder, loss)\n</code></pre>\n<p>E5 的核心判断是：通用文本嵌入的瓶颈不一定是架构，而是训练对的规模、多样性和噪声控制。论文没有设计复杂的多塔或交互式 encoder，而是从五类网络半结构化数据中挖掘自然形成的 <span class=\"kb-math kb-math-inline\">(q,p)</span> 关系，例如问题与高赞回答、实体名与百科段落、论文标题与摘要、网页标题与正文等。这些弱监督关系覆盖问答、实体描述、主题相关和语义相似等多种 embedding 需求。</p>\n<p>原始 1.3B 文本对噪声很高，因此 E5 使用 consistency-based filter。具体做法是先用噪声数据训练一个初始 embedding 模型，再把每个候选正例 <span class=\"kb-math kb-math-inline\">p</span> 放入 1M 随机 passage 池中排序；只有当模型能把真实配对排到 top-<span class=\"kb-math kb-math-inline\">k</span> 内时才保留，论文设置 <span class=\"kb-math kb-math-inline\">k=2</span>。这个过滤的直觉是：干净弱标签会先被模型学会，而随机或错配文本对很难在大候选池中保持高排名。</p>\n<p>预训练目标是标准 InfoNCE。给定文本对集合 <span class=\"kb-math kb-math-inline\">\\{(q_i,p_i)\\}_{i=1}^{n}</span> 和负例 <span class=\"kb-math kb-math-inline\">\\{p^-_{ij}\\}_{j=1}^{m}</span>，损失为：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{cont}\n=-\\frac{1}{n}\\sum_i\n\\log\n\\frac{\\exp(s_{\\theta}(q_i,p_i))}\n{\\exp(s_{\\theta}(q_i,p_i))+\\sum_j \\exp(s_{\\theta}(q_i,p^-_{ij}))}.</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">s_{\\theta}(q,p)=\\cos(\\mathbf{E}_q,\\mathbf{E}_p)/\\tau</span>，<span class=\"kb-math kb-math-inline\">\\tau=0.01</span>。E5 使用共享 Transformer encoder 并做 mean pooling，query 和 passage 的区别主要通过文本前缀体现：<code>query:</code> 表示查询语义，<code>passage:</code> 表示待检索文本语义。这个小设计很重要，因为检索常常是非对称任务，查询短而意图明确，文档长且信息密集。</p>\n<p>负例策略上，E5 选择大 batch 的 in-batch negatives，而不是复杂的 memory bank 或 pre-batch 机制。batch size 32,768 意味着一个 query 在同一批次内能看到大量其他 passage 作为负例，模型必须学习更细粒度的语义边界。论文消融显示，batch size 从 1K 增至 32K 会带来稳定提升，这解释了为什么 E5 的方法看似简单但训练资源要求并不低。</p>\n<p>第二阶段微调把弱监督预训练得到的通用语义空间推向标注任务。E5 使用 NLI 改善语义相似与分类相关能力，用 MS-MARCO 和 NQ 改善检索能力，并为 MS-MARCO/NQ 加入硬负例与 cross-encoder teacher 分数。微调目标可写为：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{ft}\n=D_{\\mathrm{KL}}(p_{\\mathrm{ce}},p_{\\mathrm{stu}})\n+\\alpha \\mathcal{L}_{cont},</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">p_{\\mathrm{ce}}</span> 来自 cross-encoder teacher，<span class=\"kb-math kb-math-inline\">p_{\\mathrm{stu}}</span> 来自 E5 bi-encoder。cross-encoder 更慢但能精细比较 query-candidate，bi-encoder 更快但交互弱；蒸馏让 E5 在保持向量检索效率的同时学习 teacher 的排序偏好。</p>\n<p>与 Contriever 这类从裁剪文本构造正例的无监督方法相比，E5 的正例来自真实网络结构，语义关系更自然；与只在 MS-MARCO 上微调的 dense retriever 相比，E5 在预训练阶段已经学到跨任务的通用匹配能力。因此 E5 可以作为 RAG 检索器、语义搜索、聚类和分类的通用 embedding backbone，而不是只服务单一检索数据集。</p>",
      "quiz": {
        "q": "E5 中 `query:` 和 `passage:` 前缀的主要目的是什么？",
        "options": [
    ...
    },
    {
      ...
      "summary": "FLARE 提出 Forward-Looking Active Retrieval：先临时生成下一句，再用低置信度 token 判断是否检索，并把前瞻句改写成查询来重新生成，解决长文本生成中固定检索过度或单次检索不足的问题。",
      "keyPoints": [
        "<strong>主动检索框架</strong>：把 RAG 从“输入前检索一次”扩展为生成过程中的动态检索。",
        "<strong>低置信触发</strong>：当临时句中存在概率低于阈值 <span class=\"kb-math kb-math-inline\">\\theta</span> 的 token 时触发检索。",
        "<strong>前瞻式查询</strong>：用即将生成的下一句，而不是已生成历史，表达当前知识缺口。",
        "<strong>两种查询策略</strong>：FLARE_direct 掩码低置信 token 后直接检索，FLARE_instruct 让 LM 生成显式搜索查询。",
        "<strong>免训练部署</strong>：只需要语言模型 logprobs 和外部检索器，不要求额外训练检索策略。",
        "<strong>评测任务</strong>：在 2WikiMQA、StrategyQA、ASQA、WikiAsp 等长文本知识密集任务上验证效果。"
      ],
      "detail": "<p><img alt=\"FLARE 工作流程\" src=\"https://ar5iv.labs.arxiv.org/html/2305.06983/assets/x2.png\" /></p>\n<p><em>图源：ar5iv 论文图 2，展示 FLARE 先预测下一句、检测低置信 token、检索并重新生成的循环。</em></p>\n<pre><code class=\"language-python\">def flare_generate(question, lm, retriever, theta=0.5, top_k=5):\n    answer = &quot;&quot;\n    evidence = []\n    while not lm.finished(answer):\n        draft, token_probs = lm.generate_next_sentence(question, answer, evidence)\n\n        if min(token_probs) &lt; theta:\n            query = mask_low_confidence_tokens(draft, token_probs, theta)\n            docs = retriever.search(query, top_k=top_k)\n            evidence.extend(docs)\n            sentence = lm.regenerate_sentence(question, answer, evidence)\n        else:\n            sentence = draft\n\n        answer += sentence\n    return answer\n</code></pre>\n<p>FLARE 的动机来自长文本生成的动态信息需求。传统 RAG 往往在用户问题到来时检索一次，这适合答案集中在少数段落的场景；但长答案会逐句展开，模型在后续句子中才暴露新的实体、时间或事件需求。固定每句检索虽然能覆盖这些需求，却会把无关文档频繁塞进上下文，增加成本并放大噪声。</p>\n<p>核心机制是把模型自身的 token 概率当作不确定性信号。给定临时下一句 <span class=\"kb-math kb-math-inline\">\\hat{s}_t=[w_1,\\dots,w_n]</span>，FLARE 用如下条件判断是否检索：</p>\n<div class=\"kb-math kb-math-display\">\\operatorname{Retrieve}(\\hat{s}_t)=\\mathbb{1}\\left[\\exists i,\\ p(w_i\\mid x,y_{&lt;t}) &lt; \\theta\\right].</div>\n<p>这里的直觉很直接：如果模型对下一句中的某些 token 信心不足，说明它可能正在凭参数记忆猜测事实；此时用外部证据重新约束生成，比等错误写进答案后再补救更有效。</p>\n<p>查询构造体现了“forward-looking”的关键差异。回顾式检索使用已经生成的文本，容易检索到上一段话的主题；FLARE 使用临时下一句表达即将写出的内容，更贴近当前缺口。FLARE_direct 会删除或掩码低置信 token，避免错误 token 污染检索；FLARE_instruct 则让 LM 根据问题和生成历史写出搜索查询，适合复杂推理任务。</p>\n<p>推理流程是句级闭环：生成草稿、检查置信度、必要时检索、基于证据重写这一句，然后进入下一句。它与 Self-RAG 的区别是 FLARE 不训练反射 token，而是使用现成模型的 logprob；代价是它依赖可访问 token 概率的模型接口，并且阈值 <span class=\"kb-math kb-math-inline\">\\theta</span> 需要按任务调节。</p>",
      "quiz": {
        "q": "FLARE 为什么用临时生成的下一句作为检索查询？",
        "options": [
    ...
    },
    {
      ...
      "summary": "RECOMP 在检索和生成之间插入专门的压缩器，把多篇检索文档压缩成面向任务的短摘要，解决 RAG 上下文过长、无关证据干扰和推理成本过高的问题。",
      "keyPoints": [
        "<strong>实际论文页</strong>：manifest 中 <code>2310.04444</code> 指向无关提示控制论文，RECOMP 实际公开论文为 <code>https://arxiv.org/abs/2310.04408</code>。",
        "<strong>Retrieve-Compress-Prepend</strong>：先检索文档，再压缩成摘要，最后把摘要而非全文拼接给 LM。",
        "<strong>抽取式压缩器</strong>：训练双编码器按任务收益选择最有帮助的句子。",
        "<strong>生成式压缩器</strong>：蒸馏大模型的 query-focused summarization 能力，生成跨文档摘要。",
        "<strong>选择性增强</strong>：如果检索结果无帮助，压缩器可以输出空串，避免错误或无关上下文污染 LM。",
        "<strong>端任务信号训练</strong>：压缩目标不是普通摘要质量，而是摘要拼接后能否提升 LM 的语言建模或 QA 表现。"
      ],
      "detail": "<p><img alt=\"RECOMP 流程图\" src=\"https://ar5iv.labs.arxiv.org/html/2310.04408/assets/x1.png\" /></p>\n<p><em>图源：ar5iv 论文图 1，展示 RECOMP 将检索文档压缩为短摘要后再送入语言模型。</em></p>\n<pre><code class=\"language-python\">def recomp_answer(query, retriever, compressor, lm, corpus):\n    docs = retriever.search(query, corpus, top_k=5)\n    summary = compressor.compress(query=query, documents=docs)\n\n    # selective augmentation: 无帮助时允许不增强\n    if summary.strip():\n        prompt = summary + &quot;\\n\\nQuestion: &quot; + query\n    else:\n        prompt = &quot;Question: &quot; + query\n\n    return lm.generate(prompt)\n\ndef train_abstractive_compressor(train_examples, teacher_lm, base_lm):\n    targets = []\n    for query, docs, gold in train_examples:\n        summaries = [teacher_lm.summarize(query, docs, p) for p in prompt_pool]\n        best = max(summaries, key=lambda s: score(base_lm, s, query, gold))\n        if score(base_lm, best, query, gold) &lt; score(base_lm, &quot;&quot;, query, gold):\n            best = &quot;&quot;\n        targets.append((query, docs, best))\n    return finetune_encoder_decoder(targets)\n</code></pre>\n<p>RECOMP 的问题设定很现实：RAG 检索到的文档通常有数百到数千 token，直接拼接会让推理成本随检索文档长度线性上涨；更糟的是，模型并不总能从长上下文中找到关键句，甚至会被中间位置或无关文档干扰。RECOMP 因此把检索文档 <span class=\"kb-math kb-math-inline\">D=[d_1,\\dots,d_N]</span> 压缩为短摘要 <span class=\"kb-math kb-math-inline\">s=c_\\theta(x,D)</span>，再让黑盒 LM 生成 <span class=\"kb-math kb-math-inline\">p(y\\mid x,s)</span>。</p>\n<p>抽取式压缩器把每个候选句子和查询分别编码，用内积估计“把这个句子放进 prompt 后是否有助于 LM 生成目标答案”。它与普通 reranker 的差异是粒度更细：检索器通常返回段落，RECOMP 的抽取器返回句子；评分依据也不是句子与问题的语义相似度，而是句子对下游 LM 生成正确输出的因果贡献。</p>\n<p>生成式压缩器面向多文档综合。论文用强教师模型为每组 <span class=\"kb-math kb-math-inline\">(x,D,y)</span> 产生多个 query-focused 摘要，再用端任务分数过滤：如果某个摘要比无检索更差，就把目标摘要设为空串。这个“空串目标”是选择性增强的关键，它让压缩器学会在检索无用时保持沉默，而不是为了形式完整强行写摘要。</p>\n<p>与 Long Context LM 或直接 top-k 文档拼接相比，RECOMP 的优势是把信息瓶颈显式放在一个小模型里。它减少 token 成本，也降低 LM 在无关证据上浪费注意力的概率；局限是压缩器可能丢掉后续推理需要的细节，因此更适合答案证据可被短文本表达的 QA、语言建模和事实补全任务。</p>",
      "quiz": {
        "q": "RECOMP 中选择性增强的含义是什么？",
        "options": [
          "只检索图片，不检索文本",
    ...
    },
    {
      ...
      "summary": "Self-RAG 训练语言模型生成特殊反射 token，让模型在推理时自主决定是否检索，并评价检索段落相关性、回答是否被支持以及输出是否有用，从而把检索控制和事实性批判内化到一个生成模型中。",
      "keyPoints": [
        "<strong>四类反射 token</strong>：Retrieve、IsRel、IsSup、IsUse 分别控制检索、相关性、证据支持和效用评分。",
        "<strong>Critic 蒸馏</strong>：先用 GPT-4 标注反射信号，再训练 Critic 为大规模语料自动插入 token。",
        "<strong>标准 LM 训练</strong>：Generator 在带反射 token 的语料上做 next-token prediction，无需 RL。",
        "<strong>按需检索</strong>：推理时由 Retrieve token 概率决定是否调用检索器，而不是固定检索。",
        "<strong>候选证据重排</strong>：对多个检索段落并行生成候选片段，再用 IsRel/IsSup/IsUse 加权选优。",
        "<strong>可控解码</strong>：推理阶段可调节不同 critique 权重，在引用精度、事实性和流畅度之间取舍。"
      ],
      "detail": "<p><img alt=\"Self-RAG 框架\" src=\"https://ar5iv.labs.arxiv.org/html/2310.11511/assets/x1.png\" /></p>\n<p><em>图源：ar5iv 论文图 1，展示 Self-RAG 通过 Retrieve 和 critique token 控制检索与自我评估。</em></p>\n<pre><code class=\"language-python\">def self_rag(query, generator, retriever, threshold=0.2, top_k=5):\n    output = []\n    while not generator.done(output):\n        p_retrieve = generator.prob(&quot;[Retrieve]&quot;, query, output)\n\n        if p_retrieve &gt; threshold:\n            passages = retriever.search(query, top_k=top_k)\n            candidates = []\n            for passage in passages:\n                text, rel, sup, use = generator.generate_with_reflection(\n                    query=query,\n                    prefix=output,\n                    passage=passage,\n                )\n                score = (\n                    generator.logprob(text)\n                    + score_relevance(rel)\n                    + score_support(sup)\n                    + 0.5 * score_utility(use)\n                )\n                candidates.append((score, text))\n            output.append(max(candidates)[1])\n        else:\n            output.append(generator.generate_without_retrieval(query, output))\n\n    return &quot;&quot;.join(output)\n</code></pre>\n<p>Self-RAG 针对传统 RAG 的两个缺陷：第一，固定检索会让简单常识问题也携带外部文档，降低模型灵活性；第二，检索到文档后，模型通常缺少显式机制判断文档是否相关、答案是否被证据支持。Self-RAG 的做法不是外接多个分类器，而是把这些判断变成模型可生成的离散 token。</p>\n<p>训练流程分为 Critic 和 Generator 两步。Critic 先学习 GPT-4 对检索必要性、段落相关性、证据支持度和输出效用的判断，然后离线标注训练样本。Generator 的词表加入这些反射 token，在标注语料上直接优化：</p>\n<div class=\"kb-math kb-math-display\">\\max_\\theta \\sum_t \\log p_\\theta(y_t \\mid x, y_{&lt;t}),</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">y_t</span> 既可以是普通文本，也可以是 <code>[Retrieve]</code>、<code>[IsRel]</code>、<code>[IsSup]</code>、<code>[IsUse]</code> 等控制 token。</p>\n<p>推理时，Retrieve token 决定是否检索；如果需要检索，模型对每个候选段落生成回答片段及 critique token。最终分数可写成：</p>\n<div class=\"kb-math kb-math-display\">S(y,d)=\\log p_\\theta(y\\mid x,d)+w_rR_{\\text{rel}}+w_sR_{\\text{sup}}+w_uR_{\\text{use}}.</div>\n<p>这让 Self-RAG 在解码阶段能偏好“相关、被支持、有用”的片段，而不是只看语言模型概率。</p>\n<p>与 FLARE 依赖 logprob 阈值不同，Self-RAG 通过训练显式学会何时检索和如何批判证据，因此不需要人工设计低置信规则；代价是需要构造反射 token 标注和训练 Generator。它适合事实性要求高、需要引用或长答案分段生成的场景。</p>",
      "quiz": {
        "q": "Self-RAG 的 IsSup token 主要用于判断什么？",
        "options": [
          "是否需要扩大模型参数量",
          "生成内容是否被检索证据支持",
          "检索器是否使用 BM25",
    ...
    },
    {
      ...
      "summary": "GraphRAG 用 LLM 从私有语料中抽取实体、关系和声明，构建图索引并预生成社区摘要，再用 map-reduce 式查询聚合回答全局问题，解决向量 RAG 难以回答“整个语料主题是什么”这类跨文档总结问题。",
      "keyPoints": [
        "<strong>图索引构建</strong>：从文本块中抽取 entity、relationship、claim，并汇总为图元素描述。",
        "<strong>社区检测</strong>：用 Leiden 等算法把实体图划分为层级社区。",
        "<strong>社区摘要</strong>：为每个社区预生成 report-like summary，形成可复用的全局语料记忆。",
        "<strong>全局查询流程</strong>：每个社区摘要先独立回答问题，再把相关中间答案汇总成最终回答。",
        "<strong>目标场景</strong>：面向 global sensemaking 和 query-focused summarization，而不是单段事实查找。",
        "<strong>优势与成本</strong>：提升答案全面性和多样性，但索引阶段需要较多 LLM 抽取与摘要调用。"
      ],
      "detail": "<p><img alt=\"GraphRAG 社区结构示意\" src=\"https://ar5iv.labs.arxiv.org/html/2404.16130/assets/Level0Multihop.jpg\" /></p>\n<p><em>图源：ar5iv 论文图 3 的 Level 0 社区可视化，展示 GraphRAG 将实体图划分为可摘要的社区。</em></p>\n<pre><code class=\"language-python\">def build_graphrag_index(documents, llm):\n    chunks = split_documents(documents, chunk_size=600)\n    graph = Graph()\n    for chunk in chunks:\n        entities, relations, claims = llm.extract_graph_elements(chunk)\n        graph.add_entities(entities)\n        graph.add_relations(relations)\n        graph.add_claims(claims)\n\n    graph.merge_and_summarize_elements(llm)\n    communities = leiden_hierarchy(graph)\n    reports = {}\n    for level, groups in communities.items():\n        for group in groups:\n            reports[(level, group.id)] = llm.summarize_community(group)\n    return graph, reports\n\ndef global_search(query, community_reports, llm, token_budget):\n    partials = []\n    for batch in pack_reports(community_reports, token_budget):\n        answer, helpfulness = llm.answer_from_reports(query, batch)\n        if helpfulness &gt; 0:\n            partials.append((helpfulness, answer))\n    context = select_by_score(partials, token_budget)\n    return llm.reduce_answers(query, context)\n</code></pre>\n<p>GraphRAG 的核心判断是：普通向量 RAG 擅长找到“局部答案所在段落”，但不擅长回答需要全语料综合的问题。例如“这个数据集中有哪些主要主题？”没有单个 chunk 能代表答案，语义检索很容易只召回几个局部片段。GraphRAG 把任务改写为 query-focused summarization，通过图社区覆盖整个语料。</p>\n<p>索引阶段先由 LLM 从每个 chunk 抽取实体、关系和声明。与传统知识图谱追求严格三元组不同，GraphRAG 更强调 LLM 可读的丰富描述：节点和边都带自然语言摘要，claim 也可以作为 covariate 附着到实体上。这样即便抽取结果有命名不一致，后续社区摘要仍能把同一主题附近的信息聚合起来。</p>\n<p>社区检测利用图的模块性。GraphRAG 把实体关系图看成无向加权图，边权通常来自关系实例计数或强度，再用 Leiden 得到层级社区。每个社区覆盖一组强相关实体、关系和声明；社区摘要相当于把局部子图压缩成可检索、可阅读的全局记忆。</p>\n<p>查询阶段是 map-reduce：map 步骤让每个社区摘要独立回答问题并给出有用度分数，reduce 步骤按分数选择中间答案并生成最终回答。这样做的优势是并行、可扩展，并且避免把整个语料塞进单个上下文；不足是索引成本较高，而且社区摘要质量受 LLM 抽取和社区划分影响。</p>",
      "quiz": {
        "q": "GraphRAG 相比普通向量 RAG 最适合解决哪类问题？",
        "options": [
          "只需要查找单个精确事实的短问题",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>小模型选择</strong>：GPT-2-Alpaca（117M）或 Alpaca-7B 作为压缩代理模型 <span class=\"kb-math kb-math-inline\">\\mathcal{M}_s</span>",
    ...
    },
    {
      ...
      "summary": "RAGAS 提出了一套面向 RAG 三元组 \\((q, c(q), a_s(q))\\) 的无参考自动评估框架，用 LLM-as-judge 将忠实度、答案相关性、上下文相关性拆成可执行的子判断，解决 RAG 系统缺少人工参考答案时难以持续评估的问题。",
      "keyPoints": [
        "<strong>RAG 三元组评估</strong>：仅依赖问题 <span class=\"kb-math kb-math-inline\">q</span>、检索上下文 <span class=\"kb-math kb-math-inline\">c(q)</span>、系统答案 <span class=\"kb-math kb-math-inline\">a_s(q)</span>，不要求 ground truth answer。",
        "<strong>Faithfulness 忠实度</strong>：先把答案拆成原子事实声明，再逐条判断声明能否由上下文推出，用于检测基于上下文的事实幻觉。",
        "<strong>Answer Relevance 答案相关性</strong>：从答案反向生成若干可能问题，再计算这些问题与原始问题的嵌入相似度，惩罚答非所问、信息缺失和冗余回答。",
        "<strong>Context Relevance 上下文相关性</strong>：让 LLM 从检索上下文中抽取回答问题所需的关键句，按关键句占全部上下文句子的比例估计检索噪声。",
        "<strong>结构化 LLM-as-judge</strong>：避免让 LLM 一次性给整体质量打分，而是把评估拆成声明生成、NLI 判断、问题生成、句子抽取等更稳定的局部任务。",
        "<strong>WikiEval 验证集</strong>：论文构造 50 个基于 2022 年后 Wikipedia 页面的样本，并用人工偏好比较验证指标与人类判断的一致性。",
        "<strong>实验结论</strong>：在 WikiEval 成对比较中，RAGAS 在 Faithfulness、Answer Relevance、Context Relevance 上分别达到 0.95、0.78、0.70 的人工一致率，优于直接 GPT Score 和 GPT Ranking。"
      ],
      "detail": "<p><img alt=\"RAGAS 评估框架图\" src=\"https://assets.zilliz.com/large_Mar_18_RAG_Evaluation_using_Ragas_20240318_080304_62e448ec81.png\" />\n<em>图：公开 RAGAS 框架示意图，展示 RAG 评估围绕问题、上下文、答案和可选参考答案展开；论文核心关注无需参考答案的 Faithfulness、Answer Relevance、Context Relevance 三项指标。</em></p>\n<pre><code class=\"language-python\"># RAGAS 三元组评估伪代码\ndef evaluate_ragas(question, context, answer, llm, embedder, n_questions=3):\n    # 1. Faithfulness: answer -&gt; statements -&gt; supported / unsupported\n    statements = llm.extract_atomic_statements(question=question, answer=answer)\n    supported = 0\n    for statement in statements:\n        verdict = llm.verify_statement(context=context, statement=statement)\n        if verdict == &quot;Yes&quot;:\n            supported += 1\n    faithfulness = supported / max(len(statements), 1)\n\n    # 2. Answer Relevance: answer -&gt; reverse questions -&gt; embedding similarity\n    generated_questions = [\n        llm.generate_question_from_answer(answer)\n        for _ in range(n_questions)\n    ]\n    q_vec = embedder.encode(question)\n    answer_relevance = mean(\n        cosine_similarity(q_vec, embedder.encode(q_i))\n        for q_i in generated_questions\n    )\n\n    # 3. Context Relevance: context -&gt; answer-supporting sentences\n    relevant_sentences = llm.extract_relevant_sentences(\n        question=question,\n        context=context,\n    )\n    total_sentences = sentence_count(context)\n    context_relevance = len(relevant_sentences) / max(total_sentences, 1)\n\n    return {\n        &quot;faithfulness&quot;: faithfulness,\n        &quot;answer_relevance&quot;: answer_relevance,\n        &quot;context_relevance&quot;: context_relevance,\n    }\n</code></pre>\n<p>RAGAS 的出发点是生产环境里的 RAG 往往没有标准答案。传统 QA 指标通常假设有人工标注答案或可抽取短答案，但真实 RAG 系统的输出是长文本，质量同时取决于检索器是否取回了聚焦上下文，以及生成器是否正确利用了这些上下文。论文因此把评估对象固定成 <span class=\"kb-math kb-math-inline\">(q, c(q), a_s(q))</span>：问题、检索到的上下文、系统生成答案。这样做的价值是可以直接接入线上日志，对每次 RAG 调用做自动诊断，而不是等人工标注集积累完再评估。</p>\n<p>Faithfulness 是最核心的幻觉检测指标。RAGAS 不直接问 LLM “这个答案是否忠实”，而是先让 LLM 把答案拆成短而集中的 statements，再逐条用上下文做蕴含判断。若答案声明集合为 <span class=\"kb-math kb-math-inline\">S</span>，被上下文支持的声明集合为 <span class=\"kb-math kb-math-inline\">V</span>，则：</p>\n<div class=\"kb-math kb-math-display\">F = \\frac{|V|}{|S|}</div>\n<p>这个设计的关键直觉是把复杂主观判断变成多个二元 NLI 式判断。一个长答案可能大部分正确、局部幻觉；整体打分容易掩盖局部错误，而 statement 级验证能定位具体不被上下文支持的事实，从而给 RAG 调参与失败分析提供更细粒度信号。</p>\n<p>Answer Relevance 只衡量“答得是否切题”，刻意不检查事实正确性。RAGAS 采用反向问题生成：如果答案确实围绕原问题，那么从答案生成的问题 <span class=\"kb-math kb-math-inline\">q_i</span> 应该与原问题 <span class=\"kb-math kb-math-inline\">q</span> 语义接近。论文用文本嵌入计算平均余弦相似度：</p>\n<div class=\"kb-math kb-math-display\">AR = \\frac{1}{n}\\sum_{i=1}^{n} \\operatorname{sim}(q, q_i)</div>\n<p>这种间接评估避免了让 LLM 凭感觉给“相关性”打分。答案若遗漏问题关键约束，反向生成的问题会变得更宽泛；答案若夹带冗余事实，生成的问题也可能偏离原始意图，最终拉低平均相似度。</p>\n<p>Context Relevance 则面向检索器。它要求 LLM 从 <span class=\"kb-math kb-math-inline\">c(q)</span> 中抽取真正有助于回答 <span class=\"kb-math kb-math-inline\">q</span> 的句子集合 <span class=\"kb-math kb-math-inline\">S_{ext}</span>，再用关键句数占总句数的比例近似上下文聚焦程度：</p>\n<div class=\"kb-math kb-math-display\">CR = \\frac{|S_{ext}|}{\\text{number of sentences in } c(q)}</div>\n<p>该指标的方向不是“检索结果越多越好”，而是惩罚噪声上下文。对于长上下文 RAG，冗余段落会增加 token 成本，也可能让模型忽略中间位置的关键信息；因此 Context Relevance 直接反映检索结果是否足够精炼。论文也指出这是三项指标里最难稳定判断的一项，因为长上下文中的关键句抽取对 LLM 本身要求更高。</p>\n<p>WikiEval 的构造用于检验这些指标是否真能对齐人类偏好。论文选取 50 个 2022 年以后发生事件相关的 Wikipedia 页面，生成可由页面引言回答的问题，再构造高低质量对比样本：无上下文回答用于制造低忠实度答案，不完整回答用于测试答案相关性，反向链接和补全内容用于制造冗余上下文。两名英语流利标注者独立比较样本，最终 RAGAS 在忠实度上达到 0.95 的人工一致率，显著高于让 ChatGPT 直接 0-10 打分或直接排序。</p>\n<div class=\"key-point\">💡 关键：RAGAS 的贡献不只是“用 LLM 当裁判”，而是提出了一套把 RAG 质量拆解为可审计中间步骤的评估流程。它牺牲了一点调用复杂度，换来更高的可解释性和更可靠的线上诊断能力。</div>",
      "quiz": {
        "q": "RAGAS 的 Faithfulness 指标为什么要把答案拆成 statements 再验证？",
        "options": [
    ...
    },
    {
      ...
      "summary": "LinearRAG 提出 relation-free Tri-Graph，用实体、句子和段落三类节点替代昂贵且不稳定的关系抽取，再通过局部语义桥接和全局重要性聚合完成多跳检索，解决 GraphRAG 索引成本高、关系噪声大和可扩展性差的问题。",
      "keyPoints": [
        "<strong>实际论文页</strong>：manifest 中 <code>2412.14833</code> 指向骨架动作识别论文；LinearRAG 实际公开论文为 <code>https://arxiv.org/abs/2510.10114</code>。",
        "<strong>Tri-Graph 索引</strong>：构建 entity、sentence、passage 三层节点，仅保留“提及/包含”边。",
        "<strong>避开关系抽取</strong>：不用 OpenIE 或 LLM 抽取三元组，减少错误关系和额外 token 成本。",
        "<strong>局部语义桥接</strong>：在 entity-sentence 子图上传播查询语义，激活隐含中间实体。",
        "<strong>全局重要性聚合</strong>：在 entity-passage 子图上用 Personalized PageRank 聚合 passage 重要性。",
        "<strong>线性扩展</strong>：句子切分、NER、稀疏邻接矩阵和 SpMM/PPR 让构建与检索近似随语料规模线性增长。"
      ],
      "detail": "<p><img alt=\"LinearRAG 框架图\" src=\"https://ar5iv.labs.arxiv.org/html/2510.10114/assets/x1.png\" /></p>\n<p><em>图源：ar5iv 论文图 1，展示 LinearRAG 的 relation-free 图构建与两阶段检索思路。</em></p>\n<pre><code class=\"language-python\">def build_trigraph(passages):\n    graph = TriGraph()\n    for passage in passages:\n        p_node = graph.add_passage(passage)\n        for sentence in split_sentences(passage):\n            s_node = graph.add_sentence(sentence)\n            for ent in ner(sentence):\n                e_node = graph.add_entity(ent)\n                graph.add_edge(e_node, s_node)  # mention matrix M\n                graph.add_edge(e_node, p_node)  # contain matrix C\n    return graph\n\ndef linearrag_retrieve(query, graph, embedder, top_k=5, delta=4):\n    seed_entities = match_query_entities(query, graph.entities)\n    activation = initialize(seed_entities)\n\n    # Stage 1: local semantic bridging on entity-sentence graph\n    for _ in range(4):\n        activation = sparse_semantic_propagation(activation, graph.M, embedder)\n        activation = prune_below_threshold(activation, delta)\n        if no_new_entities(activation):\n            break\n\n    # Stage 2: global importance aggregation on entity-passage graph\n    ppr_scores = personalized_pagerank(graph.C, seeds=activation)\n    return top_passages_by_score(ppr_scores, top_k)\n</code></pre>\n<p>LinearRAG 的出发点是对 GraphRAG 做“减法”。许多 GraphRAG 系统会先把文本抽成实体-关系三元组，再在图上做推理；但关系抽取常常把否定、条件、上下文省略或层级关系抽错。一旦错误关系进入图，检索会沿着这些边扩散，把语义相关但事实无关的段落带入上下文。</p>\n<p>Tri-Graph 只抽取实体并保留原文段落，不显式抽取关系。图中有三类节点：passage 保存完整上下文，sentence 作为局部语义桥，entity 作为跨段落锚点。两类稀疏矩阵分别表示实体被哪些句子提及、实体出现在哪些段落中。这样关系语义仍留在原文里，由生成模型在读证据时解释，而不是提前压缩成可能错误的三元组。</p>\n<p>检索第一阶段是 entity activation。查询中的实体先成为种子，然后在 entity-sentence 子图中传播相似度，找到没有字面出现在查询里、但通过句子语义连接多跳推理链的中间实体。阈值 <span class=\"kb-math kb-math-inline\">\\delta</span> 控制扩散边界，避免实体激活指数级膨胀。</p>\n<p>第二阶段是 passage retrieval。LinearRAG 把激活实体作为 Personalized PageRank 的种子，在 entity-passage 子图中聚合全局重要性。一个 passage 的分数既来自与查询的直接相似度，也来自它包含多少高激活实体以及这些实体在图中的位置。最终 top-k passage 被送给 LLM 生成答案。</p>\n<p>与 GraphRAG 相比，LinearRAG 的核心优势是“结构足够、关系留白”。它仍然利用实体连接跨文档信息，但不让不可靠的关系抽取决定推理路径；因此更适合大规模语料、多跳 QA 和需要频繁增量更新的企业知识库。</p>",
      "quiz": {
        "q": "LinearRAG 为什么刻意避免显式关系抽取？",
        "options": [
          "因为实体节点完全不能用于检索",
    ...
    },
    {
      ...
      "summary": "TM-RAG 将 Transformer 的局部精细注意力与 Mamba 的长程状态建模结合，并用 CAGF 动态融合和多层级对比学习增强证据聚合，解决长证据 RAG 中全局语义弱、细粒度事实对齐不足的问题。",
      "keyPoints": [
        "<strong>Transformer-Mamba 编码</strong>：Transformer 捕捉局部证据交互，Mamba 以线性序列建模处理长程依赖。",
        "<strong>CAGF 动态融合</strong>：通过 Cross-Attention Gated Fusion 类模块自适应融合两路特征。",
        "<strong>多层级对比学习</strong>：包含句级、槽位级和 token 级 masked-recovery contrastive learning。",
        "<strong>长文本证据聚合</strong>：目标是避免检索只偏向主题相似，而忽略事实一致性和关键槽位。",
        "<strong>评测覆盖</strong>：在中文左宗棠历史数据集、HotpotQA、MuSiQue、SQuAD 等任务上验证。",
        "<strong>公开来源限制</strong>：Springer 页面可访问摘要与元数据，正文图未暴露稳定图片直链；下方使用官方文章页作为公开图源链接。"
      ],
      "detail": "<p><img alt=\"TM-RAG 官方文章页图源\" src=\"https://link.springer.com/article/10.1007/s44443-026-00723-5\" /></p>\n<p><em>图源：Springer Open Access 文章页；该页说明 TM-RAG 由 Transformer-Mamba 编码、CAGF 融合和多层级对比目标组成。</em></p>\n<pre><code class=\"language-python\">def tm_rag_train(query, positive_evidence, negative_evidence, encoder, generator):\n    evidence = positive_evidence + negative_evidence\n\n    transformer_states = encoder.transformer_branch(query, evidence)\n    mamba_states = encoder.mamba_branch(query, evidence)\n    fused = cagf_gate(transformer_states, mamba_states)\n\n    sent_loss = contrastive_sentence(fused, positive_evidence, negative_evidence)\n    slot_loss = contrastive_slots(fused, extract_slots(positive_evidence))\n    token_loss = masked_recovery_contrastive(fused, mask_entity_time_place_slots(evidence))\n    gen_loss = generator.nll(query, positive_evidence)\n\n    return gen_loss + sent_loss + slot_loss + token_loss\n\ndef tm_rag_answer(query, retriever, tm_encoder, generator):\n    docs = retriever.search(query, top_k=20)\n    fused_context = tm_encoder.aggregate_long_evidence(query, docs)\n    return generator.generate(query, fused_context)\n</code></pre>\n<p>TM-RAG 关注的是长证据聚合，而不是单纯检索召回。真实 RAG 场景里，top-k 文档可能跨越很长上下文，普通 dense retrieval 容易根据主题相似召回材料，却没有足够机制判断“这些材料是否共同支持同一个事实”。Transformer 能做细粒度 token 交互，但在长序列上成本高；Mamba 的选择性状态空间模型更适合线性处理长程依赖，却不如注意力直观地建模局部证据对齐。</p>\n<p>混合编码器把两者分工：Transformer 分支处理查询和证据之间的局部交互，捕捉实体、时间、地点、动作等关键槽位；Mamba 分支沿长序列传播状态，保留跨段落的全局语义。CAGF 融合模块相当于一个动态门控，按样本决定更信任局部注意力还是长程状态，而不是简单拼接两路特征。</p>\n<p>多层级对比学习用于让编码器不只“读过证据”，还要区分事实支持关系。句级对比拉近查询与正证据句，推远负证据句；槽位级对比关注 subject、time、place、action 等结构化事实槽；token 级 masked recovery 则把被遮蔽的关键 token 与原始证据表示对齐，迫使模型保留细粒度事实。</p>\n<p>在 RAG 推理中，TM-RAG 的输出可以看作经过长证据聚合的上下文表示：</p>\n<div class=\"kb-math kb-math-display\">h_{\\text{fused}} = g_{\\text{CAGF}}\\left(h_{\\text{Transformer}}, h_{\\text{Mamba}}\\right),</div>\n<p>再由生成器基于 <span class=\"kb-math kb-math-inline\">h_{\\text{fused}}</span> 回答。它与 Self-RAG 的关系在于都强调“不要盲信检索结果”，但 Self-RAG 用反射 token 批判检索内容，TM-RAG 则在编码层面强化长程证据一致性。</p>",
      "quiz": {
        "q": "TM-RAG 中引入 Mamba 分支的主要目的是什么？",
        "options": [
          "替代所有检索器",
          "以更适合长序列的方式建模跨段落长程依赖",
          "把文本转换成图片",
    ...
    },
    {
      ...
      "summary": "VideoRAG 把 RAG 扩展到小时级、多视频语料，通过图式文本知识 grounding 和多模态上下文编码双通道索引视频片段，让 LLM 能从极长视频集合中检索视觉、语音和文本证据后生成回答。",
      "keyPoints": [
        "<strong>实际论文页</strong>：manifest 中 <code>2501.09885</code> 指向无关超导论文；这里依据 <code>https://arxiv.org/abs/2502.01549</code> 的 Extreme Long-Context VideoRAG。",
        "<strong>双通道架构</strong>：graph-based textual knowledge grounding + multi-modal context encoding。",
        "<strong>视频切片处理</strong>：把任意长视频切为片段，抽取 ASR 文本、采样帧和 VLM caption。",
        "<strong>跨视频知识图谱</strong>：用 LLM 从 caption/transcript 中抽取实体和关系，支持多视频知识连接。",
        "<strong>多模态检索</strong>：同时利用文本语义、图结构和视觉 embedding 找到相关片段。",
        "<strong>LongerVideos 基准</strong>：包含 160+ 视频、134+ 小时，覆盖课程、纪录片、娱乐内容。"
      ],
      "detail": "<p><img alt=\"VideoRAG 框架图\" src=\"https://ar5iv.labs.arxiv.org/html/2502.01549/assets/x1.png\" /></p>\n<p><em>图源：ar5iv 论文图 1，展示 VideoRAG 的视频知识索引、多模态检索和最终生成流程。</em></p>\n<pre><code class=\"language-python\">def index_videos(video_list, asr, vlm, text_encoder, multimodal_encoder):\n    graph = KnowledgeGraph()\n    clip_store = []\n    for video in video_list:\n        for clip in split_video(video, seconds=30):\n            transcript = asr.transcribe(clip.audio)\n            frames = sample_frames(clip, k=10)\n            caption = vlm.caption(frames, transcript)\n            text_chunk = merge(caption, transcript, clip.timestamp)\n\n            entities, relations = llm_extract_graph(text_chunk)\n            graph.update(entities, relations, source=clip.id)\n            clip_store.append({\n                &quot;clip&quot;: clip,\n                &quot;text_vec&quot;: text_encoder(text_chunk),\n                &quot;video_vec&quot;: multimodal_encoder(frames, transcript),\n            })\n    return graph, clip_store\n\ndef videorag_query(query, graph, clip_store, generator):\n    graph_hits = graph.retrieve_related_chunks(query)\n    visual_hits = multimodal_search(query, clip_store)\n    evidence = rerank_and_merge(graph_hits, visual_hits)\n    return generator.answer(query, evidence)\n</code></pre>\n<p>VideoRAG 的关键难点是视频不是普通长文档。它同时包含视觉帧、语音、字幕、场景变化和跨片段时间依赖；如果只把视频转写成文本，视觉细节会丢失；如果只把帧塞进长视频模型，小时级视频会遇到上下文和计算瓶颈。因此 VideoRAG 使用双通道索引：文本图谱保留可符号化的知识关系，视觉编码保留难以文本化的场景信息。</p>\n<p>索引阶段先把视频切成短片段。每个片段通过 ASR 得到 transcript，通过采样帧和 VLM 得到 caption，再把二者合并为结构化文本。随后 LLM 从文本中抽取实体和关系，增量构建跨视频知识图谱；同时，文本编码器和多模态编码器分别保存文本向量与视觉/音频上下文向量。</p>\n<p>检索阶段不只做单一路径相似度搜索。对于查询 <span class=\"kb-math kb-math-inline\">q</span>，VideoRAG 可以在图谱中找到相关实体和关系，也可以在多模态 embedding 空间中匹配视觉片段。最终证据由两路候选合并、重排后提供给生成模型：</p>\n<div class=\"kb-math kb-math-display\">\\operatorname{Answer}=\\operatorname{LLM}\\left(q,\\ \\psi_{\\text{text-graph}}(q,G)\\cup\\psi_{\\text{multi-modal}}(q,E_v)\\right).</div>\n<p>这套设计特别适合跨视频问题，例如“某系列课程里某概念第一次在哪一集解释、后续如何展开”。传统 LVLM 可能只能看固定帧窗口，文本 RAG 又看不到画面；VideoRAG 通过知识图谱连接多个视频片段，再用视觉检索补齐具体画面证据。</p>",
      "quiz": {
        "q": "VideoRAG 为什么需要同时使用图式文本 grounding 和多模态编码？",
        "options": [
    ...
    },
    {
      ...
      "summary": "BayesRAG 将多模态检索重写为贝叶斯证据融合问题，用查询相关 likelihood、跨模态一致性 prior 和 Dempster-Shafer 证据理论共同计算后验置信度，解决文本、图像和版面证据高相似但互相矛盾的问题。",
      "keyPoints": [
        "<strong>多模态证据 tuple</strong>：把文本、视觉元素和页面/截图布局组合成候选证据单元。",
        "<strong>贝叶斯后验排序</strong>：以 <span class=\"kb-math kb-math-inline\">P(E\\mid Q)\\propto P(Q\\mid E)P(E)</span> 重排检索结果。",
        "<strong>Dempster-Shafer likelihood</strong>：融合不同模态的相关性质量函数，处理不确定和冲突证据。",
        "<strong>一致性 prior</strong>：用 graph-topology prior 或 layout prior 衡量文本-图像是否天然属于同一证据单元。",
        "<strong>冲突惩罚</strong>：对高单模态相似但跨模态语义不一致的候选降权。",
        "<strong>适用场景</strong>：面向图文混排、表格图表丰富的长文档 QA，如 DocBench、MMLongBench-Doc。"
      ],
      "detail": "<p><img alt=\"BayesRAG 架构图\" src=\"https://ar5iv.labs.arxiv.org/html/2601.07329/assets/x1.png\" /></p>\n<p><em>图源：ar5iv 论文图 1，展示 BayesRAG 把多模态检索候选通过 likelihood、prior 和 posterior 进行证据融合。</em></p>\n<pre><code class=\"language-python\">def bayesrag_rank(query, text_hits, image_hits, page_hits, graph_or_layout):\n    candidates = make_evidence_tuples(text_hits, image_hits, page_hits)\n    ranked = []\n    for E in candidates:\n        # likelihood: 各模态与 query 的相关性，经 Dempster-Shafer 融合\n        masses = [mass_function(similarity(query, item)) for item in E]\n        likelihood = dempster_shafer_combine(masses).belief(&quot;relevant&quot;)\n\n        # prior: tuple 内部是否互相支持\n        if graph_or_layout.type == &quot;graph&quot;:\n            prior = graph_topology_consistency(E, graph_or_layout)\n        else:\n            prior = layout_proximity(E, graph_or_layout)\n\n        posterior = likelihood * prior\n        ranked.append((posterior, E))\n    return [E for _, E in sorted(ranked, reverse=True)]\n</code></pre>\n<p>BayesRAG 针对视觉丰富文档中的“bag-of-evidence”问题。普通多模态 RAG 往往分别检索文本、图片和页面，再把 top-k 合并；但高相似并不等于互相支持。例如文本候选可能说水果 apple，图像候选却是 Apple 公司 logo，二者都与查询相似，却组合成错误证据。</p>\n<p>论文将候选证据表示为 <span class=\"kb-math kb-math-inline\">E=(e_{\\text{text}}, e_{\\text{vision}}, e_{\\text{screenshot}})</span>，目标是估计：</p>\n<div class=\"kb-math kb-math-display\">P(E\\mid Q)\\propto P(Q\\mid E)P(E).</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">P(Q\\mid E)</span> 是 evidence tuple 对查询的解释能力，来自各模态 embedding 相似度；<span class=\"kb-math kb-math-inline\">P(E)</span> 是证据内部一致性，即这些文本、图片和页面元素在语义或布局上是否本来就应该关联。</p>\n<p>likelihood 部分使用 Dempster-Shafer 证据理论。每个模态根据相似度给出“相关/不相关/不确定”的质量函数，组合规则会显式处理冲突：多个模态一致支持时 belief 上升，彼此矛盾时联合置信度下降。这比简单平均相似度更适合多模态噪声场景。</p>\n<p>prior 部分有两种实现。Graph-topology prior 把文档元素构成多模态知识图，优先选择在图中连接强、语义一致的 tuple；layout prior 使用页面坐标和邻近关系，认为同页相邻的图文更可能互相解释。最终 posterior 重排让 BayesRAG 优先选择“既与查询相关，又相互 corroborate”的证据。</p>",
      "quiz": {
        "q": "BayesRAG 中 prior P(E) 主要表示什么？",
        "options": [
          "语言模型参数的先验分布",
    ...
    },
    {
      ...
      "summary": "SSRAG 提出 query augmentation、agentic routing、vector + graph hybrid retrieval 和 context unification 的混合框架，通过语义检索与结构化实体关系检索互补，解决标准 RAG 容易检索偏移、上下文不完整和事实性不足的问题。",
      "keyPoints": [
        "<strong>查询增强</strong>：抽取实体、意图、时间线索，扩展缩写并规范别名。",
        "<strong>Agentic Query Routing</strong>：用 LLM 判断 factual/temporal，把查询路由到 Wikipedia、Google API 等合适来源。",
        "<strong>混合检索</strong>：并行执行向量检索和图检索，兼顾语义相似与关系推理。",
        "<strong>Context Unification</strong>：把图结果线性化成文本向量，与向量候选统一重排、去重、截断。",
        "<strong>工程实现</strong>：FAISS 负责 dense retrieval，Neo4j/Cypher 负责实体关系子图检索。",
        "<strong>评测数据</strong>：TruthfulQA、SQuAD、WikiQA，覆盖五类 LLM 和 RAGAS/ROUGE/BLEU/事实性指标。"
      ],
      "detail": "<p><img alt=\"SSRAG 架构图\" src=\"https://ar5iv.labs.arxiv.org/html/2601.12658/assets/Learning_to_RAG_Architecture.png\" /></p>\n<p><em>图源：ar5iv 论文图 1，展示 SSRAG 的查询增强、路由、混合检索和上下文统一流程。</em></p>\n<pre><code class=\"language-python\">def ssrag_answer(query, llm, vector_index, graph_db, web_api, wiki_db, k=20):\n    augmented = enhance_query(\n        query,\n        entities=extract_entities(query),\n        intent=detect_intent(query),\n        aliases=canonicalize_aliases(query),\n    )\n\n    route = llm.classify_route(augmented, labels=[&quot;TEMPORAL&quot;, &quot;FACTUAL&quot;])\n    source = web_api if route == &quot;TEMPORAL&quot; else wiki_db\n\n    vector_hits = vector_index.search(augmented, source=source, top_k=k)\n    subgraph = graph_db.retrieve_entities_and_relations(augmented, source=source)\n    graph_texts = llm.linearize_graph(subgraph)\n    graph_vectors = embed(graph_texts)\n\n    candidates = rerank_by_cosine([*vector_hits, *graph_vectors], augmented)\n    unified = deduplicate(candidates, by=[&quot;exact_match&quot;, &quot;cosine&quot;])\n    context = take_top_k(unified, k)\n    return llm.generate(query, context)\n</code></pre>\n<p>SSRAG 的动机是标准向量 RAG 容易出现 semantic drift：查询和文档 embedding 相似，但证据并不完整，或者缺少关键实体关系。Graph RAG 能表达结构关系，却可能漏掉语义近似但图中未显式连接的材料。SSRAG 把两者做成流水线，而不是二选一。</p>\n<p>查询增强是前置的稳定化步骤。系统先识别查询中的实体、意图和时间敏感性，再扩展缩写与别名，例如把 “RL” 改写成 “reinforcement learning”。这一步降低了短查询、歧义查询和别名不一致导致的召回失败。</p>\n<p>路由模块把检索源选择显式化。对于“最新突破”“今天”“当前价格”等 temporal 查询，系统路由到实时 Web；对于历史事实或百科知识，则路由到 Wikipedia 或预构建语料。路由可以表示为：</p>\n<div class=\"kb-math kb-math-display\">r=\\operatorname{Router}(Q_{\\text{aug}})\\in\\{\\text{TEMPORAL},\\text{FACTUAL}\\}.</div>\n<p>这比所有查询都用同一个索引更稳，因为检索源的新鲜度和可信度需求并不相同。</p>\n<p>混合检索阶段并行运行向量检索和图检索。图检索返回实体、关系和子图路径，随后被 LLM 转成文本表示并嵌入到同一向量空间；系统把 graph-to-text 候选和 dense 候选统一重排，选 top-2k 后去重，再截断为最终上下文。这样既能保留结构化关系，也能在最终 prompt 中以 LLM 可读文本呈现。</p>",
      "quiz": {
        "q": "SSRAG 中 Agentic Query Routing 的核心作用是什么？",
        "options": [
          "把所有查询强制送到同一个向量库",
    ...
    },
    {
      ...
      "summary": "ViG-RAG 为长视频构建带时间戳和置信度的概率时间知识图谱（PTKG），再用语义-时间双层检索、GMM 自适应筛选和 VLM 证据融合生成答案，解决长视频 RAG 中片段割裂、时间关系缺失和静态文本匹配不准的问题。",
      "keyPoints": [
        "<strong>实际论文页</strong>：manifest 中 AAAI <code>30471</code> 是语音增强学生摘要；ViG-RAG 实际 AAAI 2026 页面为 <code>https://ojs.aaai.org/index.php/AAAI/article/view/36963</code>。",
        "<strong>PTKG 表示</strong>：事实以 <span class=\"kb-math kb-math-inline\">(h,r,t,\\tau,p)</span> 表示，额外包含时间标记和置信度。",
        "<strong>多模态内容抽取</strong>：视频切片后抽取 ASR、采样帧和 VLM caption，再由 LLM 抽取实体、关系、时间、置信度。",
        "<strong>语义-时间双层检索</strong>：Text-F 判断语义相关，Temp-F 判断时间一致和长程依赖。",
        "<strong>GMM 动态 Top-K</strong>：根据相似度分布自适应区分高置信候选，避免手工阈值。",
        "<strong>插件式增强</strong>：可作为辅助模块接入 Video-LLaVA、LongVA、Qwen2-VL、LLaVA-Video 等 LVLM。"
      ],
      "detail": "<p><img alt=\"ViG-RAG 官方 PDF 图源\" src=\"https://ojs.aaai.org/index.php/AAAI/article/download/36963/40925\" /></p>\n<p><em>图源：AAAI 2026 官方 PDF，Figure 2 展示 ViG-RAG 将视频转成 PTKG，并通过语义-时间检索和 query-aware generation 生成答案。</em></p>\n<pre><code class=\"language-python\">def build_ptkg(videos, asr, vlm, llm):\n    ptkg = []\n    for video in videos:\n        for segment in split_video(video, seconds=30):\n            transcript = asr(segment.audio)\n            frames = sample_frames(segment, k=10)\n            caption = vlm.describe(frames, transcript)\n            facts = llm.extract_quintuples(caption, transcript)\n            # fact = (head, relation, tail, timestamp, plausibility)\n            ptkg.extend(facts)\n    return merge_cross_video_facts(ptkg)\n\ndef vig_rag_query(query, ptkg, visual_index, lvml):\n    textual_candidates = retrieve_by_entities_and_anchors(query, ptkg)\n    scored = []\n    for segment in textual_candidates:\n        sem = text_f(segment, query)\n        temp = temp_f(segment, query, ptkg)\n        scored.append((alpha * sem + (1 - alpha) * temp, segment))\n\n    selected = gmm_select_high_confidence(scored)\n    frames = retrieve_visual_frames(query, selected, visual_index)\n    return lvml.generate(query, semantic_anchors=selected, frames=frames)\n</code></pre>\n<p>ViG-RAG 的核心是把视频片段从孤立 chunk 变成带时间和不确定性的图事实。普通视频 RAG 可能只把 transcript 或 caption 当文本检索，无法表达“某实体在某时间段做了什么，置信度多高”。PTKG 用 <span class=\"kb-math kb-math-inline\">(h,r,t,\\tau,p)</span> 同时编码关系、时间和 plausibility，更适合视频中事件随时间展开的场景。</p>\n<p>索引阶段先把长视频切成固定片段，提取语音转写和视觉描述。LLM 随后从每个片段中抽取实体、关系、时间信息和置信分数，合并成跨视频 PTKG。这个图既是文本检索索引，也是时间推理结构，使模型可以沿实体和时间线找证据，而不是只看静态相似度。</p>\n<p>检索阶段包含语义和时间两种过滤。Text-F 判断片段文本是否回答查询；Temp-F 判断片段是否处在正确时间范围、是否与前后事件连贯。两者加权后得到候选分数：</p>\n<div class=\"kb-math kb-math-display\">s(S,q)=\\alpha\\operatorname{TextF}(S,q)+(1-\\alpha)\\operatorname{TempF}(S,q).</div>\n<p>由于不同查询的分数分布不同，固定 top-k 或固定阈值会不稳。ViG-RAG 用 Gaussian Mixture Model 拟合候选相似度分布，自动选择高置信簇；随后再由 VLM/LVLM 整合 semantic anchors、上下文字段和选中视频帧生成答案。</p>",
      "quiz": {
        "q": "ViG-RAG 的 PTKG 相比普通知识图谱多编码了哪些关键信息？",
        "options": [
          "只多编码模型参数量",
          "时间标记和事实置信度",
          "只多编码图像分辨率",
    ...
    },
    {
      ...
      "summary": "URaG 在单一多模态 LLM 内部统一检索与生成：利用早期 Transformer 层的粗粒度证据定位能力选择相关页面，再让深层只处理保留页面，从而在长文档理解中同时提升准确率和效率。",
      "keyPoints": [
        "<strong>统一架构</strong>：不外接独立 retriever，而是在 MLLM 内部加入轻量 cross-modal retrieval module。",
        "<strong>粗到细观察</strong>：早层广泛关注整份文档，深层更集中到相关证据页。",
        "<strong>早层检索</strong>：把早期 hidden states 映射后与查询计算相似度，选择 top-k 页面/视觉 token。",
        "<strong>深层生成</strong>：丢弃不相关视觉 token，让深层 Transformer 专注证据页面。",
        "<strong>效率收益</strong>：AAAI 摘要报告计算开销降低 44-56%。",
        "<strong>官方图源</strong>：作者 GitHub 提供 URaG framework 和 layer study 图。"
      ],
      "detail": "<p><img alt=\"URaG 框架图\" src=\"https://github.com/shi-yx/URaG/raw/main/figures/urag_framework.jpg\" /></p>\n<p><em>图源：URaG 官方 GitHub，展示早层特征进入 cross-modal retrieval module，筛选 top-k 页面后送入深层生成。</em></p>\n<pre><code class=\"language-python\">def urag_forward(document_pages, query, mllm, retriever_head, k):\n    visual_tokens = mllm.vision_encoder(document_pages)\n    query_tokens = mllm.text_embed(query)\n\n    early_states = mllm.run_early_layers(visual_tokens, query_tokens)\n    page_vectors = retriever_head.map_visual_pages(early_states.visual)\n    query_vector = retriever_head.map_query(early_states.query)\n\n    sim = cosine_similarity(page_vectors, query_vector)\n    selected_pages = top_k_pages(sim, k)\n    pruned_tokens = keep_tokens(visual_tokens, selected_pages)\n\n    deep_states = mllm.run_deep_layers(pruned_tokens, query_tokens)\n    return mllm.decode_answer(deep_states)\n</code></pre>\n<p>URaG 的出发点是多模态长文档理解中的两个瓶颈：无关页面造成信息干扰，Transformer 对长视觉 token 的计算成本近似二次增长。外接 retriever 能筛页面，但会增加系统复杂度，而且检索目标和生成模型不一定端到端一致。</p>\n<p>论文的关键观察是 MLLM 本身呈现 coarse-to-fine 规律：早期层对文档页面的注意力比较广，可以作为粗检索信号；深层注意力逐渐集中到回答所需页面。因此 URaG 把早层隐藏状态拿出来做页面级相似度计算，让模型“边推理边检索”。</p>\n<p>Cross-modal retrieval module 是轻量映射头。它把视觉页面 token 和文本查询 token 投到同一相似度空间，计算页面分数：</p>\n<div class=\"kb-math kb-math-display\">s_i=\\cos\\left(W_v h_i^{\\text{page}}, W_q h^{\\text{query}}\\right),</div>\n<p>然后保留 top-k 页面对应的视觉 token。被丢弃的页面不进入后续深层，从而节省计算并减少干扰。</p>\n<p>与 token compression 的区别是，URaG 不是把所有页面压成短摘要，而是显式选择证据页面，保留被选页面的细粒度视觉信息。与外部检索器相比，它共享 MLLM 的视觉编码和查询表示，更容易和生成目标一致；局限是需要在特定 MLLM 结构上插入并训练/适配检索模块。</p>",
      "quiz": {
        "q": "URaG 为什么使用 MLLM 的早期层做检索？",
        "options": [
          "早期层已经生成最终答案",
    ...
    },
    {
      ...
      "summary": "MG-CRAG 在 CRAG 的检索纠错思想上引入段落级 PLRE 与句子级 SLRE 两个检索评估器，并用人工少量标注、Autoencoder 聚类伪标签和残差分类头弱监督训练，让 RAG 在更细粒度上过滤噪声上下文并减少不必要的 Web 搜索。",
      "keyPoints": [
        "<strong>多粒度纠正式 RAG</strong>：先用 passage-level retrieval evaluator (PLRE) 评估整段/文档，再用 sentence-level retrieval evaluator (SLRE) 评估句子级 evidence strip。",
        "<strong>弱监督四阶段训练</strong>：Retrieval → Manual Labeling → T5-GTR Embedding + Autoencoder/K-Means Clustering → Classification Head Training。",
        "<strong>三类质量标签</strong>：将候选上下文分为 high、medium、low；high 直接保留，medium 作为可补充证据，low 被过滤。",
        "<strong>T5-GTR + QNLI Prompt 编码</strong>：把 query-document 或 query-sentence 对组织成 QNLI 风格输入，再映射到 768 维向量。",
        "<strong>Autoencoder-guided pseudo-labeling</strong>：Autoencoder 将 768 维向量压到低维表示，K-Means 生成伪标签，再用少量人工标注将簇映射到 high/medium/low。",
        "<strong>Residual classification head</strong>：冻结或复用高效文本编码器表示，用带 9 个残差块的全连接分类头学习检索质量分类，降低对大规模标注和全模型微调的依赖。",
        "<strong>可调推理模式</strong>：strict、moderate、lenient 三种模式控制 PLRE/SLRE 的通过条件，在准确率、召回率和 Web 搜索调用率之间折中。",
        "<strong>实验收益</strong>：论文在 ARC-Challenge、PubHealth、PopQA 上验证，报告 ARC-Challenge 68.85% accuracy、PopQA 59.89% accuracy，并强调在 PubHealth 上以更低 Web 搜索率保持相当结果。"
      ],
      "detail": "<p><img alt=\"MG-CRAG 训练阶段图\" src=\"https://github.com/omidacoder/mg-crag/raw/main/images/train_phase.png\" />\n<em>图：MG-CRAG 官方项目公开的训练阶段图，展示检索、少量人工标注、T5-GTR 嵌入与聚类、分类头训练四阶段。</em></p>\n<p><img alt=\"MG-CRAG 推理阶段图\" src=\"https://github.com/omidacoder/mg-crag/raw/main/images/inference_phase.png\" />\n<em>图：MG-CRAG 官方项目公开的推理阶段图，展示 PLRE/SLRE 多粒度过滤、模式分支、重排序、Web 搜索补充与最终生成。</em></p>\n<pre><code class=\"language-python\"># MG-CRAG 训练与推理伪代码\ndef train_mg_crag_evaluators(queries, retriever, human_labeler):\n    # Stage 1: retrieval\n    pairs = []\n    for q in queries:\n        docs = retriever.retrieve(q, top_k=N)  # MS Contriever in the paper\n        pairs.extend((q, doc) for doc in docs)\n\n    # Stage 2: manual labeling on a small subset\n    labeled_pairs = human_labeler.label_subset(pairs)\n    unlabeled_pairs = [p for p in pairs if p not in labeled_pairs]\n\n    # Stage 3: embedding and clustering\n    all_pairs = labeled_pairs + unlabeled_pairs\n    embeddings = [\n        t5_gtr_encode(f&quot;qnli question: {q} sentence: {text}&quot;)\n        for q, text in all_pairs\n    ]  # each vector has dimension 768\n    z = autoencoder.fit_transform(\n        embeddings,\n        supervised_labels=labeled_pairs.labels,\n        losses=[&quot;reconstruction&quot;, &quot;classification&quot;],\n    )\n    cluster_ids = kmeans(z, k=3)\n    pseudo_labels = map_clusters_to_quality_labels(\n        cluster_ids,\n        labeled_pairs.labels,\n        labels=[&quot;low&quot;, &quot;medium&quot;, &quot;high&quot;],\n    )\n\n    # Stage 4: train classification heads for passage-level and sentence-level scoring\n    plre = residual_classifier.fit(embeddings, pseudo_labels)\n    slre = residual_classifier.fit(sentence_level_embeddings(all_pairs), pseudo_labels)\n    return plre, slre\n\n\ndef infer_mg_crag(query, retriever, plre, slre, reranker, web_search, generator, mode):\n    docs = retriever.retrieve(query, top_k=N)\n\n    high_docs, medium_docs = [], []\n    for doc in docs:\n        label = plre.predict(encode_qnli(query, doc))\n        if label == &quot;high&quot;:\n            high_docs.append(doc)\n        elif label == &quot;medium&quot;:\n            medium_docs.append(doc)\n\n    candidate_sentences = split_sentences(high_docs + medium_docs)\n    high_sentences, medium_sentences = [], []\n    for sent in candidate_sentences:\n        label = slre.predict(encode_qnli(query, sent))\n        if label == &quot;high&quot;:\n            high_sentences.append(sent)\n        elif label == &quot;medium&quot;:\n            medium_sentences.append(sent)\n\n    if mode == &quot;strict&quot;:\n        evidence = only_sentences_from_high_docs(high_sentences, high_docs)\n    elif mode == &quot;moderate&quot;:\n        evidence = high_sentences + medium_sentences\n    elif mode == &quot;lenient&quot;:\n        evidence = high_sentences + medium_sentences + sentences_from(medium_docs)\n\n    evidence = reranker.top_m(query, evidence)\n    if not evidence or len(high_sentences) &lt;= WEB_SEARCH_THRESHOLD:\n        web_docs = web_search(query_rewrite(query))\n        web_sentences = slre_filter_and_rerank(query, web_docs, slre, reranker)\n        evidence = reranker.top_m(query, evidence + web_sentences)\n\n    return generator.generate(query=query, evidence=evidence)\n</code></pre>\n<p>MG-CRAG 的问题设定来自 CRAG：检索器会把不相关或弱相关文本送进生成器，导致答案被噪声污染；CRAG 通过检索评估器和外部搜索做纠正，但单一粒度的评估器容易把“段落整体相关但内部有噪声句子”与“段落整体一般但包含关键句子”混为一谈。MG-CRAG 的核心改动是把纠正机制拆成两个粒度：PLRE 先在段落/文档层面做粗筛，SLRE 再在句子层面做精筛，从而让生成器看到的是更聚焦的 evidence。</p>\n<p>训练阶段的关键是弱监督。论文没有假设存在大规模 high/medium/low 标注，而是先用 MS Contriever 为 ARC-Challenge、PubHealth、PopQA 等短答案任务检索候选文档，人工只标注一小部分 query-document 对；随后 T5-GTR 用 QNLI 风格 prompt 编码每个 pair，Autoencoder 在重建损失下保留语义结构，同时借助少量标注样本的分类损失让瓶颈表示更有判别性。K-Means 在低维表示上聚成 3 类，再由人工标注子集把簇映射为 high、medium、low，形成可扩展的伪标签。</p>\n<p>分类头采用残差全连接网络，而不是对大型生成模型做端到端微调。论文附录说明输入是 T5-GTR 的 768 维输出，先投影到 2048 维并加 dropout，核心部分是 9 个残差块；每个残差块包含线性变换、LeakyReLU 和 dropout，维度变化时用线性 down-sampling 对齐 shortcut。这个结构的作用是让轻量分类头在伪标签上学习检索质量边界，同时保持梯度稳定和较低训练成本。</p>\n<p>推理时，MG-CRAG 不是简单地把所有 high/medium 文本塞给 LLM。PLRE 先把文档分成 high、medium、low，low 被丢弃；保留下来的文档被拆成句子后再由 SLRE 分类，随后根据 strict、moderate、lenient 三种模式选择 evidence。strict 追求高精度，倾向于只保留高置信文档里的高置信句子；moderate 接受 high 与 medium 证据以平衡召回；lenient 更偏向保留可能有用的 medium 文档内容，适合检索较难或开放域问题。</p>\n<p>Web 搜索在 MG-CRAG 中变成一种受控补救动作，而不是默认依赖。推理图中先对高质量句子做 reranking；若没有 high evidence，或 high evidence 数量低于阈值 <span class=\"kb-math kb-math-inline\">w_s</span>，系统才通过 query rewriting 触发 Web 搜索，再把搜索结果交给 SLRE 和 reranker 过滤。这样既保留了 CRAG 的纠错能力，又避免每次查询都付出外部搜索成本。</p>\n<p>实验部分表明，多粒度处理对短答案问答尤其有用。论文报告 MG-CRAG 在 ARC-Challenge 上达到 68.85% accuracy，在 PopQA 上达到 59.89% accuracy；在 PubHealth 上结果与强基线相当，同时 Web 搜索率更低。这个结果说明 MG-CRAG 的收益不只是提升回答准确率，也包括把“何时需要外部搜索”变成可调策略，从而控制成本和延迟。</p>\n<div class=\"key-point\">💡 关键：MG-CRAG 的创新点不是单独的 reranker 或单独的弱监督分类，而是把多粒度质量评估、伪标签训练、模式化 evidence 选择和按需 Web 搜索组合成一条纠正式 RAG 流程。</div>",
      "quiz": {
        "q": "MG-CRAG 为什么同时使用 PLRE 和 SLRE 两个检索评估器？",
        "options": [
    ...
    },
    {
      ...
      "summary": "Qwen3-Embedding 基于 Qwen3 foundation model 构建文本嵌入和重排模型系列，使用双编码器、交叉编码器、多阶段训练和 instruction-aware 输入，面向多语言、长上下文、检索与 RAG 场景提供高质量向量表示。",
      "keyPoints": [
        "<strong>模型系列</strong>：Embedding 和 Reranker 均提供 0.6B、4B、8B 三种规模。",
        "<strong>32K 上下文</strong>：嵌入与重排模型均支持长上下文输入，适合长文档检索。",
        "<strong>双编码器嵌入</strong>：Embedding 模型用最后 <code>[EOS]</code> hidden state 表示单段文本。",
        "<strong>交叉编码器重排</strong>：Reranker 输入 query-document pair，输出相关性分数。",
        "<strong>MRL 支持</strong>：Embedding 支持自定义最终向量维度，便于不同存储/延迟预算部署。",
        "<strong>多语言能力</strong>：支持 100+ 语言和代码检索，官方博客报告 8B 在 MTEB multilingual leaderboard 得分约 70.58。"
      ],
      "detail": "<p><img alt=\"Qwen3-Embedding 训练流程\" src=\"https://ar5iv.labs.arxiv.org/html/2506.05176/assets/figures/q3e-train-pipeline.png\" /></p>\n<p><em>图源：Qwen3-Embedding 技术报告的公开 ar5iv 页面，展示 Qwen3-Embedding 与 Qwen3-Reranker 的三阶段训练流程。</em></p>\n<pre><code class=\"language-python\">def embed_text(text, instruction, qwen3_embedding):\n    prompt = format_instruction(instruction, text)\n    states = qwen3_embedding.forward(prompt)\n    return normalize(states[&quot;eos_hidden_state&quot;])\n\ndef rerank(query, documents, qwen3_reranker):\n    scored = []\n    for doc in documents:\n        pair = format_pair(query, doc)\n        score = qwen3_reranker.cross_encoder_score(pair)\n        scored.append((score, doc))\n    return [doc for score, doc in sorted(scored, reverse=True)]\n\ndef rag_with_qwen3(query, corpus):\n    q_vec = embed_text(query, &quot;Represent this query for retrieval&quot;, qwen3_embedding)\n    candidates = vector_search(q_vec, corpus, top_k=100)\n    reranked = rerank(query, candidates, qwen3_reranker)\n    return llm_answer(query, reranked[:10])\n</code></pre>\n<p>Qwen3-Embedding 的工程定位是 RAG 检索栈中的 first-stage dense retriever 与 second-stage reranker。Embedding 模型采用双编码器，查询和文档可独立编码并存入向量库，适合大规模 ANN 检索；Reranker 采用 cross-encoder，推理更慢但能逐对建模 query-document 交互，适合重排 top-100 候选。</p>\n<p>嵌入模型的表示取最后 <code>[EOS]</code> token 的隐藏状态。这与许多 decoder-only embedding 模型一致：把整段文本通过自回归 backbone 编码后，用句末位置聚合语义。模型还支持 instruction-aware 输入，例如为“法律检索”“代码搜索”“跨语言问答”定制不同指令，从而让同一文本在不同任务下产生更合适的向量。</p>\n<p>训练分三阶段。第一阶段用大规模弱监督/合成 pair 做 contrastive pre-training，强化通用语义对齐；第二阶段用高质量标注数据做监督训练，提升检索任务表现；第三阶段通过采样 checkpoint merging 融合候选模型，改善泛化。Reranker 则主要使用高质量标注数据做监督训练，以提高 query-document 精细相关性判断。</p>\n<p>在 RAG 系统中，Qwen3-Embedding 的价值不只是分数高，还在于 32K 长上下文、多语言、代码检索和可调维度。实际部署中常见组合是：8B/4B embedding 负责高召回，0.6B 或 4B reranker 按延迟预算重排；若向量库成本敏感，可利用 MRL 输出较短维度向量。</p>",
      "quiz": {
        "q": "Qwen3-Embedding 与 Qwen3-Reranker 在检索栈中的典型分工是什么？",
        "options": [
    ...
    },
    {
      ...
      "summary": "RAGUARD 构建面向事实核查的误导性检索基准，把 PolitiFact 声明与 Reddit 讨论证据配对并标注 supporting、misleading、irrelevant，用来评估 RAG 系统在真实冲突证据下是否比零检索更可靠。",
      "keyPoints": [
        "<strong>实际论文页</strong>：manifest 中 <code>2410.20992</code> 指向无关信道估计论文；RAGuard 实际公开论文为 <code>https://arxiv.org/abs/2502.16101</code>。",
        "<strong>任务定位</strong>：不是评估干净 gold retrieval，而是评估 RAG 对误导性证据的鲁棒性。",
        "<strong>真实噪声来源</strong>：检索语料来自 Reddit 讨论，捕捉自然发生的错误、偏见、片面叙事和冲突信息。",
        "<strong>三类证据标签</strong>：supporting、misleading、irrelevant，区分支持、误导和无关上下文。",
        "<strong>数据构造</strong>：从 PolitiFact 收集政治声明和真伪标签，用 GPT-4 扩展关键词并检索 Reddit 文档。",
        "<strong>核心发现</strong>：多种 LLM-RAG 系统在误导性检索下表现低于 zero-shot/no retrieval 基线。"
      ],
      "detail": "<p><img alt=\"RAGuard 数据构造流程\" src=\"https://ar5iv.labs.arxiv.org/html/2502.16101/assets/figures/newconstruct.png\" /></p>\n<p><em>图源：ar5iv 论文图 4，展示 RAGuard 从事实核查声明、Reddit 检索和 LLM 辅助标注构造基准。</em></p>\n<pre><code class=\"language-python\">def build_raguard(politifact_claims, google_search, reddit_corpus, gpt4):\n    dataset = []\n    for claim, gold_label in politifact_claims:\n        keywords = gpt4.extract_keywords(claim)\n        docs = google_search(site=&quot;reddit.com&quot;, query=keywords, top_k=10)\n        for doc in docs:\n            predicted = gpt4.fact_check(claim, context=doc)\n            if predicted == gold_label:\n                tag = &quot;supporting&quot;\n            elif doc_irrelevant_to_claim(doc, claim):\n                tag = &quot;irrelevant&quot;\n            else:\n                tag = &quot;misleading&quot;\n            dataset.append((claim, gold_label, doc, tag))\n    return dataset\n\ndef evaluate_rag_guard(model, claim, retriever, mode):\n    if mode == &quot;zero_context&quot;:\n        context = []\n    elif mode == &quot;standard_rag&quot;:\n        context = retriever.search(claim, top_k=5)\n    elif mode == &quot;misleading_only&quot;:\n        context = gold_associated_docs(claim, tag=&quot;misleading&quot;)\n    return model.fact_check(claim, context)\n</code></pre>\n<p>RAGuard 的基本质疑是：RAG 并不总是提升可靠性。许多基准假设检索文档是 gold 或只有合成噪声，模型只要“利用上下文”就能得分；但真实网络检索常包含片面、过时、政治化或故意误导的信息。RAGuard 因此把检索本身变成压力测试，而不是默认可信环节。</p>\n<p>数据集从 PolitiFact 获取政治声明及真伪标签，并把多级真伪压缩为二分类。随后用 GPT-4 提取关键词，通过搜索引擎检索 Reddit 讨论，形成更接近真实网络环境的证据池。Reddit 的价值在于它包含自然出现的支持、反驳、误解、夸张和无关讨论，而不是人为注入的简单噪声。</p>\n<p>证据标注采用“模拟 LLM 考试”的方式：给定 claim 和单篇 retrieved document，让 GPT-4 基于该文档判断 claim 的真伪。如果判断与 PolitiFact gold label 一致，则文档是 supporting；如果文档让判断偏离 gold，则是 misleading；如果文档无法提供有效核查信息，则是 irrelevant。这一定义关注文档对 RAG 系统行为的实际影响。</p>\n<p>评测设置包括 zero-context、standard RAG、oracle associated documents 和 misleading-only 等模式。核心指标不是召回率越高越好，而是模型能否识别上下文可能错误。论文报告的关键现象是：当提供误导性文档时，多数系统准确率明显低于无检索基线，说明“更多上下文”可能压倒模型原有判断。</p>",
      "quiz": {
        "q": "RAGuard 与普通干净检索 QA 基准的主要区别是什么？",
        "options": [
          "RAGuard 只测试图片分类",
