/**
 * device-data.js — 由 pipeline/build.py 于 2026-05-20 17:59:40 自动生成。
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>提前退出</strong>：基于熵阈值 \\(E_T\\) 预测退出层，SST-2 任务平均仅需 3.9 层（vs 12 层），延迟降低 6.13×",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>首次全量化 FP4 训练</strong>：同时将权重 \\(W\\)、激活 \\(a\\)、梯度 \\(\\delta\\) 量化为 FP4，覆盖训练中全部三个 GEMM（Forward / Backward / Update）",
        ...
        "<strong>理论分析</strong>：证明当梯度标准差降至 \\(\\sqrt{3} \\cdot \\sigma_q\\) 以下时 FP4 训练失效，为 QAF 切换时机提供理论依据",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>双阶段网络建模</strong>：Stage 1 构建逻辑流量矩阵（与拓扑无关），Stage 2 映射到物理拓扑并执行路由仿真，比 NS-3 快 \\(10^5\\)× 且误差仅 2.12%",
