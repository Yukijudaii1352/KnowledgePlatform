/**
 * sound_generation-data.js — 由 pipeline/build.py 于 2026-05-13 12:20:49 自动生成。
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>输出帧缩减技巧</strong>：每个解码步预测 \\(r\\) 帧（reduction factor），大幅加速训练收敛和推理速度",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>多周期判别器（MPD）</strong>：由 5 个子判别器组成，分别以素数周期 \\([2, 3, 5, 7, 11]\\) 对 1D 波形重塑为 2D 后用 2D 卷积判别，捕获不同周期的隐式结构",
        ...
        "<strong>三重损失函数</strong>：LSGAN 对抗损失 + 特征匹配损失（\\(\\lambda_{fm}=2\\)）+ Mel 频谱图 L1 损失（\\(\\lambda_{mel}=45\\)）",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>残差向量量化（RVQ）</strong>：使用级联多层码本（每层 1024 个码字 = 10 bits），通过选择不同数量的量化层（\\(N_q = 2 \\sim 32\\)）实现可变比特率（1.5/3/6/12/24 kbps），单一模型支持多带宽",
