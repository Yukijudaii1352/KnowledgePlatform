/**
 * mm_sound-data.js — 由 pipeline/build.py 于 2026-06-15 18:08:27 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "detail": "<h3>系统架构示意图</h3>\n<div class=\"img-wrap\"><img src=\"../../content/mm/mm_sound/assets/assets/i_vector_architecture.png\" alt=\"i-vector 说话人验证系统架构\" loading=\"lazy\"><p class=\"img-caption\">▲ i-vector 说话人验证系统架构</p></div>\n<h3>算法伪代码</h3>\n<pre><code>算法: i-vector 提取与说话人验证\n\n========== 离线训练阶段 ==========\n\n输入: 大规模训练语音集合 {u₁, u₂, ..., uN}, 每段标注说话人ID\n输出: UBM参数 λ, 全变分矩阵 T, 后端模型参数\n\n1. [训练 UBM]\n   对所有训练数据提取 MFCC 特征 (含 Δ, ΔΔ, 通常 60 维)\n   用 EM 算法训练 C 个分量的 GMM → 得到 UBM: λ = {πc, μc, Σc}_{c=1}^{C}\n   拼接所有均值得到超向量: m = [μ₁ᵀ, μ₂ᵀ, ..., μCᵀ]ᵀ  (CF × 1)\n\n2. [训练全变分矩阵 T]  (EM 迭代)\n   随机初始化 T (CF × R), 其中 R ≪ CF (典型 R=400, CF≈30000)\n   FOR iter = 1 to max_iter:\n     // E-step: 对每段语音 u 计算后验统计量\n     FOR each utterance u:\n       计算零阶统计量: Nc(u) = Σ_t γ_t(c)           对每个高斯 c\n       计算一阶统计量: Fc(u) = Σ_t γ_t(c) · o_t     对每个高斯 c\n       中心化: F̃c(u) = Fc(u) - Nc(u) · μc\n       构造对角矩阵: N(u) = diag(N₁(u)·I_F, ..., NC(u)·I_F)\n       后验协方差: L(u) = (I + Tᵀ Σ⁻¹ N(u) T)⁻¹\n       后验均值:   E[w(u)] = L(u) · Tᵀ · Σ⁻¹ · F̃(u)\n     END FOR\n     // M-step: 更新 T 矩阵 (按高斯分量分块更新)\n     FOR c = 1 to C:\n       Ac = Σ_u Nc(u) · (L(u) + E[w(u)]·E[w(u)]ᵀ)\n       Bc = Σ_u F̃c(u) · E[w(u)]ᵀ\n       Tc = Bc · Ac⁻¹          // 更新第 c 个分块\n     END FOR\n   END FOR\n\n3. [训练后端补偿]\n   对所有训练语音提取 i-vector\n   训练 LDA 投影矩阵 A (R → d 维, 典型 d=200)\n   训练 WCCN 归一化矩阵 B\n   (可选) 训练 PLDA 模型\n\n========== 在线测试阶段 ==========\n\n输入: 注册语音 u_enroll, 测试语音 u_test\n输出: 验证得分 score, 决策 (接受/拒绝)\n\n4. [提取 i-vector]\n   FOR each utterance u ∈ {u_enroll, u_test}:\n     提取 MFCC 特征序列 {o₁, ..., oT}\n     计算 Baum-Welch 统计量: Nc(u), F̃c(u)\n     w(u) = (I + Tᵀ Σ⁻¹ N(u) T)⁻¹ · Tᵀ · Σ⁻¹ · F̃(u)   // i-vector\n   END FOR\n\n5. [信道补偿]\n   w' = B · A · w                    // LDA 降维 + WCCN 归一化\n   w' = w' / ‖w'‖                    // 长度归一化\n\n6. [评分与决策]\n   score = cos(w'_enroll, w'_test)   // 余弦评分\n   // 或: score = PLDA_score(w'_enroll, w'_test)\n   IF score &gt; θ THEN 接受 ELSE 拒绝\n</code></pre>\n<h3>方法详解</h3>\n<h4>1. 从 JFA 到全变分空间：动机与建模</h4>\n<p>传统的<strong>联合因子分析（JFA）</strong>将 GMM 超向量分解为说话人子空间和信道子空间两个独立部分：</p>\n<div class=\"kb-math kb-math-display\">M(u) = m + V \\cdot y(s) + U \\cdot x(u) + D \\cdot z(s)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">V</span> 建模说话人变化，<span class=\"kb-math kb-math-inline\">U</span> 建模信道变化。JFA 的核心假设是说话人因子 <span class=\"kb-math kb-math-inline\">y</span> 和信道因子 <span class=\"kb-math kb-math-inline\">x</span> 可以被独立估计。然而 Dehak 等人通过实验发现，<strong>信道子空间 <span class=\"kb-math kb-math-inline\">U</span> 中实际上也包含了大量说话人信息</strong>——直接用信道因子 <span class=\"kb-math kb-math-inline\">x</span> 做说话人识别竟然也能取得不错的效果。这一发现促使作者提出了一个更简洁的模型：</p>\n<div class=\"kb-math kb-math-display\">M(u) = m + T \\cdot w(u)</div>\n<p>这里只使用一个<strong>全变分矩阵 <span class=\"kb-math kb-math-inline\">T</span></strong>（Total Variability Matrix），将说话人变化和信道变化统一投影到同一个低维子空间中。向量 <span class=\"kb-math kb-math-inline\">w(u)</span> 被称为<strong>身份向量（identity vector, i-vector）</strong>，它是一个低维（通常 <span class=\"kb-math kb-math-inline\">R = 400</span> 维）的固定长度表示，包含了该段语音中所有与说话人和信道相关的信息。信道信息的去除被推迟到后端处理阶段，这种\"前端统一建模 + 后端补偿\"的策略被证明比 JFA 的前端分离更加有效。</p>\n<h4>2. 全变分矩阵 T 的训练：EM 算法</h4>\n<p>全变分矩阵 <span class=\"kb-math kb-math-inline\">T</span> 的估计采用<strong>期望最大化（EM）</strong>算法，其推导与 JFA 中信道子空间矩阵的训练完全类似。设 UBM 有 <span class=\"kb-math kb-math-inline\">C</span> 个高斯分量，每个分量的特征维度为 <span class=\"kb-math kb-math-inline\">F</span>（如 60 维 MFCC），则超向量维度为 <span class=\"kb-math kb-math-inline\">CF</span>（典型值约 <span class=\"kb-math kb-math-inline\">512 \\times 60 = 30720</span>），而 i-vector 维度 <span class=\"kb-math kb-math-inline\">R</span> 通常取 400 或 600。</p>\n<p><strong>E-step</strong>：对每段训练语音 <span class=\"kb-math kb-math-inline\">u</span>，首先利用 UBM 计算 Baum-Welch 充分统计量——零阶统计量 <span class=\"kb-math kb-math-inline\">N_c(u)</span> 表示第 <span class=\"kb-math kb-math-inline\">c</span> 个高斯分量的占有率，一阶统计量 <span class=\"kb-math kb-math-inline\">F_c(u)</span> 表示加权特征累积。然后计算 i-vector 的后验分布：</p>\n<div class=\"kb-math kb-math-display\">\\mathbb{E}[w(u)] = \\left(I + T^\\top \\Sigma^{-1} N(u) T\\right)^{-1} T^\\top \\Sigma^{-1} \\tilde{F}(u)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\Sigma</span> 是 UBM 的对角协方差矩阵拼接而成的块对角矩阵，<span class=\"kb-math kb-math-inline\">N(u)</span> 是由零阶统计量构成的对角矩阵，<span class=\"kb-math kb-math-inline\">\\tilde{F}(u)</span> 是中心化后的一阶统计量。由于 <span class=\"kb-math kb-math-inline\">\\Sigma</span> 和 <span class=\"kb-math kb-math-inline\">N(u)</span> 都是（块）对角的，矩阵求逆的实际计算复杂度仅为 <span class=\"kb-math kb-math-inline\">O(R^3)</span> 而非 <span class=\"kb-math kb-math-inline\">O((CF)^3)</span>，这使得大规模训练成为可能。</p>\n<p><strong>M-step</strong>：按高斯分量分块更新 <span class=\"kb-math kb-math-inline\">T</span> 矩阵。对第 <span class=\"kb-math kb-math-inline\">c</span> 个分量，累积所有语音的二阶统计量和交叉统计量，然后求解线性方程组更新 <span class=\"kb-math kb-math-inline\">T_c</span>。典型的训练需要 10-20 次 EM 迭代即可收敛。</p>\n<h4>3. 后端信道补偿与评分策略</h4>\n<p>提取出的 i-vector 同时包含说话人信息和信道信息，因此需要后端补偿来分离二者。论文中探讨了多种补偿策略：</p>\n<p><strong>线性判别分析（LDA）</strong>：将 i-vector 从 <span class=\"kb-math kb-math-inline\">R</span> 维投影到更低的 <span class=\"kb-math kb-math-inline\">d</span> 维空间（如 200 维），最大化说话人间方差与说话人内方差的比值。LDA 有效地去除了与说话人无关的变化方向。</p>\n<p><strong>类内协方差归一化（WCCN）</strong>：在 LDA 降维后，进一步对类内协方差进行白化处理，使得余弦距离评分更加鲁棒：</p>\n<div class=\"kb-math kb-math-display\">w&#x27; = B \\cdot A \\cdot w, \\quad \\text{其中 } B^\\top B = W^{-1}</div>\n<p><span class=\"kb-math kb-math-inline\">A</span> 为 LDA 投影矩阵，<span class=\"kb-math kb-math-inline\">W</span> 为类内协方差矩阵。</p>\n<p><strong>长度归一化</strong>：Garcia-Romero 和 Espy-Wilson (2011) 发现对 i-vector 进行 L2 长度归一化（<span class=\"kb-math kb-math-inline\">w&#x27; \\leftarrow w&#x27; / \\|w&#x27;\\|</span>）可以使其分布更接近高斯假设，显著提升 PLDA 等生成式模型的性能。</p>\n<p><strong>评分方式</strong>：论文主要比较了三种评分方法：① <strong>余弦距离评分</strong>（Cosine Distance Scoring, CDS），直接计算两个 i-vector 的余弦相似度；② <strong>SVM 评分</strong>，对每个目标说话人训练一个 SVM 分类器；③ <strong>PLDA 评分</strong>，使用概率线性判别分析计算似然比。实验表明，在 NIST SRE 2008 核心条件下，i-vector + LDA + WCCN + 余弦评分的 EER 为 3.76%，而 JFA 系统的 EER 为 5.17%，相对降低了约 <strong>27%</strong>。</p>\n<h4>4. 实验结果与历史影响</h4>\n<p>论文在 NIST SRE 2008 的多个条件下进行了全面评估。关键实验结果包括：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>系统配置</th>\n<th>EER (%)</th>\n<th>minDCF</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>JFA (基线)</td>\n<td>5.17</td>\n<td>0.0270</td>\n</tr>\n<tr>\n<td>i-vector + Cosine</td>\n<td>4.57</td>\n<td>0.0243</td>\n</tr>\n<tr>\n<td>i-vector + LDA + Cosine</td>\n<td>4.00</td>\n<td>0.0207</td>\n</tr>\n<tr>\n<td>i-vector + LDA + WCCN + Cosine</td>\n<td>3.76</td>\n<td>0.0184</td>\n</tr>\n<tr>\n<td>i-vector + LDA + WCCN + SVM</td>\n<td>3.46</td>\n<td>0.0175</td>\n</tr>\n</tbody>\n</table></div>\n<p>i-vector 方法的提出标志着说话人识别领域从<strong>生成式模型（GMM-UBM/JFA）</strong>向<strong>判别式后端处理</strong>的重要转变。其核心贡献在于：（1）证明了前端不必显式分离说话人和信道信息；（2）将变长语音压缩为固定长度的低维向量，极大简化了后端处理；（3）为后续的深度学习方法（如 d-vector、x-vector）提供了\"嵌入向量\"的设计范式。i-vector 在 2011-2017 年间一直是说话人识别的主流方法，直到基于神经网络的 x-vector 逐渐取代其地位。</p>\n<hr />"
    },
    {
      "id": "d_vector",
      "num": 4,
    ...
    },
    {
      ...
      "detail": "<h3>整体架构</h3>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2601.06896/assets/x2.png\" alt=\"TagSpeech架构图\" loading=\"lazy\"><p class=\"img-caption\">▲ TagSpeech架构图</p></div>\n<p>TagSpeech的整体框架如上图所示。原始音频波形首先被转换为Mel频谱特征 $\\mathbf{M} \\in \\mathbb{R}^{T \\times 80}$，然后并行送入两个独立的编码器：<strong>语义编码器</strong>和<strong>说话人编码器</strong>，均采用Zipformer结构。语义编码器通过序列化输出训练（SOT）进行微调，使其能够捕获重叠语音中的轮次切换模式；说话人编码器使用Auden-Voice初始化，在说话人识别、性别、年龄和情感识别等多任务上预训练，确保嵌入对说话人身份具有判别性。两个编码器的输出分别通过各自的MLP投影器（2层MLP + 时间下采样因子k）映射到LLM嵌入维度。</p>\n<h3>交错数字时间锚机制</h3>\n<p>时间锚机制是TagSpeech最核心的创新之一。定义函数 $\\mathcal{F}_{anc}(\\cdot; m)$ 在特征序列中每隔m帧插入一个时间锚token（自然数\"1\",\"2\",\"3\"...），包括序列首尾。该操作同时应用于语义流和说话人流：</p>\n<div class=\"kb-math kb-math-display\">\\mathbf{Z}_{sem} = \\mathcal{F}_{anc}(\\hat{\\mathbf{H}}_{sem}; m), \\quad \\mathbf{Z}_{spk} = \\mathcal{F}_{anc}(\\hat{\\mathbf{H}}_{spk}; m)</div>\n<p>由于两个流使用相同的确定性插入函数和间隔m，时间锚在两个流之间建立了<strong>显式的时间同步</strong>。与需要扩展词表的方法不同，数字锚利用了LLM对自然数的固有推理能力，仅需1-2个token即可表示时间位置。</p>\n<pre><code>伪代码: 交错时间锚插入\nInput: 特征序列 H = [h_1, h_2, ..., h_L], 间隔 m\nOutput: 带时间锚的序列 Z\n\nZ = [anchor(0)]  # 起始锚\ncounter = 1\nfor i = 1 to L:\n    Z.append(h_i)\n    if i % m == 0:\n        Z.append(anchor(counter))\n        counter += 1\nZ.append(anchor(counter))  # 结束锚\nreturn Z\n</code></pre>\n<h3>结构化输入-输出对齐</h3>\n<p>模型采用XML风格的标签组织输入和输出。输入格式为：</p>\n<div class=\"kb-math kb-math-display\">\\mathbf{X}_{in} = [\\mathbf{E}_{tag}, \\mathbf{Z}_{sem}, \\mathbf{E}_{tag}, \\mathbf{Z}_{spk}, \\mathbf{E}_{tag}]</div>\n<p>其中 <code>&lt;text&gt;</code> 和 <code>&lt;spk&gt;</code> 标签利用LLM预训练时对XML结构的理解能力，无需任何词表修改。输出目标序列镜像了这种解耦结构，使得语义流与转录/时间戳对齐，说话人流与说话人身份/时间戳对齐。训练目标为标准自回归负对数似然，仅优化两个投影器参数。</p>\n<h3>主要实验结果</h3>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>模型</th>\n<th>AMI DER↓</th>\n<th>AMI cpWER↓</th>\n<th>AMI SCA↑</th>\n<th>AliMeeting DER↓</th>\n<th>AliMeeting cpCER↓</th>\n<th>AliMeeting SCA↑</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Gemini-2.0-flash</td>\n<td>45.16</td>\n<td>36.24</td>\n<td>56.19</td>\n<td>50.35</td>\n<td>59.02</td>\n<td>51.14</td>\n</tr>\n<tr>\n<td>Qwen2.5-Omni-7B</td>\n<td>34.71</td>\n<td>49.86</td>\n<td>60.06</td>\n<td>37.42</td>\n<td>41.23</td>\n<td>71.12</td>\n</tr>\n<tr>\n<td>Qwen3.0-Omni-30B</td>\n<td>39.06</td>\n<td>38.83</td>\n<td>59.35</td>\n<td>34.60</td>\n<td>33.69</td>\n<td>70.20</td>\n</tr>\n<tr>\n<td>Pyannote+Whisper (级联)</td>\n<td>23.05</td>\n<td>43.57</td>\n<td>51.43</td>\n<td>26.13</td>\n<td>46.56</td>\n<td>60.68</td>\n</tr>\n<tr>\n<td><strong>TagSpeech (Ours)</strong></td>\n<td><strong>24.84</strong></td>\n<td><strong>42.55</strong></td>\n<td><strong>70.01</strong></td>\n<td><strong>22.13</strong></td>\n<td><strong>33.84</strong></td>\n<td><strong>81.63</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>TagSpeech在所有端到端系统中取得最佳DER，在AliMeeting上甚至超越了专门优化日志的级联系统。说话人计数准确率（SCA）在两个数据集上均大幅领先，表明SOT微调的语义编码器与说话人编码器协同工作能有效检测说话人切换。</p>\n<h3>消融实验关键发现</h3>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2601.06896/assets/x3.png\" alt=\"时间锚粒度影响\" loading=\"lazy\"><p class=\"img-caption\">▲ 时间锚粒度影响</p></div>\n<p><strong>编码器设计消融（Table 3）</strong>：单编码器（包括WavLM-Large）导致极高的失败率（~22-24%）和性能崩溃；双编码器架构显著优于单编码器，证明语义和说话人信息在共享表示空间中会相互干扰。SOT微调的语义编码器在所有配置中表现最佳。</p>\n<p><strong>时间锚粒度（Figure 3）</strong>：DER和cpWER均呈U型曲线——过密的锚（每帧）用时间线索淹没序列，破坏语义和说话人流的潜在结构；过稀的锚（≥16帧）则无法提供足够的时间定位。最佳平衡点在8帧间隔（对应1.28秒）。DER的改善主要来自减少漏检（miss rate），特别是在重叠语音区域。</p>\n<p><strong>时间线索类型对比（Table 5）</strong>：离散token锚一致优于连续正弦嵌入；文本锚（<code>&lt;time=0.16s&gt;</code>）DER略优（20.53% vs 21.75%），但需要4-6个token，而数字锚仅需1-2个token，是更可扩展的方案。</p>\n<h3>基线模型的线性偏差</h3>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2601.06896/assets/x5.png\" alt=\"时间线可视化\" loading=\"lazy\"><p class=\"img-caption\">▲ 时间线可视化</p></div>\n<p>论文发现Gemini和Qwen等端到端基线存在<strong>线性偏差（linearity bias）</strong>：即使在明确提示处理重叠的情况下，这些模型仍倾向于将同时发生的语音事件序列化，强制一个说话人在另一个结束后才开始。TagSpeech通过交错时间锚和SOT微调的语义编码器有效克服了这一问题，能够正确恢复重叠的时间线。此外，Gemini还表现出<strong>时间盲区</strong>：对极短输入（0.21秒）产生28秒的幻觉对话。</p>"
    },
    {
      "id": "streaming_sp_asr",
      "num": 20,
