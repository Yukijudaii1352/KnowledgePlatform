/**
 * trustworthy_learning-data.js — 由 pipeline/build.py 于 2026-05-26 14:20:27 自动生成。
  ...
  "algos": [
    {
      ...
      "keyPoints": [
        "<strong>线性假说</strong>：高维空间中，即使每维扰动极小（<span class=\"kb-math kb-math-inline\">\\|\\eta\\|_\\infty &lt; \\epsilon</span>），线性模型的激活变化可达 <span class=\"kb-math kb-math-inline\">\\epsilon \\cdot m \\cdot n</span>，随维度线性增长",
        "<strong>FGSM 攻击公式</strong>：<span class=\"kb-math kb-math-inline\">\\eta = \\epsilon \\cdot \\text{sign}(\\nabla_x J(\\theta, x, y))</span>，单步、闭式、仅需一次反向传播",
        "<strong>对抗训练目标</strong>：<span class=\"kb-math kb-math-inline\">\\tilde{J} = \\alpha J(\\theta, x, y) + (1-\\alpha) J(\\theta, x + \\epsilon \\cdot \\text{sign}(\\nabla_x J))</span>，持续用当前模型生成对抗样本进行数据增强",
        "<strong>跨模型迁移性</strong>：不同架构、不同训练集的模型对同一对抗样本均易受骗，支持线性假说的普适性",
        "<strong>实验验证</strong>：MNIST 上 <span class=\"kb-math kb-math-inline\">\\epsilon=0.25</span> 使 softmax 分类器错误率达 99.9%；对抗训练将 maxout 网络测试误差从 0.94% 降至 0.78%",
        "<strong>与非线性防御对比</strong>：通用正则化（Dropout、预训练）无法有效防御对抗样本，RBF 网络等非线性模型族可抵抗但难以训练"
      ],
      "detail": "<p><img alt=\"FGSM 对抗样本示例（Panda → Gibbon）\" src=\"https://ar5iv.labs.arxiv.org/html/1412.6572/assets/panda_577.png\" />\n<em>图：对 GoogLeNet 施加 FGSM 扰动（ε=0.007），\"panda\"（57.7% 置信度）被误分类为\"gibbon\"（99.3% 置信度），扰动对人眼不可见。</em></p>\n<pre><code class=\"language-python\"># FGSM 攻击伪代码\nimport torch\n\ndef fgsm_attack(model, x, y, epsilon, loss_fn):\n    &quot;&quot;&quot;\n    model: 目标分类器\n    x: 原始输入 (requires_grad=True)\n    y: 真实标签\n    epsilon: 扰动幅度上界\n    &quot;&quot;&quot;\n    x.requires_grad = True\n    output = model(x)\n    loss = loss_fn(output, y)\n    loss.backward()\n\n    # 核心：取梯度符号方向，乘以 epsilon\n    perturbation = epsilon * x.grad.sign()\n    x_adv = x + perturbation\n    x_adv = torch.clamp(x_adv, 0, 1)  # 保持有效像素范围\n    return x_adv\n</code></pre>\n<h5>动机与背景</h5>\n<p>Szegedy et al. (2014) 首次发现神经网络存在对抗样本现象——对输入施加人眼不可察觉的微小扰动即可使模型高置信度误分类。早期解释将此归因于深度网络的\"极端非线性\"和\"过拟合\"。Goodfellow et al. 在本文中推翻了这一假说，提出了更为简洁有力的<strong>线性假说</strong>：正是神经网络中大量线性组件（ReLU、LSTM、maxout 等分段线性激活）的累积效应导致了对抗脆弱性。</p>\n<h5>核心机制：线性假说与 FGSM 推导</h5>\n<p>考虑权重向量 <span class=\"kb-math kb-math-inline\">\\mathbf{w}</span> 与对抗输入 <span class=\"kb-math kb-math-inline\">\\tilde{x} = x + \\eta</span> 的点积：</p>\n<div class=\"kb-math kb-math-display\">\\mathbf{w}^\\top \\tilde{x} = \\mathbf{w}^\\top x + \\mathbf{w}^\\top \\eta</div>\n<p>在 <span class=\"kb-math kb-math-inline\">\\|\\eta\\|_\\infty \\leq \\epsilon</span> 约束下，为最大化 <span class=\"kb-math kb-math-inline\">\\mathbf{w}^\\top \\eta</span>，最优解为 <span class=\"kb-math kb-math-inline\">\\eta = \\epsilon \\cdot \\text{sign}(\\mathbf{w})</span>。此时激活增量为 <span class=\"kb-math kb-math-inline\">\\epsilon \\cdot m \\cdot n</span>（<span class=\"kb-math kb-math-inline\">m</span> 为权重均值绝对值，<span class=\"kb-math kb-math-inline\">n</span> 为输入维度）。</p>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：扰动的 <span class=\"kb-math kb-math-inline\">L_\\infty</span> 范数不随维度增长，但其对输出的影响随维度<strong>线性增长</strong>。这就是为什么在高维图像空间（如 MNIST 的 784 维、ImageNet 的 150528 维）中，极微小的逐像素扰动就能产生巨大的输出偏移。</div>\n<p>将此思想推广到非线性神经网络：对损失函数 <span class=\"kb-math kb-math-inline\">J(\\theta, x, y)</span> 在当前参数 <span class=\"kb-math kb-math-inline\">\\theta</span> 处做一阶泰勒展开，最大化损失的最优 <span class=\"kb-math kb-math-inline\">L_\\infty</span> 扰动即为：</p>\n<div class=\"kb-math kb-math-display\">\\eta = \\epsilon \\cdot \\text{sign}\\left(\\nabla_x J(\\theta, x, y)\\right)</div>\n<p>这就是 <strong>Fast Gradient Sign Method (FGSM)</strong>。其计算仅需一次前向传播 + 一次反向传播，复杂度与训练一步相当。</p>\n<h5>对抗训练：从攻击到防御</h5>\n<p>基于 FGSM 的高效性，作者提出将对抗样本纳入训练过程。修改后的目标函数为：</p>\n<div class=\"kb-math kb-math-display\">\\tilde{J}(\\theta, x, y) = \\alpha \\cdot J(\\theta, x, y) + (1 - \\alpha) \\cdot J\\left(\\theta, x + \\epsilon \\cdot \\text{sign}(\\nabla_x J(\\theta, x, y)), y\\right)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\alpha = 0.5</span>。这一方法的关键优势在于：\n1. <strong>在线生成</strong>：每步训练时用当前模型参数实时生成对抗样本，避免使用过时的攻击\n2. <strong>计算高效</strong>：仅增加一次前向+反向传播的开销\n3. <strong>正则化效果</strong>：等价于对模型施加局部 Lipschitz 约束，鼓励损失函数在输入邻域内平滑</p>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：作者发现对抗训练需要更大容量的模型（1600 units/layer vs 原始 240），且应使用<strong>对抗验证集误差</strong>而非标准验证集误差进行早停。</div>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方面</th>\n<th>传统解释</th>\n<th>FGSM 论文观点</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>对抗脆弱性根因</td>\n<td>非线性 + 过拟合</td>\n<td>线性特性在高维空间的累积</td>\n</tr>\n<tr>\n<td>攻击方法</td>\n<td>L-BFGS 优化（Szegedy 2014），计算昂贵</td>\n<td>单步梯度符号，闭式解</td>\n</tr>\n<tr>\n<td>防御策略</td>\n<td>Dropout、权重衰减等通用正则</td>\n<td>对抗训练（直接在扰动样本上优化）</td>\n</tr>\n<tr>\n<td>迁移性解释</td>\n<td>无</td>\n<td>不同模型学到相似的线性函数，梯度方向一致</td>\n</tr>\n</tbody>\n</table></div>\n<h5>实验关键发现</h5>\n<ul>\n<li><strong>攻击效果</strong>：MNIST 上 <span class=\"kb-math kb-math-inline\">\\epsilon=0.25</span>，softmax 错误率 99.9%，maxout 错误率 89.4%；CIFAR-10 上 <span class=\"kb-math kb-math-inline\">\\epsilon=0.1</span>，卷积 maxout 错误率 87.15%</li>\n<li><strong>对抗训练收益</strong>：MNIST 测试误差从 0.94% → 0.78%（当时 permutation-invariant MNIST SOTA）</li>\n<li><strong>鲁棒性提升</strong>：对抗训练后模型在 FGSM 攻击下错误率从 89.4% 降至 17.9%</li>\n<li><strong>RBF 网络</strong>：天然抵抗 FGSM（因其高度非线性），但在干净数据上精度较低（1.6% vs 0.78%）</li>\n</ul>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>平滑分类器定义</strong>：<span class=\"kb-math kb-math-inline\">g(x) = \\arg\\max_c \\mathbb{P}(f(x+\\varepsilon)=c)</span>，其中 <span class=\"kb-math kb-math-inline\">\\varepsilon \\sim \\mathcal{N}(0, \\sigma^2 I)</span>",
        "<strong>紧的 <span class=\"kb-math kb-math-inline\">\\ell_2</span> 认证半径</strong>：<span class=\"kb-math kb-math-inline\">R = \\sigma \\Phi^{-1}(\\underline{p_A})</span>（二分类）；一般情况 <span class=\"kb-math kb-math-inline\">R = \\frac{\\sigma}{2}[\\Phi^{-1}(\\underline{p_A}) - \\Phi^{-1}(\\overline{p_B})]</span>",
        "<strong>证明技术</strong>：利用 Neyman-Pearson 引理构造最坏情况分类器，证明认证半径的紧性",
        "<strong>Monte Carlo 认证算法</strong>：通过采样 + Clopper-Pearson 置信区间估计 <span class=\"kb-math kb-math-inline\">\\underline{p_A}</span>，给出概率性正确的认证",
        "<strong>训练方法</strong>：高斯数据增强（训练时对输入加 <span class=\"kb-math kb-math-inline\">\\mathcal{N}(0, \\sigma^2 I)</span> 噪声）",
        "<strong>ImageNet 规模验证</strong>：在 <span class=\"kb-math kb-math-inline\">\\ell_2 &lt; 0.5</span> 扰动下达到 49% certified top-1 accuracy"
      ],
      "detail": "<h5>核心示意图</h5>\n<p><img alt=\"Randomized Smoothing 示意图\" src=\"https://ar5iv.labs.arxiv.org/html/1902.02918/assets/figures/illustration.png\" />\n<em>图：左侧为基分类器 f 的决策区域（不同颜色），虚线为高斯分布 <span class=\"kb-math kb-math-inline\">\\mathcal{N}(x, \\sigma^2 I)</span> 的等高线；右侧为 <span class=\"kb-math kb-math-inline\">f(\\mathcal{N}(x, \\sigma^2 I))</span> 的类别概率分布，<span class=\"kb-math kb-math-inline\">\\underline{p_A}</span> 为 top class 的概率下界，<span class=\"kb-math kb-math-inline\">\\overline{p_B}</span> 为 runner-up class 的概率上界。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Algorithm 1: Predict &amp; Certify\n\ndef Predict(f, sigma, x, n, alpha):\n    &quot;&quot;&quot;评估平滑分类器 g(x)&quot;&quot;&quot;\n    counts = SampleUnderNoise(f, x, n, sigma)\n    c_A, c_B = top_two_classes(counts)\n    n_A, n_B = counts[c_A], counts[c_B]\n    if BinomPValue(n_A, n_A + n_B, 0.5) &lt;= alpha:\n        return c_A\n    else:\n        return ABSTAIN\n\ndef Certify(f, sigma, x, n0, n, alpha):\n    &quot;&quot;&quot;认证平滑分类器在 x 处的鲁棒半径&quot;&quot;&quot;\n    # Step 1: 用少量样本确定 top class\n    counts0 = SampleUnderNoise(f, x, n0, sigma)\n    c_A = argmax(counts0)\n\n    # Step 2: 用大量样本估计 p_A 的下界\n    counts = SampleUnderNoise(f, x, n, sigma)\n    p_A_lower = LowerConfBound(counts[c_A], n, 1 - alpha)  # Clopper-Pearson\n\n    if p_A_lower &gt; 0.5:\n        radius = sigma * Phi_inv(p_A_lower)\n        return c_A, radius\n    else:\n        return ABSTAIN, 0.0\n</code></pre>\n<h5>动机与背景</h5>\n<p>对抗样本问题是深度学习安全性的核心挑战：微小的输入扰动可导致分类器产生错误预测。<strong>可证明防御（certified defense）</strong> 旨在为分类器提供数学上的鲁棒性保证——即在给定扰动范围内，分类结果不会改变。</p>\n<p>此前的可证明防御方法（如基于 SDP 松弛、区间传播、线性松弛等）虽然在小规模数据集上可行，但由于需要分析网络的具体结构，<strong>无法扩展到 ImageNet 等大规模任务</strong>。Randomized Smoothing 的核心优势在于：它是 <strong>模型无关的（model-agnostic）</strong>——无需知道基分类器 <span class=\"kb-math kb-math-inline\">f</span> 的内部结构，只需能够查询其输出即可。</p>\n<h5>核心机制：平滑分类器与认证半径</h5>\n<p><strong>定义</strong>：给定基分类器 <span class=\"kb-math kb-math-inline\">f: \\mathbb{R}^d \\to \\mathcal{Y}</span> 和噪声标准差 <span class=\"kb-math kb-math-inline\">\\sigma</span>，平滑分类器定义为：</p>\n<div class=\"kb-math kb-math-display\">g(x) = \\arg\\max_{c \\in \\mathcal{Y}} \\mathbb{P}_{\\varepsilon \\sim \\mathcal{N}(0, \\sigma^2 I)}[f(x + \\varepsilon) = c]</div>\n<p>直觉上，<span class=\"kb-math kb-math-inline\">g(x)</span> 返回的是在 <span class=\"kb-math kb-math-inline\">x</span> 的高斯邻域内 <span class=\"kb-math kb-math-inline\">f</span> 最频繁预测的类别。</p>\n<p><strong>Theorem 1（核心定理）</strong>：设 <span class=\"kb-math kb-math-inline\">c_A = g(x)</span>，且：</p>\n<div class=\"kb-math kb-math-display\">p_A = \\mathbb{P}(f(x+\\varepsilon) = c_A) \\geq \\underline{p_A} &gt; \\overline{p_B} \\geq \\max_{c \\neq c_A} \\mathbb{P}(f(x+\\varepsilon) = c)</div>\n<p>则对所有满足 <span class=\"kb-math kb-math-inline\">\\|\\delta\\|_2 &lt; R</span> 的扰动 <span class=\"kb-math kb-math-inline\">\\delta</span>，有 <span class=\"kb-math kb-math-inline\">g(x+\\delta) = c_A</span>，其中：</p>\n<div class=\"kb-math kb-math-display\">R = \\frac{\\sigma}{2}\\left[\\Phi^{-1}(\\underline{p_A}) - \\Phi^{-1}(\\overline{p_B})\\right]</div>\n<p>在二分类情况下（<span class=\"kb-math kb-math-inline\">\\overline{p_B} = 1 - \\underline{p_A}</span>），简化为：</p>\n<div class=\"kb-math kb-math-display\">R = \\sigma \\Phi^{-1}(\\underline{p_A})</div>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：<span class=\"kb-math kb-math-inline\">\\sigma</span> 越大，认证半径越大，但基分类器在噪声下的准确率越低——存在 accuracy-robustness trade-off。</div>\n<h5>证明思路：Neyman-Pearson 引理</h5>\n<p>证明的核心思想是寻找\"最坏情况\"基分类器。固定扰动 <span class=\"kb-math kb-math-inline\">\\delta</span>，问题变为：</p>\n<blockquote>\n<p>在所有满足 <span class=\"kb-math kb-math-inline\">\\mathbb{P}(f(x+\\varepsilon)=c_A) \\geq \\underline{p_A}</span> 的分类器 <span class=\"kb-math kb-math-inline\">f</span> 中，哪一个使得 <span class=\"kb-math kb-math-inline\">\\mathbb{P}(f(x+\\delta+\\varepsilon)=c_A)</span> 最小？</p>\n</blockquote>\n<p>由 <strong>Neyman-Pearson 引理</strong>，最坏情况分类器 <span class=\"kb-math kb-math-inline\">f^*</span> 是一个线性分类器，其决策边界垂直于扰动方向 <span class=\"kb-math kb-math-inline\">\\delta</span>：</p>\n<div class=\"kb-math kb-math-display\">f^*(x&#x27;) = \\begin{cases} c_A &amp; \\text{if } \\delta^T(x&#x27;-x) \\leq \\sigma\\|\\delta\\|_2 \\Phi^{-1}(\\underline{p_A}) \\\\ c_B &amp; \\text{otherwise} \\end{cases}</div>\n<p>该最坏情况分类器将 <span class=\"kb-math kb-math-inline\">\\mathcal{N}(x+\\delta, \\sigma^2 I)</span> 分类为 <span class=\"kb-math kb-math-inline\">c_A</span> 的概率为：</p>\n<div class=\"kb-math kb-math-display\">\\Phi\\left(\\Phi^{-1}(\\underline{p_A}) - \\frac{\\|\\delta\\|_2}{\\sigma}\\right)</div>\n<p>要保证此概率 <span class=\"kb-math kb-math-inline\">&gt; 1/2</span>，解得 <span class=\"kb-math kb-math-inline\">\\|\\delta\\|_2 &lt; \\sigma \\Phi^{-1}(\\underline{p_A})</span>。</p>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：该认证半径是<strong>紧的（tight）</strong>——存在满足约束的分类器恰好在半径边界处失效，因此无法进一步改进。</div>\n<h5>统计认证：Monte Carlo 估计</h5>\n<p>实际中 <span class=\"kb-math kb-math-inline\">p_A</span> 未知，需通过采样估计。Certify 算法的流程：</p>\n<ol>\n<li><strong>采样</strong>：抽取 <span class=\"kb-math kb-math-inline\">n</span> 个高斯噪声样本 <span class=\"kb-math kb-math-inline\">\\varepsilon_i \\sim \\mathcal{N}(0, \\sigma^2 I)</span>，统计 <span class=\"kb-math kb-math-inline\">f(x+\\varepsilon_i) = c_A</span> 的次数 <span class=\"kb-math kb-math-inline\">n_A</span></li>\n<li><strong>置信下界</strong>：用 Clopper-Pearson 方法计算 <span class=\"kb-math kb-math-inline\">p_A</span> 的 <span class=\"kb-math kb-math-inline\">(1-\\alpha)</span> 单侧置信下界 <span class=\"kb-math kb-math-inline\">\\underline{p_A}</span></li>\n<li><strong>计算半径</strong>：<span class=\"kb-math kb-math-inline\">R = \\sigma \\Phi^{-1}(\\underline{p_A})</span></li>\n</ol>\n<div class=\"key-point\">💡 <strong>概率保证</strong>：以至少 <span class=\"kb-math kb-math-inline\">1-\\alpha</span> 的概率，返回的半径是正确的（即在该半径内分类确实不变）。</div>\n<h5>训练方法</h5>\n<p>论文发现最有效的训练方法是简单的<strong>高斯数据增强</strong>：</p>\n<div class=\"kb-math kb-math-display\">\\min_\\theta \\mathbb{E}_{(x,y)\\sim\\mathcal{D}} \\mathbb{E}_{\\varepsilon \\sim \\mathcal{N}(0, \\sigma^2 I)} [\\ell(f_\\theta(x + \\varepsilon), y)]</div>\n<p>即在标准训练中，每个 mini-batch 的输入都加上与认证时相同方差的高斯噪声。这比对抗训练简单得多，且在 ImageNet 上可行。</p>\n<h5>与先前工作的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>认证范数</th>\n<th>可扩展性</th>\n<th>认证紧性</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>SDP 松弛</td>\n<td><span class=\"kb-math kb-math-inline\">\\ell_\\infty</span></td>\n<td>小网络</td>\n<td>松</td>\n</tr>\n<tr>\n<td>区间传播 (IBP)</td>\n<td><span class=\"kb-math kb-math-inline\">\\ell_\\infty</span></td>\n<td>中等</td>\n<td>松</td>\n</tr>\n<tr>\n<td>Lecuyer et al. (PixelDP)</td>\n<td><span class=\"kb-math kb-math-inline\">\\ell_2</span></td>\n<td>ImageNet</td>\n<td>松（Rényi 散度）</td>\n</tr>\n<tr>\n<td>Li et al.</td>\n<td><span class=\"kb-math kb-math-inline\">\\ell_2</span></td>\n<td>ImageNet</td>\n<td>松（信息论）</td>\n</tr>\n<tr>\n<td><strong>本文 (Cohen et al.)</strong></td>\n<td><span class=\"kb-math kb-math-inline\">\\ell_2</span></td>\n<td><strong>ImageNet</strong></td>\n<td><strong>紧</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>本文的关键改进：Lecuyer et al. 和 Li et al. 也使用了随机平滑框架，但它们的认证半径是松的（基于 Rényi 散度或互信息的界）。本文通过 Neyman-Pearson 引理直接推导出紧的认证半径，在相同条件下给出更大的认证区域。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "防御机制：在 <span class=\"kb-math kb-math-inline\">\\ell_2</span> 球约束内，通过多步梯度下降迭代最小化能量，将对抗样本拉回正确分类区域",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>原型映射机制</strong>：将每个个体通过 softmax 概率映射到 K 个可学习原型上，形成中间表示向量 <span class=\"kb-math kb-math-inline\">\\{M_{n,k}\\}_{k=1}^K</span>",
        "<strong>三项联合目标函数</strong>：<span class=\"kb-math kb-math-inline\">L = A_z \\cdot L_z + A_x \\cdot L_x + A_y \\cdot L_y</span>，分别对应公平性、信息保留和分类准确性",
        "<strong>统计平等约束（<span class=\"kb-math kb-math-inline\">L_z</span>）</strong>：强制受保护组与非受保护组在每个原型上的平均映射概率相等",
        "<strong>重构损失（<span class=\"kb-math kb-math-inline\">L_x</span>）</strong>：确保中间表示保留除受保护属性外的尽可能多的信息",
        "<strong>分类损失（<span class=\"kb-math kb-math-inline\">L_y</span>）</strong>：通过原型到标签的线性映射保证预测准确性",
        "<strong>可学习距离度量</strong>：为每个特征维度引入权重 <span class=\"kb-math kb-math-inline\">\\alpha_i</span>，且受保护组/非受保护组使用不同权重 <span class=\"kb-math kb-math-inline\">\\alpha^+, \\alpha^-</span>",
      ...
      "detail": "<pre><code class=\"language-python\"># LFR 核心算法伪代码\nimport numpy as np\n\ndef lfr_objective(X, S, Y, V, W, alpha_plus, alpha_minus, Az, Ax, Ay):\n    &quot;&quot;&quot;\n    X: 输入数据 (N, D)\n    S: 受保护属性 (N,), 0/1\n    Y: 标签 (N,)\n    V: 原型位置 (K, D)\n    W: 原型分类权重 (K,), 值在[0,1]\n    alpha_plus, alpha_minus: 特征权重 (D,)\n    &quot;&quot;&quot;\n    N, D = X.shape\n    K = V.shape[0]\n\n    # Step 1: 计算映射概率 M_{n,k}\n    M = np.zeros((N, K))\n    for n in range(N):\n        alpha = alpha_plus if S[n] == 1 else alpha_minus\n        for k in range(K):\n            dist = np.sum(alpha * (X[n] - V[k])**2)\n            M[n, k] = np.exp(-dist)\n        M[n] /= M[n].sum()  # softmax 归一化\n\n    # Step 2: 统计平等损失 Lz\n    M_plus = M[S == 1].mean(axis=0)   # 受保护组平均映射\n    M_minus = M[S == 0].mean(axis=0)  # 非受保护组平均映射\n    Lz = np.abs(M_plus - M_minus).sum()\n\n    # Step 3: 重构损失 Lx\n    X_hat = M @ V  # 重构: x_hat_n = sum_k M_{n,k} * v_k\n    Lx = np.sum((X - X_hat)**2)\n\n    # Step 4: 分类损失 Ly (交叉熵)\n    Y_hat = M @ W  # 预测: y_hat_n = sum_k M_{n,k} * w_k\n    Y_hat = np.clip(Y_hat, 1e-7, 1-1e-7)\n    Ly = -np.sum(Y * np.log(Y_hat) + (1-Y) * np.log(1-Y_hat))\n\n    # 总损失\n    L = Az * Lz + Ax * Lx + Ay * Ly\n    return L\n</code></pre>\n<h5>动机与背景</h5>\n<p>传统公平分类方法主要分为两类：<strong>数据篡改策略</strong>（修改训练标签使正标签在两组中比例相等）和<strong>正则化策略</strong>（在分类目标中加入歧视度惩罚项）。这些方法存在根本局限：</p>\n<ol>\n<li>数据篡改方法是临时性的（ad hoc），无法保证对新数据的泛化；</li>\n<li>正则化方法与特定分类器绑定，无法组合使用；</li>\n<li>两者都未考虑<strong>个体公平</strong>——即相似个体应获得相似待遇。</li>\n</ol>\n<p>Dwork et al. (2011) 首次提出通过中间表示实现公平的框架，但其假设距离度量已知且不可学习，也无法泛化到新数据。LFR 正是为解决这些问题而提出。</p>\n<h5>核心机制详解</h5>\n<p><strong>1. 原型映射（Prototype Mapping）</strong></p>\n<p>LFR 定义 K 个原型 <span class=\"kb-math kb-math-inline\">\\{v_k\\}_{k=1}^K</span>，每个原型是与输入同维度的向量。个体 <span class=\"kb-math kb-math-inline\">x_n</span> 到原型 <span class=\"kb-math kb-math-inline\">v_k</span> 的映射概率通过带权距离的 softmax 计算：</p>\n<div class=\"kb-math kb-math-display\">M_{n,k} = P(Z=k|x_n) = \\frac{\\exp(-d(x_n, v_k))}{\\sum_{j=1}^K \\exp(-d(x_n, v_j))}</div>\n<p>其中距离函数为加权欧氏距离：</p>\n<div class=\"kb-math kb-math-display\">d(x_n, v_k) = \\sum_{i=1}^D \\alpha_i (x_{ni} - v_{ki})^2</div>\n<div class=\"key-point\">💡 关键：向量 <span class=\"kb-math kb-math-inline\">\\{M_{n,k}\\}_{k=1}^K</span> 即为个体 <span class=\"kb-math kb-math-inline\">x_n</span> 的\"公平表示\"——它是一个 K 维概率分布，编码了个体信息但抹除了受保护属性。</div>\n<p><strong>2. 统计平等约束（Statistical Parity）</strong></p>\n<p>公平性通过强制两组在原型空间中的分布相同来实现：</p>\n<div class=\"kb-math kb-math-display\">L_z = \\sum_{k=1}^K |M_k^+ - M_k^-|</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">M_k^+ = \\frac{1}{|X_0^+|}\\sum_{n \\in X_0^+} M_{n,k}</span> 是受保护组映射到原型 k 的平均概率。</p>\n<div class=\"warn-box\">⚠️ 注意：当 <span class=\"kb-math kb-math-inline\">L_z \\to 0</span> 时，可以证明 <span class=\"kb-math kb-math-inline\">Z</span> 与 <span class=\"kb-math kb-math-inline\">S</span> 之间的互信息趋近于零，即表示中不包含关于受保护属性的信息。</div>\n<p><strong>3. 重构损失（Information Preservation）</strong></p>\n<p>为确保表示不丢失有用信息，LFR 要求从原型表示能重构原始输入：</p>\n<div class=\"kb-math kb-math-display\">L_x = \\sum_{n=1}^N \\|x_n - \\hat{x}_n\\|^2, \\quad \\hat{x}_n = \\sum_{k=1}^K M_{n,k} \\cdot v_k</div>\n<p>这使得模型类似于一个带公平约束的自编码器，原型充当\"码本\"角色。</p>\n<p><strong>4. 分类损失（Classification Accuracy）</strong></p>\n<p>每个原型学习一个分类概率 <span class=\"kb-math kb-math-inline\">w_k \\in [0,1]</span>，个体的预测通过边际化获得：</p>\n<div class=\"kb-math kb-math-display\">\\hat{y}_n = \\sum_{k=1}^K M_{n,k} \\cdot w_k</div>\n<p>分类损失为标准交叉熵（公式 10）。</p>\n<div class=\"key-point\">💡 关键性质：当统计平等约束满足时，由于 <span class=\"kb-math kb-math-inline\">\\hat{y}_n</span> 是 <span class=\"kb-math kb-math-inline\">M_n</span> 和 <span class=\"kb-math kb-math-inline\">w</span> 的线性函数，可以证明两组的平均预测概率自动相等，即分类决策也满足公平性。</div>\n<p><strong>5. 分组距离度量学习</strong></p>\n<p>LFR 为受保护组和非受保护组分别学习特征权重 <span class=\"kb-math kb-math-inline\">\\alpha^+</span> 和 <span class=\"kb-math kb-math-inline\">\\alpha^-</span>，这解决了 Dwork et al. 提出的\"反转问题\"（inversion problem）——不同群体中不同特征可能具有不同的预测意义。</p>\n<h5>训练与推理流程</h5>\n<p><strong>训练阶段</strong>：使用 L-BFGS 或梯度下降联合优化所有参数 <span class=\"kb-math kb-math-inline\">\\{v_k, w_k, \\alpha^+, \\alpha^-\\}</span>，最小化总目标 <span class=\"kb-math kb-math-inline\">L = A_z L_z + A_x L_x + A_y L_y</span>。超参数 <span class=\"kb-math kb-math-inline\">A_z, A_x, A_y</span> 控制三个目标之间的权衡。</p>\n<p><strong>推理阶段</strong>：对新个体 <span class=\"kb-math kb-math-inline\">x_{new}</span>，先计算其到各原型的映射概率 <span class=\"kb-math kb-math-inline\">M_{new}</span>，然后：\n- 公平表示：直接使用 <span class=\"kb-math kb-math-inline\">M_{new}</span> 作为特征输入任意下游分类器；\n- 直接预测：<span class=\"kb-math kb-math-inline\">\\hat{y}_{new} = \\sum_k M_{new,k} \\cdot w_k</span>。</p>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>数据篡改方法</th>\n<th>正则化方法</th>\n<th>LFR</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>可泛化到新数据</td>\n<td>✗</td>\n<td>✓</td>\n<td>✓</td>\n</tr>\n<tr>\n<td>与分类器解耦</td>\n<td>✗</td>\n<td>✗</td>\n<td>✓</td>\n</tr>\n<tr>\n<td>个体公平</td>\n<td>✗</td>\n<td>✗</td>\n<td>✓（相似输入→相似表示）</td>\n</tr>\n<tr>\n<td>学习距离度量</td>\n<td>—</td>\n<td>—</td>\n<td>✓</td>\n</tr>\n<tr>\n<td>群体公平</td>\n<td>✓</td>\n<td>✓</td>\n<td>✓</td>\n</tr>\n</tbody>\n</table></div>\n<p>LFR 的核心优势在于<strong>可组合性</strong>：一旦学到公平表示，任何下游分类器都可以在此表示上训练，无需额外公平约束，实现了\"公平即服务\"的理念。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>VAE 框架下的公平表示学习</strong>：将公平性问题建模为学习对敏感变量 <span class=\"kb-math kb-math-inline\">s</span> 不变的隐表示 <span class=\"kb-math kb-math-inline\">z_1</span>，基于变分自编码器实现端到端训练",
        "<strong>分解先验 (Factorized Prior)</strong>：设计先验 <span class=\"kb-math kb-math-inline\">p(z_1, s) = p(z_1)p(s)</span> 强制隐变量与敏感属性的先验独立性",
        "<strong>半监督两层隐变量架构</strong>：引入标签 <span class=\"kb-math kb-math-inline\">y</span> 和辅助隐变量 <span class=\"kb-math kb-math-inline\">z_2</span>，生成模型为 <span class=\"kb-math kb-math-inline\">y, z_2 \\to z_1 \\to x</span>，利用未标注数据提升表示质量",
        "<strong>MMD 正则化</strong>：对不同敏感组的边际后验 <span class=\"kb-math kb-math-inline\">q(z_1|s=0)</span> 和 <span class=\"kb-math kb-math-inline\">q(z_1|s=1)</span> 施加 MMD 惩罚，防止信息通过标签相关性\"泄露\"到后验中",
        "<strong>Random Fourier Features 加速</strong>：使用随机厨房水槽方法将 MMD 计算从 <span class=\"kb-math kb-math-inline\">O(M^2)</span> 降至 <span class=\"kb-math kb-math-inline\">O(MD)</span>，其中 <span class=\"kb-math kb-math-inline\">D=500</span> 为随机特征维度",
        "<strong>联合训练策略</strong>：当 <span class=\"kb-math kb-math-inline\">y</span> 与 <span class=\"kb-math kb-math-inline\">s</span> 相关时，联合训练分类器和生成模型（而非分开训练）避免退化表示",
        "<strong>多任务验证</strong>：在公平分类（German/Adult/Health）、域适应（Amazon Reviews）和不变表示学习（Extended Yale B）三类任务上验证有效性"
      ],
      "detail": "<p><img alt=\"VFAE 模型架构图\" src=\"https://arxiv.org/html/1511.00830v2/extracted/figures/model.png\" />\n<em>图：VFAE 的生成模型（左）与推断模型（右）。生成过程：<span class=\"kb-math kb-math-inline\">y, z_2 \\to z_1 \\to x</span>，其中 <span class=\"kb-math kb-math-inline\">s</span> 仅在解码器中作为输入，先验中与 <span class=\"kb-math kb-math-inline\">z_1</span> 独立。推断过程：<span class=\"kb-math kb-math-inline\">x, s \\to z_1 \\to y \\to z_2</span>。</em></p>\n<pre><code class=\"language-python\"># VFAE 训练伪代码\n# 输入: 标注数据 (x_l, s_l, y_l), 未标注数据 (x_u, s_u)\n# 超参: alpha (分类权重), beta (MMD 权重)\n\nfor epoch in range(num_epochs):\n    for (x_l, s_l, y_l), (x_u, s_u) in minibatches:\n        # === 标注数据 ELBO (L_s) ===\n        z1_l = encoder_z1(x_l, s_l)          # q(z1|x,s)\n        y_pred = classifier(z1_l)             # q(y|z1)\n        z2_l = encoder_z2(z1_l, y_l)          # q(z2|z1,y)\n        x_recon_l = decoder(z1_l, s_l)        # p(x|z1,s)\n\n        L_s = recon_loss(x_l, x_recon_l) \n              - KL(q(z1|x,s) || p(z1|z2,y)) \n              - KL(q(z2|z1,y) || p(z2))\n\n        # === 未标注数据 ELBO (L_u) ===\n        z1_u = encoder_z1(x_u, s_u)\n        y_marginal = classifier(z1_u)         # q(y|z1) 枚举所有 y\n        # 对所有可能的 y 值求期望\n        L_u = sum_y q(y|z1) * [recon + KL terms for each y]\n\n        # === MMD 正则化 ===\n        z1_all = concat(z1_l, z1_u)\n        s_all = concat(s_l, s_u)\n        z1_s0 = z1_all[s_all == 0]\n        z1_s1 = z1_all[s_all == 1]\n        mmd = compute_mmd_rff(z1_s0, z1_s1, D=500)\n\n        # === 总损失 ===\n        loss = -(L_s + L_u) + alpha * cross_entropy(y_pred, y_l) + beta * mmd\n        optimizer.step(loss)\n</code></pre>\n<p><strong>动机与背景</strong></p>\n<p>在机器学习的公平性研究中，核心挑战是：如何学习一个数据表示，使其不包含关于敏感属性（如性别、种族、年龄）的信息，同时仍然保留足够的信息用于下游预测任务？先前工作 LFR (Zemel et al., 2013) 提出了基于聚类的方法，但其局限在于：(1) 聚类表示无法利用分布式表示的表达能力；(2) 仅匹配一阶矩（均值），无法保证高阶矩的一致性，信息仍可能通过方差等高阶统计量泄露。</p>\n<p>VFAE 的核心洞察是：变分自编码器的概率框架天然适合建模\"不变性\"约束——通过设计先验分布的因子化结构，可以在概率意义上强制独立性。</p>\n<p><strong>核心机制一：分解先验实现先验独立</strong></p>\n<p>VFAE 的生成模型设计为：</p>\n<div class=\"kb-math kb-math-display\">p(\\mathbf{x}, \\mathbf{z}_1, \\mathbf{z}_2, \\mathbf{y}, \\mathbf{s}) = p(\\mathbf{s})p(\\mathbf{y})p(\\mathbf{z}_2)p_\\theta(\\mathbf{z}_1|\\mathbf{z}_2, \\mathbf{y})p_\\theta(\\mathbf{x}|\\mathbf{z}_1, \\mathbf{s})</div>\n<p>关键设计：先验中 <span class=\"kb-math kb-math-inline\">\\mathbf{z}_1</span> 仅依赖于 <span class=\"kb-math kb-math-inline\">\\mathbf{z}_2</span> 和 <span class=\"kb-math kb-math-inline\">\\mathbf{y}</span>，与 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 完全独立。而解码器 <span class=\"kb-math kb-math-inline\">p_\\theta(\\mathbf{x}|\\mathbf{z}_1, \\mathbf{s})</span> 接收 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 作为输入，这意味着 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 对 <span class=\"kb-math kb-math-inline\">\\mathbf{x}</span> 的影响完全通过解码器的直接路径解释，<span class=\"kb-math kb-math-inline\">\\mathbf{z}_1</span> 无需编码任何关于 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 的信息。</p>\n<div class=\"key-point\">💡 关键：将 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 同时输入编码器和解码器，使得模型有一条\"捷径\"来解释 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 对 <span class=\"kb-math kb-math-inline\">\\mathbf{x}</span> 的影响，从而减轻 <span class=\"kb-math kb-math-inline\">\\mathbf{z}_1</span> 编码 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 信息的压力。</div>\n<p><strong>核心机制二：半监督两层隐变量结构</strong></p>\n<p>为了在去除敏感信息的同时保留标签预测能力，VFAE 采用半监督 VAE 架构：</p>\n<ul>\n<li><strong>推断网络</strong>分解为三部分：</li>\n<li><span class=\"kb-math kb-math-inline\">q_\\phi(\\mathbf{z}_1|\\mathbf{x}, \\mathbf{s})</span>：从输入编码不变表示</li>\n<li><span class=\"kb-math kb-math-inline\">q_\\phi(\\mathbf{y}|\\mathbf{z}_1)</span>：从不变表示预测标签（即分类器）</li>\n<li>\n<p><span class=\"kb-math kb-math-inline\">q_\\phi(\\mathbf{z}_2|\\mathbf{z}_1, \\mathbf{y})</span>：捕获给定 <span class=\"kb-math kb-math-inline\">\\mathbf{z}_1</span> 和 <span class=\"kb-math kb-math-inline\">\\mathbf{y}</span> 后的残余变异</p>\n</li>\n<li>\n<p><strong>标注数据的 ELBO</strong>：</p>\n</li>\n</ul>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_s = \\mathbb{E}_{q(\\mathbf{z}_1|\\mathbf{x},\\mathbf{s})q(\\mathbf{z}_2|\\mathbf{z}_1,\\mathbf{y})}[\\log p_\\theta(\\mathbf{x}|\\mathbf{z}_1, \\mathbf{s})] - \\text{KL}[q(\\mathbf{z}_1|\\mathbf{x},\\mathbf{s}) \\| p_\\theta(\\mathbf{z}_1|\\mathbf{z}_2, \\mathbf{y})] - \\text{KL}[q(\\mathbf{z}_2|\\mathbf{z}_1,\\mathbf{y}) \\| p(\\mathbf{z}_2)]</div>\n<ul>\n<li><strong>未标注数据的 ELBO</strong>：将 <span class=\"kb-math kb-math-inline\">\\mathbf{y}</span> 视为缺失变量，对所有可能的 <span class=\"kb-math kb-math-inline\">\\mathbf{y}</span> 值求期望：</li>\n</ul>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_u = \\sum_y q_\\phi(\\mathbf{y}=y|\\mathbf{z}_1) \\cdot \\mathcal{L}_s(\\mathbf{x}, \\mathbf{s}, y)</div>\n<ul>\n<li><strong>联合训练目标</strong>（Eq. 5）：</li>\n</ul>\n<div class=\"kb-math kb-math-display\">\\mathcal{F}_{\\text{VAE}} = \\mathcal{L}_s + \\mathcal{L}_u + \\alpha \\cdot \\log q_\\phi(\\mathbf{y}|\\mathbf{z}_1)</div>\n<div class=\"warn-box\">⚠️ 注意：当 <span class=\"kb-math kb-math-inline\">\\mathbf{y}</span> 与 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 高度相关时（如在公平分类场景中），必须联合训练分类器和生成模型。若分开训练（如 Kingma et al. 2014），分类器可能学到依赖 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 的特征，导致表示退化。</div>\n<p><strong>核心机制三：MMD 正则化防止后验信息泄露</strong></p>\n<p>尽管先验独立性提供了归纳偏置，但近似后验 <span class=\"kb-math kb-math-inline\">q_\\phi(\\mathbf{z}_1|\\mathbf{s})</span> 仍可能因 <span class=\"kb-math kb-math-inline\">\\mathbf{y}</span> 与 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 的相关性而保留敏感信息。VFAE 引入 MMD 惩罚直接约束后验：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{F}_{\\text{VFAE}} = \\mathcal{F}_{\\text{VAE}} - \\beta \\cdot \\ell_{\\text{MMD}}(\\mathbf{Z}_{1_{s=0}}, \\mathbf{Z}_{1_{s=1}})</div>\n<p>其中 MMD 度量两个分布之间的距离：</p>\n<div class=\"kb-math kb-math-display\">\\ell_{\\text{MMD}}(\\mathbf{X}, \\mathbf{X}&#x27;) = \\frac{1}{N_0^2}\\sum_{n,m}k(\\mathbf{x}_n, \\mathbf{x}_m) + \\frac{1}{N_1^2}\\sum_{n,m}k(\\mathbf{x}&#x27;_n, \\mathbf{x}&#x27;_m) - \\frac{2}{N_0 N_1}\\sum_{n,m}k(\\mathbf{x}_n, \\mathbf{x}&#x27;_m)</div>\n<p>使用高斯核 <span class=\"kb-math kb-math-inline\">k(x, x&#x27;) = e^{-\\gamma\\|x-x&#x27;\\|^2}</span> 时，MMD 为零当且仅当两个分布完全相同。</p>\n<p><strong>高效计算：Random Fourier Features</strong></p>\n<p>为避免 <span class=\"kb-math kb-math-inline\">O(M^2)</span> 的 Gram 矩阵计算，VFAE 使用随机傅里叶特征近似核函数：</p>\n<div class=\"kb-math kb-math-display\">\\psi_{\\mathbf{W}}(\\mathbf{x}) = \\sqrt{\\frac{2}{D}} \\cos\\left(\\sqrt{\\frac{2}{\\gamma}} \\mathbf{x}\\mathbf{W} + \\mathbf{b}\\right)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\mathbf{W} \\in \\mathbb{R}^{K \\times D}</span> 为标准高斯随机矩阵，<span class=\"kb-math kb-math-inline\">\\mathbf{b}</span> 为 <span class=\"kb-math kb-math-inline\">[0, 2\\pi]</span> 均匀随机向量。这将 MMD 计算降至 <span class=\"kb-math kb-math-inline\">O(MD)</span> 复杂度，实验中取 <span class=\"kb-math kb-math-inline\">D=500</span>。</p>\n<p><strong>与 LFR 的关键区别</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方面</th>\n<th>LFR (Zemel et al., 2013)</th>\n<th>VFAE</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>表示类型</td>\n<td>聚类（局部表示）</td>\n<td>连续隐变量（分布式表示）</td>\n</tr>\n<tr>\n<td>不变性约束</td>\n<td>匹配一阶矩（聚类比例）</td>\n<td>先验独立 + MMD（匹配所有矩）</td>\n</tr>\n<tr>\n<td>训练方式</td>\n<td>优化组合目标</td>\n<td>变分推断 + ELBO</td>\n</tr>\n<tr>\n<td>半监督</td>\n<td>不支持</td>\n<td>天然支持未标注数据</td>\n</tr>\n<tr>\n<td>生成能力</td>\n<td>无</td>\n<td>可生成新样本</td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>实验验证</strong></p>\n<p>在三个公平分类数据集上（Adult: 45,222 样本；German: 1,000 样本；Health: 147,473 样本），VFAE 相比 LFR 和普通 VAE：\n- 更有效地去除敏感信息（通过 Random Forest 和 Logistic Regression 预测 <span class=\"kb-math kb-math-inline\">\\mathbf{s}</span> 的准确率更接近随机水平）\n- MMD 惩罚显著降低了歧视度量（discrimination metric）\n- 在域适应（Amazon Reviews 12 个跨域任务）和不变表示学习（Extended Yale B，准确率 84.6% vs 基线 82%）上同样有效</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>问题设定</strong>：无人口统计标签的公平学习（Fairness without Demographics）——训练和推理时均无受保护群体标签 <span class=\"kb-math kb-math-inline\">S</span>",
        "<strong>公平目标</strong>：Rawlsian Max-Min 公平性 <span class=\"kb-math kb-math-inline\">h^* = \\arg\\max_h \\min_s U_{D_s}(h)</span>，最大化最差子群的效用",
        "<strong>核心假设</strong>：受保护群体 <span class=\"kb-math kb-math-inline\">S</span> 与可观测特征 <span class=\"kb-math kb-math-inline\">X</span> 和标签 <span class=\"kb-math kb-math-inline\">Y</span> 存在相关性，使得高损失区域\"计算可辨识\"",
        "<strong>对抗框架</strong>：Learner <span class=\"kb-math kb-math-inline\">h_\\theta</span> 与 Adversary <span class=\"kb-math kb-math-inline\">f_\\phi</span> 构成 minimax 博弈，Adversary 输出样本权重 <span class=\"kb-math kb-math-inline\">\\lambda_\\phi(x_i, y_i) \\in [0,1]</span>",
    ...
    },
    {
      ...
      "detail": "<p><img alt=\"LIME 核心思想示意图\" src=\"https://ar5iv.labs.arxiv.org/html/1602.04938/assets/x3.png\" />\n<em>图：LIME 解释文本分类器预测的示例。高亮词为对预测贡献最大的特征。</em></p>\n<p><img alt=\"LIME 局部近似直觉\" src=\"https://ar5iv.labs.arxiv.org/html/1602.04938/assets/x1.png\" />\n<em>图：LIME 的核心直觉。粗红色十字为待解释实例，蓝色/粉色背景为黑盒模型的真实决策边界，虚线为 LIME 学到的局部线性解释。采样点大小表示与待解释实例的相似度权重。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># LIME 核心算法 (Algorithm 1)\ndef LIME_explain(instance x, classifier f, num_samples N, kernel_width σ):\n    # 1. 生成可解释表示\n    x' = to_interpretable(x)  # 如文本→词袋二值向量，图像→超像素二值向量\n\n    # 2. 邻域采样\n    Z = []\n    for i in range(N):\n        z'_i = perturb(x')           # 随机关闭部分可解释特征\n        z_i = to_original(z'_i)       # 映射回原始空间\n        weight_i = exp_kernel(x', z'_i, σ)  # 计算距离权重 π_x(z)\n        label_i = f(z_i)             # 获取黑盒模型预测\n        Z.append((z'_i, label_i, weight_i))\n\n    # 3. 拟合加权稀疏线性模型\n    g = weighted_lasso(Z, K)  # K为最大特征数\n    return g  # 返回可解释模型（各特征的权重即为解释）\n</code></pre>\n<h5>动机与背景</h5>\n<p>机器学习模型在实际部署中面临\"信任\"问题：即使模型在测试集上表现优异，用户仍然需要理解<strong>为什么</strong>模型做出某个预测，才能决定是否信任并采纳该预测。传统方法要么只适用于特定模型（如决策树本身可解释），要么试图全局近似复杂模型（往往不够忠实）。LIME 的核心洞察是：<strong>即使全局行为极其复杂的模型，在单个样本的局部邻域内也可以用简单的线性模型忠实近似</strong>。</p>\n<h5>核心机制：局部可解释近似</h5>\n<p>LIME 的目标函数定义为：</p>\n<div class=\"kb-math kb-math-display\">\\xi(x) = \\arg\\min_{g \\in G} \\; \\mathcal{L}(f, g, \\pi_x) + \\Omega(g)</div>\n<p>其中：\n- <span class=\"kb-math kb-math-inline\">f</span> 是待解释的黑盒模型\n- <span class=\"kb-math kb-math-inline\">G</span> 是可解释模型族（如线性模型、决策树）\n- <span class=\"kb-math kb-math-inline\">\\pi_x(z)</span> 是以 <span class=\"kb-math kb-math-inline\">x</span> 为中心的邻域核函数，定义样本权重\n- <span class=\"kb-math kb-math-inline\">\\mathcal{L}(f, g, \\pi_x)</span> 度量 <span class=\"kb-math kb-math-inline\">g</span> 在 <span class=\"kb-math kb-math-inline\">\\pi_x</span> 定义的局部邻域内对 <span class=\"kb-math kb-math-inline\">f</span> 的近似忠实度\n- <span class=\"kb-math kb-math-inline\">\\Omega(g)</span> 是模型复杂度惩罚（确保解释足够简洁）</p>\n<div class=\"key-point\">💡 关键：LIME 不要求 <span class=\"kb-math kb-math-inline\">g</span> 全局忠实于 <span class=\"kb-math kb-math-inline\">f</span>，只要求在 <span class=\"kb-math kb-math-inline\">x</span> 的局部邻域内忠实。这使得即使 <span class=\"kb-math kb-math-inline\">f</span> 极其复杂，<span class=\"kb-math kb-math-inline\">g</span> 仍然可以是简单的线性模型。</div>\n<p>具体地，当 <span class=\"kb-math kb-math-inline\">G</span> 为线性模型时，局部加权损失为：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}(f, g, \\pi_x) = \\sum_{z, z&#x27; \\in \\mathcal{Z}} \\pi_x(z) \\left( f(z) - g(z&#x27;) \\right)^2</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\pi_x(z) = \\exp\\left(-D(x, z)^2 / \\sigma^2\\right)</span> 为指数核函数，<span class=\"kb-math kb-math-inline\">D</span> 为距离度量（文本用余弦距离，图像用 L2 距离）。</p>\n<h5>可解释表示与扰动策略</h5>\n<p>LIME 引入<strong>可解释表示</strong>（interpretable representation）<span class=\"kb-math kb-math-inline\">x&#x27; \\in \\{0, 1\\}^{d&#x27;}</span>，将原始高维特征映射为人类可理解的二值向量：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>数据类型</th>\n<th>原始表示</th>\n<th>可解释表示</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>文本</td>\n<td>词嵌入/TF-IDF</td>\n<td>词是否出现（二值向量）</td>\n</tr>\n<tr>\n<td>图像</td>\n<td>像素矩阵</td>\n<td>超像素是否激活（二值向量）</td>\n</tr>\n<tr>\n<td>表格</td>\n<td>连续/离散特征</td>\n<td>特征是否在原始值附近（离散化）</td>\n</tr>\n</tbody>\n</table></div>\n<p>扰动采样通过随机将可解释表示中的部分维度置零实现，然后映射回原始空间（如文本中移除对应词，图像中将对应超像素置灰）。</p>\n<h5>SP-LIME：全局代表性解释选择</h5>\n<p>单个 LIME 解释只能解释一个预测。为帮助用户全面理解模型行为，SP-LIME 提出选择 <span class=\"kb-math kb-math-inline\">B</span> 个最具代表性的样本及其解释：</p>\n<div class=\"kb-math kb-math-display\">\\text{Pick}(W, I) = \\arg\\max_{V, |V| \\leq B} \\sum_{j=1}^{d&#x27;} \\mathbb{1}\\left[\\exists\\, i \\in V : W_{ij} &gt; 0\\right]</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">W_{ij} = |w_{g_{i,j}}|</span> 为第 <span class=\"kb-math kb-math-inline\">i</span> 个样本解释中第 <span class=\"kb-math kb-math-inline\">j</span> 个特征的重要性权重绝对值。</p>\n<div class=\"warn-box\">⚠️ 注意：该优化问题是 NP-hard 的（加权集合覆盖问题），但由于目标函数是子模的（submodular）且单调非递减，贪心算法可保证 <span class=\"kb-math kb-math-inline\">(1 - 1/e)</span> 近似比。</div>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>模型依赖</th>\n<th>解释范围</th>\n<th>可解释性</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>梯度/显著性图</td>\n<td>需可微模型</td>\n<td>局部</td>\n<td>像素级，难理解</td>\n</tr>\n<tr>\n<td>决策树代理</td>\n<td>模型无关</td>\n<td>全局</td>\n<td>全局近似不忠实</td>\n</tr>\n<tr>\n<td>SHAP（后续工作）</td>\n<td>模型无关</td>\n<td>局部</td>\n<td>基于博弈论</td>\n</tr>\n<tr>\n<td><strong>LIME</strong></td>\n<td><strong>模型无关</strong></td>\n<td><strong>局部</strong></td>\n<td><strong>人类可理解的稀疏特征</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>LIME 的核心优势在于：(1) 完全模型无关，只需黑盒预测接口；(2) 使用人类可理解的可解释表示而非原始特征；(3) 通过局部忠实性在简洁性和准确性之间取得平衡。</p>\n<h5>实验验证</h5>\n<p>论文在文本分类（20 Newsgroups）和图像分类（ImageNet + Google Inception）上验证了 LIME 的有效性：\n- <strong>信任单个预测</strong>：用户通过 LIME 解释能识别出模型利用了虚假相关（如通过邮件头而非内容分类）\n- <strong>模型选择</strong>：非专家用户借助 LIME 解释能选出真正泛化更好的模型，优于仅看准确率\n- <strong>特征工程</strong>：用户通过 SP-LIME 识别出数据集中的噪声特征，移除后模型性能提升</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "定义\"加性特征归因方法\"类（Definition 1）：解释模型 <span class=\"kb-math kb-math-inline\">g(z&#x27;) = \\phi_0 + \\sum_{i=1}^{M} \\phi_i z&#x27;_i</span>，统一 LIME、DeepLIFT、LRP、Shapley regression values、Shapley sampling values、QII 六种方法",
        "唯一性定理（Theorem 1）：在加性特征归因框架下，同时满足 Local Accuracy、Missingness、Consistency 三条性质的解唯一，即 Shapley 值",
        "SHAP 值定义：<span class=\"kb-math kb-math-inline\">\\phi_i = \\sum_{S \\subseteq F \\setminus \\{i\\}} \\frac{|S|!(M-|S|-1)!}{M!} [f_x(S \\cup \\{i\\}) - f_x(S)]</span>，其中 <span class=\"kb-math kb-math-inline\">f_x(S) = E[f(x) | x_S]</span>",
        "Kernel SHAP（Theorem 2）：证明 LIME 框架在特定核函数 <span class=\"kb-math kb-math-inline\">\\pi_{x&#x27;}(z&#x27;) = \\frac{M-1}{\\binom{M}{|z&#x27;|}|z&#x27;|(M-|z&#x27;|)}</span>、平方损失、零正则化下恢复精确 Shapley 值",
        "Deep SHAP：利用 DeepLIFT 反向传播规则递归组合各层 SHAP 值，实现深度网络的高效近似归因",
        "模型特定方法：Linear SHAP（<span class=\"kb-math kb-math-inline\">\\phi_i = w_i(x_i - E[x_i])</span>）、Max SHAP（<span class=\"kb-math kb-math-inline\">O(M^2)</span> 复杂度）、Low-Order SHAP（低阶交互近似）"
      ],
      "detail": "<p><img alt=\"SHAP 核心框架：Shapley 核权重与 Deep SHAP 组合规则\" src=\"https://ar5iv.labs.arxiv.org/html/1705.07874/assets/x2.png\" />\n<em>图：(A) Shapley 核权重按联盟大小对称分布，与启发式核显著不同；(B) Deep SHAP 利用网络组合结构递归反向传播 SHAP 值</em></p>\n<p><strong>算法伪代码 — Kernel SHAP 估计流程：</strong></p>\n<pre><code class=\"language-python\"># Kernel SHAP: 通过加权线性回归估计 SHAP 值\ndef kernel_shap(f, x, M, num_samples):\n    &quot;&quot;&quot;\n    f: 原始模型\n    x: 待解释样本\n    M: 特征数量\n    num_samples: 采样联盟数\n    &quot;&quot;&quot;\n    phi_0 = E[f(x)]  # 基线值（模型在训练集上的期望输出）\n\n    # 1. 采样二值向量 z' ∈ {0,1}^M（排除全0和全1）\n    Z = sample_coalitions(M, num_samples)\n\n    # 2. 对每个 z'，构造映射样本 h_x(z') 并计算模型输出\n    y = []\n    weights = []\n    for z_prime in Z:\n        x_masked = map_to_original(z_prime, x)  # 缺失特征用条件期望填充\n        y.append(f(x_masked))\n        # Shapley 核权重\n        s = sum(z_prime)\n        w = (M - 1) / (comb(M, s) * s * (M - s))\n        weights.append(w)\n\n    # 3. 加权最小二乘回归求解 φ\n    # min_φ Σ_z' π(z') [f(h_x(z')) - (φ_0 + Σ φ_i z'_i)]²\n    phi = weighted_linear_regression(Z, y, weights)\n\n    return phi  # φ_1, ..., φ_M 即为各特征的 SHAP 值\n</code></pre>\n<h5>动机与背景</h5>\n<p>随着机器学习模型复杂度不断提升（深度网络、集成方法等），模型预测的可解释性成为关键需求。2017 年前已有多种解释方法被独立提出——LIME 通过局部线性近似解释、DeepLIFT/LRP 通过反向传播归因、Shapley regression values 通过博弈论分配——但这些方法之间缺乏统一的理论联系，用户难以判断何种方法更优、各方法的理论保证是什么。</p>\n<p>SHAP 的核心动机是：<strong>能否找到一个统一框架，将这些方法纳入同一类别，并从公理化角度确定最优解？</strong></p>\n<h5>加性特征归因方法的统一框架</h5>\n<p>SHAP 首先定义了\"加性特征归因方法\"（Additive Feature Attribution Methods）：</p>\n<div class=\"kb-math kb-math-display\">g(z&#x27;) = \\phi_0 + \\sum_{i=1}^{M} \\phi_i z&#x27;_i, \\quad z&#x27; \\in \\{0,1\\}^M</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">z&#x27;</span> 是简化输入空间中的二值向量，<span class=\"kb-math kb-math-inline\">z&#x27;_i = 1</span> 表示第 <span class=\"kb-math kb-math-inline\">i</span> 个特征\"存在\"，<span class=\"kb-math kb-math-inline\">\\phi_i \\in \\mathbb{R}</span> 是第 <span class=\"kb-math kb-math-inline\">i</span> 个特征的归因值。论文证明 LIME、DeepLIFT、LRP、Shapley regression values、Shapley sampling values 和 QII 都属于此类。</p>\n<div class=\"key-point\">💡 关键：这一统一视角揭示了看似不同的方法实际上都在求解同一形式的线性解释模型，区别仅在于如何确定 <span class=\"kb-math kb-math-inline\">\\phi_i</span> 的值。</div>\n<h5>三条公理与唯一性定理</h5>\n<p>论文提出三条期望性质：</p>\n<ol>\n<li>\n<p><strong>Local Accuracy（局部准确性）</strong>：解释模型在原始输入处的输出等于原模型输出：\n<div class=\"kb-math kb-math-display\">f(x) = g(x&#x27;) = \\phi_0 + \\sum_{i=1}^{M} \\phi_i</div></p>\n</li>\n<li>\n<p><strong>Missingness（缺失性）</strong>：缺失的特征不应有归因值：\n<div class=\"kb-math kb-math-display\">x&#x27;_i = 0 \\Rightarrow \\phi_i = 0</div></p>\n</li>\n<li>\n<p><strong>Consistency（一致性）</strong>：若某特征在新模型中的边际贡献不低于旧模型，则其归因值不应降低：\n<div class=\"kb-math kb-math-display\">f&#x27;_x(S \\cup \\{i\\}) - f&#x27;_x(S) \\geq f_x(S \\cup \\{i\\}) - f_x(S), \\forall S \\subseteq F \\setminus \\{i\\} \\Rightarrow \\phi_i(f&#x27;, x) \\geq \\phi_i(f, x)</div></p>\n</li>\n</ol>\n<p><strong>Theorem 1</strong> 证明：在加性特征归因方法类中，同时满足以上三条性质的解唯一，即 Shapley 值：</p>\n<div class=\"kb-math kb-math-display\">\\phi_i(f, x) = \\sum_{S \\subseteq F \\setminus \\{i\\}} \\frac{|S|!(M - |S| - 1)!}{M!} \\left[ f_x(S \\cup \\{i\\}) - f_x(S) \\right]</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">f_x(S) = E[f(x) | x_S]</span> 表示给定特征子集 <span class=\"kb-math kb-math-inline\">S</span> 的值后模型输出的条件期望。</p>\n<div class=\"warn-box\">⚠️ 注意：精确计算 SHAP 值需要遍历所有 <span class=\"kb-math kb-math-inline\">2^M</span> 个特征子集，复杂度为 <span class=\"kb-math kb-math-inline\">O(2^M)</span>，因此实际应用中必须使用近似方法。</div>\n<h5>Kernel SHAP：将 LIME 与 Shapley 值统一</h5>\n<p>Kernel SHAP 是论文最重要的算法贡献之一。它证明（Theorem 2）：当 LIME 的三个自由参数取特定值时，其解恰好等于 Shapley 值：</p>\n<ul>\n<li><strong>损失函数</strong>：平方损失 <span class=\"kb-math kb-math-inline\">L(f, g, \\pi_{x&#x27;}) = \\sum_{z&#x27;} \\pi_{x&#x27;}(z&#x27;) [f(h_x(z&#x27;)) - g(z&#x27;)]^2</span></li>\n<li><strong>核函数</strong>：Shapley 核 <span class=\"kb-math kb-math-inline\">\\pi_{x&#x27;}(z&#x27;) = \\frac{(M-1)}{\\binom{M}{|z&#x27;|} \\cdot |z&#x27;| \\cdot (M - |z&#x27;|)}</span></li>\n<li><strong>正则化</strong>：<span class=\"kb-math kb-math-inline\">\\Omega = 0</span></li>\n</ul>\n<p>Shapley 核的直觉是：<strong>越接近全空或全满的联盟（<span class=\"kb-math kb-math-inline\">|z&#x27;|</span> 接近 0 或 <span class=\"kb-math kb-math-inline\">M</span>）权重越大</strong>，因为这些联盟提供了关于单个特征边际贡献最清晰的信息。这与 LIME 原始使用的指数核或余弦核形成鲜明对比。</p>\n<p>在实践中，Kernel SHAP 通过采样联盟子集并求解加权最小二乘问题来近似 SHAP 值，既保留了 LIME 的模型无关性，又获得了 Shapley 值的理论保证。</p>\n<h5>Deep SHAP：深度网络的组合近似</h5>\n<p>对于深度神经网络，Kernel SHAP 的采样效率不够高。Deep SHAP 利用网络的层次组合结构，将整体 SHAP 值分解为各组件 SHAP 值的递归组合：</p>\n<div class=\"kb-math kb-math-display\">m_{x_j f_3} = \\frac{\\phi_i(f_3, x)}{x_j - E[x_j]}</div>\n<div class=\"kb-math kb-math-display\">m_{y_i f_3} = \\sum_{j=1}^{2} m_{y_i f_j} \\cdot m_{x_j f_3} \\quad \\text{(链式法则)}</div>\n<div class=\"kb-math kb-math-display\">\\phi_i(f_3, y) \\approx m_{y_i f_3} \\cdot (y_i - E[y_i]) \\quad \\text{(线性近似)}</div>\n<p>Deep SHAP 的核心思想是：\n1. 对网络中每个简单组件（线性层、激活函数、max pooling）解析计算局部 SHAP 值\n2. 利用 DeepLIFT 风格的反向传播规则将各层的乘子（multiplier）递归组合\n3. 最终得到输入特征对输出的 SHAP 值近似</p>\n<div class=\"key-point\">💡 关键：与原始 DeepLIFT 不同，Deep SHAP 不需要启发式选择线性化规则，而是从各组件的 Shapley 值推导出有效的线性化方式。例如对 max 函数，Deep SHAP 的归因优于 DeepLIFT 的启发式规则。</div>\n<h5>与传统方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>理论保证</th>\n<th>模型依赖</th>\n<th>计算复杂度</th>\n<th>统一框架</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>LIME</td>\n<td>无公理保证</td>\n<td>模型无关</td>\n<td>中等</td>\n<td>✗</td>\n</tr>\n<tr>\n<td>DeepLIFT</td>\n<td>满足 Local Accuracy + Missingness</td>\n<td>深度网络</td>\n<td>快</td>\n<td>✗</td>\n</tr>\n<tr>\n<td>Shapley regression</td>\n<td>满足全部三条</td>\n<td>模型无关</td>\n<td><span class=\"kb-math kb-math-inline\">O(2^M)</span></td>\n<td>✗</td>\n</tr>\n<tr>\n<td><strong>Kernel SHAP</strong></td>\n<td><strong>满足全部三条</strong></td>\n<td><strong>模型无关</strong></td>\n<td><strong>可控采样</strong></td>\n<td><strong>✓</strong></td>\n</tr>\n<tr>\n<td><strong>Deep SHAP</strong></td>\n<td><strong>近似满足三条</strong></td>\n<td><strong>深度网络</strong></td>\n<td><strong>快（反向传播）</strong></td>\n<td><strong>✓</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>实验表明：(1) Kernel SHAP 在保持理论保证的同时，计算效率优于经典 Shapley sampling；(2) Deep SHAP 在 MNIST 数字识别任务上的归因质量优于原始 DeepLIFT，通过遮蔽实验验证了更好的 Shapley 近似带来更准确的特征重要性排序。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>核心公式</strong>：对目标类别得分关于最后卷积层特征图的梯度做全局平均池化，得到通道重要性权重 <span class=\"kb-math kb-math-inline\">\\alpha_k^c</span>",
        "<strong>类别判别性热力图</strong>：<span class=\"kb-math kb-math-inline\">L_{\\text{Grad-CAM}}^c = \\text{ReLU}(\\sum_k \\alpha_k^c A^k)</span>，ReLU 保留正影响区域",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>概念瓶颈架构</strong>：模型分为 <span class=\"kb-math kb-math-inline\">g: x \\to \\hat{c}</span>（输入→概念）和 <span class=\"kb-math kb-math-inline\">f: \\hat{c} \\to \\hat{y}</span>（概念→目标）两阶段，预测完全通过概念层传递",
      ...
      "detail": "<p><img alt=\"CBM 概念瓶颈模型架构示意图\" src=\"https://proceedings.mlr.press/v119/koh20a/koh20a-Figure1-1.png\" />\n<em>图：概念瓶颈模型在两个任务上的示意。上方为骨关节炎 X 光分级（输入→临床概念→KLG 等级），下方为鸟类识别（输入→视觉属性→物种）。</em></p>\n<h5>动机与背景</h5>\n<p>深度学习模型虽然在医学影像、细粒度识别等任务上表现优异，但其\"黑箱\"特性严重限制了在高风险场景中的部署。现有可解释性方法（如 TCAV、Network Dissection）主要做<strong>事后分析</strong>——从已训练好的模型中探测概念，但这种方式：\n1. 概念精度较低（线性探测难以准确恢复概念）\n2. 无法支持干预（即使找到与概念相关的神经元组合，也不清楚如何修改激活来改变单一概念的预测）</p>\n<p>CBM 的核心洞察是：如果任务本身就是通过一组标准概念来定义的（如\"骨关节炎由骨刺、关节间隙变窄等标志判定\"），那么直接将这些概念作为模型的中间表示，既能保证可解释性，又能支持人工干预。</p>\n<h5>模型形式化</h5>\n<p>给定输入 <span class=\"kb-math kb-math-inline\">x \\in \\mathbb{R}^d</span>，目标 <span class=\"kb-math kb-math-inline\">y \\in \\mathbb{R}</span>，概念向量 <span class=\"kb-math kb-math-inline\">c \\in \\mathbb{R}^k</span>，CBM 定义为：</p>\n<div class=\"kb-math kb-math-display\">\\hat{y} = f(g(x)), \\quad \\text{其中} \\quad \\hat{c} = g(x) \\in \\mathbb{R}^k</div>\n<p>模型的预测完全通过概念瓶颈 <span class=\"kb-math kb-math-inline\">\\hat{c}</span> 传递。训练时同时优化概念损失和任务损失。</p>\n<p><strong>四种训练变体的损失函数：</strong></p>\n<ol>\n<li><strong>Independent</strong>：分别独立优化\n<div class=\"kb-math kb-math-display\">\\hat{g} = \\arg\\min_g \\sum_{i,j} L_{C_j}(g_j(x^{(i)}); c_j^{(i)}), \\quad \\hat{f} = \\arg\\min_f \\sum_i L_Y(f(c^{(i)}); y^{(i)})</div></li>\n</ol>\n<div class=\"key-point\">💡 关键：Independent 的 <span class=\"kb-math kb-math-inline\">f</span> 在训练时使用<strong>真实概念</strong> <span class=\"kb-math kb-math-inline\">c</span>，但测试时使用<strong>预测概念</strong> <span class=\"kb-math kb-math-inline\">\\hat{c}</span>，存在 train-test mismatch。</div>\n<ol>\n<li>\n<p><strong>Sequential</strong>：先训练 <span class=\"kb-math kb-math-inline\">\\hat{g}</span>，再用 <span class=\"kb-math kb-math-inline\">\\hat{g}(x)</span> 的输出训练 <span class=\"kb-math kb-math-inline\">\\hat{f}</span>，消除了 mismatch。</p>\n</li>\n<li>\n<p><strong>Joint</strong>：联合优化加权目标\n<div class=\"kb-math kb-math-display\">\\hat{f}, \\hat{g} = \\arg\\min_{f,g} \\sum_i \\left[ L_Y(f(g(x^{(i)})); y^{(i)}) + \\lambda \\sum_j L_{C_j}(g_j(x^{(i)}); c_j^{(i)}) \\right]</div></p>\n</li>\n</ol>\n<div class=\"warn-box\">⚠️ 注意：<span class=\"kb-math kb-math-inline\">\\lambda</span> 控制概念精度与任务精度的权衡。<span class=\"kb-math kb-math-inline\">\\lambda \\to 0</span> 退化为 Standard 模型，<span class=\"kb-math kb-math-inline\">\\lambda \\to \\infty</span> 等价于 Sequential。</div>\n<ol>\n<li><strong>Standard</strong>：忽略概念，直接端到端优化 <span class=\"kb-math kb-math-inline\">L_Y</span>。</li>\n</ol>\n<h5>实现方式</h5>\n<p>将标准深度网络（如 Inception-v3）的某一层调整为 <span class=\"kb-math kb-math-inline\">k</span> 个神经元，使其与概念数量匹配。对于分类任务，<span class=\"kb-math kb-math-inline\">g(x)</span> 输出概念 logits <span class=\"kb-math kb-math-inline\">\\hat{\\ell} \\in \\mathbb{R}^k</span>，通过 sigmoid 转为概率：<span class=\"kb-math kb-math-inline\">P(\\hat{c}_j = 1) = \\sigma(\\hat{\\ell}_j)</span>。</p>\n<pre><code class=\"language-python\"># CBM 概念瓶颈模型伪代码\nclass ConceptBottleneckModel:\n    def __init__(self, backbone, n_concepts, n_classes):\n        self.g = nn.Sequential(backbone, nn.Linear(hidden_dim, n_concepts))  # x → c\n        self.f = nn.Linear(n_concepts, n_classes)  # c → y\n\n    def forward(self, x):\n        c_hat = self.g(x)           # 概念预测（logits）\n        y_hat = self.f(c_hat)       # 任务预测\n        return y_hat, c_hat\n\n    def intervene(self, x, concept_idx, true_value):\n        &quot;&quot;&quot;测试时干预：将指定概念替换为真实值&quot;&quot;&quot;\n        c_hat = self.g(x)\n        c_hat[concept_idx] = true_value  # 人工修正\n        y_hat = self.f(c_hat)\n        return y_hat\n\n# Joint 训练\nfor x, y, c in dataloader:\n    y_hat, c_hat = model(x)\n    loss = L_Y(y_hat, y) + lambda_ * sum(L_Cj(c_hat_j, c_j) for j in range(k))\n    loss.backward()\n    optimizer.step()\n</code></pre>\n<h5>测试时概念干预</h5>\n<p>CBM 的核心优势在于支持<strong>测试时干预</strong>（test-time intervention）：领域专家可以检查模型的概念预测，修正错误的概念，观察最终预测如何变化。</p>\n<p>干预机制：\n- <strong>OAI 任务</strong>：直接将预测概念 <span class=\"kb-math kb-math-inline\">\\hat{c}_j</span> 替换为真实值 <span class=\"kb-math kb-math-inline\">c_j</span>（概念为连续有序变量）\n- <strong>CUB 任务</strong>：按概念组（如\"翅膀颜色\"包含 15 个二值属性）进行干预，使用 CDF 匹配将二值概念组转换为单一有序值</p>\n<p>实验结果表明：\n- OAI 上干预仅 2 个概念（共 10 个），RMSE 从 &gt;0.4 降至 ≈0.3\n- CUB 上干预约 8 个概念组（共 28 组），错误率从 ≈0.24 降至 ≈0.05\n- Independent 模型在所有概念被替换时表现最好（因为 <span class=\"kb-math kb-math-inline\">f</span> 直接在真实概念上训练）\n- 概念精度与干预效果高度相关：概念精度越高，干预收益越大</p>\n<div class=\"key-point\">💡 关键：干预使得\"单个放射科医生 + CBM\"可能超越\"单独的放射科医生\"或\"单独的模型\"，实现真正的人机协作。</div>\n<h5>与事后解释方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>CBM（概念瓶颈）</th>\n<th>Post-hoc 探测（如 TCAV）</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>概念精度</td>\n<td>高（OAI: r=0.84, CUB: F1=0.92）</td>\n<td>低（OAI: r=0.72, CUB: F1=0.77）</td>\n</tr>\n<tr>\n<td>支持干预</td>\n<td>✅ 直接修改瓶颈层</td>\n<td>❌ 无法清晰修改激活</td>\n</tr>\n<tr>\n<td>训练要求</td>\n<td>需要概念标注</td>\n<td>仅需训练后的探测数据</td>\n</tr>\n<tr>\n<td>任务精度</td>\n<td>略有损失（Joint 接近 Standard）</td>\n<td>不影响原模型</td>\n</tr>\n</tbody>\n</table></div>\n<h5>鲁棒性实验</h5>\n<p>在构造的 CUB 背景偏移实验中（训练时每个鸟类物种与特定背景绑定，测试时打乱），Standard 模型严重依赖虚假背景特征导致性能崩溃，而 CBM 由于概念（如翅膀颜色）在多个物种间共享、跨越多种背景，因此对背景偏移更鲁棒。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>SAE 方法</strong>：训练带 L1 正则化的自编码器，学习过完备（overcomplete）特征字典，将 <span class=\"kb-math kb-math-inline\">d</span>-维激活映射到 <span class=\"kb-math kb-math-inline\">m \\gg d</span> 维稀疏表示",
    ...
    },
    {
      ...
      "keyPoints": [
        "提出 DP-SGD 算法：每步对每样本梯度做 L2 裁剪（阈值 <span class=\"kb-math kb-math-inline\">C</span>）→ 求和 → 加高斯噪声 <span class=\"kb-math kb-math-inline\">\\mathcal{N}(0, \\sigma^2 C^2 I)</span> → 除以批大小 → 梯度下降",
        "引入 Moments Accountant：通过追踪隐私损失随机变量的对数矩生成函数，实现比强组合定理紧 <span class=\"kb-math kb-math-inline\">\\sqrt{\\log(T/\\delta)}</span> 因子的隐私分析",
        "利用 Poisson 子采样的隐私放大效应：每条记录独立以概率 <span class=\"kb-math kb-math-inline\">q = L/N</span> 被选中",
        "差分隐私 PCA：对协方差矩阵加噪后取主方向，用于降维预处理",
        "TensorFlow 实现：per-example gradient 算子 + Sanitizer（裁剪加噪）+ PrivacyAccountant（隐私追踪）",
        "超参数洞察：最优批大小约 <span class=\"kb-math kb-math-inline\">\\sqrt{N}</span>；裁剪阈值取梯度范数中位数；网络增大不降低准确率"
      ],
      "detail": "<p><img alt=\"DP-SGD Algorithm\" src=\"https://arxiv.org/html/1607.00133v1/extracted/figures/algorithm1.png\" />\n<em>图：DP-SGD 算法流程——对每样本梯度裁剪后加噪聚合</em></p>\n<div class=\"warn-box\">⚠️ 注意：若上图不可访问，核心流程见下方伪代码。</div>\n<pre><code class=\"language-python\"># Algorithm 1: Differentially Private SGD (DP-SGD)\n# 输入: 数据集 {x_1,...,x_N}, 损失函数 L(θ), 学习率 η_t\n#       噪声尺度 σ, 裁剪阈值 C, 批大小 L\n\nfor t in range(T):\n    # 1. Poisson子采样: 每条记录独立以概率 q=L/N 被选中\n    L_t = poisson_sample(dataset, q=L/N)\n\n    # 2. 计算每样本梯度\n    for x_i in L_t:\n        g_i = compute_gradient(θ_t, x_i)\n\n    # 3. 梯度裁剪 (L2范数)\n    for x_i in L_t:\n        g_i_clipped = g_i / max(1, ||g_i||_2 / C)\n\n    # 4. 加噪聚合\n    g_noisy = (1/L) * (sum(g_i_clipped) + N(0, σ²C²I))\n\n    # 5. 参数更新\n    θ_{t+1} = θ_t - η_t * g_noisy\n</code></pre>\n<h5>动机与背景</h5>\n<p>深度学习模型在训练过程中可能记忆训练数据的敏感信息。模型反演攻击（Fredrikson et al., 2015）已证明可以从训练好的模型中恢复训练数据。差分隐私（Dwork et al., 2006）提供了形式化的隐私保证，但此前的 DP 方法主要针对凸优化问题（如 output perturbation、objective perturbation），无法直接应用于深度学习的非凸优化。</p>\n<p>已有工作的不足：\n- Shokri &amp; Shmatikov (2015)：分布式训练中按单个参数计算隐私损失，总隐私预算达数千级别\n- 强组合定理：对 <span class=\"kb-math kb-math-inline\">T</span> 步迭代的隐私损失估计过于宽松（<span class=\"kb-math kb-math-inline\">O(\\sqrt{T \\log(1/\\delta)} \\cdot \\varepsilon_0)</span>）\n- 凸 ERM 方法（Wu et al.）：MNIST 仅达 83% 准确率</p>\n<h5>核心机制：梯度裁剪 + 高斯噪声</h5>\n<p><strong>梯度裁剪</strong>确保单样本梯度的 L2 灵敏度有界：</p>\n<div class=\"kb-math kb-math-display\">\\bar{\\mathbf{g}}_t(x_i) = \\mathbf{g}_t(x_i) / \\max\\left(1, \\frac{\\|\\mathbf{g}_t(x_i)\\|_2}{C}\\right)</div>\n<div class=\"key-point\">💡 关键：裁剪后 <span class=\"kb-math kb-math-inline\">\\|\\bar{\\mathbf{g}}_t(x_i)\\|_2 \\leq C</span>，因此聚合梯度对任意单样本的 L2 灵敏度至多为 <span class=\"kb-math kb-math-inline\">C</span>。</div>\n<p><strong>高斯机制</strong>：对灵敏度为 <span class=\"kb-math kb-math-inline\">C</span> 的查询添加 <span class=\"kb-math kb-math-inline\">\\mathcal{N}(0, \\sigma^2 C^2 I)</span> 噪声，满足 <span class=\"kb-math kb-math-inline\">(\\varepsilon, \\delta)</span>-DP（当 <span class=\"kb-math kb-math-inline\">\\sigma</span> 足够大时）。</p>\n<p><strong>加噪聚合</strong>：</p>\n<div class=\"kb-math kb-math-display\">\\tilde{\\mathbf{g}}_t = \\frac{1}{L}\\left(\\sum_{i \\in L_t} \\bar{\\mathbf{g}}_t(x_i) + \\mathcal{N}(0, \\sigma^2 C^2 \\mathbf{I})\\right)</div>\n<h5>核心机制：Moments Accountant</h5>\n<p>传统的强组合定理对 <span class=\"kb-math kb-math-inline\">T</span> 次 <span class=\"kb-math kb-math-inline\">(\\varepsilon_0, \\delta_0)</span>-DP 机制的组合给出：</p>\n<div class=\"kb-math kb-math-display\">(\\varepsilon_0 \\sqrt{2T \\ln(1/\\delta&#x27;)} + T\\varepsilon_0(e^{\\varepsilon_0}-1),\\; T\\delta_0 + \\delta&#x27;)</div>\n<p>Moments Accountant 通过追踪隐私损失的<strong>对数矩生成函数</strong>获得更紧的界：</p>\n<div class=\"kb-math kb-math-display\">\\alpha_{\\mathcal{M}}(\\lambda) = \\max_{d, d&#x27;} \\log \\mathbb{E}_{o \\sim \\mathcal{M}(d)}\\left[\\exp\\left(\\lambda \\cdot \\log \\frac{\\Pr[\\mathcal{M}(d)=o]}{\\Pr[\\mathcal{M}(d&#x27;)=o]}\\right)\\right]</div>\n<p><strong>关键性质</strong>：\n1. <strong>可组合性</strong>：<span class=\"kb-math kb-math-inline\">\\alpha_{\\mathcal{M}_{1:T}}(\\lambda) \\leq \\sum_{t=1}^T \\alpha_{\\mathcal{M}_t}(\\lambda)</span>\n2. <strong>尾界转换</strong>：<span class=\"kb-math kb-math-inline\">\\delta = \\min_\\lambda \\exp(\\alpha(\\lambda) - \\lambda\\varepsilon)</span></p>\n<p>对于采样率 <span class=\"kb-math kb-math-inline\">q</span> 的子采样高斯机制：</p>\n<div class=\"kb-math kb-math-display\">\\alpha(\\lambda) \\leq \\frac{q^2 \\lambda(\\lambda+1)}{(1-q)\\sigma^2} + O(q^3/\\sigma^3)</div>\n<div class=\"key-point\">💡 关键：Moments Accountant 比强组合定理省去 <span class=\"kb-math kb-math-inline\">\\sqrt{\\log(T/\\delta)}</span> 因子。实测 <span class=\"kb-math kb-math-inline\">T=10000</span> 时，<span class=\"kb-math kb-math-inline\">\\varepsilon</span> 从 9.34 降至 1.26（改善 7.4×）。</div>\n<p><strong>主定理</strong>：存在常数 <span class=\"kb-math kb-math-inline\">c_1, c_2</span>，Algorithm 1 在 <span class=\"kb-math kb-math-inline\">T</span> 步后满足 <span class=\"kb-math kb-math-inline\">(\\varepsilon, \\delta)</span>-DP，只要：</p>\n<div class=\"kb-math kb-math-display\">\\sigma \\geq c_2 \\frac{q\\sqrt{T \\log(1/\\delta)}}{\\varepsilon}</div>\n<h5>训练流程与实现</h5>\n<ol>\n<li><strong>差分隐私 PCA 预处理</strong>：将输入归一化为单位向量，对 <span class=\"kb-math kb-math-inline\">A^T A</span> 加高斯噪声后取主方向投影（如 MNIST 784→60 维）</li>\n<li><strong>DP-SGD 训练</strong>：在投影空间中训练全连接网络</li>\n<li><strong>隐私预算分配</strong>：PCA 和 SGD 各消耗部分 <span class=\"kb-math kb-math-inline\">(\\varepsilon, \\delta)</span> 预算，通过组合定理合并</li>\n<li><strong>TensorFlow 实现</strong>：利用 Goodfellow (2015) 的 per-example gradient 技巧高效计算每样本梯度</li>\n</ol>\n<h5>实验结果</h5>\n<p><strong>MNIST</strong>（模型：60维PCA → 1000隐藏单元ReLU → Softmax；非隐私基线 98.3%）：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th><span class=\"kb-math kb-math-inline\">(\\varepsilon, \\delta)</span></th>\n<th>噪声 <span class=\"kb-math kb-math-inline\">(\\sigma, \\sigma_p)</span></th>\n<th>测试准确率</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>(0.5, 10⁻⁵)</td>\n<td>(8, 16)</td>\n<td>90%</td>\n</tr>\n<tr>\n<td>(2, 10⁻⁵)</td>\n<td>(4, 7)</td>\n<td>95%</td>\n</tr>\n<tr>\n<td>(8, 10⁻⁵)</td>\n<td>(2, 4)</td>\n<td>97%</td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>CIFAR-10</strong>（预训练卷积层 + DP全连接层；非隐私基线 ~80%）：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th><span class=\"kb-math kb-math-inline\">(\\varepsilon, \\delta)</span></th>\n<th>测试准确率</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>(2, 10⁻⁵)</td>\n<td>67%</td>\n</tr>\n<tr>\n<td>(4, 10⁻⁵)</td>\n<td>70%</td>\n</tr>\n<tr>\n<td>(8, 10⁻⁵)</td>\n<td>73%</td>\n</tr>\n</tbody>\n</table></div>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>适用范围</th>\n<th>MNIST准确率</th>\n<th>隐私保证</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Output Perturbation</td>\n<td>凸优化</td>\n<td>~83%</td>\n<td>严格DP</td>\n</tr>\n<tr>\n<td>Shokri &amp; Shmatikov</td>\n<td>分布式DL</td>\n<td>未报告</td>\n<td>ε~数千</td>\n</tr>\n<tr>\n<td><strong>DP-SGD (本文)</strong></td>\n<td><strong>通用DL</strong></td>\n<td><strong>95%@ε=2</strong></td>\n<td><strong>严格DP</strong></td>\n</tr>\n</tbody>\n</table></div>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "三个关键超参数：客户端采样比例 <span class=\"kb-math kb-math-inline\">C</span>、本地训练轮数 <span class=\"kb-math kb-math-inline\">E</span>、本地 mini-batch 大小 <span class=\"kb-math kb-math-inline\">B</span>",
        "FedSGD 作为特殊情况：<span class=\"kb-math kb-math-inline\">B = \\infty, E = 1</span> 等价于分布式梯度下降",
      ...
      "detail": "<p><img alt=\"FedAvg 框架示意图\" src=\"https://production-media.paperswithcode.com/methods/fedavg.png\" />\n<em>图：FedAvg 联邦学习框架——服务器协调多个客户端进行本地训练与全局聚合</em></p>\n<pre><code class=\"language-python\"># FedAvg 算法伪代码 (Algorithm 1)\n# === 服务器端 ===\ndef server_execute():\n    w = initialize_global_model()\n    for each_round t = 1, 2, ...:\n        m = max(C * K, 1)  # C: 客户端采样比例, K: 总客户端数\n        S_t = random_sample(m, clients)  # 随机选取 m 个客户端\n        for each client k in S_t (in parallel):\n            w_k = client_update(k, w)\n        # 加权平均聚合\n        w = sum(n_k / n * w_k for k in S_t)  # n_k: 客户端k数据量, n: 总数据量\n\n# === 客户端 k ===\ndef client_update(k, w):\n    B_k = split(P_k, batch_size=B)  # 将本地数据分为大小为B的batch\n    for epoch in range(E):           # 本地训练E个epoch\n        for batch b in B_k:\n            w = w - η * gradient(loss(w, b))  # 本地SGD更新\n    return w\n</code></pre>\n<p><strong>动机与背景</strong></p>\n<p>传统分布式机器学习假设数据可集中存储在数据中心，通过数据并行加速训练。然而现实中，大量有价值的训练数据产生于用户移动设备（手机、平板），出于隐私保护、法规合规和通信成本考虑，这些数据不应也不能上传至中心服务器。FedAvg 提出的联邦学习范式直接在数据产生的设备上进行模型训练，仅传输模型更新，从根本上解决了数据隐私与分布式学习的矛盾。</p>\n<p>联邦学习面临的核心挑战包括：\n1. <strong>Non-IID 数据</strong>：每个用户的本地数据反映其个人使用习惯，不代表全局分布\n2. <strong>不平衡性</strong>：不同用户产生的数据量差异巨大\n3. <strong>大规模参与者</strong>：客户端数量远超每个客户端的数据量\n4. <strong>通信瓶颈</strong>：上行带宽有限（通常 1MB/s 或更低），通信成本远高于计算成本</p>\n<p><strong>核心机制</strong></p>\n<p>FedAvg 的核心思想是<strong>增加本地计算量来换取通信效率</strong>。具体而言，算法通过两个维度增加本地计算：</p>\n<ol>\n<li><strong>增大本地 epoch 数 <span class=\"kb-math kb-math-inline\">E</span></strong>：每个客户端在本地数据上训练多个完整 epoch，而非仅计算一次梯度</li>\n<li><strong>减小 mini-batch 大小 <span class=\"kb-math kb-math-inline\">B</span></strong>：更小的 batch 意味着每个 epoch 内更多的梯度更新步骤</li>\n</ol>\n<p>服务器端的聚合采用加权平均：</p>\n<div class=\"kb-math kb-math-display\">w_{t+1} = \\sum_{k=1}^{K} \\frac{n_k}{n} w_{t+1}^k</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">n_k</span> 为客户端 <span class=\"kb-math kb-math-inline\">k</span> 的本地数据量，<span class=\"kb-math kb-math-inline\">n = \\sum_k n_k</span> 为参与聚合的总数据量。这一加权方式等价于对全局损失函数的无偏估计：</p>\n<div class=\"kb-math kb-math-display\">f(w) = \\sum_{k=1}^{K} \\frac{n_k}{n} F_k(w), \\quad F_k(w) = \\frac{1}{n_k} \\sum_{i \\in \\mathcal{P}_k} f_i(w)</div>\n<div class=\"key-point\">💡 关键：当 <span class=\"kb-math kb-math-inline\">B = \\infty</span>（即使用全部本地数据作为一个 batch）且 <span class=\"kb-math kb-math-inline\">E = 1</span> 时，FedAvg 退化为 FedSGD——每个客户端仅计算一次全量梯度，服务器聚合后等价于集中式 SGD 的一步更新。</div>\n<p><strong>为什么模型平均有效？</strong></p>\n<p>论文通过实验（Figure 1）揭示了一个关键洞察：<strong>共享初始化使得模型平均有效</strong>。从相同初始点出发的两个模型，即使在不同数据子集上训练，其参数空间中的路径足够接近，使得简单的参数平均能产生有意义的模型。这与随机初始化的模型平均形成鲜明对比——后者由于参数置换对称性（permutation symmetry），平均后的模型性能极差。</p>\n<div class=\"warn-box\">⚠️ 注意：当本地训练轮数 <span class=\"kb-math kb-math-inline\">E</span> 过大时，各客户端模型可能偏离过远（client drift），导致聚合后模型质量下降甚至发散。论文建议在实践中适当衰减本地计算量。</div>\n<p><strong>训练流程与通信模式</strong></p>\n<p>每一轮通信的流程为：\n1. 服务器选取 <span class=\"kb-math kb-math-inline\">\\lceil C \\cdot K \\rceil</span> 个客户端（<span class=\"kb-math kb-math-inline\">C</span> 为采样比例）\n2. 服务器将当前全局模型 <span class=\"kb-math kb-math-inline\">w_t</span> 下发给选中的客户端\n3. 各客户端并行执行本地 SGD 训练（<span class=\"kb-math kb-math-inline\">E</span> 个 epoch，batch 大小为 <span class=\"kb-math kb-math-inline\">B</span>）\n4. 客户端将更新后的模型 <span class=\"kb-math kb-math-inline\">w_{t+1}^k</span> 上传至服务器\n5. 服务器执行加权平均得到新的全局模型 <span class=\"kb-math kb-math-inline\">w_{t+1}</span></p>\n<p>每轮的通信量为 <span class=\"kb-math kb-math-inline\">O(|w|)</span>（模型参数量），与本地计算量 <span class=\"kb-math kb-math-inline\">E \\cdot \\lceil n_k / B \\rceil</span> 无关，因此增加本地计算不增加通信开销。</p>\n<p><strong>与传统方法的区别</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>本地计算</th>\n<th>通信轮次</th>\n<th>数据假设</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>并行 SGD (同步)</td>\n<td>1 次梯度计算</td>\n<td>每步通信</td>\n<td>IID 划分</td>\n</tr>\n<tr>\n<td>FedSGD</td>\n<td>1 次全量梯度</td>\n<td>每步通信</td>\n<td>允许 Non-IID</td>\n</tr>\n<tr>\n<td><strong>FedAvg</strong></td>\n<td><strong>E 个 epoch</strong></td>\n<td><strong>大幅减少</strong></td>\n<td><strong>允许 Non-IID</strong></td>\n</tr>\n<tr>\n<td>ADMM/DANE</td>\n<td>求解子问题</td>\n<td>较少</td>\n<td>凸优化假设</td>\n</tr>\n</tbody>\n</table></div>\n<p>FedAvg 的关键优势在于：(1) 无需凸性假设，直接适用于深度网络；(2) 实现极其简单，仅需标准 SGD 加参数平均；(3) 通信效率提升显著且稳定。</p>\n<p><strong>实验验证</strong></p>\n<p>在 CIFAR-10 上，FedAvg（<span class=\"kb-math kb-math-inline\">C=0.1, E=5, B=50</span>）仅需 2000 轮通信即达到 85% 测试精度，而标准 SGD 需要 197,500 次 mini-batch 更新，FedSGD 在相同精度目标下也需要数倍于 FedAvg 的通信轮次。在大规模社交网络下一词预测任务（50 万+客户端）上，FedAvg 同样展现出优于 FedSGD 的收敛速度和最终精度。</p>",
      "quiz": {
