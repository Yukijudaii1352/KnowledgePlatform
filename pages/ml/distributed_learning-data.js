/**
 * distributed_learning-data.js — 由 pipeline/build.py 于 2026-05-26 10:35:20 自动生成。
  ...
  "algos": [
    {
      ...
      "keyPoints": [
        ...
        "核心算法 <strong>FedAvg</strong>：每轮随机选取 <span class=\"kb-math kb-math-inline\">C</span> 比例客户端，各自用本地数据执行 <span class=\"kb-math kb-math-inline\">E</span> 个 epoch、batch size 为 <span class=\"kb-math kb-math-inline\">B</span> 的 SGD，再将模型参数按数据量加权平均",
        "<strong>FedSGD</strong> 作为特例：当 <span class=\"kb-math kb-math-inline\">E=1, B=\\infty</span> 时退化为分布式全批量梯度下降",
        "关键发现：增大本地计算量（减小 <span class=\"kb-math kb-math-inline\">B</span>、增大 <span class=\"kb-math kb-math-inline\">E</span>）可显著减少通信轮次，但过大的 <span class=\"kb-math kb-math-inline\">E</span> 在 Non-IID 数据上可能导致发散",
        "实验覆盖 5 种模型架构：MLP、两种 CNN、字符级 LSTM、大规模词级 LSTM",
        "验证了在 IID 和 pathological Non-IID 数据划分下的有效性"
      ],
      "detail": "<p><img alt=\"FedAvg 参数平均损失曲面\" src=\"https://ar5iv.labs.arxiv.org/html/1602.05629/assets/x1.png\" />\n<em>图：对两个独立训练的 MNIST 模型进行参数平均后的损失变化。当从同一随机初始化出发时（右），平均后的模型损失接近两个端点，说明参数平均在共享初始化条件下是可行的聚合策略。</em></p>\n<pre><code class=\"language-python\"># FedAvg 算法伪代码 (Algorithm 1)\n# 服务器端执行：\ndef ServerUpdate():\n    w = initialize_global_model()\n    for each round t = 1, 2, ...:\n        S_t = random_sample(clients, fraction=C)\n        for each client k in S_t (in parallel):\n            w_k = ClientUpdate(k, w)\n        # 加权平均聚合\n        w = sum(n_k / n * w_k for k in S_t)\n    return w\n\n# 客户端 k 执行：\ndef ClientUpdate(k, w):\n    B = split_local_data_into_batches(P_k, batch_size=B)\n    for epoch in range(E):\n        for batch b in B:\n            w = w - η * gradient(loss(w, b))\n    return w\n</code></pre>\n<p><strong>动机与背景</strong></p>\n<p>传统分布式 SGD（如数据中心内的同步/异步 SGD）假设数据可以在节点间自由 shuffle，通信带宽高且延迟低。然而在移动设备场景下，用户数据因隐私原因不能上传至服务器，且移动网络带宽有限（上行尤其慢）、设备可用性不稳定。这要求学习算法必须<strong>最小化通信轮次</strong>，同时容忍 Non-IID 的数据分布和不平衡的数据量。此前的朴素方法 FedSGD 每轮仅让客户端计算一个梯度就上传，通信效率极低。</p>\n<p><strong>核心机制：本地多步 SGD + 加权聚合</strong></p>\n<p>FedAvg 的关键洞察是：与其每轮只做一步梯度计算，不如让客户端在本地多跑几轮 SGD，再把最终模型参数发回服务器做加权平均。具体地，全局优化目标为：</p>\n<div class=\"kb-math kb-math-display\">f(w) = \\sum_{k=1}^{K} \\frac{n_k}{n} F_k(w), \\quad F_k(w) = \\frac{1}{n_k}\\sum_{i \\in \\mathcal{P}_k} \\ell(x_i, y_i; w)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\mathcal{P}_k</span> 是客户端 <span class=\"kb-math kb-math-inline\">k</span> 的本地数据集，<span class=\"kb-math kb-math-inline\">n_k = |\\mathcal{P}_k|</span>。FedSGD 的聚合为：</p>\n<div class=\"kb-math kb-math-display\">w_{t+1} \\leftarrow w_t - \\eta \\sum_{k=1}^{K} \\frac{n_k}{n} \\nabla F_k(w_t)</div>\n<p>当 <span class=\"kb-math kb-math-inline\">\\eta</span> 固定时，这等价于对各客户端执行一步 SGD 后的模型做加权平均。FedAvg 将此推广为<strong>多步本地更新</strong>：每个客户端从全局模型 <span class=\"kb-math kb-math-inline\">w_t</span> 出发，用本地数据跑 <span class=\"kb-math kb-math-inline\">E</span> 个 epoch（每 epoch 遍历 <span class=\"kb-math kb-math-inline\">\\lceil n_k/B \\rceil</span> 个 mini-batch），得到 <span class=\"kb-math kb-math-inline\">w_k^{t+1}</span>，服务器再聚合：</p>\n<div class=\"kb-math kb-math-display\">w_{t+1} \\leftarrow \\sum_{k \\in S_t} \\frac{n_k}{n} w_k^{t+1}</div>\n<p>三个关键超参数控制计算-通信权衡：\n- <span class=\"kb-math kb-math-inline\">C</span>：每轮参与的客户端比例（如 <span class=\"kb-math kb-math-inline\">C=0.1</span> 表示 10% 客户端参与）\n- <span class=\"kb-math kb-math-inline\">B</span>：本地 mini-batch 大小（<span class=\"kb-math kb-math-inline\">B=\\infty</span> 退化为全批量）\n- <span class=\"kb-math kb-math-inline\">E</span>：本地训练 epoch 数</p>\n<div class=\"key-point\">💡 关键：增大 <span class=\"kb-math kb-math-inline\">E</span> 或减小 <span class=\"kb-math kb-math-inline\">B</span> 都会增加每轮的本地计算量，从而减少达到目标精度所需的通信轮次。但这并非无限有效——过大的 <span class=\"kb-math kb-math-inline\">E</span> 会使各客户端模型偏离过远，尤其在 Non-IID 数据下可能导致聚合后性能下降。</div>\n<p><strong>参数平均的可行性与共享初始化</strong></p>\n<p>一个自然的疑问是：独立训练的神经网络参数平均后是否还有意义？论文通过实验（Figure 1）表明，如果两个模型从<strong>相同的随机初始化</strong>出发分别训练，它们的参数平均后的损失接近各自的损失，说明它们收敛到了同一个损失盆地。这为 FedAvg 提供了直觉支撑——每轮通信重置了共享起点，各客户端的本地更新不会偏离太远。</p>\n<p><strong>与传统分布式 SGD 的关键区别</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>数据中心分布式 SGD</th>\n<th>FedAvg</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>数据分布</td>\n<td>IID（随机 shuffle）</td>\n<td>Non-IID、不平衡</td>\n</tr>\n<tr>\n<td>通信频率</td>\n<td>每 1 步或几步</td>\n<td>每 <span class=\"kb-math kb-math-inline\">E</span> 个 epoch</td>\n</tr>\n<tr>\n<td>参与者</td>\n<td>全部节点</td>\n<td>随机子集（<span class=\"kb-math kb-math-inline\">C</span> 比例）</td>\n</tr>\n<tr>\n<td>通信瓶颈</td>\n<td>带宽充足</td>\n<td>上行带宽有限</td>\n</tr>\n<tr>\n<td>隐私</td>\n<td>数据可集中</td>\n<td>数据永不离开设备</td>\n</tr>\n</tbody>\n</table></div>\n<p>FedAvg 在 MNIST CNN 上仅需 FedSGD 通信轮次的 1/23 即可达到相同精度，在大规模 LSTM 上也展现出 23× 的通信效率提升。</p>\n<p><strong>局限与未来方向</strong></p>\n<p>论文指出 FedAvg 在极端 Non-IID 场景下可能不稳定，后续工作可结合差分隐私（DP）和安全多方计算（Secure Aggregation）提供更强的隐私保障。这些技术天然适配 FedAvg 的同步聚合范式。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>近端项修正</strong>：本地目标函数添加 <span class=\"kb-math kb-math-inline\">\\frac{\\mu}{2}\\|w - w^t\\|^2</span>，限制本地更新对全局模型的偏离，<span class=\"kb-math kb-math-inline\">\\mu=0</span> 时退化为 FedAvg",
        "<strong><span class=\"kb-math kb-math-inline\">\\gamma</span>-inexact 求解</strong>：允许设备不精确求解本地子问题，只需满足 <span class=\"kb-math kb-math-inline\">\\|\\nabla h_k(w^*)\\| \\leq \\gamma \\|\\nabla h_k(w^t)\\|</span>，天然容忍系统异构",
        "<strong>B-local dissimilarity 度量</strong>：定义 <span class=\"kb-math kb-math-inline\">B(w) = \\sqrt{\\frac{\\mathbb{E}_k[\\|\\nabla F_k(w)\\|^2]}{\\|\\nabla f(w)\\|^2}}</span> 量化设备间统计异构程度",
        "<strong>收敛保证</strong>：在非凸设定下，对异构数据和部分设备参与提供 <span class=\"kb-math kb-math-inline\">O(1/T)</span> 收敛率",
        "<strong>FedAvg 的严格泛化</strong>：FedAvg 是 FedProx 在 <span class=\"kb-math kb-math-inline\">\\mu=0</span>、SGD 求解器、固定 <span class=\"kb-math kb-math-inline\">\\gamma</span> 下的特例",
        "<strong>实验验证</strong>：在 4 个真实数据集 + 合成数据上，FedProx 相比 FedAvg 平均提升 22% 测试准确率（异构场景）"
      ],
      "detail": "<h5>核心实验结果</h5>\n<p><img alt=\"FedProx vs FedAvg 收敛对比\" src=\"https://ar5iv.labs.arxiv.org/html/1812.06127/assets/x1.png\" />\n<em>图：FedProx 在异构联邦场景下相比 FedAvg 的显著收敛改进。在 5 个数据集上，FedProx 展现出更稳定的训练损失下降曲线。</em></p>\n<h5>算法伪代码</h5>\n<pre><code>FedProx (Algorithm 2)\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n服务器端:\n  输入: w⁰, T轮, K个设备, μ (近端项系数)\n  for t = 0, 1, ..., T-1:\n      S_t ← 采样部分设备子集\n      for 每个设备 k ∈ S_t (并行):\n          发送 w^t 给设备 k\n          设备 k 求解: min_w h_k(w) = F_k(w) + μ/2·||w - w^t||²\n          返回 w_k^(t+1) (γ-inexact 解)\n      聚合: w^(t+1) = Σ (n_k/n) · w_k^(t+1)\n  return w^T\n\n客户端 γ-inexact 条件:\n  ||∇h_k(w*)|| ≤ γ · ||∇h_k(w^t)||,  γ ∈ [0, 1)\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n</code></pre>\n<h5>动机与背景</h5>\n<p>联邦学习面临两大核心挑战：</p>\n<ol>\n<li>\n<p><strong>统计异构性（Statistical Heterogeneity）</strong>：各设备的本地数据分布不同（non-IID），导致本地目标函数 <span class=\"kb-math kb-math-inline\">F_k</span> 之间差异显著。FedAvg 在此场景下可能发散或收敛到次优解。</p>\n</li>\n<li>\n<p><strong>系统异构性（Systems Heterogeneity）</strong>：设备的计算能力、网络带宽差异巨大，部分设备可能掉线（stragglers）。FedAvg 要求所有设备完成固定轮次的本地 SGD，无法适应这种不均衡。</p>\n</li>\n</ol>\n<p>传统分布式优化方法（如 DANE、ADMM）假设数据中心化或 IID 分布，无法直接应用于联邦场景。FedAvg 虽然实用，但缺乏理论保证且在异构环境下不稳定。</p>\n<h5>核心机制：近端项约束</h5>\n<p>FedProx 的核心创新是将本地子问题从：</p>\n<div class=\"kb-math kb-math-display\">\\min_w F_k(w)</div>\n<p>修改为：</p>\n<div class=\"kb-math kb-math-display\">\\min_w h_k(w; w^t) = F_k(w) + \\frac{\\mu}{2}\\|w - w^t\\|^2</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">w^t</span> 是当前轮次的全局模型参数。</p>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：近端项 <span class=\"kb-math kb-math-inline\">\\frac{\\mu}{2}\\|w - w^t\\|^2</span> 相当于一个\"弹簧\"，将本地更新\"拉回\"全局模型附近。当本地数据与全局分布偏差越大时，这个约束越重要——它防止某个设备的本地模型\"跑偏\"太远，从而稳定全局聚合。</div>\n<p><strong><span class=\"kb-math kb-math-inline\">\\mu</span> 的作用</strong>：\n- <span class=\"kb-math kb-math-inline\">\\mu = 0</span>：退化为 FedAvg，无约束\n- <span class=\"kb-math kb-math-inline\">\\mu</span> 较大：本地更新被强约束在全局模型附近，类似于只做一步梯度下降\n- <span class=\"kb-math kb-math-inline\">\\mu</span> 适中：平衡本地适应性与全局一致性</p>\n<p>从优化角度看，当 <span class=\"kb-math kb-math-inline\">F_k</span> 非凸时，若 <span class=\"kb-math kb-math-inline\">\\mu</span> 足够大，<span class=\"kb-math kb-math-inline\">h_k</span> 可变为凸函数（Hessian 正定），大幅改善优化景观。</p>\n<h5><span class=\"kb-math kb-math-inline\">\\gamma</span>-Inexact 求解机制</h5>\n<p>FedProx 不要求本地子问题被精确求解。定义 <span class=\"kb-math kb-math-inline\">\\gamma</span>-inexact 解：</p>\n<div class=\"kb-math kb-math-display\">\\|\\nabla h_k(w^*; w^t)\\| \\leq \\gamma \\|\\nabla h_k(w^t; w^t)\\|, \\quad \\gamma \\in [0, 1)</div>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：<span class=\"kb-math kb-math-inline\">\\gamma</span> 衡量本地求解的\"完成度\"。<span class=\"kb-math kb-math-inline\">\\gamma = 0</span> 表示精确解，<span class=\"kb-math kb-math-inline\">\\gamma</span> 接近 1 表示几乎没有优化。不同设备可以有不同的 <span class=\"kb-math kb-math-inline\">\\gamma_k^t</span>，自然适应系统异构——算力强的设备多迭代（小 <span class=\"kb-math kb-math-inline\">\\gamma</span>），算力弱或即将掉线的设备少迭代（大 <span class=\"kb-math kb-math-inline\">\\gamma</span>）也能贡献有效更新。</div>\n<p>这与 FedAvg 的\"固定 E 轮本地 SGD\"形成对比：FedAvg 中掉线设备的更新被直接丢弃，而 FedProx 中部分完成的更新仍然有效。</p>\n<h5>收敛分析</h5>\n<p>收敛分析基于 <strong>B-local dissimilarity</strong>（Definition 3）：</p>\n<div class=\"kb-math kb-math-display\">\\mathbb{E}_k[\\|\\nabla F_k(w)\\|^2] \\leq B^2 \\|\\nabla f(w)\\|^2</div>\n<p>该度量刻画了本地梯度与全局梯度的偏差程度。<span class=\"kb-math kb-math-inline\">B=1</span> 对应 IID 情形，<span class=\"kb-math kb-math-inline\">B</span> 越大异构性越强。</p>\n<p><strong>Theorem 6（非凸收敛）</strong>：在 Assumption 1（有界 dissimilarity）下，FedProx 以 <span class=\"kb-math kb-math-inline\">O(1/T)</span> 的速率收敛到近似驻点，收敛率与 <span class=\"kb-math kb-math-inline\">B</span>、<span class=\"kb-math kb-math-inline\">\\mu</span>、<span class=\"kb-math kb-math-inline\">\\gamma</span> 相关。具体地，更大的 <span class=\"kb-math kb-math-inline\">B</span>（更异构）需要更大的 <span class=\"kb-math kb-math-inline\">\\mu</span> 来补偿。</p>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：FedAvg（<span class=\"kb-math kb-math-inline\">\\mu=0</span>）在理论上更难分析，因为缺少近端项时本地目标可能非凸且无界，导致收敛分析需要更强的假设。</div>\n<h5>与 FedAvg 的关键区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>FedAvg</th>\n<th>FedProx</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>本地目标</td>\n<td><span class=\"kb-math kb-math-inline\">F_k(w)</span></td>\n<td><span class=\"kb-math kb-math-inline\">F_k(w) + \\frac{\\mu}{2}\\|w-w^t\\|^2</span></td>\n</tr>\n<tr>\n<td>本地求解器</td>\n<td>固定为 SGD</td>\n<td>任意求解器</td>\n</tr>\n<tr>\n<td>本地计算量</td>\n<td>固定 E 轮</td>\n<td>可变（<span class=\"kb-math kb-math-inline\">\\gamma</span>-inexact）</td>\n</tr>\n<tr>\n<td>设备掉线处理</td>\n<td>丢弃更新</td>\n<td>部分更新仍有效</td>\n</tr>\n<tr>\n<td>收敛保证</td>\n<td>无（non-IID 下）</td>\n<td><span class=\"kb-math kb-math-inline\">O(1/T)</span> 非凸收敛</td>\n</tr>\n</tbody>\n</table></div>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "首次严格证明 FedAvg 在异构数据下存在不可消除的收敛偏差项 <span class=\"kb-math kb-math-inline\">B^2/\\mu</span>，该偏差源于客户漂移",
        "提出 SCAFFOLD 算法：为每个客户端和服务器维护控制变量 <span class=\"kb-math kb-math-inline\">\\boldsymbol{c}_i</span> 和 <span class=\"kb-math kb-math-inline\">\\boldsymbol{c}</span>，用于校正本地梯度方向",
        "收敛速率完全不依赖梯度异构性 <span class=\"kb-math kb-math-inline\">G</span>，仅依赖随机噪声 <span class=\"kb-math kb-math-inline\">\\sigma^2</span> 和平滑常数 <span class=\"kb-math kb-math-inline\">\\beta</span>",
        "两种控制变量更新策略：Option I（额外全梯度计算）和 Option II（利用已有梯度，无额外开销）",
        "支持部分客户参与（partial participation）：每轮仅采样 <span class=\"kb-math kb-math-inline\">S</span> 个客户",
        "在二次函数上证明 local steps 的加速效果依赖于 Hessian 相似度 <span class=\"kb-math kb-math-inline\">\\delta</span>",
        "实验在 EMNIST 上验证：SCAFFOLD 在凸和非凸设置下均一致优于 FedAvg、FedProx 和 SGD"
      ],
      "detail": "<h5>核心示意图</h5>\n<p><img alt=\"Client-drift 示意图\" src=\"https://ar5iv.labs.arxiv.org/html/1910.06378/assets/x1.png\" />\n<em>图1：FedAvg 中的客户漂移现象。两个客户端（N=2, K=3）的本地更新分别收敛到各自局部最优 <span class=\"kb-math kb-math-inline\">\\boldsymbol{x}_1^\\star</span> 和 <span class=\"kb-math kb-math-inline\">\\boldsymbol{x}_2^\\star</span>，而非全局最优 <span class=\"kb-math kb-math-inline\">\\boldsymbol{x}^\\star</span>。</em></p>\n<p><img alt=\"SCAFFOLD 校正机制\" src=\"https://ar5iv.labs.arxiv.org/html/1910.06378/assets/x2.png\" />\n<em>图2：SCAFFOLD 在单个客户端上的更新步骤。本地梯度（黑色虚线）指向局部最优，但校正项 <span class=\"kb-math kb-math-inline\">(\\boldsymbol{c} - \\boldsymbol{c}_i)</span>（红色）确保更新方向朝向全局最优。</em></p>\n<p><img alt=\"实验对比\" src=\"https://ar5iv.labs.arxiv.org/html/1910.06378/assets/x3.png\" />\n<em>图3：模拟数据上 SGD、FedAvg 和 SCAFFOLD 的对比。FedAvg 随 local steps 增加而变慢，SCAFFOLD 则持续加速且不受异构性 G 影响。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># SCAFFOLD 算法核心流程\n# 初始化\nx = x_0                    # 服务器模型\nc = 0                      # 服务器控制变量\nc_i = 0 for all i in [N]   # 客户端控制变量\n\nfor round r in range(R):\n    # 服务器采样客户子集\n    S = sample(clients, size=S)\n\n    for client i in S (parallel):\n        # 接收服务器参数\n        y_i = x\n\n        # 本地更新 K 步（核心：加入控制变量校正）\n        for k in range(K):\n            g_i = stochastic_gradient(y_i, local_data_i)\n            y_i = y_i - eta_l * (g_i + c - c_i)  # 校正梯度方向\n\n        # 更新本地控制变量（Option II，无额外开销）\n        c_i_new = c_i - c + (1 / (K * eta_l)) * (x - y_i)\n\n        # 发送更新给服务器\n        send(delta_x = y_i - x, delta_c = c_i_new - c_i)\n        c_i = c_i_new\n\n    # 服务器聚合\n    x = x + (eta_g / |S|) * sum(delta_x for i in S)\n    c = c + (1 / N) * sum(delta_c for i in S)\n</code></pre>\n<h5>动机与背景：客户漂移问题</h5>\n<p>联邦学习的核心优化目标为：</p>\n<div class=\"kb-math kb-math-display\">f(\\boldsymbol{x}) = \\frac{1}{N}\\sum_{i=1}^{N} f_i(\\boldsymbol{x})</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">f_i</span> 是第 <span class=\"kb-math kb-math-inline\">i</span> 个客户端的本地目标函数。FedAvg 让每个客户端执行 <span class=\"kb-math kb-math-inline\">K</span> 步本地 SGD 后聚合，以减少通信轮次。</p>\n<p><strong>问题核心</strong>：当各客户端数据分布不同（non-iid）时，本地更新会使各客户端模型漂移向各自的局部最优 <span class=\"kb-math kb-math-inline\">\\boldsymbol{x}_i^\\star</span>，而非全局最优 <span class=\"kb-math kb-math-inline\">\\boldsymbol{x}^\\star</span>。论文严格证明了 FedAvg 的收敛上界包含不可消除项：</p>\n<div class=\"kb-math kb-math-display\">R_{\\text{FedAvg}} = \\mathcal{O}\\left(\\frac{\\sigma^2}{\\mu K S \\epsilon} + \\frac{B^2}{\\mu}\\right)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">B^2 = \\frac{1}{N}\\sum_i \\|\\nabla f_i(\\boldsymbol{x}^\\star)\\|^2</span> 度量数据异构程度。即使通信轮次 <span class=\"kb-math kb-math-inline\">R \\to \\infty</span>，FedAvg 也无法收敛到精确解。</p>\n<div class=\"key-point\">💡 关键：客户漂移的根源在于本地梯度 <span class=\"kb-math kb-math-inline\">\\nabla f_i(\\boldsymbol{x})</span> 不是全局梯度 <span class=\"kb-math kb-math-inline\">\\nabla f(\\boldsymbol{x})</span> 的无偏估计——它们有系统性偏差。</div>\n<h5>核心机制：控制变量校正</h5>\n<p>SCAFFOLD 的核心思想借鉴了方差缩减（variance reduction）技术中的控制变量方法。</p>\n<p><strong>理想更新</strong>：如果通信不受限，每步理想更新应为：</p>\n<div class=\"kb-math kb-math-display\">\\boldsymbol{y}_i \\leftarrow \\boldsymbol{y}_i - \\eta_l \\cdot \\frac{1}{N}\\sum_{j=1}^{N} g_j(\\boldsymbol{y}_i)</div>\n<p>这等价于在 iid 数据上运行 FedAvg。但这需要每步都与所有客户端通信。</p>\n<p><strong>SCAFFOLD 的近似</strong>：维护控制变量使得 <span class=\"kb-math kb-math-inline\">\\boldsymbol{c}_j \\approx \\nabla f_j(\\boldsymbol{y}_i)</span>，<span class=\"kb-math kb-math-inline\">\\boldsymbol{c} \\approx \\frac{1}{N}\\sum_j \\nabla f_j(\\boldsymbol{y}_i)</span>。则本地更新变为：</p>\n<div class=\"kb-math kb-math-display\">\\boldsymbol{y}_i \\leftarrow \\boldsymbol{y}_i - \\eta_l\\left(g_i(\\boldsymbol{y}_i) - \\boldsymbol{c}_i + \\boldsymbol{c}\\right)</div>\n<p>此时校正后的梯度估计为：</p>\n<div class=\"kb-math kb-math-display\">g_i(\\boldsymbol{y}_i) - \\boldsymbol{c}_i + \\boldsymbol{c} \\approx g_i(\\boldsymbol{y}_i) - \\nabla f_i(\\boldsymbol{y}_i) + \\frac{1}{N}\\sum_j \\nabla f_j(\\boldsymbol{y}_i) \\approx \\frac{1}{N}\\sum_j g_j(\\boldsymbol{y}_i)</div>\n<div class=\"key-point\">💡 关键：控制变量 <span class=\"kb-math kb-math-inline\">(\\boldsymbol{c} - \\boldsymbol{c}_i)</span> 作为校正项，消除了本地梯度的系统性偏差，使得本地更新方向始终指向全局最优。</div>\n<p><strong>控制变量更新</strong>有两种方案：\n- <strong>Option I</strong>：<span class=\"kb-math kb-math-inline\">\\boldsymbol{c}_i^+ = \\nabla f_i(\\boldsymbol{x})</span>，需额外一次全数据梯度计算，更稳定\n- <strong>Option II</strong>：<span class=\"kb-math kb-math-inline\">\\boldsymbol{c}_i^+ = \\boldsymbol{c}_i - \\boldsymbol{c} + \\frac{1}{K\\eta_l}(\\boldsymbol{x} - \\boldsymbol{y}_i)</span>，利用已有计算结果推导，无额外开销</p>\n<p>Option II 的直觉：<span class=\"kb-math kb-math-inline\">\\frac{1}{K\\eta_l}(\\boldsymbol{x} - \\boldsymbol{y}_i)</span> 实际上是 <span class=\"kb-math kb-math-inline\">K</span> 步本地更新中使用的校正梯度的平均值。</p>\n<h5>收敛理论：消除异构性依赖</h5>\n<p>SCAFFOLD 的收敛定理（Theorem III）表明，对于 <span class=\"kb-math kb-math-inline\">\\mu</span>-强凸函数：</p>\n<div class=\"kb-math kb-math-display\">R_{\\text{SCAFFOLD}} = \\tilde{\\mathcal{O}}\\left(\\frac{\\sigma^2}{\\mu K S \\epsilon} + \\frac{\\beta}{\\mu} + \\frac{N}{S}\\right)</div>\n<p><strong>关键对比</strong>：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>指标</th>\n<th>FedAvg</th>\n<th>SCAFFOLD</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>异构性依赖</td>\n<td><span class=\"kb-math kb-math-inline\">B^2/\\mu</span>（不可消除）</td>\n<td><strong>无</strong></td>\n</tr>\n<tr>\n<td>噪声项</td>\n<td><span class=\"kb-math kb-math-inline\">\\sigma^2/(\\mu KS\\epsilon)</span></td>\n<td><span class=\"kb-math kb-math-inline\">\\sigma^2/(\\mu KS\\epsilon)</span></td>\n</tr>\n<tr>\n<td>通信下界</td>\n<td><span class=\"kb-math kb-math-inline\">\\beta/\\mu + B^2/\\mu</span></td>\n<td><span class=\"kb-math kb-math-inline\">\\beta/\\mu + N/S</span></td>\n</tr>\n<tr>\n<td>local steps 加速</td>\n<td>仅在 iid 时有效</td>\n<td>始终有效</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"warn-box\">⚠️ 注意：SCAFFOLD 的 <span class=\"kb-math kb-math-inline\">N/S</span> 项来自部分参与的方差，当所有客户参与（<span class=\"kb-math kb-math-inline\">S=N</span>）时消失。<span class=\"kb-math kb-math-inline\">\\beta/\\mu</span> 项是条件数的固有下界。</div>\n<h5>双步长机制</h5>\n<p>论文引入全局步长 <span class=\"kb-math kb-math-inline\">\\eta_g</span> 和本地步长 <span class=\"kb-math kb-math-inline\">\\eta_l</span> 的分离设计：\n- <span class=\"kb-math kb-math-inline\">\\eta_g = \\sqrt{S}</span>：较大的全局步长确保聚合后有足够进展\n- <span class=\"kb-math kb-math-inline\">\\eta_l \\leq \\frac{1}{81\\beta K \\eta_g}</span>：较小的本地步长控制漂移</p>\n<p>这种设计使得即使 FedAvg 也能获得改进的收敛率（相比之前的分析），但仍无法消除异构性偏差。</p>\n<h5>与相关方法的对比</h5>\n<ul>\n<li><strong>FedProx</strong>（Li et al., 2018）：添加近端正则项 <span class=\"kb-math kb-math-inline\">\\frac{\\mu}{2}\\|\\boldsymbol{x} - \\boldsymbol{y}_i\\|^2</span>，理论复杂度与 FedAvg 相同（<span class=\"kb-math kb-math-inline\">B^2/\\mu</span>），无本质改进</li>\n<li><strong>DANE</strong>（Shamir et al., 2014）：在二次函数上需要 <span class=\"kb-math kb-math-inline\">(\\delta/\\mu)^2</span> 轮，SCAFFOLD 仅需 <span class=\"kb-math kb-math-inline\">\\delta/\\mu</span> 轮</li>\n<li><strong>SGD</strong>（单步通信）：SCAFFOLD 通过 local steps 实现线性加速，当 <span class=\"kb-math kb-math-inline\">\\delta=0</span> 时 <span class=\"kb-math kb-math-inline\">K</span> 步等价于 <span class=\"kb-math kb-math-inline\">K</span> 倍加速</li>\n</ul>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Globally Perceptive Optimization (GPO)</strong>：辅助分支（后续适配器 + 输出层）计算全局损失，<span class=\"kb-math kb-math-inline\">\\text{Loss}_m = \\text{Local Loss} + \\lambda \\cdot \\text{Global Loss}</span>，防止短视优化",
        "<strong>Function-Oriented Adaptive Tuning (FOAT)</strong>：基于 CKA 相似度识别最优起始层 <span class=\"kb-math kb-math-inline\">L_{\\text{start}}</span>，跳过通用浅层，仅微调任务关键层",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>MoE 架构设计</strong>：每个客户端包含全局专家（同构小特征提取器 <span class=\"kb-math kb-math-inline\">\\mathcal{G}(\\theta)</span>）、本地专家（异构大特征提取器 <span class=\"kb-math kb-math-inline\">\\mathcal{F}_k^{ex}</span>）和轻量门控网络 <span class=\"kb-math kb-math-inline\">\\mathcal{H}(\\varphi_k)</span>",
        "<strong>数据级别个性化</strong>：门控网络为每个样本独立生成权重 <span class=\"kb-math kb-math-inline\">[\\alpha^{\\mathcal{G}}, \\alpha^{\\mathcal{F}_k}]</span>，动态平衡泛化与个性化表示",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "定义稀疏可分（Sparse Separable）代价函数结构：<span class=\"kb-math kb-math-inline\">f(x) = \\sum_{e \\in E} f_e(x_e)</span>，每个子函数仅依赖少量变量",
        "引入三个稀疏度量指标：<span class=\"kb-math kb-math-inline\">\\Omega</span>（单次梯度影响的最大变量数）、<span class=\"kb-math kb-math-inline\">\\Delta</span>（单个变量被影响的最大子函数数）、<span class=\"kb-math kb-math-inline\">\\rho</span>（两个子函数共享变量的最大概率）",
        "理论证明：当最大延迟 <span class=\"kb-math kb-math-inline\">\\tau</span> 满足 <span class=\"kb-math kb-math-inline\">\\tau \\leq O(n^{1/4})</span> 时（<span class=\"kb-math kb-math-inline\">n</span> 为变量维度），算法以近线性加速比收敛",
        "收敛速率为 <span class=\"kb-math kb-math-inline\">O(1/k)</span>，与串行 SGD 相同量级，额外误差项与 <span class=\"kb-math kb-math-inline\">\\tau^2 \\cdot \\rho</span> 成正比",
        "实验验证：稀疏 SVM、矩阵补全（Matrix Completion）、图割（Graph Cuts）三类任务均获得显著加速"
      ],
      "detail": "<p><img alt=\"Hogwild! 稀疏结构示意图\" src=\"https://ar5iv.labs.arxiv.org/html/1106.5730/assets/x1.png\" />\n<em>图 1：代价函数诱导的稀疏图结构示例——稀疏 SVM 中每个样本仅涉及少量特征，变量间冲突概率极低</em></p>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：当优化问题足够稀疏时，不同处理器同时更新的变量几乎不重叠，因此即使没有锁保护，冲突概率极低，不会破坏收敛性。</div>\n<p><strong>算法伪代码：</strong></p>\n<pre><code class=\"language-python\"># Hogwild! 无锁并行 SGD (Algorithm 1)\n# 共享内存中的决策变量 x，所有处理器并发执行以下循环：\n\ndef hogwild_worker(shared_x, data_samples, step_size_gamma):\n    while not converged:\n        # 1. 随机采样一个子函数 e\n        e = sample_uniformly(data_samples)\n        # 2. 读取当前共享变量的相关分量（可能读到过时值）\n        x_e = read(shared_x, indices=e)  # 无锁读取\n        # 3. 计算该子函数的梯度\n        G_e = compute_gradient(f_e, x_e)\n        # 4. 对涉及的每个变量分量执行原子加操作\n        for v in support(G_e):\n            shared_x[v] -= gamma * G_e[v]  # 无锁写入（硬件原子加）\n</code></pre>\n<p><strong>动机与背景</strong></p>\n<p>传统并行 SGD 方法通常依赖以下策略之一：(1) 使用互斥锁保护共享变量，但锁竞争在高并发下成为严重瓶颈；(2) 采用 mini-batch 方式将梯度聚合后统一更新，但这引入了同步等待开销并改变了算法的统计特性。Hogwild! 的核心洞察是：对于许多实际的机器学习问题（如稀疏特征的分类、推荐系统中的矩阵分解），目标函数具有天然的稀疏结构——每个训练样本仅涉及极少数特征维度。在这种条件下，多个处理器同时写入同一变量的概率极低，因此完全可以省去锁机制。</p>\n<p><strong>核心机制：稀疏可分结构与无锁更新</strong></p>\n<p>Hogwild! 将优化问题形式化为：</p>\n<div class=\"kb-math kb-math-display\">\\min_{x \\in \\mathbb{R}^n} f(x) = \\sum_{e \\in E} f_e(x_e)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">E</span> 是样本/子函数的索引集，<span class=\"kb-math kb-math-inline\">x_e \\subseteq x</span> 表示子函数 <span class=\"kb-math kb-math-inline\">f_e</span> 实际依赖的变量子集。定义三个关键稀疏度量：</p>\n<ul>\n<li><strong><span class=\"kb-math kb-math-inline\">\\Omega</span></strong>：单个子函数梯度的最大非零分量数，即 <span class=\"kb-math kb-math-inline\">\\max_e |e|</span></li>\n<li><strong><span class=\"kb-math kb-math-inline\">\\Delta</span></strong>：单个变量被多少子函数共同依赖，即 <span class=\"kb-math kb-math-inline\">\\max_v |\\{e : v \\in e\\}|</span></li>\n<li><strong><span class=\"kb-math kb-math-inline\">\\rho</span></strong>：任意两个随机采样的子函数共享至少一个变量的最大概率</li>\n</ul>\n<p>当 <span class=\"kb-math kb-math-inline\">\\rho \\ll 1</span> 时，并发写冲突几乎不发生。论文证明，即使存在延迟（处理器读取的是 <span class=\"kb-math kb-math-inline\">\\tau</span> 步之前的旧值），只要稀疏度足够高，算法仍能收敛。</p>\n<p><strong>收敛性理论保证</strong></p>\n<p>论文的核心理论结果（Proposition 4.1）表明，在以下条件下：\n1. <span class=\"kb-math kb-math-inline\">f</span> 是强凸的，Hessian 特征值在 <span class=\"kb-math kb-math-inline\">[c, L]</span> 之间\n2. 步长 <span class=\"kb-math kb-math-inline\">\\gamma</span> 满足 <span class=\"kb-math kb-math-inline\">\\gamma &lt; \\frac{c}{L \\cdot \\Omega \\cdot (\\tau \\Delta + 1)}</span>\n3. 梯度的方差有界</p>\n<p>则经过 <span class=\"kb-math kb-math-inline\">k</span> 步更新后，期望误差满足：</p>\n<div class=\"kb-math kb-math-display\">\\mathbb{E}[f(x_k) - f(x^*)] \\leq O\\left(\\frac{1}{c \\cdot k}\\right) + O\\left(\\frac{\\tau^2 \\cdot L^2 \\cdot \\Omega \\cdot \\rho}{c^2 \\cdot k}\\right)</div>\n<div class=\"warn-box\">⚠️ <strong>关键条件</strong>：当处理器数 <span class=\"kb-math kb-math-inline\">p</span> 满足 <span class=\"kb-math kb-math-inline\">p \\leq O(n^{1/2} / \\Omega)</span> 时（因为 <span class=\"kb-math kb-math-inline\">\\tau \\approx p</span>），额外误差项可忽略，算法实现近线性加速。对于典型的稀疏问题（<span class=\"kb-math kb-math-inline\">\\Omega = O(1)</span>），这意味着可以使用 <span class=\"kb-math kb-math-inline\">O(\\sqrt{n})</span> 个处理器而不损失收敛质量。</div>\n<p><strong>与传统方法的区别</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>同步机制</th>\n<th>通信开销</th>\n<th>适用场景</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>加锁 SGD</td>\n<td>互斥锁</td>\n<td>高（锁竞争）</td>\n<td>通用但慢</td>\n</tr>\n<tr>\n<td>Mini-batch SGD</td>\n<td>Barrier 同步</td>\n<td>中（聚合梯度）</td>\n<td>密集问题</td>\n</tr>\n<tr>\n<td><strong>Hogwild!</strong></td>\n<td><strong>无锁</strong></td>\n<td><strong>零（共享内存）</strong></td>\n<td><strong>稀疏问题</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>Hogwild! 的优势在于完全消除了同步开销，但其理论保证依赖于问题的稀疏性假设。对于密集问题（如全连接神经网络的所有参数都被每个样本更新），冲突概率高，Hogwild! 的理论保证不再成立（尽管实践中仍常被使用）。</p>\n<p><strong>实验验证</strong></p>\n<p>论文在三个典型稀疏学习任务上验证了 Hogwild!：</p>\n<ol>\n<li><strong>稀疏 SVM</strong>（RCV1 数据集，<span class=\"kb-math kb-math-inline\">n=677,399</span> 特征）：10 线程加速比约 <strong>6.4×</strong></li>\n<li><strong>矩阵补全</strong>（Netflix 数据集，<span class=\"kb-math kb-math-inline\">n=17,770 \\times 480,189</span> 的低秩分解）：10 线程加速比约 <strong>4.3×</strong></li>\n<li><strong>图割</strong>（随机图上的 s-t 最小割问题）：10 线程加速比约 <strong>7.5×</strong></li>\n</ol>\n<p>所有实验均在共享内存多核机器上进行，证实了理论预测的近线性加速。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>逻辑环拓扑</strong>：将 <span class=\"kb-math kb-math-inline\">N</span> 个 GPU 排列为环形，每个 GPU 仅与左右邻居通信，消除中心瓶颈",
        "<strong>两阶段算法</strong>：scatter-reduce（分散规约）+ allgather（全收集），各执行 <span class=\"kb-math kb-math-inline\">N-1</span> 轮迭代",
        "<strong>带宽最优</strong>：每个 GPU 总传输数据量为 <span class=\"kb-math kb-math-inline\">2(N-1) \\cdot K/N</span>，与 GPU 数量 <span class=\"kb-math kb-math-inline\">N</span> 无关（<span class=\"kb-math kb-math-inline\">K</span> 为参数总量）",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Embedding 并行</strong>：词表维度切分 + 将输出 logits 的 cross-entropy 融合计算，避免传输巨大的 <span class=\"kb-math kb-math-inline\">b \\times s \\times v</span> 张量",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "ZeRO-DP 三阶段渐进式优化：Stage 1 分区优化器状态（<span class=\"kb-math kb-math-inline\">P_{os}</span>）、Stage 2 分区梯度（<span class=\"kb-math kb-math-inline\">P_{os+g}</span>）、Stage 3 分区参数（<span class=\"kb-math kb-math-inline\">P_{os+g+p}</span>）",
        "内存效率：混合精度 Adam 下，从每设备 <span class=\"kb-math kb-math-inline\">16\\Psi</span> 字节降至 Stage 1 的 <span class=\"kb-math kb-math-inline\">4\\Psi + \\frac{12\\Psi}{N_d}</span>、Stage 2 的 <span class=\"kb-math kb-math-inline\">2\\Psi + \\frac{14\\Psi}{N_d}</span>、Stage 3 的 <span class=\"kb-math kb-math-inline\">\\frac{16\\Psi}{N_d}</span>",
        "通信效率：Stage 1 和 Stage 2 通信量与标准数据并行相同（<span class=\"kb-math kb-math-inline\">2\\Psi</span>），Stage 3 仅增加 50%（<span class=\"kb-math kb-math-inline\">3\\Psi</span>）",
      ...
      "detail": "<h5>核心示意图</h5>\n<p><img alt=\"ZeRO-DP 三阶段内存消耗对比\" src=\"https://ar5iv.labs.arxiv.org/html/1910.02054/assets/x1.png\" />\n<em>图：ZeRO-DP 三个阶段的每设备内存消耗对比。以 7.5B 参数模型、64 张 GPU（<span class=\"kb-math kb-math-inline\">N_d=64</span>）为例，展示了从基线数据并行的 120GB 逐步降低到 Stage 3 的 1.9GB 的过程。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># ZeRO-DP Stage 1 (Pos): 优化器状态分区\n# 每个进程 rank_i 只维护 1/Nd 的优化器状态\n\ndef zero_dp_stage1_training_step(model, data, Nd, rank):\n    # Forward pass (每个进程持有完整模型参数副本)\n    loss = model.forward(data)\n\n    # Backward pass (每个进程计算完整梯度)\n    gradients = loss.backward()  # 大小: Ψ\n\n    # Reduce-Scatter: 每个进程获得自己负责分区的聚合梯度\n    my_grad_partition = reduce_scatter(gradients)  # 大小: Ψ/Nd\n\n    # 每个进程只更新自己负责的参数分区\n    my_param_partition = optimizer_step(\n        my_grad_partition, \n        my_optimizer_states  # 大小: 12Ψ/Nd (Adam: fp32参数+动量+方差)\n    )\n\n    # All-Gather: 收集所有进程更新后的参数，重建完整模型\n    all_params = all_gather(my_param_partition)  # 每个进程获得完整 Ψ\n    model.update_params(all_params)\n\n# ZeRO-DP Stage 3 (Pos+g+p): 全分区\ndef zero_dp_stage3_forward_backward(model, data, Nd, rank):\n    # Forward: 逐层 All-Gather 参数 → 计算 → 丢弃非本分区参数\n    for layer in model.layers:\n        full_params = all_gather(layer.my_partition)  # 临时获取完整层参数\n        activation = layer.forward(input, full_params)\n        discard(full_params - layer.my_partition)     # 释放非本分区\n\n    # Backward: 逐层反向 All-Gather 参数 → 计算梯度 → Reduce-Scatter 梯度\n    for layer in reversed(model.layers):\n        full_params = all_gather(layer.my_partition)\n        grad = layer.backward(activation, full_params)\n        my_grad_partition = reduce_scatter(grad)      # 只保留本分区梯度\n        discard(full_params - layer.my_partition)\n\n    # 每个进程用本分区梯度更新本分区优化器状态和参数\n    optimizer_step(my_grad_partition, my_optimizer_states)\n</code></pre>\n<h5>方法深入解释</h5>\n<p><strong>动机与背景：数据并行的内存冗余问题</strong></p>\n<p>当前大模型训练主要依赖数据并行（DP）和模型并行（MP）。模型并行虽能减少每设备内存，但受限于计算/通信比，通常只能在单节点内高效扩展（如 Megatron-LM 最多 8-way MP）。数据并行虽然通信效率高、扩展性好，但每个进程都持有完整的模型状态副本——这在大模型场景下造成了巨大的内存浪费。</p>\n<p>以混合精度训练 + Adam 优化器为例，对于参数量为 <span class=\"kb-math kb-math-inline\">\\Psi</span> 的模型，每个 DP 进程需要存储：\n- fp16 模型参数：<span class=\"kb-math kb-math-inline\">2\\Psi</span> 字节\n- fp16 梯度：<span class=\"kb-math kb-math-inline\">2\\Psi</span> 字节<br />\n- fp32 优化器状态（Adam）：<span class=\"kb-math kb-math-inline\">12\\Psi</span> 字节（fp32 参数副本 <span class=\"kb-math kb-math-inline\">4\\Psi</span> + fp32 动量 <span class=\"kb-math kb-math-inline\">4\\Psi</span> + fp32 方差 <span class=\"kb-math kb-math-inline\">4\\Psi</span>）</p>\n<p>总计每设备需 <span class=\"kb-math kb-math-inline\">16\\Psi</span> 字节。一个 GPT-2 级别的 1.5B 参数模型就需要 24GB，而 GPT-3 级别的 175B 参数模型则需要 2.8TB——远超单卡显存。关键观察是：<strong>这些内存在所有 DP 进程中完全冗余复制</strong>。</p>\n<div class=\"key-point\">💡 关键洞察：数据并行中，每个进程在任意时刻只需要完整参数做前向/反向计算，而优化器状态只在参数更新时使用，且每个参数的更新是独立的——这为分区提供了天然基础。</div>\n<p><strong>核心机制：ZeRO-DP 三阶段渐进分区</strong></p>\n<p>ZeRO-DP 的核心思想是：将模型状态按参数维度均匀分区到 <span class=\"kb-math kb-math-inline\">N_d</span> 个数据并行进程中，每个进程只存储 <span class=\"kb-math kb-math-inline\">1/N_d</span> 的状态，需要时通过集合通信临时获取。</p>\n<p><strong>Stage 1 — 优化器状态分区（<span class=\"kb-math kb-math-inline\">P_{os}</span>）</strong>：每个进程只保留 <span class=\"kb-math kb-math-inline\">1/N_d</span> 的优化器状态。反向传播后，通过 Reduce-Scatter 操作让每个进程获得其负责分区的聚合梯度，然后各自更新本分区参数，最后通过 All-Gather 同步更新后的完整参数。内存从 <span class=\"kb-math kb-math-inline\">4\\Psi + 12\\Psi = 16\\Psi</span> 降至 <span class=\"kb-math kb-math-inline\">4\\Psi + \\frac{12\\Psi}{N_d}</span>。当 <span class=\"kb-math kb-math-inline\">N_d = 64</span> 时，约为 <span class=\"kb-math kb-math-inline\">4.19\\Psi</span>，实现约 <strong>4 倍</strong>内存节省。</p>\n<p><strong>Stage 2 — 梯度分区（<span class=\"kb-math kb-math-inline\">P_{os+g}</span>）</strong>：在 Stage 1 基础上，梯度也只保留本分区部分。由于每个进程只需要本分区的聚合梯度来更新本分区参数，反向传播中一旦某层梯度被 Reduce-Scatter 完成，非本分区的梯度即可释放。内存降至 <span class=\"kb-math kb-math-inline\">2\\Psi + \\frac{14\\Psi}{N_d}</span>，约 <strong>8 倍</strong>节省。</p>\n<p><strong>Stage 3 — 参数分区（<span class=\"kb-math kb-math-inline\">P_{os+g+p}</span>）</strong>：连模型参数也分区存储。前向和反向传播时，通过 All-Gather 临时获取当前层的完整参数，计算完成后立即释放。内存降至 <span class=\"kb-math kb-math-inline\">\\frac{16\\Psi}{N_d}</span>，实现与 <span class=\"kb-math kb-math-inline\">N_d</span> 成线性的内存缩减。</p>\n<div class=\"warn-box\">⚠️ 注意：Stage 1 和 Stage 2 的通信量与标准数据并行完全相同（<span class=\"kb-math kb-math-inline\">2\\Psi</span> 元素），因为标准 All-Reduce 本质上等价于 Reduce-Scatter + All-Gather。Stage 3 额外增加一次 All-Gather（前向传播时），总通信量为 <span class=\"kb-math kb-math-inline\">3\\Psi</span>，仅增加 50%。</div>\n<p><strong>通信量分析</strong></p>\n<p>标准数据并行使用 All-Reduce 同步梯度，通信量为 <span class=\"kb-math kb-math-inline\">2\\Psi</span>（Reduce-Scatter <span class=\"kb-math kb-math-inline\">\\Psi</span> + All-Gather <span class=\"kb-math kb-math-inline\">\\Psi</span>）。</p>\n<div class=\"kb-math kb-math-display\">\\text{Stage 1/2 通信量} = \\underbrace{\\Psi}_{\\text{Reduce-Scatter 梯度}} + \\underbrace{\\Psi}_{\\text{All-Gather 参数}} = 2\\Psi</div>\n<div class=\"kb-math kb-math-display\">\\text{Stage 3 通信量} = \\underbrace{\\Psi}_{\\text{All-Gather (前向)}} + \\underbrace{\\Psi}_{\\text{All-Gather (反向)}} + \\underbrace{\\Psi}_{\\text{Reduce-Scatter 梯度}} = 3\\Psi</div>\n<p><strong>ZeRO-R：残余内存优化</strong></p>\n<p>除模型状态外，激活值（activations）、临时缓冲区和内存碎片也消耗大量显存。ZeRO-R 提出三项互补优化：</p>\n<ol>\n<li>\n<p><strong>激活值分区（<span class=\"kb-math kb-math-inline\">P_a</span>）</strong>：将激活值的 checkpoint 也按 DP 进程分区存储，需要时通过 All-Gather 重建。配合激活重计算（activation checkpointing），可将激活内存从 <span class=\"kb-math kb-math-inline\">O(layers \\times hidden)</span> 降至 <span class=\"kb-math kb-math-inline\">O(layers \\times hidden / N_d)</span>。</p>\n</li>\n<li>\n<p><strong>恒定大小缓冲区（<span class=\"kb-math kb-math-inline\">C_B</span>）</strong>：大模型中 All-Reduce 等操作常需要与模型大小成正比的临时融合缓冲区。ZeRO-R 使用固定大小的缓冲区，在效率和内存间取得平衡。</p>\n</li>\n<li>\n<p><strong>内存碎片整理（<span class=\"kb-math kb-math-inline\">M_D</span>）</strong>：训练过程中频繁的内存分配/释放导致碎片化，即使总空闲内存充足也可能 OOM。ZeRO-R 通过预分配连续内存块并动态管理子分配来解决此问题。</p>\n</li>\n</ol>\n<p><strong>与传统方法的对比</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>标准数据并行</th>\n<th>模型并行 (Megatron)</th>\n<th>ZeRO-DP</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>内存效率</td>\n<td>差（完全冗余）</td>\n<td>好（按层/张量切分）</td>\n<td>极好（线性缩减）</td>\n</tr>\n<tr>\n<td>通信效率</td>\n<td>好（<span class=\"kb-math kb-math-inline\">2\\Psi</span>）</td>\n<td>差（<span class=\"kb-math kb-math-inline\">O(hidden \\times batch)</span> 每层）</td>\n<td>好（<span class=\"kb-math kb-math-inline\">2\\Psi</span> ~ <span class=\"kb-math kb-math-inline\">3\\Psi</span>）</td>\n</tr>\n<tr>\n<td>扩展性</td>\n<td>受内存限制</td>\n<td>受通信限制（≤8 GPU）</td>\n<td>可扩展至数千 GPU</td>\n</tr>\n<tr>\n<td>实现复杂度</td>\n<td>低</td>\n<td>高（需改模型代码）</td>\n<td>中（对用户透明）</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 关键优势：ZeRO 实现了\"鱼与熊掌兼得\"——既获得了模型并行级别的内存效率，又保持了数据并行的通信效率和易用性。</div>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>交错式 1F1B 调度</strong>：每个设备分配 v 个虚拟流水线阶段，气泡比例从 <span class=\"kb-math kb-math-inline\">\\frac{p-1}{m}</span> 降至 <span class=\"kb-math kb-math-inline\">\\frac{p-1}{m \\cdot v}</span>，吞吐提升 10%+",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>灵活分片因子 <span class=\"kb-math kb-math-inline\">F</span></strong>：<span class=\"kb-math kb-math-inline\">F=1</span> 为纯复制（等价 DDP），<span class=\"kb-math kb-math-inline\">F=N</span> 为全分片（ZeRO-3），<span class=\"kb-math kb-math-inline\">1&lt;F&lt;N</span> 为混合分片（Hybrid Sharding），在内存与通信间灵活权衡",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>Index-based Token Dispatch</strong>：用紧凑索引数据结构（expert_token_indices + expert_offsets + token_index_map）替代传统的全量routing buffer，避免物化 <span class=\"kb-math kb-math-inline\">O(L \\times E \\times d)</span> 的token副本",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>随机量化算子 <span class=\"kb-math kb-math-inline\">Q_s</span></strong>：将梯度向量的每个分量随机映射到 <span class=\"kb-math kb-math-inline\">s</span> 个均匀量化级别之一，保证无偏性（<span class=\"kb-math kb-math-inline\">\\mathbb{E}[Q_s(\\mathbf{v})] = \\mathbf{v}</span>）",
        "<strong>方差-通信权衡</strong>：方差放大因子为 <span class=\"kb-math kb-math-inline\">\\min(n/s^2, \\sqrt{n}/s)</span>，<span class=\"kb-math kb-math-inline\">s=\\sqrt{n}</span> 时方差仅放大 2 倍",
        "<strong>Elias 递归编码</strong>：利用量化后梯度的稀疏性，将通信开销从 32n 比特压缩至 <span class=\"kb-math kb-math-inline\">2.8n+32</span> 比特",
        "<strong>信息论最优性</strong>：证明该权衡不可渐近改进，任何保证常数方差放大的算法都需 <span class=\"kb-math kb-math-inline\">\\Omega(n)</span> 比特",
      ...
      "detail": "<p><img alt=\"QSGD 量化示意图\" src=\"https://ar5iv.labs.arxiv.org/html/1610.02132/assets/x1.png\" />\n<em>图：5 级随机量化示意。每个梯度分量被归一化后随机映射到相邻的两个量化级别之一，概率与距离成正比，保证期望无偏。</em></p>\n<p><strong>算法伪代码（Algorithm 1: Parallel QSGD）：</strong></p>\n<pre><code class=\"language-python\"># Parallel QSGD with K workers\nfor t in range(T):\n    for each worker i in parallel:\n        # 1. Compute stochastic gradient\n        g_i = stochastic_gradient(f, x_t)\n\n        # 2. Encode: quantize gradient\n        M_i = Encode(Q_s(g_i))  # Apply Q_s then Elias coding\n\n        # 3. Broadcast encoded message to all peers\n        broadcast(M_i)\n\n        # 4. Receive and decode from all peers\n        for each peer l:\n            M_l = receive(l)\n            g_hat_l = Decode(M_l)  # Recover quantized gradient\n\n        # 5. Aggregate and update\n        x_{t+1} = x_t - (eta_t / K) * sum(g_hat_l for l in range(K))\n</code></pre>\n<h5>动机与背景</h5>\n<p>在数据并行分布式训练中，每轮迭代需要在 <span class=\"kb-math kb-math-inline\">K</span> 个工作节点之间同步梯度。对于含 <span class=\"kb-math kb-math-inline\">n</span> 个参数的模型（如 ResNet-152 有 6000 万参数），每轮全精度通信需传输 <span class=\"kb-math kb-math-inline\">32n</span> 比特。当 GPU 计算速度远超网络带宽时，<strong>通信成为训练瓶颈</strong>。实验表明，在 16 GPU 训练 VGG 时，通信占总时间的 80% 以上。</p>\n<p>传统方法要么牺牲收敛性（如直接截断低位），要么缺乏理论保证（如 1BitSGD）。QSGD 的核心洞察是：<strong>量化引入的额外方差可以被视为 SGD 本身随机性的一部分</strong>，从而利用已有的 SGD 收敛理论来分析量化对收敛速度的影响。</p>\n<h5>核心机制：随机量化算子 <span class=\"kb-math kb-math-inline\">Q_s</span></h5>\n<p>对于向量 <span class=\"kb-math kb-math-inline\">\\mathbf{v} \\in \\mathbb{R}^n</span>，量化算子定义为：</p>\n<div class=\"kb-math kb-math-display\">Q_s(v_i) = \\|\\mathbf{v}\\|_2 \\cdot \\text{sgn}(v_i) \\cdot \\xi_i(\\mathbf{v}, s)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\xi_i(\\mathbf{v}, s)</span> 是独立随机变量。设 <span class=\"kb-math kb-math-inline\">\\ell</span> 为满足 <span class=\"kb-math kb-math-inline\">|v_i|/\\|\\mathbf{v}\\|_2 \\in [\\ell/s, (\\ell+1)/s]</span> 的整数，则：</p>\n<div class=\"kb-math kb-math-display\">\\xi_i(\\mathbf{v}, s) = \\begin{cases} \\ell/s &amp; \\text{概率 } 1 - p(|v_i|/\\|\\mathbf{v}\\|_2, s) \\\\ (\\ell+1)/s &amp; \\text{否则} \\end{cases}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">p(a, s) = as - \\ell</span>。</p>\n<div class=\"key-point\">💡 <strong>直觉</strong>：每个分量先除以向量范数归一化到 <span class=\"kb-math kb-math-inline\">[0,1]</span>，然后随机\"舍入\"到最近的两个量化点之一。舍入概率与到两端的距离成正比，这保证了 <span class=\"kb-math kb-math-inline\">\\mathbb{E}[\\xi_i] = |v_i|/\\|\\mathbf{v}\\|_2</span>，即<strong>无偏性</strong>。</div>\n<p><strong>关键性质（Lemma 3.1）：</strong></p>\n<ol>\n<li><strong>无偏性</strong>：<span class=\"kb-math kb-math-inline\">\\mathbb{E}[Q_s(\\mathbf{v})] = \\mathbf{v}</span></li>\n<li><strong>方差界</strong>：<span class=\"kb-math kb-math-inline\">\\mathbb{E}[\\|Q_s(\\mathbf{v}) - \\mathbf{v}\\|_2^2] \\leq \\min\\left(\\frac{n}{s^2}, \\frac{\\sqrt{n}}{s}\\right) \\|\\mathbf{v}\\|_2^2</span></li>\n<li><strong>稀疏性</strong>：<span class=\"kb-math kb-math-inline\">\\mathbb{E}[\\|Q_s(\\mathbf{v})\\|_0] \\leq s(s + \\sqrt{n})</span></li>\n</ol>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：方差界有两个分支——<span class=\"kb-math kb-math-inline\">n/s^2</span> 来自最坏情况分析，<span class=\"kb-math kb-math-inline\">\\sqrt{n}/s</span> 来自利用归一化后分量的 <span class=\"kb-math kb-math-inline\">\\ell_1/\\ell_2</span> 范数比值（Cauchy-Schwarz 不等式）。实际中后者通常更紧。</div>\n<h5>高效编码：Elias 递归编码</h5>\n<p>量化后的梯度表示为三元组 <span class=\"kb-math kb-math-inline\">(\\|\\mathbf{v}\\|_2, \\boldsymbol{\\sigma}, \\boldsymbol{\\zeta})</span>：范数（32位浮点）、符号向量、量化整数向量。关键观察是<strong>较大的量化整数出现概率较低</strong>，因此可用变长编码压缩。</p>\n<p>Elias 递归编码对正整数 <span class=\"kb-math kb-math-inline\">k</span> 的编码长度为 <span class=\"kb-math kb-math-inline\">|\\text{Elias}(k)| \\leq (1+o(1))\\log k + 1</span>。编码方案：\n1. 用 32 位编码 <span class=\"kb-math kb-math-inline\">\\|\\mathbf{v}\\|_2</span>\n2. 用 Elias 编码第一个非零位置\n3. 逐个编码非零项的符号（1位）+ 量化值（Elias 编码）+ 到下一非零项的距离（Elias 编码）</p>\n<p><strong>通信开销（Corollary 3.3）：</strong> 当 <span class=\"kb-math kb-math-inline\">s = \\sqrt{n}</span> 时，期望通信量仅为 <span class=\"kb-math kb-math-inline\">2.8n + 32</span> 比特，相比全精度 <span class=\"kb-math kb-math-inline\">32n</span> 比特压缩约 <strong>11.4 倍</strong>。</p>\n<h5>收敛保证</h5>\n<p><strong>凸情形（Theorem 3.4）：</strong> 对 <span class=\"kb-math kb-math-inline\">L</span>-光滑凸函数，<span class=\"kb-math kb-math-inline\">K</span> 个处理器并行 QSGD 以步长 <span class=\"kb-math kb-math-inline\">\\eta_t = 1/(L + \\sqrt{K}/\\gamma)</span> 运行，收敛到 <span class=\"kb-math kb-math-inline\">\\epsilon</span> 精度需要：</p>\n<div class=\"kb-math kb-math-display\">T = O\\left(R^2 \\cdot \\max\\left(\\frac{2B&#x27;}{K\\epsilon^2}, \\frac{L}{\\epsilon}\\right)\\right)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">B&#x27; = \\min(n/s^2, \\sqrt{n}/s) \\cdot B</span> 是量化后的二阶矩界。</p>\n<div class=\"key-point\">💡 <strong>关键洞察</strong>：量化仅影响方差项 <span class=\"kb-math kb-math-inline\">B&#x27; / (K\\epsilon^2)</span>，不影响光滑性项 <span class=\"kb-math kb-math-inline\">L/\\epsilon</span>。当 <span class=\"kb-math kb-math-inline\">K</span> 足够大使得光滑性项主导时，量化对收敛速度<strong>几乎无影响</strong>。</div>\n<p><strong>非凸情形（Theorem 3.5）：</strong> 对 <span class=\"kb-math kb-math-inline\">L</span>-光滑（可能非凸）函数：</p>\n<div class=\"kb-math kb-math-display\">\\frac{1}{L}\\mathbb{E}\\left[\\|\\nabla f(\\mathbf{x})\\|_2^2\\right] \\leq O\\left(\\frac{\\sqrt{L(f(\\mathbf{x}_1) - f^*)}}{N} + \\frac{\\min(n/s^2, \\sqrt{n}/s) \\cdot B}{L}\\right)</div>\n<h5>与传统方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>通信比特/轮</th>\n<th>方差放大</th>\n<th>理论保证</th>\n<th>可调性</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>全精度 SGD</td>\n<td><span class=\"kb-math kb-math-inline\">32n</span></td>\n<td>1×</td>\n<td>✓</td>\n<td>—</td>\n</tr>\n<tr>\n<td>1BitSGD</td>\n<td><span class=\"kb-math kb-math-inline\">n + 64</span></td>\n<td>无界</td>\n<td>✗</td>\n<td>✗</td>\n</tr>\n<tr>\n<td>TernGrad</td>\n<td><span class=\"kb-math kb-math-inline\">2n</span></td>\n<td><span class=\"kb-math kb-math-inline\">\\sqrt{n}</span>×</td>\n<td>部分</td>\n<td>✗</td>\n</tr>\n<tr>\n<td><strong>QSGD</strong> (<span class=\"kb-math kb-math-inline\">s=\\sqrt{n}</span>)</td>\n<td><span class=\"kb-math kb-math-inline\">2.8n + 32</span></td>\n<td>2×</td>\n<td>✓</td>\n<td>✓</td>\n</tr>\n<tr>\n<td><strong>QSGD</strong> (<span class=\"kb-math kb-math-inline\">s=1</span>)</td>\n<td><span class=\"kb-math kb-math-inline\">O(\\sqrt{n}\\log n)</span></td>\n<td><span class=\"kb-math kb-math-inline\">\\sqrt{n}</span>×</td>\n<td>✓</td>\n<td>✓</td>\n</tr>\n</tbody>\n</table></div>\n<h5>实验结果</h5>\n<p>在 Amazon EC2 p2.16xlarge（16× NVIDIA K80 GPU）上的实验结果：</p>\n<ul>\n<li><strong>AlexNet/ImageNet</strong>（16 GPU）：通信时间减少 4×，端到端训练加速 2.5×，精度无损</li>\n<li><strong>ResNet-152/ImageNet</strong>（16 GPU）：端到端收敛时间减少约 2×</li>\n<li><strong>LSTM 语音识别</strong>（2 GPU）：通信时间减少 6.8×，训练加速 2.7×</li>\n<li><strong>VGG/CIFAR-10</strong>：通信占比从 &gt;80% 降至 &lt;40%，精度保持不变</li>\n<li>所有实验中量化训练的最终精度与全精度训练<strong>几乎完全一致</strong></li>\n</ul>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "去中心化通信拓扑：用双随机权重矩阵 <span class=\"kb-math kb-math-inline\">W</span> 编码节点间通信关系，每个节点仅与邻居交换参数",
        "通信复杂度优势：每节点每轮通信 <span class=\"kb-math kb-math-inline\">O(\\text{degree})</span>，远低于中心化的 <span class=\"kb-math kb-math-inline\">O(n)</span>；环形拓扑下为 <span class=\"kb-math kb-math-inline\">O(1)</span>",
        "收敛率：非凸目标下达到 <span class=\"kb-math kb-math-inline\">O\\left(\\frac{1}{\\sqrt{nK}}\\right)</span>，与中心化 mini-batch SGD 相同",
        "线性加速条件：环形拓扑下，当 <span class=\"kb-math kb-math-inline\">n = O(K^{1/9})</span>（共享数据）或 <span class=\"kb-math kb-math-inline\">n = O(K^{1/13})</span>（分区数据）时可实现",
        "共识收敛：所有节点局部变量以 <span class=\"kb-math kb-math-inline\">O(1/K)</span> 速率收敛到一致",
        "实验验证：在 112 GPU 集群上训练 ResNet，低带宽/高延迟网络下比中心化方法快 10 倍以上",
        "不依赖有界梯度假设：理论分析仅需 Lipschitz 梯度和有界方差，比已有去中心化分析更一般"
      ],
      "detail": "<pre><code>中心化 (Parameter Server)          去中心化 (D-PSGD, Ring Topology)\n\n        [PS]                          [W1] --- [W2]\n       / | \\                          |           |\n      /  |  \\                         |           |\n   [W1] [W2] [W3]                    [W4] --- [W3]\n      \\  |  /\n       \\ | /                    每节点通信量: O(degree)\n    每节点通信量: O(n)           无中心瓶颈，带宽均匀分布\n    PS 为通信瓶颈\n</code></pre>\n<p><em>图：中心化 vs 去中心化通信模式对比。D-PSGD 中每个节点仅与邻居交换参数，避免了参数服务器的带宽瓶颈。</em></p>\n<div class=\"key-point\">💡 <strong>核心直觉</strong>：中心化 SGD 中所有节点必须与参数服务器通信，形成 <span class=\"kb-math kb-math-inline\">O(n)</span> 的通信瓶颈；D-PSGD 让每个节点只与固定数量的邻居交换信息，通信负载恒定，同时通过混合矩阵的谱隙保证全局信息最终传播到所有节点。</div>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># D-PSGD 算法 (Algorithm 1)\n# 输入: 初始点 x_{0,i}, 学习率 γ, 权重矩阵 W, 迭代次数 K\n# W 为 n×n 对称双随机矩阵 (W1=1, 1^T W=1^T, W=W^T)\n\nfor k in range(K):\n    for i in range(n):  # 所有节点并行执行\n        # Step 1: 从本地数据采样\n        xi_k_i = sample_local_data(node_i)\n\n        # Step 2: 计算本地随机梯度\n        grad_i = compute_gradient(F_i, x_k_i, xi_k_i)\n\n        # Step 3: 邻居加权平均 (通信步)\n        x_half = sum(W[i][j] * x_k_j for j in neighbors(i))\n\n        # Step 4: 梯度更新\n        x_{k+1}_i = x_half - gamma * grad_i\n\n# 输出: (1/n) * sum(x_K_i for i in range(n))\n</code></pre>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：Step 2 和 Step 3 可以并行执行——当通信时间小于计算时间时，通信开销可被完全隐藏。</div>\n<h5>动机与背景</h5>\n<p>传统分布式 SGD 采用中心化架构（参数服务器或 AllReduce），存在两个根本问题：</p>\n<ol>\n<li><strong>通信瓶颈</strong>：参数服务器模式下，中心节点需处理所有 <span class=\"kb-math kb-math-inline\">n</span> 个 worker 的梯度，带宽需求为 <span class=\"kb-math kb-math-inline\">O(n)</span>；AllReduce 虽然分散了负载，但每个节点仍需参与 <span class=\"kb-math kb-math-inline\">O(\\log n)</span> 次通信。</li>\n<li><strong>单点故障</strong>：中心化架构对网络拓扑有严格要求，不适用于带宽异构或延迟较高的集群。</li>\n</ol>\n<p>D-PSGD 的核心思想是：<strong>将全局同步替换为局部同步</strong>。每个节点只需与固定数量的邻居交换参数，然后做加权平均。这种设计使得：\n- 每节点通信量从 <span class=\"kb-math kb-math-inline\">O(n)</span> 降至 <span class=\"kb-math kb-math-inline\">O(\\text{degree})</span>\n- 网络中不存在通信热点\n- 天然适配带宽受限的异构网络</p>\n<h5>核心机制：双随机混合矩阵</h5>\n<p>D-PSGD 的关键数学工具是<strong>双随机矩阵</strong> <span class=\"kb-math kb-math-inline\">W \\in \\mathbb{R}^{n \\times n}</span>，满足：</p>\n<div class=\"kb-math kb-math-display\">W_{ij} \\geq 0, \\quad W = W^T, \\quad W\\mathbf{1} = \\mathbf{1}</div>\n<p>矩阵 <span class=\"kb-math kb-math-inline\">W</span> 编码了网络拓扑：<span class=\"kb-math kb-math-inline\">W_{ij} &gt; 0</span> 当且仅当节点 <span class=\"kb-math kb-math-inline\">i</span> 和 <span class=\"kb-math kb-math-inline\">j</span> 相连。双随机性保证了：\n- 加权平均保持全局均值不变\n- 反复混合后所有节点趋于一致（共识）</p>\n<p>混合速度由<strong>谱隙</strong> <span class=\"kb-math kb-math-inline\">\\rho = \\max(|\\lambda_2(W)|, |\\lambda_n(W)|)</span> 决定，<span class=\"kb-math kb-math-inline\">\\rho &lt; 1</span> 越小收敛越快。</p>\n<h5>收敛理论</h5>\n<p><strong>假设条件</strong>（Assumption 1）：\n1. <span class=\"kb-math kb-math-inline\">L</span>-Lipschitz 梯度：<span class=\"kb-math kb-math-inline\">\\|\\nabla f_i(x) - \\nabla f_i(y)\\| \\leq L\\|x-y\\|</span>\n2. 有界方差：<span class=\"kb-math kb-math-inline\">\\mathbb{E}\\|\\nabla F_i(x;\\xi) - \\nabla f_i(x)\\|^2 \\leq \\sigma^2</span>\n3. 有界数据异质性：<span class=\"kb-math kb-math-inline\">\\frac{1}{n}\\sum_i \\|\\nabla f_i(x) - \\nabla f(x)\\|^2 \\leq \\varsigma^2</span></p>\n<p><strong>主定理</strong>（非凸情况）：选择学习率 <span class=\"kb-math kb-math-inline\">\\gamma = O\\left(\\frac{1}{\\sqrt{nK}}\\right)</span>，D-PSGD 满足：</p>\n<div class=\"kb-math kb-math-display\">\\frac{1}{K}\\sum_{k=0}^{K-1}\\mathbb{E}\\left\\|\\nabla f(\\bar{x}_k)\\right\\|^2 \\leq O\\left(\\frac{\\sigma}{\\sqrt{nK}} + \\frac{n^{1/3}\\varsigma^{2/3}}{K^{2/3}(1-\\rho)^{2/3}}\\right)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\bar{x}_k = \\frac{1}{n}\\sum_i x_{k,i}</span> 是所有节点的平均。</p>\n<div class=\"key-point\">💡 <strong>关键洞察</strong>：第一项 <span class=\"kb-math kb-math-inline\">O(1/\\sqrt{nK})</span> 表明 <span class=\"kb-math kb-math-inline\">n</span> 个节点带来线性加速（等效于 <span class=\"kb-math kb-math-inline\">n</span> 倍 mini-batch）；第二项是去中心化的代价，依赖谱隙 <span class=\"kb-math kb-math-inline\">\\rho</span> 和数据异质性 <span class=\"kb-math kb-math-inline\">\\varsigma</span>，当 <span class=\"kb-math kb-math-inline\">K</span> 足够大时可忽略。</div>\n<h5>与中心化方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>C-PSGD (中心化)</th>\n<th>D-PSGD (去中心化)</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>每节点通信量/轮</td>\n<td><span class=\"kb-math kb-math-inline\">O(n)</span></td>\n<td><span class=\"kb-math kb-math-inline\">O(\\text{degree})</span></td>\n</tr>\n<tr>\n<td>收敛率</td>\n<td><span class=\"kb-math kb-math-inline\">O(1/\\sqrt{nK})</span></td>\n<td><span class=\"kb-math kb-math-inline\">O(1/\\sqrt{nK})</span></td>\n</tr>\n<tr>\n<td>网络瓶颈</td>\n<td>参数服务器带宽</td>\n<td>无</td>\n</tr>\n<tr>\n<td>拓扑要求</td>\n<td>星形/全连接</td>\n<td>任意连通图</td>\n</tr>\n<tr>\n<td>适用场景</td>\n<td>高速互联集群</td>\n<td>带宽受限/异构网络</td>\n</tr>\n</tbody>\n</table></div>\n<p>D-PSGD 在收敛率上与中心化方法完全匹配，但通信效率在稀疏拓扑（如环形网络，degree=2）下有数量级优势。实验表明，在 10Mbps 带宽或 5ms 延迟的网络条件下，D-PSGD 比中心化实现快 10 倍以上。</p>\n<h5>实验结果</h5>\n<ul>\n<li><strong>数据集/模型</strong>：CIFAR-10 上训练 ResNet-20/56</li>\n<li><strong>规模</strong>：最大 112 GPU（AWS EC2 集群）</li>\n<li><strong>关键发现</strong>：</li>\n<li>收敛曲线（按 epoch）与中心化 SGD 几乎重合</li>\n<li>低带宽网络下 wall-clock 时间快 10 倍</li>\n<li>16 机器线性加速：达到 0.2 训练损失所需 epoch 从 80（1机）降至 5（16机）</li>\n<li>泛化能力：测试误差 7.15%-7.46%，优于 He et al. 报告的 7.51%</li>\n<li>网络利用率仅约 50%，远未达到瓶颈</li>\n</ul>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>线性加速比保证</strong>：理论证明当同步间隔 <span class=\"kb-math kb-math-inline\">H = O\\bigl(\\sqrt{T/(Kb)}\\bigr)</span> 时，收敛率为 <span class=\"kb-math kb-math-inline\">O\\bigl(\\frac{G^2}{\\mu b K T}\\bigr)</span>，与全同步 mini-batch SGD 完全一致",
        "<strong>通信轮次大幅削减</strong>：相比 mini-batch SGD 的 T 轮通信，Local SGD 仅需 <span class=\"kb-math kb-math-inline\">T/H = O(\\sqrt{KbT})</span> 轮，减少因子达 <span class=\"kb-math kb-math-inline\">\\sqrt{T/(Kb)}</span>",
        "<strong>理论覆盖强凸与凸两种设定</strong>：分别给出 <span class=\"kb-math kb-math-inline\">\\mu</span>-强凸和一般凸函数的收敛界",
        "<strong>支持异步实现</strong>：理论结果同样适用于异步版本的 Local SGD",
        "<strong>与增大 batch size 正交的加速思路</strong>：不增大 mini-batch（避免泛化退化），而是降低通信频率来提升计算通信比"
      ],
      "detail": "<p><img alt=\"Local SGD 加速示意图\" src=\"https://ar5iv.labs.arxiv.org/html/1805.09767/assets/x1.png\" />\n<em>图：增大 mini-batch size b（路径 1→2）与增大同步间隔 H（路径 1→2）对训练加速效果的对比示意。两者都能提升计算通信比，但 Local SGD 通过增大 H 实现，避免了大 batch 带来的泛化问题。</em></p>\n<p><strong>算法伪代码：</strong></p>\n<pre><code class=\"language-python\"># Algorithm 1: Local SGD\n# 输入: K个worker, 总步数T, 同步间隔H, 学习率序列{η_t}\n# 初始化: 所有worker共享相同初始参数 x_0\n\nfor t in range(T):\n    # 每个worker并行执行\n    for k in range(K):  # parallel\n        g_t_k = stochastic_gradient(x_t_k)  # 采样计算随机梯度\n\n        if (t + 1) % H == 0:  # 同步轮次\n            # 先本地更新，再全局平均\n            x_{t+1}_k = (1/K) * sum([x_t_j - η_t * g_t_j for j in range(K)])\n        else:  # 非同步轮次\n            # 仅本地更新，不通信\n            x_{t+1}_k = x_t_k - η_t * g_t_k\n\n# 输出: 加权平均 x̂_T = Σ w_t * x̄_t (x̄_t 为K个worker的平均)\n</code></pre>\n<h5>动机与背景</h5>\n<p>分布式 SGD 的核心瓶颈在于<strong>通信开销</strong>。标准的并行 mini-batch SGD 要求每步迭代都进行一次 AllReduce 操作来同步梯度，当网络带宽有限或延迟较高时，通信时间可能远超计算时间。</p>\n<p>已有的两种应对策略各有局限：\n1. <strong>增大 mini-batch size</strong>：提高每次通信的计算量，但实践中过大的 batch 会导致泛化性能下降（generalization gap）\n2. <strong>梯度压缩/量化</strong>：减少每次通信的数据量，但引入额外近似误差</p>\n<p>Local SGD 提出了第三条路径：<strong>降低通信频率</strong>。每个 worker 独立执行多步 SGD 后再同步，从根本上减少通信轮次。这一思想虽早已在实践中被使用（如 federated learning 中的 FedAvg），但此前缺乏严格的理论保证——甚至无法证明在凸问题上平均是否真的有帮助。</p>\n<h5>核心理论结果</h5>\n<p><strong>Theorem 2.2（强凸情形）：</strong> 设 <span class=\"kb-math kb-math-inline\">f</span> 为 <span class=\"kb-math kb-math-inline\">L</span>-光滑且 <span class=\"kb-math kb-math-inline\">\\mu</span>-强凸函数，梯度方差有界 <span class=\"kb-math kb-math-inline\">\\mathbb{E}\\|\\nabla f_i(\\mathbf{x}) - \\nabla f(\\mathbf{x})\\|^2 \\leq \\sigma^2</span>，二阶矩有界 <span class=\"kb-math kb-math-inline\">\\mathbb{E}\\|\\nabla f_i(\\mathbf{x})\\|^2 \\leq G^2</span>。对于 K 个 worker 运行 Local SGD，同步间隔为 H，使用递减学习率 <span class=\"kb-math kb-math-inline\">\\eta_t = \\frac{4}{\\mu(a+t)}</span>，则加权平均迭代 <span class=\"kb-math kb-math-inline\">\\hat{\\mathbf{x}}_T</span> 满足：</p>\n<div class=\"kb-math kb-math-display\">\\mathbb{E}[f(\\hat{\\mathbf{x}}_T)] - f^\\star = O\\left(\\frac{L}{\\mu^2 T^2} + \\frac{\\sigma^2}{\\mu K T} + \\frac{H G^2}{\\mu T}\\right)</div>\n<div class=\"key-point\">💡 <strong>关键洞察</strong>：收敛界中有三项——(1) 优化偏差项（与初始距离相关），(2) 方差项（被 K 个 worker 线性减少），(3) 同步延迟项（与 H 成正比）。当 <span class=\"kb-math kb-math-inline\">H = O(\\sqrt{T/K})</span> 时，第三项不超过第二项，从而保持线性加速比。</div>\n<p><strong>Corollary（通信复杂度）：</strong> 为达到精度 <span class=\"kb-math kb-math-inline\">\\epsilon</span>，Local SGD 需要：\n- 每个 worker 的梯度计算次数：<span class=\"kb-math kb-math-inline\">T = O\\bigl(\\frac{G^2}{\\mu K \\epsilon}\\bigr)</span>（与 mini-batch SGD 相同）\n- 通信轮次：<span class=\"kb-math kb-math-inline\">T/H = O\\bigl(\\sqrt{\\frac{K G^2}{\\mu \\epsilon}}\\bigr)</span>（比 mini-batch SGD 少 <span class=\"kb-math kb-math-inline\">\\sqrt{T/K}</span> 倍）</p>\n<h5>证明核心思路</h5>\n<p>证明的关键技术难点在于：Local SGD 中各 worker 的迭代序列 <span class=\"kb-math kb-math-inline\">\\mathbf{x}_t^k</span> 会逐渐偏离（drift），如何控制这种偏离是核心。</p>\n<p>作者的证明策略分两步：</p>\n<ol>\n<li>\n<p><strong>虚拟序列分析</strong>：定义虚拟平均序列 <span class=\"kb-math kb-math-inline\">\\bar{\\mathbf{x}}_t = \\frac{1}{K}\\sum_{k=1}^K \\mathbf{x}_t^k</span>，分析其收敛性。关键观察是即使各 worker 不同步，虚拟平均序列的更新方向仍然是无偏的梯度估计，且方差被 K 个独立样本降低。</p>\n</li>\n<li>\n<p><strong>偏离量控制</strong>：证明各 worker 与虚拟平均的偏离 <span class=\"kb-math kb-math-inline\">\\mathbb{E}\\|\\mathbf{x}_t^k - \\bar{\\mathbf{x}}_t\\|^2</span> 可以被 <span class=\"kb-math kb-math-inline\">O(\\eta_t^2 H^2 G^2)</span> 控制。这一偏离引入的额外误差在学习率递减时可以被吸收。</p>\n</li>\n</ol>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：证明要求使用递减学习率 <span class=\"kb-math kb-math-inline\">\\eta_t = O(1/t)</span>。对于常数学习率，同步间隔 H 的允许范围会更受限。</div>\n<h5>与传统方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>每步通信</th>\n<th>通信轮次</th>\n<th>收敛率</th>\n<th>适用场景</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Mini-batch SGD</td>\n<td>每步 AllReduce</td>\n<td>T</td>\n<td><span class=\"kb-math kb-math-inline\">O(\\frac{\\sigma^2}{\\mu KT})</span></td>\n<td>低延迟网络</td>\n</tr>\n<tr>\n<td>Local SGD</td>\n<td>每 H 步 AllReduce</td>\n<td>T/H</td>\n<td><span class=\"kb-math kb-math-inline\">O(\\frac{G^2}{\\mu KT})</span></td>\n<td>高延迟/低带宽</td>\n</tr>\n<tr>\n<td>One-shot Averaging</td>\n<td>仅最终一次</td>\n<td>1</td>\n<td>无线性加速</td>\n<td>理论极限</td>\n</tr>\n</tbody>\n</table></div>\n<p>Local SGD 的核心优势在于：在保持与 mini-batch SGD 相同计算效率的前提下，将通信复杂度从 <span class=\"kb-math kb-math-inline\">O(T)</span> 降低到 <span class=\"kb-math kb-math-inline\">O(\\sqrt{KT})</span>，这对于跨数据中心训练、联邦学习等通信受限场景具有重要实际意义。</p>\n<h5>实际影响与局限</h5>\n<p><strong>实际影响：</strong>\n- 为 FedAvg（联邦学习中的核心算法）提供了理论基础\n- 指导了大规模分布式训练中同步频率的选择：<span class=\"kb-math kb-math-inline\">H \\propto \\sqrt{T/K}</span>\n- 证明了\"不需要用大 batch，用 Local SGD\"这一实践经验的理论正确性</p>\n<p><strong>局限性：</strong>\n- 理论分析限于凸/强凸设定，深度学习的非凸情形需要额外假设\n- 收敛界中使用的是二阶矩界 <span class=\"kb-math kb-math-inline\">G^2</span> 而非方差 <span class=\"kb-math kb-math-inline\">\\sigma^2</span>，在某些情况下可能不够紧\n- 递减学习率的要求与实践中常用的分段常数学习率有差距</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>结构化更新</strong>：限制模型更新矩阵为低秩形式 <span class=\"kb-math kb-math-inline\">H = AB^T</span> 或随机稀疏掩码形式",
      ...
      "detail": "<h5>框架总览</h5>\n<p><img alt=\"联邦学习通信压缩：结构化更新 vs 草图更新\" src=\"https://ar5iv.labs.arxiv.org/html/1610.05492/assets/cifar_figures/rank_rounds.png\" />\n<em>图：CIFAR-10 数据集上结构化低秩更新在不同压缩比下的收敛曲线。rank 越低压缩比越高，rank=5 时约 256x 压缩仍能收敛。</em></p>\n<h5>问题设定</h5>\n<p>联邦学习的标准流程为：服务器将全局模型 <span class=\"kb-math kb-math-inline\">W_t</span> 下发给选中的客户端子集，每个客户端在本地数据上训练得到更新 <span class=\"kb-math kb-math-inline\">H_t = W_t&#x27; - W_t</span>，再将 <span class=\"kb-math kb-math-inline\">H_t</span> 上传至服务器聚合。由于移动设备的上行带宽远小于下行带宽，<strong>上行通信成为主要瓶颈</strong>。</p>\n<p>本文的目标是在保持模型收敛性的前提下，最小化每轮上传的比特数：</p>\n<div class=\"kb-math kb-math-display\">\\min \\text{bits}(H_t) \\quad \\text{s.t.} \\quad \\text{model quality} \\approx \\text{uncompressed baseline}</div>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># 联邦学习通信压缩框架\ndef federated_round(server_model, selected_clients, compress_method):\n    updates = []\n    for client in selected_clients:\n        # 1. 客户端接收全局模型\n        local_model = server_model.copy()\n\n        # 2. 本地训练（多个 epoch 的 SGD）\n        for epoch in range(E):\n            for batch in client.local_data:\n                local_model.sgd_step(batch)\n\n        # 3. 计算更新\n        H = local_model - server_model\n\n        # 4. 压缩更新（核心步骤）\n        if compress_method == &quot;structured_lowrank&quot;:\n            # 直接在低秩空间训练：H = A @ B.T, A∈R^(n×k), B∈R^(m×k)\n            H_compressed = lowrank_update(H, rank=k)\n        elif compress_method == &quot;structured_mask&quot;:\n            # 随机稀疏掩码：仅更新随机选择的条目\n            H_compressed = sparse_mask_update(H, sparsity=s)\n        elif compress_method == &quot;sketched&quot;:\n            # 子采样 + 量化\n            H_rotated = random_rotation(H)       # 可选预处理\n            H_subsampled = subsample(H_rotated, rate=p)\n            H_compressed = quantize(H_subsampled, bits=b)\n\n        updates.append(H_compressed)\n\n    # 5. 服务器聚合\n    server_model += aggregate(updates)\n    return server_model\n</code></pre>\n<h5>方法一：结构化更新（Structured Updates）</h5>\n<p>结构化更新的核心思想是<strong>在训练阶段就限制更新的结构</strong>，使其天然具有低通信成本。</p>\n<p><strong>低秩更新（Low-rank）：</strong> 对于全连接层权重矩阵 <span class=\"kb-math kb-math-inline\">W \\in \\mathbb{R}^{n \\times m}</span>，将更新限制为：</p>\n<div class=\"kb-math kb-math-display\">H = A \\cdot B^T, \\quad A \\in \\mathbb{R}^{n \\times k}, \\; B \\in \\mathbb{R}^{m \\times k}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">k \\ll \\min(n, m)</span>。客户端只需上传 <span class=\"kb-math kb-math-inline\">A</span> 和 <span class=\"kb-math kb-math-inline\">B</span>，通信量从 <span class=\"kb-math kb-math-inline\">O(nm)</span> 降至 <span class=\"kb-math kb-math-inline\">O((n+m)k)</span>。</p>\n<div class=\"key-point\">💡 关键：训练时固定 <span class=\"kb-math kb-math-inline\">B</span> 为随机生成的矩阵（由共享随机种子确定），只优化 <span class=\"kb-math kb-math-inline\">A</span>。这样客户端仅需上传 <span class=\"kb-math kb-math-inline\">A</span> 和随机种子，进一步减少通信。</div>\n<p><strong>随机掩码更新（Random Mask）：</strong> 使用共享随机种子生成稀疏掩码，客户端仅更新掩码选中的参数子集：</p>\n<div class=\"kb-math kb-math-display\">H_{ij} = \\begin{cases} \\text{trained value} &amp; \\text{if } (i,j) \\in \\text{Mask} \\\\ 0 &amp; \\text{otherwise} \\end{cases}</div>\n<h5>方法二：草图更新（Sketched Updates）</h5>\n<p>草图更新允许客户端<strong>自由训练完整模型更新</strong>，然后在上传前进行压缩编码。</p>\n<p><strong>子采样（Subsampling）：</strong> 随机选取更新向量中的一部分坐标上传，未选中的置零。为保持无偏性，选中的值需乘以 <span class=\"kb-math kb-math-inline\">1/p</span>（<span class=\"kb-math kb-math-inline\">p</span> 为采样概率）。</p>\n<p><strong>概率量化（Probabilistic Quantization）：</strong> 将更新值量化到有限精度。对标量 <span class=\"kb-math kb-math-inline\">v \\in [a, b]</span>，量化为：</p>\n<div class=\"kb-math kb-math-display\">Q(v) = \\begin{cases} a &amp; \\text{w.p. } \\frac{b-v}{b-a} \\\\ b &amp; \\text{w.p. } \\frac{v-a}{b-a} \\end{cases}</div>\n<p>此量化是<strong>无偏的</strong>：<span class=\"kb-math kb-math-inline\">\\mathbb{E}[Q(v)] = v</span>。</p>\n<div class=\"warn-box\">⚠️ 注意：量化的效果依赖于更新值的分布。如果值集中在少数大值上，均匀量化的误差较大。</div>\n<p><strong>结构化随机旋转预处理：</strong> 为改善量化效果，在量化前对更新向量施加随机旋转：</p>\n<div class=\"kb-math kb-math-display\">\\hat{H} = R \\cdot H</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">R</span> 为随机正交矩阵。旋转后各坐标的方差趋于一致，使量化更高效。实际使用 Walsh-Hadamard 变换实现 <span class=\"kb-math kb-math-inline\">O(d \\log d)</span> 复杂度的快速旋转。</p>\n<h5>组合策略与压缩比分析</h5>\n<p>两类方法可以组合使用。例如：\n- 子采样率 <span class=\"kb-math kb-math-inline\">p</span> + <span class=\"kb-math kb-math-inline\">b</span> 位量化 → 压缩比 = <span class=\"kb-math kb-math-inline\">\\frac{32}{p \\cdot b}</span>（原始 32 位浮点）\n- 低秩 <span class=\"kb-math kb-math-inline\">k</span> + 量化 → 压缩比 = <span class=\"kb-math kb-math-inline\">\\frac{nm \\cdot 32}{(n+m) \\cdot k \\cdot b}</span></p>\n<p><img alt=\"草图更新对比实验\" src=\"https://ar5iv.labs.arxiv.org/html/1610.05492/assets/cifar_figures/no_quant_rounds_125.png\" />\n<em>图：CIFAR-10 上结构化随机掩码更新与草图子采样更新的对比。两者在相同压缩比下表现相当。</em></p>\n<p><img alt=\"随机旋转预处理效果\" src=\"https://ar5iv.labs.arxiv.org/html/1610.05492/assets/cifar_figures/sketching_100_rounds.png\" />\n<em>图：加入随机旋转预处理后，草图更新（子采样+量化）的效果显著提升，接近无压缩基线。</em></p>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>传统梯度压缩</th>\n<th>本文方法</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>压缩对象</td>\n<td>单步梯度</td>\n<td>多步本地训练的完整更新</td>\n</tr>\n<tr>\n<td>适用场景</td>\n<td>数据中心分布式训练</td>\n<td>联邦学习（非IID、设备异构）</td>\n</tr>\n<tr>\n<td>隐私兼容</td>\n<td>不考虑</td>\n<td>兼容 Secure Aggregation</td>\n</tr>\n<tr>\n<td>压缩时机</td>\n<td>每次通信</td>\n<td>本地多轮训练后一次压缩</td>\n</tr>\n<tr>\n<td>结构约束</td>\n<td>无</td>\n<td>可在训练阶段直接施加结构约束</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 关键创新：本文首次将通信压缩问题置于<strong>联邦学习</strong>框架下考虑，不仅关注压缩比，还关注与隐私保护协议的兼容性，以及在非IID数据分布下的鲁棒性。</div>\n<h5>实验结果</h5>\n<ul>\n<li><strong>CIFAR-10 CNN</strong>：结构化低秩更新（rank=5）实现 ~256x 压缩，100 轮后准确率仅比无压缩基线低约 2%</li>\n<li><strong>Reddit LSTM</strong>：草图更新（1-bit 量化 + 10% 子采样）实现 ~50x 压缩，困惑度接近基线</li>\n<li>随机旋转预处理在所有量化方案中均带来显著改善</li>\n</ul>",
      "quiz": {
