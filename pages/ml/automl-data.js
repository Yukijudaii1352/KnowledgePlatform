/**
 * automl-data.js — 由 pipeline/build.py 于 2026-05-26 14:20:25 自动生成。
  ...
  "algos": [
    {
      ...
      "keyPoints": [
        ...
        "采用 <strong>Expected Improvement (EI)</strong> 作为采集函数，自动平衡探索与利用；具体使用对数变换下的 <span class=\"kb-math kb-math-inline\">E[I_{\\exp}]</span> 准则",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>逆向建模思路</strong>：不直接建模 <span class=\"kb-math kb-math-inline\">P(y|x)</span>（如高斯过程），而是建模 <span class=\"kb-math kb-math-inline\">P(x|y&lt;y^*)</span> 和 <span class=\"kb-math kb-math-inline\">P(x|y \\geq y^*)</span>，即条件密度 <span class=\"kb-math kb-math-inline\">l(x)</span> 和 <span class=\"kb-math kb-math-inline\">g(x)</span>",
        "<strong>EI 等价简化</strong>：证明 Expected Improvement 正比于 <span class=\"kb-math kb-math-inline\">\\gamma + (1-\\gamma) \\cdot g(x)/l(x)</span> 的倒数，最大化 EI 等价于最大化 <span class=\"kb-math kb-math-inline\">l(x)/g(x)</span>",
        ...
        "<strong>阈值分位数 <span class=\"kb-math kb-math-inline\">\\gamma</span></strong>：以观测值的 <span class=\"kb-math kb-math-inline\">\\gamma</span> 分位数作为 <span class=\"kb-math kb-math-inline\">y^*</span>，典型取 <span class=\"kb-math kb-math-inline\">\\gamma=0.15\\sim0.25</span>",
      ...
      "detail": "<pre><code>                        TPE 密度估计示意图\n\n  目标函数值 y\n       ▲\n       │         ×  ×\n  y*  ─┼─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─   ← 阈值 (γ 分位数)\n       │    ●  ●     ●   ●  ●\n       │  ●      ●  ●  ●     ●\n       └──────────────────────────→ 超参数 x\n\n       ● = 好的观测 (y &lt; y*)  →  拟合 l(x)\n       × = 差的观测 (y ≥ y*)  →  拟合 g(x)\n\n  密度\n   ▲   l(x): 好观测的KDE          g(x): 差观测的KDE\n   │    ╱╲                           ╱──╲\n   │  ╱    ╲    ╱╲                 ╱      ╲\n   │╱        ╲╱    ╲             ╱          ╲\n   └──────────────────→ x    └──────────────────→ x\n\n  选择策略: 从 l(x) 采样候选点, 按 l(x)/g(x) 排序取最优\n</code></pre>\n<p><em>图：TPE 将观测按阈值 <span class=\"kb-math kb-math-inline\">y^*</span> 分为两组，分别用核密度估计建模 <span class=\"kb-math kb-math-inline\">l(x)</span>（好的观测）和 <span class=\"kb-math kb-math-inline\">g(x)</span>（差的观测），新候选点从 <span class=\"kb-math kb-math-inline\">l(x)</span> 中采样并按 <span class=\"kb-math kb-math-inline\">l(x)/g(x)</span> 排序选择</em></p>\n<pre><code class=\"language-python\"># TPE 核心算法伪代码\ndef tpe_suggest(observations, gamma=0.25):\n    &quot;&quot;&quot;\n    observations: 历史观测 {x_i, y_i}\n    gamma: 分位数阈值比例\n    &quot;&quot;&quot;\n    # Step 1: 按目标值排序，确定阈值 y*\n    y_star = quantile(observations.y, gamma)\n\n    # Step 2: 将观测分为两组\n    D_good = {x_i | y_i &lt; y_star}   # 好的观测\n    D_bad  = {x_i | y_i &gt;= y_star}  # 差的观测\n\n    # Step 3: 分别拟合核密度估计\n    l_x = fit_parzen_estimator(D_good)  # P(x | y &lt; y*)\n    g_x = fit_parzen_estimator(D_bad)   # P(x | y &gt;= y*)\n\n    # Step 4: 从 l(x) 中采样候选点\n    candidates = sample(l_x, n_candidates=24)\n\n    # Step 5: 按 l(x)/g(x) 排序，选择比值最大的点\n    scores = [l_x.pdf(c) / g_x.pdf(c) for c in candidates]\n    return candidates[argmax(scores)]\n</code></pre>\n<h5>动机与背景</h5>\n<p>超参数优化（HPO）是机器学习中的关键问题。传统方法如网格搜索在高维空间中效率极低（指数级增长），随机搜索虽然更好但仍未利用历史信息。基于高斯过程（GP）的贝叶斯优化虽然能建模目标函数的后验分布，但存在以下局限：</p>\n<ol>\n<li><strong>计算复杂度</strong>：GP 的推断复杂度为 <span class=\"kb-math kb-math-inline\">O(n^3)</span>，随观测数增长迅速变慢</li>\n<li><strong>条件空间处理困难</strong>：GP 假设固定维度的连续空间，难以自然处理条件/层级超参数（如\"当 kernel=RBF 时才有 gamma 参数\"）</li>\n<li><strong>高维性能退化</strong>：GP 在超过 10-20 维时性能显著下降</li>\n</ol>\n<p>TPE 的核心创新在于<strong>逆转建模方向</strong>：不建模 <span class=\"kb-math kb-math-inline\">P(y|x)</span>，而是建模 <span class=\"kb-math kb-math-inline\">P(x|y)</span>。</p>\n<h5>核心机制：从 EI 到密度比</h5>\n<p>Expected Improvement（EI）的标准定义为：</p>\n<div class=\"kb-math kb-math-display\">\\text{EI}_{y^*}(x) = \\int_{-\\infty}^{y^*} (y^* - y) \\cdot p(y|x) \\, dy</div>\n<p>TPE 利用贝叶斯公式进行变换。定义：</p>\n<div class=\"kb-math kb-math-display\">p(x|y) = \\begin{cases} l(x) &amp; \\text{if } y &lt; y^* \\\\ g(x) &amp; \\text{if } y \\geq y^* \\end{cases}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">y^*</span> 是使得 <span class=\"kb-math kb-math-inline\">P(y &lt; y^*) = \\gamma</span> 的分位数阈值。通过贝叶斯公式：</p>\n<div class=\"kb-math kb-math-display\">p(x) = \\gamma \\cdot l(x) + (1-\\gamma) \\cdot g(x)</div>\n<p>将 EI 重写后可以证明：</p>\n<div class=\"kb-math kb-math-display\">\\text{EI}_{y^*}(x) \\propto \\left(\\gamma + (1-\\gamma) \\frac{g(x)}{l(x)}\\right)^{-1}</div>\n<div class=\"key-point\">💡 关键：最大化 EI 等价于最大化 <span class=\"kb-math kb-math-inline\">l(x)/g(x)</span>。直觉上，我们希望找到在\"好的观测\"中概率高、在\"差的观测\"中概率低的超参数配置。</div>\n<h5>密度估计方法</h5>\n<p>对于连续超参数，TPE 使用<strong>自适应带宽的 Parzen 窗（核密度估计）</strong>：</p>\n<div class=\"kb-math kb-math-display\">l(x) = \\frac{1}{|D_{\\text{good}}|} \\sum_{i \\in D_{\\text{good}}} \\mathcal{N}(x; x_i, \\sigma_i^2)</div>\n<p>带宽 <span class=\"kb-math kb-math-inline\">\\sigma_i</span> 设置为相邻观测点间距的最大值，确保密度估计平滑且自适应。对于离散/类别超参数，使用加权的类别分布（带有均匀分布的平滑项）。</p>\n<h5>树结构处理条件空间</h5>\n<p>\"树结构\"是 TPE 的关键特性。在超参数空间中，许多参数是条件性的：</p>\n<pre><code>algorithm_choice: {SVM, RandomForest, MLP}\n├── if SVM:\n│   ├── kernel: {linear, rbf, poly}\n│   │   ├── if rbf: gamma: [1e-5, 1e2]\n│   │   └── if poly: degree: {2, 3, 4, 5}\n│   └── C: [1e-3, 1e3]\n├── if RandomForest:\n│   ├── n_estimators: [10, 1000]\n│   └── max_depth: [2, 50]\n└── if MLP:\n    ├── hidden_size: [32, 512]\n    └── learning_rate: [1e-5, 1e-1]\n</code></pre>\n<p>TPE 按照这种树结构<strong>独立地</strong>对每个超参数进行密度估计，只使用该参数被激活时的观测子集。这使得 TPE 能自然处理任意深度的条件依赖关系，而无需像 GP 那样将条件参数编码为固定维度向量。</p>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>GP-BO</th>\n<th>SMAC</th>\n<th>TPE</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>代理模型</td>\n<td><span class=\"kb-math kb-math-inline\">P(y\\|x)</span> 高斯过程</td>\n<td><span class=\"kb-math kb-math-inline\">P(y\\|x)</span> 随机森林</td>\n<td><span class=\"kb-math kb-math-inline\">P(x\\|y)</span> 密度估计</td>\n</tr>\n<tr>\n<td>计算复杂度</td>\n<td><span class=\"kb-math kb-math-inline\">O(n^3)</span></td>\n<td><span class=\"kb-math kb-math-inline\">O(n \\log n)</span></td>\n<td><span class=\"kb-math kb-math-inline\">O(n)</span></td>\n</tr>\n<tr>\n<td>条件空间</td>\n<td>需特殊编码</td>\n<td>天然支持</td>\n<td>天然支持</td>\n</tr>\n<tr>\n<td>高维表现</td>\n<td>差（&gt;20维）</td>\n<td>好</td>\n<td>好</td>\n</tr>\n<tr>\n<td>采集函数优化</td>\n<td>需数值优化</td>\n<td>需数值优化</td>\n<td>从 <span class=\"kb-math kb-math-inline\">l(x)</span> 直接采样</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"warn-box\">⚠️ 注意：TPE 的一个局限是各维度的密度估计是独立的（轴对齐），不能直接捕获超参数间的交互作用。后续工作（如 Falkner et al. 2018 的 BOHB）通过多保真度策略缓解了这一问题。</div>\n<h5>实际影响</h5>\n<p>TPE 是 <strong>Hyperopt</strong> 库的核心算法，广泛应用于：\n- 深度学习超参数调优（学习率、网络架构等）\n- AutoML 流水线（CASH 问题：同时选择算法和调参）\n- 神经架构搜索的早期工作</p>\n<p>其设计思想也影响了后续的 BOHB（Bayesian Optimization + HyperBand）和 Optuna 中的 TPE 变体。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Figure 1 直觉</strong>：在 2D 空间中，<span class=\"kb-math kb-math-inline\">3 \\times 3</span> 网格仅在每个维度上采样 3 个不同值；而 9 个随机点在每个维度的投影上产生 9 个不同值",
        "<strong>GP 分析</strong>：用高斯过程回归拟合验证集性能函数 <span class=\"kb-math kb-math-inline\">Y</span>，通过各维度长度尺度参数量化有效维度，证实不同数据集的重要超参数各不相同",
      ...
      "detail": "<p><img alt=\"Grid Search vs Random Search\" src=\"https://www.jmlr.org/papers/volume13/bergstra12a/bergstra12a.pdf\" />\n<em>图：论文 Figure 1 —— 左侧为网格搜索（Grid Layout），右侧为随机搜索（Random Layout）。绿色曲线表示目标函数仅依赖于一个维度（低有效维度）。网格的 9 个点在重要维度上仅覆盖 3 个不同值，而随机的 9 个点覆盖 9 个不同值，因此更有可能找到最优区域。</em></p>\n<pre><code class=\"language-python\"># Random Search 超参数优化伪代码\nimport numpy as np\n\ndef random_search(objective_fn, param_distributions, n_trials=32, seed=42):\n    &quot;&quot;&quot;\n    Random Search for Hyperparameter Optimization (Bergstra &amp; Bengio, 2012)\n\n    Args:\n        objective_fn: 评估函数 f(params) -&gt; validation_score\n        param_distributions: dict, 每个超参数的采样分布\n            e.g. {&quot;lr&quot;: (&quot;log_uniform&quot;, 1e-5, 1.0),\n                   &quot;hidden_units&quot;: (&quot;log_uniform_int&quot;, 128, 4000),\n                   &quot;dropout&quot;: (&quot;uniform&quot;, 0.0, 0.5)}\n        n_trials: 随机试验次数 (论文建议 ≥ 32)\n        seed: 随机种子\n    Returns:\n        best_params, best_score\n    &quot;&quot;&quot;\n    rng = np.random.RandomState(seed)\n    best_score = -np.inf\n    best_params = None\n\n    for trial in range(n_trials):\n        # 从各维度独立采样 —— 关键：每个维度独立随机\n        params = {}\n        for name, (dist_type, low, high) in param_distributions.items():\n            if dist_type == &quot;log_uniform&quot;:\n                params[name] = np.exp(rng.uniform(np.log(low), np.log(high)))\n            elif dist_type == &quot;uniform&quot;:\n                params[name] = rng.uniform(low, high)\n            elif dist_type == &quot;log_uniform_int&quot;:\n                params[name] = int(np.exp(rng.uniform(np.log(low), np.log(high))))\n\n        # 评估当前配置（训练模型 + 验证集评估）\n        score = objective_fn(params)\n\n        if score &gt; best_score:\n            best_score = score\n            best_params = params\n\n    return best_params, best_score\n</code></pre>\n<p><strong>动机与背景：网格搜索的根本缺陷</strong></p>\n<p>在深度学习和机器学习实践中，超参数优化长期依赖网格搜索（Grid Search）。网格搜索在每个超参数维度上选取若干离散值，然后评估所有组合。其根本问题在于：当搜索空间为 <span class=\"kb-math kb-math-inline\">d</span> 维、每维取 <span class=\"kb-math kb-math-inline\">n</span> 个值时，总试验数为 <span class=\"kb-math kb-math-inline\">n^d</span>，呈指数增长。更关键的是，如果目标函数 <span class=\"kb-math kb-math-inline\">Y(\\boldsymbol{\\lambda})</span> 实际上只依赖于 <span class=\"kb-math kb-math-inline\">d</span> 维中的 <span class=\"kb-math kb-math-inline\">d_{\\text{eff}} \\ll d</span> 个维度（即具有<strong>低有效维度</strong>），那么网格搜索在不重要维度上的所有变化都是浪费——每个重要维度上实际只有 <span class=\"kb-math kb-math-inline\">n</span> 个不同的采样点，而非 <span class=\"kb-math kb-math-inline\">n^d</span> 个。</p>\n<div class=\"key-point\">💡 关键：对于具有低有效维度的函数，<span class=\"kb-math kb-math-inline\">N</span> 次网格搜索在重要维度上仅提供 <span class=\"kb-math kb-math-inline\">N^{1/d}</span> 个不同值，而 <span class=\"kb-math kb-math-inline\">N</span> 次随机搜索在每个维度上提供 <span class=\"kb-math kb-math-inline\">N</span> 个不同值。</div>\n<p><strong>核心机制：为什么随机搜索更优</strong></p>\n<p>设超参数空间为 <span class=\"kb-math kb-math-inline\">\\Lambda = \\Lambda_1 \\times \\Lambda_2 \\times \\cdots \\times \\Lambda_d</span>，目标函数为验证集损失 <span class=\"kb-math kb-math-inline\">Y: \\Lambda \\to \\mathbb{R}</span>。假设 <span class=\"kb-math kb-math-inline\">Y</span> 仅依赖于前 <span class=\"kb-math kb-math-inline\">d_{\\text{eff}}</span> 个维度（低有效维度假设）：</p>\n<div class=\"kb-math kb-math-display\">Y(\\lambda_1, \\lambda_2, \\ldots, \\lambda_d) \\approx g(\\lambda_1, \\ldots, \\lambda_{d_{\\text{eff}}})</div>\n<p>对于网格搜索，<span class=\"kb-math kb-math-inline\">N = n^d</span> 个试验在每个维度上仅有 <span class=\"kb-math kb-math-inline\">n = N^{1/d}</span> 个不同值。在有效子空间的投影中，只有 <span class=\"kb-math kb-math-inline\">n^{d_{\\text{eff}}} = N^{d_{\\text{eff}}/d}</span> 个不同的函数值被评估。</p>\n<p>而随机搜索的 <span class=\"kb-math kb-math-inline\">N</span> 个试验在有效子空间的投影中（以概率 1）产生 <span class=\"kb-math kb-math-inline\">N</span> 个不同的点。当 <span class=\"kb-math kb-math-inline\">d_{\\text{eff}} / d</span> 较小时，随机搜索的覆盖效率远超网格搜索。</p>\n<div class=\"warn-box\">⚠️ 注意：这一优势不依赖于事先知道哪些维度是重要的——随机搜索在<strong>任意</strong>子空间投影上都保持良好覆盖。</div>\n<p><strong>GP 分析量化有效维度</strong></p>\n<p>论文使用高斯过程（GP）回归拟合函数 <span class=\"kb-math kb-math-inline\">Y</span>，采用各向异性平方指数核（ARD 核）：</p>\n<div class=\"kb-math kb-math-display\">k(\\boldsymbol{\\lambda}, \\boldsymbol{\\lambda}&#x27;) = \\sigma^2 \\exp\\left(-\\sum_{i=1}^{d} \\frac{(\\lambda_i - \\lambda_i&#x27;)^2}{2 \\ell_i^2}\\right)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\ell_i</span> 为第 <span class=\"kb-math kb-math-inline\">i</span> 个超参数的<strong>长度尺度</strong>（length scale）。若 <span class=\"kb-math kb-math-inline\">\\ell_i</span> 很大，说明 <span class=\"kb-math kb-math-inline\">Y</span> 对第 <span class=\"kb-math kb-math-inline\">i</span> 维不敏感（该维度不重要）；若 <span class=\"kb-math kb-math-inline\">\\ell_i</span> 很小，说明 <span class=\"kb-math kb-math-inline\">Y</span> 在该维度变化剧烈（重要维度）。</p>\n<p>实验结果表明：\n- 不同数据集的重要超参数<strong>各不相同</strong>（如 mnist_basic 对学习率敏感，而 mnist_rotated 对网络宽度敏感）\n- 大多数超参数在多数数据集上的长度尺度很大，证实了低有效维度假设\n- 这解释了为什么固定的网格设计无法适应不同问题——预先不知道哪些维度重要</p>\n<p><strong>与传统方法的对比及实践意义</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>试验数</th>\n<th>有效覆盖</th>\n<th>适应性</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Grid Search</td>\n<td><span class=\"kb-math kb-math-inline\">n^d</span></td>\n<td>每维 <span class=\"kb-math kb-math-inline\">n</span> 个值</td>\n<td>需预知重要维度</td>\n</tr>\n<tr>\n<td>Random Search</td>\n<td><span class=\"kb-math kb-math-inline\">N</span></td>\n<td>每维 <span class=\"kb-math kb-math-inline\">N</span> 个值</td>\n<td>自动适应任意有效子空间</td>\n</tr>\n<tr>\n<td>Quasi-MC (Sobol)</td>\n<td><span class=\"kb-math kb-math-inline\">N</span></td>\n<td>略优于随机（低维）</td>\n<td>低维有效，高维退化为随机</td>\n</tr>\n</tbody>\n</table></div>\n<p>论文在 Section 5 中进一步展示，在 DBN 的 32 维超参数空间中（包括每层隐藏单元数、学习率、预训练迭代次数、权重初始化方式等），随机搜索仅用数百次试验就匹配了人工专家经过数周调参的结果。在 7 个基准数据集中的 5 个上，随机搜索达到或超越了 Larochelle et al. (2007) 精心设计的网格搜索+手动调优的最佳结果。</p>\n<p>论文最终指出，随机搜索应作为所有自适应超参数优化算法（如贝叶斯优化、TPE、SMAC 等）的<strong>自然基线</strong>——任何声称优于随机搜索的方法都必须在公平条件下证明其优势。这一观点深刻影响了后续 AutoML 研究的实验设计范式。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "元学习热启动：利用 140 个数据集上的 38 维元特征，通过 <span class=\"kb-math kb-math-inline\">k</span>-NN 选择相似数据集的最优配置来初始化 SMAC",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>两阶段交替训练</strong>：(1) 固定控制器参数 <span class=\"kb-math kb-math-inline\">\\theta</span>，用标准 SGD 在训练集上更新共享权重 <span class=\"kb-math kb-math-inline\">\\omega</span>；(2) 固定 <span class=\"kb-math kb-math-inline\">\\omega</span>，用 REINFORCE 在验证集上更新 <span class=\"kb-math kb-math-inline\">\\theta</span>",
    ...
    },
    {
      ...
      "keyPoints": [
        "提出复合缩放法则：用统一的复合系数 <span class=\"kb-math kb-math-inline\">\\phi</span> 同时缩放深度、宽度和分辨率，约束条件为 <span class=\"kb-math kb-math-inline\">\\alpha \\cdot \\beta^2 \\cdot \\gamma^2 \\approx 2</span>",
        "基线网络 EfficientNet-B0 通过多目标 NAS（优化 ACC(m) × [FLOPS(m)/T]^w）搜索得到，以 MBConv（Mobile Inverted Bottleneck）为核心构建块",
        "缩放系数通过小规模网格搜索确定：<span class=\"kb-math kb-math-inline\">\\alpha=1.2, \\beta=1.1, \\gamma=1.15</span>",
        "系列模型 B1-B7 通过逐步增大 <span class=\"kb-math kb-math-inline\">\\phi</span> 从 B0 缩放得到",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "特征工程：对原始特征迭代应用非线性变换（exp, log, sqrt, <span class=\"kb-math kb-math-inline\">x^2</span>, <span class=\"kb-math kb-math-inline\">x^3</span>, 1/x, |x|）及算术组合（+, -, ×, /），指数级扩展特征空间",
        "特征选择核心：多步 L1 正则化，通过添加噪声特征作为基线过滤无关特征",
        "分块策略（Chunking）：将大规模特征池拆分为多个子集分别进行 Lasso 回归，解决 <span class=\"kb-math kb-math-inline\">p \\gg n</span> 问题",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>超网（Once-for-All Network）</strong>：单一共享权重网络包含 <span class=\"kb-math kb-math-inline\">&gt;10^{19}</span> 个子网络，覆盖深度、宽度、卷积核大小、输入分辨率四个弹性维度",
    ...
    },
    {
      ...
      "keyPoints": [
        "将特征工程形式化为双层优化的程序搜索问题：外层搜索最优变换程序 <span class=\"kb-math kb-math-inline\">T^*</span>，内层训练预测模型评估变换质量",
        "LLM 作为知识引导的进化优化器：利用结构化提示（指令 + 数据集描述 + 评估函数 + 上下文示例）生成特征变换假设",
        "多种群记忆（Island Model）：维护 <span class=\"kb-math kb-math-inline\">m</span> 个独立岛屿缓冲区，按 Boltzmann 分布采样父代程序，防止早熟收敛",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>组合模式搜索（Composition Pattern Search）</strong>：以\"段（Segment）\"为单位搜索算子排列，每段包含 <span class=\"kb-math kb-math-inline\">K</span> 层，搜索最优的算子类型分配",
