/**
 * video_vision-data.js — 由 pipeline/build.py 于 2026-05-20 17:59:38 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>统一的3D卷积核尺寸</strong>：系统实验证明 \\(3 \\times 3 \\times 3\\) 是3D卷积的最优核尺寸，兼顾时间和空间建模能力",
        "<strong>C3D网络架构</strong>：8层卷积 + 5层池化 + 2层全连接（4096维），输入为16帧 \\(112 \\times 112\\) 的视频片段",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>膨胀策略（Inflation）</strong>：将 2D 卷积核 \\(N \\times N\\) 扩展为 3D 卷积核 \\(N \\times N \\times N\\)，使网络能够学习时空特征",
        "<strong>Boring-Video Fixed Point 初始化</strong>：将 2D 预训练权重沿时间维度重复 N 次后除以 N，保证对静态视频的输出与原 2D 网络一致",
        "<strong>时间感受野节奏控制（Receptive Field Pacing）</strong>：前两个 max-pooling 不做时间池化（\\(1 \\times 3 \\times 3\\)），后续使用对称核，平衡时空感受野增长",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>关键超参数</strong>：速度比 \\(\\alpha = 8\\)（Fast 帧率是 Slow 的 8 倍），通道比 \\(\\beta = 1/8\\)（Fast 通道数仅为 Slow 的 1/8）",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>统一 4D 表示</strong>：单次前向推理同时输出 RGB 视频、逐像素点云 \\(P_i\\)、相机内外参 \\(C_i\\)、3D 场景流 \\(F_i^{3d}\\) 和动态前景掩码 \\(M_i\\)，所有几何量统一在首帧规范坐标系下",
        ...
        "<strong>两阶段训练策略</strong>：第一阶段分别训练 DiT（扩散损失）和 DPT heads（多任务损失）；第二阶段端到端联合训练，加入背景深度一致性正则 \\(\\mathcal{L}_{\\text{reg}}^{\\text{depth}}\\) 和前景流平滑正则 \\(\\mathcal{L}_{\\text{reg}}^{\\text{flow}}\\)",
