/**
 * body_vision-data.js — 由 pipeline/build.py 于 2026-05-13 12:20:50 自动生成。
  ...
  "algos": [
    {
      ...
      "keyPoints": [
        "<strong>DNN 直接回归关节坐标</strong>：以整张图像为输入，通过 DNN 直接输出所有关节的归一化 \\((x, y)\\) 坐标，取代传统的图模型 + 手工特征范式",
        "<strong>基于 AlexNet 的骨干网络</strong>：采用 7 层卷积网络（5 卷积 + 2 全连接），以 \\(220 \\times 220\\) 图像作为输入",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>极简姿态估计网络</strong>：ResNet 骨干 + 3 层转置卷积（256 通道、4×4 核、步长 2）+ 1×1 卷积生成 \\(K\\) 通道热力图，无需复杂的多阶段或跳跃连接设计",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Anatomy-aware Loss</strong>：按身体部位（躯干、四肢、手部）分解速度场监督，配合 \\(x_0\\) 重建损失、速度/加速度正则化",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>多种验证度量</strong>：加权 \\(\\chi^2\\) 距离（由线性 SVM 学习权重）和 Siamese 网络端到端度量学习",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>ArcFace 损失</strong>：在 softmax 的目标类角度 \\(\\theta_{y_i}\\) 上直接加一个角度间隔 \\(m\\)，即 \\(\\cos(\\theta_{y_i} + m)\\)，使决策边界在超球面上具有恒定的测地距离惩罚",
        "<strong>归一化机制</strong>：对特征向量和分类权重均做 L2 归一化，将 logit 简化为 \\(s \\cdot \\cos\\theta\\)，其中 \\(s=64\\) 为特征缩放因子",
        "<strong>统一框架</strong>：将 SphereFace（乘性角度间隔 \\(m_1\\)）、ArcFace（加性角度间隔 \\(m_2\\)）、CosFace（加性余弦间隔 \\(m_3\\)）统一为 \\(\\cos(m_1\\theta + m_2) - m_3\\)",
        "<strong>Sub-center ArcFace</strong>：为每个类别引入 \\(K\\) 个子中心，自动将噪声样本隔离到非主导子类中，实现大规模 web 数据的自动清洗",
      ...
      "detail": "<p><img alt=\"ArcFace 框架图\" src=\"https://ar5iv.labs.arxiv.org/html/1801.07698v4/assets/x4.png\" />\n<em>图：ArcFace 训练流程。特征 \\(x_i\\) 和权重 \\(W\\) 均经 L2 归一化后计算角度 \\(\\theta\\)，对目标类角度添加间隔 \\(m\\)，再乘以缩放因子 \\(s\\) 送入 softmax。</em></p>\n<pre><code class=\"language-python\"># ArcFace 核心前向计算伪代码\nimport torch\nimport torch.nn.functional as F\nimport math\n\ndef arcface_forward(features, weights, labels, s=64.0, m=0.5):\n    # Step 1: L2 归一化\n    features = F.normalize(features, dim=1)   # (B, 512)\n    weights = F.normalize(weights, dim=1)     # (N_classes, 512)\n\n    # Step 2: 计算 cos(θ) = 特征与权重的内积\n    cosine = features @ weights.T             # (B, N_classes)\n\n    # Step 3: 对目标类添加角度间隔\n    theta = torch.acos(cosine.clamp(-1+1e-7, 1-1e-7))\n    target_logits = torch.cos(theta[range(len(labels)), labels] + m)\n\n    # Step 4: 替换目标类 logit，缩放后计算交叉熵\n    logits = cosine.clone()\n    logits[range(len(labels)), labels] = target_logits\n    logits *= s\n\n    loss = F.cross_entropy(logits, labels)\n    return loss\n</code></pre>\n<p><strong>动机与背景</strong></p>\n<p>传统 softmax 损失虽然能训练出可分的特征，但缺乏显式的类间间隔约束，导致特征在开集验证场景下判别力不足。度量学习方法（如 triplet loss）虽然直接优化特征距离，但面临组合爆炸的采样困难和训练不稳定问题。SphereFace 首次引入角度间隔的思想，但其乘性间隔 \\(\\cos(m\\theta)\\) 在数学上需要复杂的倍角公式且收敛困难，需要联合 softmax 监督进行退火训练。</p>\n<div class=\"key-point\">💡 关键：ArcFace 的核心洞察是——在角度空间中添加<strong>加性</strong>间隔比乘性间隔更自然，因为加性角度间隔直接对应超球面上的<strong>测地距离</strong>，在整个角度区间内提供恒定的惩罚强度。</div>\n<p><strong>核心机制</strong></p>\n<p>ArcFace 的损失函数从标准 softmax 出发，经过三步演进：</p>\n<p><strong>Step 1 — 归一化 Softmax：</strong> 将权重 \\(W_j\\) 和特征 \\(x_i\\) 均做 L2 归一化，使 \\(W_j^T x_i = \\cos\\theta_j\\)，将分类问题转化为超球面上的角度分类：</p>\n<p>$$L_2 = -\\log \\frac{e^{s \\cdot \\cos\\theta_{y_i}}}{e^{s \\cdot \\cos\\theta_{y_i}} + \\sum_{j \\neq y_i} e^{s \\cdot \\cos\\theta_j}}$$</p>\n<p><strong>Step 2 — 添加角度间隔：</strong> 对目标类角度 \\(\\theta_{y_i}\\) 加上间隔 \\(m\\)：</p>\n<p>$$L_3 = -\\log \\frac{e^{s \\cdot \\cos(\\theta_{y_i} + m)}}{e^{s \\cdot \\cos(\\theta_{y_i} + m)} + \\sum_{j \\neq y_i} e^{s \\cdot \\cos\\theta_j}}$$</p>\n<p>这使得样本不仅需要与正确类中心的角度最小，还需要额外克服 \\(m\\) 的角度惩罚才能被正确分类，从而在训练时强制拉大类间边界。</p>\n<p><strong>Step 3 — 统一框架：</strong> 将三种主流间隔方法统一为：</p>\n<p>$$\\cos(m_1 \\theta_{y_i} + m_2) - m_3$$</p>\n<p>其中 SphereFace 对应 \\((m_1, m_2, m_3) = (1.5, 0, 0)\\)，ArcFace 对应 \\((1, 0.5, 0)\\)，CosFace 对应 \\((1, 0, 0.35)\\)。</p>\n<div class=\"warn-box\">⚠️ 注意：ArcFace 的几何优势在于其决策边界是<strong>线性</strong>的角度间隔（在整个 \\([0, \\pi]\\) 区间内恒定为 \\(m\\)），而 SphereFace 和 CosFace 的角度间隔是非线性的，在不同角度处惩罚强度不同。</div>\n<p><strong>超参数设计</strong></p>\n<ul>\n<li><strong>缩放因子 \\(s = 64\\)</strong>：控制 softmax 的温度。\\(s\\) 过小导致收敛困难，过大导致梯度消失。论文证明当 \\(s \\geq \\frac{N-1}{N} \\cdot \\frac{\\log((N-1) \\cdot P_W)}{1 - \\cos(m)}\\) 时可保证期望分类精度 \\(P_W\\)。</li>\n<li><strong>角度间隔 \\(m = 0.5\\)</strong>（约 28.6°）：在判别力和收敛性之间取得平衡。</li>\n</ul>\n<p><strong>Sub-center ArcFace</strong></p>\n<p>针对大规模 web 数据中不可避免的标签噪声问题，论文提出为每个类别维护 \\(K\\) 个子中心（默认 \\(K=3\\)），样本只需与最近的子中心满足间隔约束：</p>\n<p>$$L_7 = -\\log \\frac{e^{s \\cdot \\cos(\\theta_{\\min} + m)}}{e^{s \\cdot \\cos(\\theta_{\\min} + m)} + \\sum_{j \\neq y_i} e^{s \\cdot \\cos\\theta_j}}$$</p>\n<p>其中 \\(\\theta_{\\min} = \\min_{k=1}^K \\theta_k\\)。训练完成后，主导子中心（包含多数干净样本）可被识别，与主导子中心角度超过 75° 的样本被判定为噪声并移除。这一机制无需额外标注即可实现自动数据清洗。</p>\n<p><strong>与传统方法的对比</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>间隔类型</th>\n<th>决策边界</th>\n<th>收敛性</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>SphereFace</td>\n<td>乘性角度 \\(\\cos(m\\theta)\\)</td>\n<td>非线性</td>\n<td>需退火策略</td>\n</tr>\n<tr>\n<td>CosFace</td>\n<td>加性余弦 \\(\\cos\\theta - m\\)</td>\n<td>非线性角度间隔</td>\n<td>稳定</td>\n</tr>\n<tr>\n<td><strong>ArcFace</strong></td>\n<td><strong>加性角度</strong> \\(\\cos(\\theta + m)\\)</td>\n<td><strong>恒定线性角度间隔</strong></td>\n<td><strong>稳定</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>ArcFace 在 LFW 上达到 99.83%，在 MegaFace 上（refined, large protocol）达到 98.98% Rank-1 识别率和 99.08% 验证率（TPR@FPR=1e-6），在 IJB-C 上（TPR@FPR=1e-4）达到 96.03%，全面超越同期方法。</p>",
      "quiz": {
