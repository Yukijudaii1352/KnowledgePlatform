/**
 * device-data.js — 由 pipeline/build.py 于 2026-06-15 18:08:24 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "detail": "<h3>TPU v4: 基于光路交换的可重构ML超级计算机</h3>\n<pre><code class=\"language-yaml\">id: tpu_v4\nname: &quot;TPU v4&quot;\nyear: 2023\norganization: Google\ncategory: tpu\nparent: tpu_v2v3\npaper: &quot;TPU v4: An Optically Reconfigurable Supercomputer for Machine Learning with Hardware Support for Embeddings&quot;\narxiv: &quot;2304.01433&quot;\nvenue: ISCA 2023\nauthors: &quot;Norm Jouppi, George Kurian, Sheng Li, Peter Ma, et al.&quot;\n</code></pre>\n<hr />\n<h2>一句话总结</h2>\n<p>TPU v4 通过<strong>光路交换机（OCS）实现 4096 芯片的 3D Torus 拓扑动态重构</strong>，并引入 <strong>SparseCore 硬件加速嵌入训练</strong>，在同等规模下比 A100 快 1.2–1.7×、能效高 1.3–1.9×。</p>\n<hr />\n<h2>核心要点</h2>\n<ol>\n<li>\n<p><strong>OCS 可重构互连</strong>：64 个 4×4×4 电气立方体通过 48 个 136 端口 MEMS 光路交换机组成 4096 芯片超算，支持按作业动态切换 3D Torus 拓扑（含 twisted torus），OCS 成本 &lt;5% 总资本、&lt;3% 总功耗。</p>\n</li>\n<li>\n<p><strong>SparseCore 嵌入加速</strong>：每芯片 4 个 SparseCore（仅占 ~5% 面积/功耗），含 16 个 compute tile + 5 个跨通道单元，为推荐模型嵌入训练提供专用 scatter/gather 和 all-to-all 通信支持，比 CPU 方案快 30×。</p>\n</li>\n<li>\n<p><strong>芯片架构升级</strong>：7nm 工艺、2 个 TensorCore（各含 4 个 128×128 MXU）、新增 128 MiB CMEM 片上缓存、275 TFLOPS (bf16)、32 GiB HBM2 @ 1200 GB/s，峰值性能 2.2× TPU v3。</p>\n</li>\n<li>\n<p><strong>Twisted Torus 拓扑</strong>：对 n×n×2n 形状切片，通过 OCS 重编程实现扭曲环面，all-to-all 吞吐提升 1.31–1.63×，无需物理重新布线。</p>\n</li>\n<li>\n<p><strong>PA-NAS 协同优化</strong>：平台感知神经架构搜索可同时优化模型结构和 TPU 拓扑，DLRM 端到端性能提升 &gt;10%，LLM 拓扑搜索提升可达 2.3×。</p>\n</li>\n<li>\n<p><strong>生产性能</strong>：同等切片大小下 TPU v4 比 v3 快 1.5–3.5×；MLPerf 中比 A100 快 1.15×(BERT)–1.67×(ResNet)，功耗低 30–48%；运营碳排放比本地 DSA 低 ~18×。</p>\n</li>\n<li>\n<p><strong>可用性与调度</strong>：OCS 路由绕过故障节点，主机可用性 99.0% 时仍有合理 goodput；每个 4³ 块独立部署，调度器可从任意位置拼接切片，支持非 2 的幂切片（如 4×4×12=192）。</p>\n</li>\n</ol>\n<hr />\n<h2>深入细节</h2>\n<h3>1. 系统架构总览</h3>\n<pre><code>┌─────────────────────────────────────────────────────────┐\n│                TPU v4 Supercomputer (4096 chips)         │\n│                                                         │\n│  ┌─────────┐  ┌─────────┐       ┌─────────┐            │\n│  │ 4³ Block │  │ 4³ Block │ ...  │ 4³ Block │  × 64     │\n│  │ (64 chip)│  │ (64 chip)│       │ (64 chip)│           │\n│  │ 1 rack   │  │ 1 rack   │       │ 1 rack   │           │\n│  └────┬─────┘  └────┬─────┘       └────┬─────┘           │\n│       │              │                  │                 │\n│       └──────────────┼──────────────────┘                 │\n│                      ▼                                    │\n│         ┌──────────────────────┐                          │\n│         │   48 × Palomar OCS   │  136×136 MEMS mirrors   │\n│         │   (光路交换机层)       │  毫秒级切换             │\n│         └──────────────────────┘                          │\n└─────────────────────────────────────────────────────────┘\n</code></pre>\n<p><strong>关键设计决策</strong>：\n- <strong>为什么选 4³ = 64 芯片作为电气构建块？</strong> 3D Torus 中立方体有最佳等分带宽。4 个 TPU v4 共享一个 CPU 主机，64 芯片 + 16 主机恰好装入一个机架。\n- <strong>为什么用 OCS 而非 InfiniBand？</strong> OCS 是无源光反射（MEMS 镜面），支持任意带宽（波分复用可达 Tbit/s 级），功耗极低；IB 交换机需要有源包处理，568 台 IB 交换机成本远高于 48 台 OCS，且 all-reduce 慢 1.8–2.4×。\n- <strong>每个 4³ 块的 96 条光纤</strong>：6 个面 × 16 条/面 = 96 条光纤，对面连同一 OCS，故 96/2 = 48 个 OCS。</p>\n<h3>2. OCS 拓扑重构机制</h3>\n<pre><code>常规 Torus (4×2 示例):          Twisted Torus:\n  ┌──0,0──0,1──0,2──0,3──┐      ┌──0,0──0,1──0,2──0,3──┐\n  │   │    │    │    │    │      │   │    │    │    │    │\n  └──1,0──1,1──1,2──1,3──┘      └──1,2──1,3──1,0──1,1──┘\n       电气连接(固定)                    ↑ OCS 重编程光连接\n       光连接(可重构)                    (坐标偏移, 降低最坏延迟)\n</code></pre>\n<p><strong>Twisted Torus 原理</strong>：对 k×k×2k 形状切片，通过 OCS 重新编程光连接的路由表（无需物理重新布线），将环绕链路的目标偏移，使得最坏情况下的跳数减少。实测 all-to-all 吞吐：\n- 4×4×8 切片：twisted 比 regular 快 <strong>1.63×</strong>\n- 4×8×8 切片：twisted 比 regular 快 <strong>1.31×</strong></p>\n<p><strong>生产使用统计</strong>（2022 年 11 月采样）：\n- 29% 作业 &lt; 64 芯片（仅 2D mesh）\n- 71% ≥ 64 芯片中，48% 可 twist（n×n×2n 或 n×2n×2n）\n- 可 twist 中 86% 实际选择了 twisted torus\n- 最受欢迎切片：4×4×4 (14%)、4×4×8_T (16%)、4×8×8_T (9.2%)、8×8×8 (9.6%)</p>\n<h3>3. SparseCore 架构详解</h3>\n<pre><code>┌─────────────────── SparseCore ───────────────────────┐\n│                                                       │\n│  ┌─────────────────────────────────────────────────┐  │\n│  │          5 个 Cross-Channel Units (金色)          │  │\n│  │  ┌──────────┬──────────┬──────────┬──────────┐  │  │\n│  │  │Dedup Unit│Distribute│ Sort/    │Reduce    │  │  │\n│  │  │(去重)    │Unit(分发)│ Merge    │Unit(规约)│  │  │\n│  │  └──────────┴──────────┴──────────┴──────────┘  │  │\n│  └─────────────────────────────────────────────────┘  │\n│                         ↕                             │\n│  ┌──────── 2.5 MiB Sparse Vector Memory (Spmem) ──┐  │\n│  │  Bank 0 │ Bank 1 │ ... │ Bank 15               │  │\n│  └─────────┴────────┴─────┴───────────────────────┘  │\n│       ↕          ↕              ↕                     │\n│  ┌─ Tile 0 ─┐ ┌─ Tile 1 ─┐  ┌─ Tile 15 ─┐          │\n│  │ Fetch Unit│ │ Fetch Unit│  │ Fetch Unit │          │\n│  │ scVPU(8w) │ │ scVPU(8w) │  │ scVPU(8w)  │          │\n│  │ Flush Unit│ │ Flush Unit│  │ Flush Unit │          │\n│  │    ↕      │ │    ↕      │  │    ↕       │          │\n│  │ HBM Ch.0 │ │ HBM Ch.1 │  │ HBM Ch.15  │          │\n│  └──────────┘ └──────────┘  └───────────┘           │\n└───────────────────────────────────────────────────────┘\n</code></pre>\n<p><strong>嵌入训练流水线</strong>：</p>\n<pre><code>1. Fetch: 从 HBM 读取嵌入向量 → Spmem\n2. Dedup: 对高频特征值去重（减少冗余内存访问和通信量）\n3. Distribute: 跨芯片 all-to-all 交换嵌入向量\n4. scVPU: 8-wide SIMD 计算（前向/反向传播中的向量运算）\n5. Sort/Reduce: 梯度聚合\n6. Flush: 更新后的参数写回 HBM\n</code></pre>\n<p><strong>为什么不用 TensorCore 做嵌入？</strong>\n- 嵌入是小粒度 scatter/gather 访存，算术强度极低\n- TensorCore 的宽 MXU 和 VPU 针对稠密运算优化，处理稀疏访存效率低\n- 嵌入放 CPU 内存则受限于 Amdahl 定律（4:1 TPU/CPU 比率放大瓶颈）</p>\n<p><strong>性能关键</strong>：3D Torus 等分带宽 ∝ N^(2/3)（vs 2D 的 N^(1/2)），在 256 芯片时 TPU v4 等分带宽比 v3 高 4×，嵌入性能提升 1.1–2.0×。</p>\n<h3>4. 芯片微架构</h3>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>TPU v4</th>\n<th>TPU v3</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>工艺</td>\n<td>7nm, &lt;600mm²</td>\n<td>16nm, &lt;700mm²</td>\n</tr>\n<tr>\n<td>晶体管</td>\n<td>220 亿</td>\n<td>100 亿</td>\n</tr>\n<tr>\n<td>峰值算力</td>\n<td>275 TFLOPS (bf16/int8)</td>\n<td>123 TFLOPS (bf16)</td>\n</tr>\n<tr>\n<td>时钟</td>\n<td>1050 MHz</td>\n<td>940 MHz</td>\n</tr>\n<tr>\n<td>MXU</td>\n<td>2×4 = 8 个 128×128</td>\n<td>2×2 = 4 个 128×128</td>\n</tr>\n<tr>\n<td>片上存储</td>\n<td>128 MiB CMEM + 32 MiB VMEM + 10 MiB spMEM</td>\n<td>32 MiB VMEM + 5 MiB spMEM</td>\n</tr>\n<tr>\n<td>HBM</td>\n<td>32 GiB, 1200 GB/s</td>\n<td>32 GiB, 900 GB/s</td>\n</tr>\n<tr>\n<td>ICI</td>\n<td>6 links × 50 GB/s = 300 GB/s</td>\n<td>4 links × 70 GB/s = 280 GB/s</td>\n</tr>\n<tr>\n<td>SparseCore</td>\n<td>4 个/芯片</td>\n<td>2 个/芯片</td>\n</tr>\n<tr>\n<td>功耗(实测)</td>\n<td>121/170/192 W (min/mean/max)</td>\n<td>175/220/262 W</td>\n</tr>\n<tr>\n<td>最大规模</td>\n<td>4096 芯片</td>\n<td>1024 芯片</td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>CMEM 的价值</strong>：128 MiB 共享片上缓存是 v4 新增的关键特性。关闭 CMEM 后性能下降 1.18×，RNN1（小权重小 batch）下降 2×。CMEM 带宽远高于 HBM，减少了对外部存储的依赖。</p>\n<h3>5. 性能对比与 Roofline 分析</h3>\n<p><strong>TPU v4 vs TPU v3（同等切片大小）</strong>：\n- 通用负载：1.5–2.0×\n- DLRM0：3.0–3.5×（SC 数量翻倍 + 更快时钟）\n- RNN1：3.3×（受益于 CMEM 带宽）\n- 整体：2.1× 性能，2.7× 性能/瓦特（~40% 来自工艺，~60% 来自设计）</p>\n<p><strong>TPU v4 vs A100（MLPerf Training 2.0）</strong>：\n- BERT：TPU v4 快 1.15×（4096 芯片）\n- ResNet：TPU v4 快 1.67×（4096 vs 4216 芯片）\n- 功耗：A100 高 1.3–1.9×（BERT: 380W vs 197W, ResNet: 273W vs 206W）</p>\n<p><strong>为什么 A100 峰值 FLOPS 更高但实际更慢？</strong>\n- A100 boost 频率 1410 MHz 实测被功耗限制到 ~1280 MHz\n- TPU v4 的 128×128 MXU 复用率（128×）远高于 A100 的 4×4 阵列（4×）\n- TPU v4 片上 SRAM 4× 大（160 vs 40 MiB），减少 DRAM 访问\n- A100 多线程需要 27 MiB 寄存器堆（vs TPU v4 的 0.25 MiB），增加能耗</p>\n<h3>6. PA-NAS 拓扑协同优化</h3>\n<pre><code class=\"language-python\"># PA-NAS 搜索空间伪代码\ndef pa_nas_search(model, chip_count):\n    best_config = None\n    for topology in generate_topologies(chip_count):\n        # e.g., 8×8×8, 4×8×16, 4×4×32 for 512 chips\n        for partition in generate_partitions(topology):\n            # [pipeline_depth, data_parallel, model_dim1, model_dim2]\n            for activation_spec in [&quot;1D&quot;, &quot;2D&quot;]:\n                for weight_spec in [&quot;1D&quot;, &quot;2D&quot;]:\n                    perf = simulate(model, topology, partition,\n                                   activation_spec, weight_spec)\n                    quality = evaluate_quality(model)\n                    if pareto_optimal(perf, quality, best_config):\n                        best_config = (topology, partition, ...)\n    return best_config\n\n# 实际案例: 512-chip LLM\n# Novice: 4×8×16, [1,1,16,32], 2D/2D → 17.9 seq/s\n# PA-NAS: 8×8×8, [1,1,64,8],  1D/2D → 41.3 seq/s (2.3×)\n</code></pre>\n<p><strong>DLRM 优化</strong>：原始 DLRM0 中 SC 空闲 ~25%（SC-TC 负载不均衡），PA-NAS 通过调整稀疏层和稠密层的计算分配，实现近乎完美的 SC-TC 负载平衡，端到端性能提升 &gt;10%。</p>\n<hr />\n<h2>练习题</h2>\n<h3>Q1: OCS 构建块大小选择</h3>\n<p><strong>问题</strong>：为什么 TPU v4 选择 4×4×4 = 64 芯片作为电气构建块，而非 8×8×8 = 512？如果选择 2×2×2 = 8 芯片会有什么问题？</p>\n<p><strong>参考答案</strong>：\n- 4³ 选择原因：(1) 3D 立方体有最佳等分带宽；(2) 4 TPU/主机 × 16 主机 = 64 芯片恰好装入一个机架；(3) 512 需要多机架，增加电气布线复杂度。\n- 2³ 的问题：(1) 每块仅 8 芯片，需要 512 个块和更多 OCS 端口来组成 4096 芯片系统；(2) 光纤数量爆炸（每块 6 面 × 4 链路 = 24 光纤，512 块共 12288 条）；(3) 更多光电转换增加延迟和成本。</p>\n<h3>Q2: 等分带宽与拓扑维度</h3>\n<p><strong>问题</strong>：解释为什么 3D Torus 的等分带宽 ∝ N^(2/3) 而 2D Torus ∝ N^(1/2)，并计算 1024 芯片时两者的比值。</p>\n<p><strong>参考答案</strong>：\n- 对于 d 维 Torus（每维 k 个节点，N = k^d），等分切面穿过 d-1 维的 k^(d-1) 条链路，故等分带宽 ∝ k^(d-1) = N^((d-1)/d)。\n- 2D: N^(1/2)；3D: N^(2/3)。\n- 1024 芯片：2D = 1024^(1/2) = 32；3D = 1024^(2/3) ≈ 101.6。比值 ≈ 3.17×，与论文 Figure 8 中 2–4× 的范围一致。</p>\n<h3>Q3: SparseCore vs TensorCore 设计权衡</h3>\n<p><strong>问题</strong>：SparseCore 仅占 ~5% 芯片面积和功耗，却使 DLRM 性能提升 5–7×（相比嵌入放 CPU 内存）。分析这种\"小投入大回报\"的根本原因。</p>\n<p><strong>参考答案</strong>：\n- <strong>Amdahl 定律放大效应</strong>：4:1 TPU/CPU 比率意味着 CPU 成为严重瓶颈，即使 CPU 处理嵌入只占模型一小部分，也会限制整体吞吐。\n- <strong>访存模式匹配</strong>：嵌入是稀疏 scatter/gather，SC 的 16 个 tile 各有独立 HBM 通道，天然支持高并发随机访问；TensorCore 的宽 SIMD 对此效率低下。\n- <strong>通信优化</strong>：SC 直连 ICI 网络，all-to-all 嵌入交换无需经过 CPU 或 PCIe 瓶颈。\n- <strong>去重硬件</strong>：专用 Dedup Unit 减少冗余内存访问和网络通信，这在软件实现中开销大。</p>\n<h3>Q4: Twisted Torus 的适用条件</h3>\n<p><strong>问题</strong>：为什么只有 n×n×2n 或 n×2n×2n 形状的切片才能使用 twisted torus？对于 8×8×8 的完美立方体切片，twisted torus 是否有意义？</p>\n<p><strong>参考答案</strong>：\n- Twisted torus 通过偏移环绕链路来减少最坏跳数，要求至少一个维度是另一个的 2 倍（k×k×2k），这样偏移后坐标映射才能保持一致性且降低直径。\n- 对于 8×8×8 完美立方体，所有维度对称，已经是最优的对称 torus（最小延迟、最大等分带宽），twisting 不会带来改善。论文 Table 2 中 8×8×8 (512 chips, 9.6%) 确实列在 \"Regular Tori\" 而非 \"Twisted\" 类别。</p>\n<h3>Q5: OCS 可用性分析</h3>\n<p><strong>问题</strong>：假设 CPU 主机可用性为 99.5%，4096 芯片系统中平均有多少主机故障？在无 OCS 的静态连接中，为什么 2048 芯片切片的 goodput 仅约 50%？</p>\n<p><strong>参考答案</strong>：\n- 1024 主机 × 0.5% 故障率 = ~5 台主机故障（~20 芯片不可用）。\n- 无 OCS 时，切片必须是物理连续的。2048 芯片切片占总 4096 的 50%，需要一大块连续区域。故障主机\"打断\"连续区域，使得最多只能调度一个 2048 切片，剩余 2048 芯片作为备用（无法利用），goodput = 50%。\n- 有 OCS 时，可以从任意位置的 4³ 块拼接切片，绕过故障块，goodput 显著提升。</p>"
    },
    {
      "id": "tpu_v7",
      "num": 11,
    ...
    },
    {
      ...
      "detail": "<h3>1. 问题背景：为什么低比特 KV Cache 用不上 Tensor Core？</h3>\n<p>在 LLM 解码阶段，注意力计算的核心是 <span class=\"kb-math kb-math-inline\">S = Q \\cdot K^T</span> 和 <span class=\"kb-math kb-math-inline\">O = P \\cdot V</span>，其中 <span class=\"kb-math kb-math-inline\">Q</span> 是 FP16 而 <span class=\"kb-math kb-math-inline\">K, V</span> 被量化为低比特。这构成了<strong>混合精度矩阵乘法（mpGEMM）</strong>问题。</p>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2503.18773v3/assets/x1.png\" alt=\"Weight Quantization vs KV Cache Quantization\" loading=\"lazy\"><p class=\"img-caption\">▲ Weight Quantization vs KV Cache Quantization</p></div>\n<p><strong>权重量化 vs KV Cache 量化的关键区别</strong>：权重可以离线预处理（pre-packing）为 Tensor Core 友好的布局，但 KV Cache 在自回归解码中<strong>逐 token 动态生成</strong>，无法预处理。</p>\n<p><strong>Tensor Core 布局不兼容问题</strong>：</p>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2503.18773v3/assets/x3.png\" alt=\"Packing Layout Mismatch\" loading=\"lazy\"><p class=\"img-caption\">▲ Packing Layout Mismatch</p></div>\n<p>Tensor Core 的 <code>ldmatrix</code> 指令要求数据按特定的交错模式排列（如上图右侧）。当多个低比特值被 pack 到一个寄存器中时，pack 后的数据布局与 <code>ldmatrix</code> 期望的布局产生<strong>不可调和的错位</strong>。现有方案（如 QServe、Atom）因此只能退回到 CUDA Core 的逐元素 FMA 指令，浪费了 Tensor Core 的巨大算力。</p>\n<h3>2. BitFusion 方案：残差 KV Cache + Tensor Core 对齐</h3>\n<p>BitDecoding 的核心创新是 <strong>BitFusion 方案</strong>，将 KV Cache 拆分为两部分：</p>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2503.18773v3/assets/x5.png\" alt=\"BitFusion System Overview\" loading=\"lazy\"><p class=\"img-caption\">▲ BitFusion System Overview</p></div>\n<p><strong>系统架构</strong>由三个 GPU Kernel 组成：</p>\n<ol>\n<li><strong>Residual Kernel（FP16 精度）</strong>：处理残差 KV Cache（未量化的尾部 token），同时执行量化 + packing 融合操作</li>\n<li><strong>Packing Kernel（低比特）</strong>：处理已量化的 KV Cache 主体，利用 Tensor Core 进行高效计算</li>\n<li><strong>Combined Kernel</strong>：合并两个 kernel 的结果（类似 FlashDecoding 的 reduce）</li>\n</ol>\n<p><strong>Tensor Cores-Centric 设计的关键洞察</strong>：</p>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2503.18773v3/assets/x6.png\" alt=\"Tensor Cores-Centric Design\" loading=\"lazy\"><p class=\"img-caption\">▲ Tensor Cores-Centric Design</p></div>\n<p>Residual Kernel 在量化打包时，数据天然存储在寄存器中，其布局<strong>已经符合 <code>ldmatrix</code> 的交错格式</strong>。因此：\n- 在 Residual Kernel 中，量化后的 packed 数据直接按 Tensor Core 布局写入全局内存\n- 在 Packing Kernel 中，用 <code>ldmatrix</code> 加载后直接反量化，保持寄存器对齐</p>\n<p><strong>残差块大小 <span class=\"kb-math kb-math-inline\">N_r</span></strong> 由 Tensor Core 的 warp tiling 决定：</p>\n<div class=\"kb-math kb-math-display\">N_r = 8 \\times W_n \\times \\frac{16}{\\text{num\\_bits}}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">W_n</span> 是沿 N 维的 warp 数，<code>num_bits</code> 是量化位宽。例如 INT8 + <span class=\"kb-math kb-math-inline\">W_n=1</span> 时 <span class=\"kb-math kb-math-inline\">N_r = 16</span>。</p>\n<h3>3. 多 Warp 并行反量化</h3>\n<p>传统单 warp 设计中，反量化操作在 CUDA Core 上执行，成为 Tensor Core MMA 指令的瓶颈：</p>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2503.18773v3/assets/x4.png\" alt=\"Single Warp vs Multi-Warp\" loading=\"lazy\"><p class=\"img-caption\">▲ Single Warp vs Multi-Warp</p></div>\n<p>BitDecoding 采用<strong>多 warp 沿 N 维并行</strong>的策略：</p>\n<ul>\n<li>将 <span class=\"kb-math kb-math-inline\">T_n</span>（KV 序列长度 tile）分配给 <span class=\"kb-math kb-math-inline\">W_n</span> 个 warp 并行处理</li>\n<li>每个 warp 独立执行 <code>ldmatrix</code> → 反量化 → MMA</li>\n<li>迭代次数从 <span class=\"kb-math kb-math-inline\">T_n / 8</span> 降低为 <span class=\"kb-math kb-math-inline\">T_n / (W_n \\times 8)</span></li>\n</ul>\n<p><strong>多 Warp 协作 Softmax</strong>：由于注意力分数 <span class=\"kb-math kb-math-inline\">S = QK^T</span> 分散在多个 warp 的寄存器中，需要跨 warp 的 rowmax 归约。BitDecoding 引入：\n- <code>sTMP</code>：大小为 <span class=\"kb-math kb-math-inline\">W_n</span> 的共享内存缓冲区，用于跨 warp 归约\n- <code>sAcc</code>：大小为 <span class=\"kb-math kb-math-inline\">T_m \\times T_n</span> 的共享内存缓冲区，暂存 softmax 结果 <span class=\"kb-math kb-math-inline\">P</span>，再通过 <code>ldmatrix</code> 重新加载以对齐 Tensor Core 布局</p>\n<pre><code>Algorithm: Multi-warps Cooperative Softmax\n─────────────────────────────────────────\nInput: Q_i ∈ R^{T_m×d}, K_j/V_j ∈ R^{T_n×d} in REG\nShared: sTMP ∈ R^{W_n}, sAcc ∈ R^{T_m×T_n}\n\n1. S_i = Q_i · K_j^T                    // MMA on Tensor Cores\n2. m_new = max(m_i, rowmax(S_i, sTMP))   // Cross-warp reduction via SMEM\n3. P_i = exp(S_i - m_new)                // Element-wise on CUDA Cores\n4. sAcc = tiled_copy_r2s(P_i)            // Write P to shared memory\n5. P'_i = tiled_copy_s2r(sAcc)           // Reload with ldmatrix (TC-aligned)\n6. O_new = P'_i · V_j + diag(e^{m_i - m_new}) · O_i  // MMA + rescale\n</code></pre>\n<blockquote>\n<p>在 Hopper 架构上，WGMMA 可直接从共享内存读取，省去显式 <code>ldmatrix</code> 步骤。</p>\n</blockquote>\n<h3>4. 细粒度异步流水线</h3>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2503.18773v3/assets/x9.png\" alt=\"Software Pipeline\" loading=\"lazy\"><p class=\"img-caption\">▲ Software Pipeline</p></div>\n<p>低比特解码引入了额外的量化参数（scale/zeros）加载和反量化开销。BitDecoding 设计了三级异步流水线：</p>\n<p><strong>Global → Shared Memory</strong>：\n- 使用 <code>cp.async.cg</code> 加载 <span class=\"kb-math kb-math-inline\">Q</span>、<span class=\"kb-math kb-math-inline\">K_{\\text{pack}}</span>、<span class=\"kb-math kb-math-inline\">V_{\\text{pack}}</span>（仅缓存在全局内存）\n- 使用 <code>cp.async.ca</code> 加载量化参数 <span class=\"kb-math kb-math-inline\">K_p</span>、<span class=\"kb-math kb-math-inline\">V_p</span>（更细粒度的字节对齐）\n- Hopper 架构使用 <code>tma.copy</code> 指令</p>\n<p><strong>Shared Memory → Register</strong>：\n- 使用 <code>ldmatrix</code> 加载 packed 数据，保持 Tensor Core tiling 布局\n- 使用 XOR swizzle 消除 bank conflict：<span class=\"kb-math kb-math-inline\">\\text{col}_{id} = \\text{row}_{id} \\oplus \\text{col}_{id}</span></p>\n<p><strong>计算流水线</strong>：\n- 第 <span class=\"kb-math kb-math-inline\">i</span> 个 slice 在 Tensor Core 上执行 MMA 的同时\n- 第 <span class=\"kb-math kb-math-inline\">i+1</span> 个 slice 在 CUDA Core 上执行 <code>ldmatrix</code> + 反量化\n- 实现 Tensor Core 和 CUDA Core 的<strong>完全重叠</strong></p>\n<h3>5. 推理执行流程</h3>\n<pre><code>Algorithm: BitDecoding Inference Execution Flow\n─────────────────────────────────────────────────\nPrefill Phase:\n  1. 正常计算 attention\n  2. 初始化 kv_cache_pack = [], kv_cache_residual = []\n  3. 将前 L - (L % N_r) 个 token 量化打包 → kv_cache_pack\n  4. 剩余 L % N_r 个 token → kv_cache_residual\n\nDecoding Phase (每个新 token):\n  1. 将新 token 的 KV 追加到 kv_cache_residual\n  2. If len(residual) == N_r:\n       Residual Kernel: 计算 attention + 量化打包 → kv_cache_pack\n       清空 residual\n  3. Packing Kernel: 对 kv_cache_pack 计算低比特 attention\n  4. Combined Kernel: 合并两个 kernel 的输出\n</code></pre>\n<h3>6. 反量化实现：Lop3 位操作</h3>\n<p>BitDecoding 使用 PTX 级别的 <code>lop3</code> 指令实现高效反量化。对于 INT4 → FP16 的转换：</p>\n<div class=\"kb-math kb-math-display\">\\text{FP16}(x) = \\text{lop3}(x_{\\text{packed}}, \\text{mask}, \\text{bias})</div>\n<p><code>lop3</code> 是一个三输入逻辑运算指令，可以在单条指令中完成位提取 + 类型转换，避免了传统的移位-掩码-转换多步操作。</p>\n<h3>7. 实验结果</h3>\n<p><strong>Kernel 级别性能</strong>（Packing Kernel，seq_len=128K，head_dim=128）：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>GPU</th>\n<th>对比 FP16 FlashDecoding</th>\n<th>对比 QServe</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>RTX 4090</td>\n<td><strong>7.5×</strong></td>\n<td><strong>4.3×</strong></td>\n</tr>\n<tr>\n<td>A100</td>\n<td><strong>4.8×</strong></td>\n<td><strong>2.1×</strong></td>\n</tr>\n<tr>\n<td>H100</td>\n<td><strong>8.9×</strong> (vs FD-v3)</td>\n<td>—</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"img-wrap\"><img src=\"https://ar5iv.labs.arxiv.org/html/2503.18773v3/assets/x10.png\" alt=\"Kernel Performance on RTX4090\" loading=\"lazy\"><p class=\"img-caption\">▲ Kernel Performance on RTX4090</p></div>\n<p><strong>端到端推理</strong>（LLaMA-3.1-8B，128K 序列长度）：\n- 单 batch 解码延迟降低 <strong>3×</strong>\n- 服务吞吐量提升 <strong>4×</strong></p>\n<p><strong>Breakdown 分析</strong>（各优化贡献，RTX4090 INT4，seq=128K）：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>优化组合</th>\n<th>延迟 (ms)</th>\n<th>加速比</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>无优化 baseline</td>\n<td>2.13</td>\n<td>1.0×</td>\n</tr>\n<tr>\n<td>+ Lop3 TC mapping</td>\n<td>1.63</td>\n<td>1.3×</td>\n</tr>\n<tr>\n<td>+ Warp-efficient</td>\n<td>1.09</td>\n<td>2.0×</td>\n</tr>\n<tr>\n<td>+ Async pipeline</td>\n<td><strong>0.61</strong></td>\n<td><strong>3.5×</strong></td>\n</tr>\n</tbody>\n</table></div>\n<blockquote>\n<p>Warp 并行贡献最大（&gt;2×），Lop3 + 异步流水线各贡献约 1.2×–1.5×。</p>\n</blockquote>\n<hr />"
    },
    {
      "id": "nvidia_ising",
      "num": 57,
