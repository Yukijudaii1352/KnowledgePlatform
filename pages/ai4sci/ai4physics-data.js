/**
 * ai4physics-data.js — 由 pipeline/build.py 于 2026-05-20 16:45:34 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>核心思想</strong>：不直接拟合 \\(\\dot{\\mathbf{q}}, \\dot{\\mathbf{p}}\\)，而是让 NN 输出标量哈密顿量 \\(H_\\theta\\)，再通过辛梯度 \\((\\partial H/\\partial \\mathbf{p},\\; -\\partial H/\\partial \\mathbf{q})\\) 得到动力学，结构性地保证能量守恒",
        ...
        "<strong>像素扩展</strong>：Autoencoder + HNN 联合训练，辅助损失（Eq 7）使潜空间的后半部分 \\(\\mathbf{z_p}\\) 近似 \\(\\mathbf{z_q}\\) 的时间导数，从而满足正则坐标条件",
      ...
      "detail": "<h5>核心架构示意图</h5>\n<p><img alt=\"HNN 核心思想对比图\" src=\"https://ar5iv.labs.arxiv.org/html/1906.01563/assets/x1.png\" />\n<em>图 1：左侧为基线方法——直接用 NN 拟合状态导数 \\((\\dot{q}, \\dot{p})\\)；右侧为 HNN——NN 输出标量 \\(H_\\theta\\)，再通过辛梯度（自动微分）得到动力学。HNN 的相空间轨迹保持在等能量面上（右下角），而基线轨迹逐渐偏离（左下角）。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># HNN 训练与推理伪代码\nimport torch\nimport torch.autograd as autograd\n\n# === 模型定义 ===\nclass HNN(torch.nn.Module):\n    def __init__(self, input_dim, hidden_dim=200):\n        super().__init__()\n        self.net = torch.nn.Sequential(\n            torch.nn.Linear(input_dim, hidden_dim),  # (q,p) → hidden\n            torch.nn.Tanh(),\n            torch.nn.Linear(hidden_dim, hidden_dim),\n            torch.nn.Tanh(),\n            torch.nn.Linear(hidden_dim, 1)            # → 标量 H\n        )\n\n    def forward(self, q, p):\n        x = torch.cat([q, p], dim=-1)\n        return self.net(x)  # 输出标量哈密顿量\n\n    def time_derivative(self, q, p):\n        &quot;&quot;&quot;通过辛梯度计算 dq/dt, dp/dt&quot;&quot;&quot;\n        q.requires_grad_(True)\n        p.requires_grad_(True)\n        H = self.forward(q, p)\n        dH_dq = autograd.grad(H.sum(), q, create_graph=True)[0]\n        dH_dp = autograd.grad(H.sum(), p, create_graph=True)[0]\n        dq_dt = dH_dp       # Hamilton 方程: dq/dt = ∂H/∂p\n        dp_dt = -dH_dq      # Hamilton 方程: dp/dt = -∂H/∂q\n        return dq_dt, dp_dt\n\n# === 训练循环 ===\nmodel = HNN(input_dim=2)  # 1D 系统: q, p 各 1 维\noptimizer = torch.optim.Adam(model.parameters(), lr=1e-3)\n\nfor step in range(2000):\n    # 从数据中采样 (q, p, dq/dt_true, dp/dt_true)\n    q, p, dq_true, dp_true = sample_batch(data, batch_size=200)\n    dq_pred, dp_pred = model.time_derivative(q, p)\n    # 损失: 预测导数 vs 真实导数 (Eq 3)\n    loss = ((dq_pred - dq_true)**2 + (dp_pred - dp_true)**2).mean()\n    optimizer.zero_grad()\n    loss.backward()\n    optimizer.step()\n\n# === 推理: 用 RK4 积分生成轨迹 ===\ndef rk4_step(model, q, p, dt):\n    def f(q, p):\n        return model.time_derivative(q, p)\n    k1q, k1p = f(q, p)\n    k2q, k2p = f(q + dt/2*k1q, p + dt/2*k1p)\n    k3q, k3p = f(q + dt/2*k2q, p + dt/2*k2p)\n    k4q, k4p = f(q + dt*k3q, p + dt*k3p)\n    q_new = q + dt/6 * (k1q + 2*k2q + 2*k3q + k4q)\n    p_new = p + dt/6 * (k1p + 2*k2p + 2*k3p + k4p)\n    return q_new, p_new\n</code></pre>\n<h5>动机与背景</h5>\n<p>物理系统的动力学建模是科学计算的核心任务。传统的神经网络方法（如 Neural ODE）直接用网络拟合状态的时间导数 \\(\\dot{\\mathbf{x}} = f_\\theta(\\mathbf{x})\\)，虽然灵活，但<strong>完全忽略了物理系统的守恒律</strong>。对于保守力学系统，总能量 \\(H(\\mathbf{q}, \\mathbf{p})\\) 是一个运动常数——沿真实轨迹恒定不变。普通 NN 无法保证这一点，导致长时间积分时能量漂移、轨迹发散。</p>\n<div class=\"key-point\">💡 <strong>关键洞察</strong>：哈密顿力学提供了一个天然的归纳偏置——只要动力学由某个标量函数 \\(H\\) 的辛梯度给出，能量就自动守恒。HNN 的核心贡献就是将这一结构性约束嵌入神经网络。</div>\n<h5>哈密顿力学基础</h5>\n<p>对于一个具有广义坐标 \\(\\mathbf{q}\\) 和共轭动量 \\(\\mathbf{p}\\) 的力学系统，哈密顿量 \\(H(\\mathbf{q}, \\mathbf{p})\\) 是系统的总能量。<strong>哈密顿正则方程</strong>给出了系统的时间演化：</p>\n<p>$$\\frac{d\\mathbf{q}}{dt} = \\frac{\\partial H}{\\partial \\mathbf{p}}, \\qquad \\frac{d\\mathbf{p}}{dt} = -\\frac{\\partial H}{\\partial \\mathbf{q}} \\tag{1}$$</p>\n<p>这组方程具有<strong>辛结构</strong>（symplectic structure），可以紧凑地写为：</p>\n<p>$$\\frac{d}{dt}\\begin{pmatrix} \\mathbf{q} \\\\ \\mathbf{p} \\end{pmatrix} = \\begin{pmatrix} 0 & I \\\\ -I & 0 \\end{pmatrix} \\nabla_{(\\mathbf{q},\\mathbf{p})} H \\tag{2}$$</p>\n<p>其中 \\(J = \\begin{pmatrix} 0 & I \\\\ -I & 0 \\end{pmatrix}\\) 是辛矩阵。辛结构的直接推论是：</p>\n<p>$$\\frac{dH}{dt} = \\nabla H \\cdot \\dot{\\mathbf{x}} = \\nabla H \\cdot J \\nabla H = 0$$</p>\n<p>即 <strong>\\(H\\) 沿轨迹恒为常数</strong>——能量自动守恒，无需额外约束。</p>\n<h5>HNN 的核心机制</h5>\n<p>HNN 的设计极为优雅：</p>\n<ol>\n<li>\n<p><strong>参数化哈密顿量</strong>：用一个神经网络 \\(H_\\theta: \\mathbb{R}^{2n} \\to \\mathbb{R}\\) 将相空间坐标 \\((\\mathbf{q}, \\mathbf{p})\\) 映射为标量。网络不直接预测动力学，而是预测一个\"能量景观\"。</p>\n</li>\n<li>\n<p><strong>辛梯度提取动力学</strong>：利用自动微分计算 \\(\\partial H_\\theta / \\partial \\mathbf{p}\\) 和 \\(\\partial H_\\theta / \\partial \\mathbf{q}\\)，再通过哈密顿方程得到 \\(\\dot{\\mathbf{q}}\\) 和 \\(\\dot{\\mathbf{p}}\\)。这一步是 HNN 的灵魂——它将物理结构硬编码进了计算图。</p>\n</li>\n<li>\n<p><strong>损失函数（Eq 3）</strong>：</p>\n</li>\n</ol>\n<p>$$\\mathcal{L}_{\\text{HNN}} = \\left\\| \\frac{\\partial H_\\theta}{\\partial \\mathbf{p}} - \\frac{d\\mathbf{q}}{dt} \\right\\|^2 + \\left\\| \\frac{\\partial H_\\theta}{\\partial \\mathbf{q}} + \\frac{d\\mathbf{p}}{dt} \\right\\|^2 \\tag{3}$$</p>\n<div class=\"warn-box\">⚠️ <strong>注意</strong>：训练数据只需要状态-导数对 \\((\\mathbf{q}, \\mathbf{p}, \\dot{\\mathbf{q}}, \\dot{\\mathbf{p}})\\)，<strong>不需要能量标签</strong>。能量守恒是结构的自然结果，而非显式监督的目标。</div>\n<h5>从坐标到像素：Autoencoder + HNN</h5>\n<p>论文最具创新性的实验是 <strong>Task 5: Pixel Pendulum</strong>——直接从 28×28 灰度图像序列中学习哈密顿动力学。</p>\n<p><img alt=\"像素摆实验结果\" src=\"https://ar5iv.labs.arxiv.org/html/1906.01563/assets/x4.png\" />\n<em>图 4：像素摆实验。HNN 在潜空间中保持能量守恒，预测轨迹数百帧后仍接近真实值；基线模型迅速衰减到低能态。</em></p>\n<p>方法设计：\n- <strong>输入</strong>：连续两帧 28×28 图像拼接（batch × 28 × 28 × 2），双帧使速度可观测\n- <strong>Autoencoder</strong>：4 层全连接（200 隐藏单元，ReLU + 残差连接），潜空间维度为 2（\\(\\mathbf{z} = (\\mathbf{z_q}, \\mathbf{z_p})\\)）\n- <strong>HNN</strong>：在潜空间上运行，架构与坐标实验相同\n- <strong>辅助损失（Eq 7）</strong>：</p>\n<p>$$\\mathcal{L}_{CC} = \\left\\| \\mathbf{z}^t_{\\mathbf{p}} - (\\mathbf{z}^t_{\\mathbf{q}} - \\mathbf{z}^{t+1}_{\\mathbf{q}}) \\right\\|_2 \\tag{7}$$</p>\n<p>该损失鼓励 \\(\\mathbf{z_p}\\) 近似 \\(\\dot{\\mathbf{z}}_{\\mathbf{q}}\\)（有限差分），使潜空间具有正则坐标 \\((\\mathbf{q}, \\mathbf{p})\\) 的性质——这是哈密顿力学成立的前提条件。</p>\n<p>总损失 = HNN 损失 + 自编码器重建损失（L2 像素损失）+ 辅助正则坐标损失。</p>\n<h5>实验结果与对比</h5>\n<p>论文在 5 个任务上对比了 HNN 与基线（直接拟合导数的同架构 NN）：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>任务</th>\n<th>基线能量 MSE (×10³)</th>\n<th>HNN 能量 MSE (×10³)</th>\n<th>提升倍数</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>理想弹簧</td>\n<td>170 ± 20</td>\n<td><strong>0.38 ± 0.1</strong></td>\n<td>~450×</td>\n</tr>\n<tr>\n<td>理想单摆</td>\n<td>42 ± 10</td>\n<td><strong>25 ± 5</strong></td>\n<td>~1.7×</td>\n</tr>\n<tr>\n<td>真实单摆</td>\n<td>390 ± 7</td>\n<td><strong>14 ± 5</strong></td>\n<td>~28×</td>\n</tr>\n<tr>\n<td>两体问题</td>\n<td>—</td>\n<td>—</td>\n<td>约 10×</td>\n</tr>\n<tr>\n<td>像素单摆</td>\n<td>—</td>\n<td>—</td>\n<td>数量级提升</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 <strong>关键发现</strong>：HNN 与基线的训练/测试损失相当（两者拟合能力相似），但 HNN 在<strong>能量守恒</strong>指标上以压倒性优势胜出。这说明辛结构归纳偏置的价值不在于更好的拟合，而在于更好的<strong>泛化和长期稳定性</strong>。</div>\n<h5>与传统方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>传统 NN (Neural ODE)</th>\n<th>HNN</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>输出</td>\n<td>直接预测 \\(\\dot{\\mathbf{q}}, \\dot{\\mathbf{p}}\\)</td>\n<td>预测标量 \\(H_\\theta\\)，辛梯度得动力学</td>\n</tr>\n<tr>\n<td>能量守恒</td>\n<td>无保证，长期漂移</td>\n<td>结构性保证（精确到数值积分误差）</td>\n</tr>\n<tr>\n<td>物理先验</td>\n<td>无</td>\n<td>哈密顿辛结构</td>\n</tr>\n<tr>\n<td>训练数据</td>\n<td>状态-导数对</td>\n<td>同样是状态-导数对（无需能量标签）</td>\n</tr>\n<tr>\n<td>长期积分</td>\n<td>轨迹迅速发散</td>\n<td>轨迹长期稳定</td>\n</tr>\n<tr>\n<td>局限</td>\n<td>灵活但不稳定</td>\n<td>要求系统为保守系统（无耗散）</td>\n</tr>\n</tbody>\n</table></div>\n<h5>讨论与局限</h5>\n<ul>\n<li><strong>正则坐标要求</strong>：HNN 假设输入为正则坐标 \\((\\mathbf{q}, \\mathbf{p})\\)，对于像素等非正则输入需要额外的 Autoencoder 和辅助损失来学习正则表示</li>\n<li><strong>保守系统假设</strong>：HNN 天然不能处理耗散系统（如有摩擦的系统），后续工作如 Dissipative HNN 对此进行了扩展</li>\n<li><strong>数值积分误差</strong>：虽然 HNN 结构上保证 \\(dH/dt = 0\\)，但实际使用 RK4 等非辛积分器时仍有微小能量漂移；使用辛积分器（如 Leapfrog）可进一步改善</li>\n<li><strong>可扩展性</strong>：论文在两体和三体问题上展示了扩展性，但更高维系统的效果有待验证</li>\n</ul>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>可学习坐标变换</strong>：学习微分同胚映射 \\(\\phi^{-1}: D_a \\to D_c\\)，将不规则物理域 \\(D_a\\) 映射到单位环面 \\(D_c = [0,1]^d\\)",
        "<strong>几何傅里叶变换</strong>：在计算域上定义正向/逆向几何傅里叶变换 \\(\\mathcal{F}_a, \\mathcal{F}_a^{-1}\\)，仅需 \\(\\phi^{-1}\\) 即可完成双向变换",
        ...
        "<strong>变形网络设计</strong>：采用残差连接 \\(\\xi = f(x,a) + x\\)（初始化为恒等映射）+ 正弦特征提升表达力",
        "<strong>多场景验证</strong>：弹性力学（点云输入）、塑性锻造、跨声速翼型流动、弯管流动四类 PDE 问题",
        "<strong>逆向设计能力</strong>：训练后可端到端优化几何参数（如翼型形状），实现气动逆设计"
      ],
      "detail": "<h5>核心架构示意图</h5>\n<p><img alt=\"Geo-FNO 架构图\" src=\"https://ar5iv.labs.arxiv.org/html/2207.05209/assets/x1.png\" />\n<em>图：Geo-FNO 架构。(a) 标准 FNO 在规则域上操作；(b) Geo-FNO 通过坐标变换 \\(\\phi_a\\) 将不规则物理域映射到规则计算域，在计算域上执行 FFT，再映射回物理域。</em></p>\n<p><img alt=\"实验场景\" src=\"https://ar5iv.labs.arxiv.org/html/2207.05209/assets/x2.png\" />\n<em>图：弹性力学（含孔洞的单元胞）和塑性锻造问题示例。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Geo-FNO 前向传播伪代码\ndef geo_fno_forward(x_phys, a, phi_inv_net, fno_layers, P, Q):\n    &quot;&quot;&quot;\n    x_phys: 物理域网格点坐标 [N, d]\n    a:      输入函数值（如几何参数、边界条件）[N, d_a]\n    phi_inv_net: 变形网络 φ^{-1}\n    &quot;&quot;&quot;\n    # Step 1: 坐标变换 — 物理域 → 计算域\n    xi = phi_inv_net(x_phys, a)  # ξ = f(x, a) + x (残差连接)\n    # xi 现在是 [0,1]^d 上的均匀网格\n\n    # Step 2: 提升通道维度\n    v = P(a)  # [N, d_a] → [N, d_v]\n\n    # Step 3: L 层 Fourier 卷积（在计算域上）\n    for l in range(L):\n        # 几何傅里叶变换（首层用 F_a，中间层用标准 FFT）\n        v_hat = FFT(v)                    # 在均匀计算网格上做 FFT\n        v_hat = R_l @ v_hat               # 频域线性变换（截断高频）\n        v_freq = IFFT(v_hat)              # 逆 FFT\n        v_local = W_l @ v + b_l           # 局部线性变换\n        v = activation(v_freq + v_local)  # 残差 + 激活\n\n    # Step 4: 投影到输出空间\n    u = Q(v)  # [N, d_v] → [N, d_u]\n\n    # Step 5: 逆变换回物理域（通过 ξ → x 对应关系）\n    return u  # 物理域上的解\n</code></pre>\n<h5>方法详解</h5>\n<p><strong>1. 动机与背景：FNO 的几何局限</strong></p>\n<p>标准 Fourier Neural Operator (FNO) 通过在频域进行全局卷积来学习 PDE 的解算子，其核心优势在于利用 FFT 实现 \\(O(N \\log N)\\) 的高效计算。然而，FFT 要求输入数据定义在<strong>均匀网格</strong>和<strong>规则域</strong>（如矩形/环面）上，这严重限制了 FNO 在实际工程问题中的应用——真实 PDE 问题通常涉及复杂几何（翼型、含孔洞结构等）和非均匀自适应网格。</p>\n<p>现有的解决方案包括：(1) 将不规则域插值到规则网格再用 FNO，但插值引入额外误差；(2) 使用图神经网络（GNO）处理任意网格，但失去了频域全局卷积的效率优势。Geo-FNO 的核心洞察是：<strong>与其改变算子，不如改变坐标系</strong>。</p>\n<div class=\"key-point\">💡 <strong>关键直觉</strong>：如果我们能找到一个光滑的坐标变换，把不规则的物理域\"拉直\"成规则的计算域，就可以在计算域上直接用 FFT，同时保持与物理域的精确对应关系。</div>\n<p><strong>2. 核心机制：可微坐标变换</strong></p>\n<p>Geo-FNO 的数学基础是微分同胚映射。定义坐标变换：</p>\n<p>$$\\phi_a: D^c \\to D_a, \\quad \\xi \\mapsto x$$</p>\n<p>其中 \\(D^c = [0,1]^d\\) 是单位环面（计算域），\\(D_a\\) 是物理域。该映射将计算域上的均匀网格 \\(\\mathcal{T}^c\\) 推前（pushforward）为物理域上的自适应网格：</p>\n<p>$$\\mathcal{T}_a \\coloneqq \\phi_a(\\mathcal{T}^c), \\quad \\psi_a(x) \\coloneqq \\psi^c \\circ \\phi_a^{-1}(x)$$</p>\n<p>对于物理域上的函数 \\(v(x)\\)，通过拉回（pullback）变换到计算域：</p>\n<p>$$v^c(\\xi) \\coloneqq v(\\phi_a(\\xi))$$</p>\n<p><strong>3. 几何傅里叶变换</strong></p>\n<p>基于坐标变换，定义正向几何傅里叶变换：</p>\n<p>$$(\\mathcal{F}_a v)(k) = \\int_{D^c} v^c(\\xi) e^{-2i\\pi \\langle \\xi, k \\rangle} d\\xi \\approx \\frac{1}{|\\mathcal{T}^i|} \\sum_{x \\in \\mathcal{T}^i} m(x) v(x) e^{-2i\\pi \\langle \\phi^{-1}(x), k \\rangle}$$</p>\n<p>逆变换为：</p>\n<p>$$(\\mathcal{F}_a^{-1} \\hat{v})(x) = \\sum_k \\hat{v}(k) e^{2i\\pi \\langle \\phi^{-1}(x), k \\rangle}$$</p>\n<div class=\"warn-box\">⚠️ <strong>重要性质</strong>：正向和逆向变换都只需要 \\(\\phi^{-1}\\)（物理域→计算域方向），无需显式计算 \\(\\phi\\)，这大大简化了实现。</div>\n<p>当 \\(\\phi^{-1}\\) 将输入网格映射为均匀网格时，权重 \\(m(x) = 1\\)，几何傅里叶变换退化为标准 FFT。</p>\n<p><strong>4. 变形网络的设计</strong></p>\n<p>变形网络 \\(\\phi_\\theta^{-1}\\) 将物理坐标和几何参数映射到计算坐标：</p>\n<p>$$\\phi_\\theta^{-1}: (x_1, x_2, a) \\mapsto (\\xi_1, \\xi_2)$$</p>\n<p>关键设计选择：\n- <strong>残差连接</strong>：\\(\\xi = f(x, a) + x\\)，使 \\(\\phi^{-1}\\) 初始化为恒等映射，训练更稳定\n- <strong>正弦位置编码</strong>：使用 \\(\\sin(2^i x)\\) 特征提升网络对高频几何细节的表达能力\n- <strong>端到端训练</strong>：变形网络与 FNO 主体联合优化，损失函数为相对 L2 误差</p>\n<p><strong>5. 两种使用场景</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>场景</th>\n<th>坐标映射方式</th>\n<th>是否需要学习</th>\n<th>示例</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>结构化网格</td>\n<td>索引归一化：\\(\\phi^{-1}: \\mathcal{T}^i[i_1,...,i_d] \\mapsto (i_1/s_1,...,i_d/s_d)\\)</td>\n<td>否</td>\n<td>翼型、管道</td>\n</tr>\n<tr>\n<td>点云/非结构网格</td>\n<td>神经网络参数化</td>\n<td>是</td>\n<td>弹性力学</td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>6. Fourier 延拓处理拓扑不规则域</strong></p>\n<p>当物理域拓扑不规则（如含孔洞，不同胚于圆盘或环面）时，不存在到 \\(D^c\\) 的微分同胚。此时 Geo-FNO 先将域嵌入更大的规则域 \\(D_a \\hookrightarrow \\bar{D}_a\\)（如将含孔方形补全为完整方形），在 \\(\\bar{D}_a\\) 上做变换。训练时仅在原域 \\(D_a\\) 上计算损失，网络隐式学习延拓。</p>\n<p><strong>7. 实验结果</strong></p>\n<p>在弹性力学（点云输入）基准上，Geo-FNO 显著优于其他方法：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>模型</th>\n<th>测试误差</th>\n<th>训练时间/epoch</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td><strong>Geo-FNO (learned)</strong></td>\n<td><strong>2.29%</strong></td>\n<td>1s</td>\n</tr>\n<tr>\n<td>Geo-FNO (O-mesh)</td>\n<td>3.63%</td>\n<td>0.5s</td>\n</tr>\n<tr>\n<td>FNO + 插值</td>\n<td>5.08%</td>\n<td>0.5s</td>\n</tr>\n<tr>\n<td>UNet + 插值</td>\n<td>5.31%</td>\n<td>0.9s</td>\n</tr>\n<tr>\n<td>DeepONet</td>\n<td>9.65%</td>\n<td>45s</td>\n</tr>\n<tr>\n<td>GNO</td>\n<td>12.60%</td>\n<td>32s</td>\n</tr>\n</tbody>\n</table></div>\n<p>在翼型和管道流动（结构化网格）上，Geo-FNO 同样优于插值方法（翼型测试误差 1.38% vs FNO+插值 4.21%）。推理速度约 0.01 秒/样本，比数值求解器快 \\(10^5\\) 倍。</p>\n<div class=\"key-point\">💡 <strong>关键发现</strong>：学习到的变形比手工设计的启发式变形（R-mesh、O-mesh）更优，说明端到端学习坐标变换的有效性。训练后的 Geo-FNO 还可直接用于逆向设计——通过反向传播优化翼型形状参数以最小化阻力、最大化升力。</div>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>频域平流求解</strong>：利用 Fourier 谱方法将平流方程 \\(\\partial u / \\partial t + \\mathbf{v} \\cdot \\nabla u = 0\\) 转化为频域的逐点乘法，仅需单次 FFT + IFFT",
        "<strong>速度场学习</strong>：通过神经网络学习大气速度向量场 \\(\\mathbf{v}(x,t)\\)，驱动频域平流算子",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>边局部参考系</strong>：为每条边 \\(ij\\) 构建三个正交基向量 \\(\\vec{a}_{ij}, \\vec{b}_{ij}, \\vec{c}_{ij}\\)，满足 SO(3) 等变、T(3) 不变、节点交换反对称",
        "<strong>反对称力解码</strong>：力 \\(\\vec{F}_{ij} = \\sum_k \\psi_{e_f}(\\boldsymbol{\\epsilon}'_{ij})[k] \\cdot \\text{basis}_k\\)，由于基向量反对称，自动满足牛顿第三定律 \\(\\vec{F}_{ij} = -\\vec{F}_{ji}\\)，严格保守线性动量",
        "<strong>反对称力矩解码</strong>：角动量交互向量 \\(\\vec{A}_{ij} = -\\vec{A}_{ji}\\)，通过分离轨道分量得到自旋力矩，严格保守总角动量（轨道 + 自旋）",
