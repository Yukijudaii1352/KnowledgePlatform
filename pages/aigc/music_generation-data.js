/**
 * music_generation-data.js — 由 pipeline/build.py 于 2026-05-20 16:45:35 自动生成。
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>MuLan 条件机制</strong>：利用 MuLan 音乐-文本联合嵌入模型，训练时使用音频端嵌入 \\(M_A\\)，推理时替换为文本端嵌入 \\(M_T\\)，实现纯文本到音乐的零样本生成",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>潜在扩散模型</strong>：基于 SDE 的扩散/去噪过程，在连续潜在空间中生成语音，以 WaveNet 为骨干网络，直接预测 \\(\\hat{z}_0\\) 而非 score",
        "<strong>三项联合损失</strong>：数据重建损失 + score 匹配损失 + 新颖的 CE-RVQ 正则化损失（跨残差量化器的交叉熵约束）",
        "<strong>Prior 模型</strong>：Phoneme Encoder（Transformer）+ Duration/Pitch Predictor（卷积），提供帧级条件信息 \\(c\\)",
