/**
 * llm_safety-data.js — 由 pipeline/build.py 于 2026-05-20 17:45:12 自动生成。
  ...
  "algos": [
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>灰盒基线对比</strong>：同时提出基于 token 概率（Avg/Max \\(-\\log p\\)）和熵（Avg/Max \\(H\\)）的灰盒方法作为对照",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "训练策略：联合训练查询编码器 \\(BERT_q\\) 和 BART 生成器，<strong>文档编码器和索引保持冻结</strong>，无需显式检索监督",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>涉及 4 个模型</strong>：SFT Model、Reward Model (6B)、Policy Model (\\(\\pi_\\phi^{\\text{RL}}\\))、Reference Model (\\(\\pi^{\\text{SFT}}\\))",
        "<strong>奖励模型</strong>：基于人类对 K=4\\~9 个输出的排序，利用 \\(\\binom{K}{2}\\) 对比较对进行 pairwise 训练，6B 参数效果最优",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>无需奖励模型</strong>：跳过 RLHF 中的奖励建模阶段，直接利用偏好对 \\((y_w, y_l)\\) 优化策略",
        "<strong>闭式最优策略</strong>：推导出 KL 约束 RL 目标下最优策略 \\(\\pi^*(y|x) \\propto \\pi_{\\text{ref}}(y|x) \\exp\\!\\bigl(\\tfrac{1}{\\beta} r^*(x,y)\\bigr)\\) 的解析解",
        "<strong>奖励重参数化</strong>：将奖励函数表示为策略对数比 \\(r(x,y) = \\beta \\log \\frac{\\pi_\\theta(y|x)}{\\pi_{\\text{ref}}(y|x)} + \\beta \\log Z(x)\\)，代入 Bradley-Terry 模型消去配分函数",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>轻量 MLP 探针</strong>：3 层 MLP（512→256→128），ReLU 激活，二分类输出幻觉概率分数 \\(s^j \\in [0,1]\\)",
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>安全约束→无约束等价变换</strong>：定义修正奖励 \\(r_c(x,y) = r(x,y)\\) 若回答安全，否则 \\(r_c(x,y) = -\\infty\\)，将带约束的安全优化问题（Eq.8）等价转化为标准无约束 RLHF 目标（Eq.11），理论上保证最优策略一致（Proposition 4.2）",
        "<strong>数据需求大幅简化</strong>：仅需有用性偏好对 \\((y_w \\succ y_l)\\) 加上每个回答的<strong>二值安全标签</strong> \\(h \\in \\{0, 1\\}\\)，完全不需要有害性偏好数据（Safe RLHF 需要），降低了标注成本和数据收集难度",
        "<strong>偏好重排序变换 \\(\\mathcal{T}\\)</strong>：当不安全的回答被偏好于安全回答时（\\(\\tilde{h}_w > \\tilde{h}_l\\)），交换偏好顺序，确保安全回答始终被优先选择；对重排后的数据直接应用 DPO 损失即为 SafeDPO（Eq.14）",
        "<strong>增强版 SafeDPO（Enhanced SafeDPO）</strong>：在 DPO 损失的 sigmoid 内部添加偏移量 \\(-(\\tilde{h}_l - \\tilde{h}_w)\\Delta\\)（\\(\\Delta \\geq 0\\)），进一步拉大安全与不安全回答的偏好差距，提升安全性；当 \\(\\Delta = 0\\) 时退化为基础版",
        "<strong>理论保证完备</strong>：Proposition 4.3 证明变换 \\(\\mathcal{T}\\) 下的 DPO 梯度是修正奖励下真实梯度的无偏估计；Proposition 4.4 证明 Enhanced SafeDPO 的最优解与基础版一致，\\(\\Delta\\) 仅影响优化景观而不改变最优点",
      ...
      "detail": "<p><img alt=\"SafeDPO Pipeline\" src=\"https://ar5iv.labs.arxiv.org/html/2505.20065/assets/x1.png\" /></p>\n<pre><code>算法: SafeDPO / Enhanced SafeDPO\n────────────────────────────────────────────\n输入: \n  - 有用性偏好数据集 D = {(x, y_w, y_l, h_w, h_l)}\n    其中 y_w ≻ y_l 表示有用性偏好, h ∈ {0,1} 为安全标签(1=安全)\n  - 参考策略 π_ref\n  - 超参数 β &gt; 0, Δ ≥ 0\n\n步骤 1: 计算安全指示量\n  对每个样本: h̃_w = 1 - h_w,  h̃_l = 1 - h_l\n  (h̃ = 0 表示安全, h̃ = 1 表示不安全)\n\n步骤 2: 偏好重排序 (变换 T)\n  对每个样本 (x, y_w, y_l):\n    if h̃_w &gt; h̃_l:           // 被偏好的回答不安全, 未被偏好的安全\n      交换: (y_w, y_l) ← (y_l, y_w)   // 强制安全回答被偏好\n      交换: (h̃_w, h̃_l) ← (h̃_l, h̃_w)\n\n步骤 3: 计算 Enhanced SafeDPO 损失\n  对每个样本计算:\n    u = β·[log π_θ(y_w|x)/π_ref(y_w|x) - log π_θ(y_l|x)/π_ref(y_l|x)]\n    offset = -(h̃_l - h̃_w) · Δ\n    L = -log σ(u + offset)\n  总损失 = 所有样本的 L 的均值\n\n步骤 4: 梯度下降优化 π_θ\n  使用标准优化器最小化总损失\n\n输出: 安全对齐后的策略 π_θ\n────────────────────────────────────────────\n注: Δ = 0 时退化为基础 SafeDPO (Eq.14)\n    offset 仅在 h̃_l ≠ h̃_w 时非零\n</code></pre>\n<p><strong>问题建模与修正奖励函数。</strong> SafeDPO 的核心洞察来自对安全约束优化问题的重新建模。标准的安全 RLHF 目标是一个带约束的优化问题：</p>\n<p>$$\\max_\\pi \\mathbb{E}_{x \\sim \\mathcal{D}_\\mathcal{X}} \\mathbb{E}_{y \\sim \\pi(\\cdot|x)} [r(x,y)] - \\beta \\, \\text{KL}[\\pi \\| \\pi_{\\text{ref}}], \\quad \\text{s.t.} \\quad c(x,y) \\leq 0$$</p>\n<p>其中 \\(r(x,y)\\) 是奖励函数，\\(c(x,y)\\) 是代价函数（正值表示不安全）。Safe RLHF 通过 Lagrangian 方法求解此问题，需要分别训练奖励模型和代价模型，再用 PPO-Lagrangian 优化策略，流程复杂且不稳定。SafeDPO 的关键创新在于定义<strong>修正奖励函数</strong> \\(r_c(x,y)\\)：当回答安全时 \\(r_c = r\\)，当回答不安全时 \\(r_c = -\\infty\\)。Proposition 4.2 严格证明了在此修正奖励下的无约束优化问题与原始带约束问题具有相同的最优解集合，从而将安全约束\"编码\"进了奖励函数本身。</p>\n<p><strong>偏好重排序变换 \\(\\mathcal{T}\\) 与 SafeDPO 损失。</strong> 将修正奖励 \\(r_c\\) 代入 DPO 的 Bradley-Terry 偏好模型后，可以推导出修正奖励下的偏好概率。关键观察是：如果 \\(y_w\\) 不安全而 \\(y_l\\) 安全，则在修正奖励下 \\(y_l\\) 应当被偏好（因为 \\(r_c(x, y_w) = -\\infty\\)）。这自然导出了变换 \\(\\mathcal{T}\\) 的定义——当 \\(\\tilde{h}_w > \\tilde{h}_l\\) 时交换偏好顺序。对变换后的数据集 \\(\\mathcal{T}(\\mathcal{D})\\) 应用标准 DPO 损失即得到 SafeDPO 的训练目标：</p>\n<p>$$\\mathcal{L}_{\\text{SafeDPO}}(\\pi_\\theta; \\pi_{\\text{ref}}) = -\\mathbb{E}_{(x, y_w', y_l') \\sim \\mathcal{T}(\\mathcal{D})} \\left[ \\log \\sigma \\left( \\beta \\log \\frac{\\pi_\\theta(y_w'|x)}{\\pi_{\\text{ref}}(y_w'|x)} - \\beta \\log \\frac{\\pi_\\theta(y_l'|x)}{\\pi_{\\text{ref}}(y_l'|x)} \\right) \\right]$$</p>\n<p>其中 \\((y_w', y_l')\\) 是经过变换 \\(\\mathcal{T}\\) 重排后的偏好对。Proposition 4.3 进一步证明此损失的梯度是修正奖励下真实 DPO 梯度的无偏估计量，保证了优化的正确性。</p>\n<p><strong>Enhanced SafeDPO 与超参数 \\(\\Delta\\) 的作用。</strong> 基础 SafeDPO 虽然理论上正确，但在有限数据下可能对安全性的强调不够。Enhanced SafeDPO 通过在 sigmoid 函数内部引入偏移量来解决这一问题：</p>\n<p>$$\\mathcal{L}_{\\text{E-SafeDPO}} = -\\mathbb{E} \\left[ \\log \\sigma \\left( \\beta \\log \\frac{\\pi_\\theta(y_w'|x)}{\\pi_{\\text{ref}}(y_w'|x)} - \\beta \\log \\frac{\\pi_\\theta(y_l'|x)}{\\pi_{\\text{ref}}(y_l'|x)} - (\\tilde{h}_l - \\tilde{h}_w)\\Delta \\right) \\right]$$</p>\n<p>当被拒绝的回答不安全（\\(\\tilde{h}_l = 1, \\tilde{h}_w = 0\\)）时，偏移量为 \\(-\\Delta < 0\\)，使得 sigmoid 的输入更小，产生更大的梯度，从而更强烈地惩罚不安全回答。Proposition 4.4 证明了无论 \\(\\Delta\\) 取何值，Enhanced SafeDPO 的全局最优解与基础版完全一致——\\(\\Delta\\) 仅改变损失景观的形状（使安全相关样本的梯度更陡峭），而不改变最优点的位置。实验中 \\(\\Delta \\in \\{0, 2, 5, 10, 20\\}\\) 的测试表明性能对 \\(\\Delta\\) 的选择相当鲁棒，\\(\\Delta = 10\\) 通常是较好的默认值。在 PKU-SafeRLHF-30K 数据集上，SafeDPO 以 Alpaca-7B（基于 LLaMA-2-7B）为基座，在安全率上达到 97-100%，同时有用性得分优于或持平 Safe RLHF、SACPO 等需要多阶段训练的基线方法。</p>",
      "quiz": {
