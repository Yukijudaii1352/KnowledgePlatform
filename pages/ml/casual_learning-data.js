/**
 * casual_learning-data.js — 由 pipeline/build.py 于 2026-06-08 12:12:03 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "detail": "<h5>核心框架示意</h5>\n<pre><code>观测数据 → 条件独立性检验 → 骨架发现 → 碰撞识别 → 方向传播 → CPDAG\n     ↓              ↓              ↓           ↓           ↓\n  样本矩阵    统计检验(偏相关/   删除独立边    X→Z←Y     Meek规则\n              χ²/核方法)        保留依赖边    模式识别    递归定向\n</code></pre>\n<p><em>图：PC 算法的完整流程。从完全无向图出发，通过条件独立性检验逐步精炼，最终输出因果图的等价类表示（CPDAG）。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># PC Algorithm 伪代码\ndef PC_algorithm(V, CI_test, alpha):\n    &quot;&quot;&quot;\n    V: 变量集合 {X1, X2, ..., Xp}\n    CI_test: 条件独立性检验函数\n    alpha: 显著性水平\n    &quot;&quot;&quot;\n    # === 阶段1: 骨架发现 (Skeleton Discovery) ===\n    G = complete_undirected_graph(V)  # 从完全图开始\n    sep_set = {}  # 记录分离集\n    d = 0  # 条件集大小，从0开始递增\n\n    while d &lt;= max_degree(G):\n        for each edge (X, Y) in G:\n            # 关键优化：仅在X的邻居中搜索（排除Y）\n            neighbors = adj(G, X) \\ {Y}\n            for each subset S ⊆ neighbors, |S| = d:\n                if CI_test(X, Y | S) &gt; alpha:  # X ⊥ Y | S\n                    remove_edge(G, X, Y)\n                    sep_set[(X,Y)] = S\n                    break\n        d += 1\n\n    # === 阶段2: 碰撞结构识别 (V-structure Orientation) ===\n    for each triple (X, Z, Y) where X-Z-Y and X,Y not adjacent:\n        if Z not in sep_set[(X,Y)]:\n            orient as X → Z ← Y  # Z是碰撞节点\n\n    # === 阶段3: 方向传播 (Orientation Propagation, Meek Rules) ===\n    repeat until no change:\n        # Rule 1: X → Z - Y 且 X,Y不相邻 → Z → Y\n        # Rule 2: X → Z → Y 且 X - Y → X → Y\n        # Rule 3: X - Z → Y, X - W → Y, Z,W不相邻 → X → Y\n        apply_meek_rules(G)\n\n    return G  # CPDAG\n</code></pre>\n<h5>动机与背景</h5>\n<p><strong>核心问题</strong>：给定一组变量的观测数据（非实验数据），能否仅从统计关联中恢复变量间的因果关系？</p>\n<p>传统方法的局限：\n- <strong>随机对照实验</strong>（RCT）是因果推断的金标准，但在许多场景中不可行（伦理、成本、时间）\n- <strong>相关性≠因果性</strong>：仅凭相关矩阵无法区分 <span class=\"kb-math kb-math-inline\">X \\to Y</span>、<span class=\"kb-math kb-math-inline\">Y \\to X</span> 和 <span class=\"kb-math kb-math-inline\">X \\leftarrow Z \\to Y</span>\n- <strong>贝叶斯方法</strong>（如 K2 算法）需要已知变量排序，且计算代价高</p>\n<p>PC 算法的突破在于：利用<strong>忠实性假设</strong>，将因果图结构与条件独立性关系建立双射，从而可以通过统计检验反推因果结构。</p>\n<h5>核心理论基础</h5>\n<p><strong>1. 因果马尔可夫性 (Causal Markov Property)</strong></p>\n<p>在因果 DAG <span class=\"kb-math kb-math-inline\">G</span> 中，每个变量在给定其父节点后，与所有非后代变量条件独立：</p>\n<div class=\"kb-math kb-math-display\">X \\perp\\!\\!\\!\\perp \\text{NonDesc}(X) \\mid \\text{Pa}(X)</div>\n<p>这意味着因果图编码了一组条件独立性约束。</p>\n<p><strong>2. d-分离 (d-separation)</strong></p>\n<p>d-分离是判断图中条件独立性的图论准则。路径 <span class=\"kb-math kb-math-inline\">X \\to \\cdots \\to Y</span> 被条件集 <span class=\"kb-math kb-math-inline\">S</span> 阻断，当且仅当路径上存在：\n- <strong>链/叉</strong>节点 <span class=\"kb-math kb-math-inline\">Z</span>（<span class=\"kb-math kb-math-inline\">X \\to Z \\to Y</span> 或 <span class=\"kb-math kb-math-inline\">X \\leftarrow Z \\to Y</span>）且 <span class=\"kb-math kb-math-inline\">Z \\in S</span>\n- <strong>碰撞</strong>节点 <span class=\"kb-math kb-math-inline\">Z</span>（<span class=\"kb-math kb-math-inline\">X \\to Z \\leftarrow Y</span>）且 <span class=\"kb-math kb-math-inline\">Z \\notin S</span> 且 <span class=\"kb-math kb-math-inline\">Z</span> 的后代均不在 <span class=\"kb-math kb-math-inline\">S</span> 中</p>\n<div class=\"kb-math kb-math-display\">X \\perp\\!\\!\\!\\perp_G Y \\mid S \\iff \\text{所有} X\\text{-}Y \\text{路径被} S \\text{d-分离}</div>\n<p><strong>3. 忠实性 (Faithfulness)</strong></p>\n<p>分布 <span class=\"kb-math kb-math-inline\">P</span> 对图 <span class=\"kb-math kb-math-inline\">G</span> 忠实，意味着 <span class=\"kb-math kb-math-inline\">P</span> 中的条件独立性<strong>恰好</strong>是 <span class=\"kb-math kb-math-inline\">G</span> 中 d-分离所蕴含的那些——不多也不少：</p>\n<div class=\"kb-math kb-math-display\">X \\perp\\!\\!\\!\\perp_P Y \\mid S \\iff X \\perp\\!\\!\\!\\perp_G Y \\mid S</div>\n<div class=\"key-point\">💡 关键：忠实性排除了\"巧合性\"的独立——即参数恰好取特殊值导致的非结构性独立。这使得我们可以从独立性关系唯一地反推图结构。</div>\n<p><strong>4. 碰撞结构的可辨识性</strong></p>\n<p>考虑三变量模式 <span class=\"kb-math kb-math-inline\">X - Z - Y</span>（X 和 Y 不直接相连）：\n- <strong>链</strong> <span class=\"kb-math kb-math-inline\">X \\to Z \\to Y</span>：<span class=\"kb-math kb-math-inline\">X \\perp\\!\\!\\!\\perp Y \\mid Z</span>（条件于 Z 后独立）\n- <strong>叉</strong> <span class=\"kb-math kb-math-inline\">X \\leftarrow Z \\to Y</span>：<span class=\"kb-math kb-math-inline\">X \\perp\\!\\!\\!\\perp Y \\mid Z</span>（条件于 Z 后独立）\n- <strong>碰撞</strong> <span class=\"kb-math kb-math-inline\">X \\to Z \\leftarrow Y</span>：<span class=\"kb-math kb-math-inline\">X \\perp\\!\\!\\!\\perp Y</span> 但 <span class=\"kb-math kb-math-inline\">X \\not\\!\\perp\\!\\!\\!\\perp Y \\mid Z</span>（条件于 Z 后<strong>反而依赖</strong>）</p>\n<div class=\"warn-box\">⚠️ 注意：碰撞结构（v-structure）是唯一可以从观测数据中区分方向的三变量模式。链和叉产生相同的独立性模式，因此属于同一马尔可夫等价类。</div>\n<h5>PC 与 SGS 的关键区别</h5>\n<p>SGS 算法（Spirtes-Glymour-Scheines）是 PC 的前身，两者的骨架发现结果相同，但搜索策略不同：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>SGS</th>\n<th>PC</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>条件集搜索范围</td>\n<td>所有其他变量的子集</td>\n<td>仅当前邻居的子集</td>\n</tr>\n<tr>\n<td>最坏复杂度</td>\n<td><span class=\"kb-math kb-math-inline\">O(2^p)</span></td>\n<td><span class=\"kb-math kb-math-inline\">O(p^{d+2})</span>，<span class=\"kb-math kb-math-inline\">d</span> 为最大度</td>\n</tr>\n<tr>\n<td>稀疏图性能</td>\n<td>指数级</td>\n<td>多项式级</td>\n</tr>\n<tr>\n<td>正确性</td>\n<td>一致</td>\n<td>一致（等价于SGS）</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 关键：PC 算法的核心洞察是——如果 <span class=\"kb-math kb-math-inline\">X \\perp\\!\\!\\!\\perp Y \\mid S</span> 对某个集合 <span class=\"kb-math kb-math-inline\">S</span> 成立，那么必然存在一个仅包含 <span class=\"kb-math kb-math-inline\">X</span> 或 <span class=\"kb-math kb-math-inline\">Y</span> 的邻居的子集 <span class=\"kb-math kb-math-inline\">S&#x27;</span> 也使得 <span class=\"kb-math kb-math-inline\">X \\perp\\!\\!\\!\\perp Y \\mid S&#x27;</span> 成立。这一性质保证了仅搜索邻居集的充分性。</div>\n<h5>条件独立性检验的实现</h5>\n<p>PC 算法是模块化的——核心逻辑与具体的条件独立性检验方法解耦：</p>\n<p><strong>高斯线性情况</strong>：检验偏相关系数是否为零</p>\n<div class=\"kb-math kb-math-display\">\\rho_{XY \\cdot S} = 0 \\iff X \\perp\\!\\!\\!\\perp Y \\mid S</div>\n<p>使用 Fisher z-变换构造检验统计量：</p>\n<div class=\"kb-math kb-math-display\">z = \\frac{1}{2} \\sqrt{n - |S| - 3} \\cdot \\ln\\frac{1 + \\hat{\\rho}_{XY \\cdot S}}{1 - \\hat{\\rho}_{XY \\cdot S}} \\sim N(0,1) \\text{ under } H_0</div>\n<p><strong>离散情况</strong>：使用 <span class=\"kb-math kb-math-inline\">\\chi^2</span> 检验或 G-检验</p>\n<p><strong>非参数情况</strong>：核方法（KCIT）、互信息估计等</p>\n<h5>Meek 方向传播规则</h5>\n<p>在碰撞结构确定后，以下规则递归应用以确定更多边的方向：</p>\n<ul>\n<li><strong>Rule 1</strong>：若 <span class=\"kb-math kb-math-inline\">X \\to Y - Z</span> 且 <span class=\"kb-math kb-math-inline\">X, Z</span> 不相邻，则 <span class=\"kb-math kb-math-inline\">Y \\to Z</span>（避免产生新碰撞）</li>\n<li><strong>Rule 2</strong>：若 <span class=\"kb-math kb-math-inline\">X \\to Y \\to Z</span> 且 <span class=\"kb-math kb-math-inline\">X - Z</span>，则 <span class=\"kb-math kb-math-inline\">X \\to Z</span>（避免产生环）</li>\n<li><strong>Rule 3</strong>：若存在 <span class=\"kb-math kb-math-inline\">X - Y \\to Z</span> 和 <span class=\"kb-math kb-math-inline\">X - W \\to Z</span>，且 <span class=\"kb-math kb-math-inline\">Y, W</span> 不相邻，则 <span class=\"kb-math kb-math-inline\">X \\to Z</span></li>\n</ul>\n<p>这些规则保证：(1) 不引入新的碰撞结构；(2) 不引入有向环；(3) 结果是唯一的 CPDAG。</p>\n<h5>一致性与局限性</h5>\n<p><strong>一致性定理</strong>：若条件独立性检验是一致的（即随样本量趋于无穷，检验功效趋于1），则：</p>\n<div class=\"kb-math kb-math-display\">\\lim_{n \\to \\infty} \\Pr(\\hat{G}_n \\neq G) = 0</div>\n<p><strong>重要局限</strong>：\n1. <strong>无均匀一致性</strong>（Robins et al., 2003）：不存在对所有分布均匀收敛的因果发现方法——对手可以让依赖性任意弱\n2. <strong>因果充分性假设</strong>：若存在未观测的混杂变量，PC 算法可能产生错误结果（此时需使用 FCI 算法）\n3. <strong>忠实性假设</strong>：在参数空间中测度为零的集合上可能违反，但实践中可能出现近似违反\n4. <strong>检验顺序敏感性</strong>：有限样本下，不同的边检验顺序可能导致不同结果（稳定版 PC-stable 解决此问题）</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>无环性的连续刻画</strong>：提出 <span class=\"kb-math kb-math-inline\">h(W) = \\text{tr}(e^{W \\circ W}) - d = 0</span> 作为 DAG 的充要条件，将离散的无环约束转化为光滑可微的等式约束",
        "<strong>连续优化框架</strong>：将结构学习从组合搜索空间转移到实数矩阵空间 <span class=\"kb-math kb-math-inline\">\\mathbb{R}^{d \\times d}</span>，使用增广拉格朗日方法（Augmented Lagrangian）求解",
        "<strong>评分函数</strong>：采用最小二乘损失 <span class=\"kb-math kb-math-inline\">F(W) = \\frac{1}{2n}\\|X - XW\\|_F^2</span> 作为评分函数，支持 <span class=\"kb-math kb-math-inline\">\\ell_1</span> 正则化实现稀疏性",
      ...
      "detail": "<h5>问题背景与动机</h5>\n<p>从观测数据中学习有向无环图（DAG）的结构是机器学习和因果推断中的核心问题。DAG（也称贝叶斯网络）广泛应用于生物学、遗传学和因果推断等领域。然而，DAG 结构学习是一个 NP-hard 问题，其核心困难在于：</p>\n<ol>\n<li><strong>搜索空间超指数增长</strong>：<span class=\"kb-math kb-math-inline\">d</span> 个节点的 DAG 数量随 <span class=\"kb-math kb-math-inline\">d</span> 超指数增长（例如 5 个节点有 29281 个 DAG）</li>\n<li><strong>无环约束的组合性</strong>：传统方法必须在离散的 DAG 空间中搜索，依赖各种局部启发式策略（如贪心搜索、爬山法）来强制满足无环性</li>\n</ol>\n<p>传统的基于评分的方法（如 GES、Hill-Climbing）在离散空间中进行贪心搜索，面临局部最优和可扩展性问题。基于约束的方法（如 PC 算法）通过条件独立性检验推断图结构，但对检验误差敏感。NOTEARS 的核心动机是：<strong>能否将离散的无环约束转化为连续可微的约束，从而利用成熟的连续优化工具？</strong></p>\n<h5>核心示意图</h5>\n<p><img alt=\"NOTEARS 结构学习示例\" src=\"https://ar5iv.labs.arxiv.org/html/1803.01422v6/assets/x1.png\" />\n<em>图：NOTEARS 在 ER-2 随机图上的结构学习结果。(a) 真实图结构；(b) n=1000 样本时的估计结果，权重矩阵与真实值高度一致。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># NOTEARS 算法核心流程\n# 输入: 数据矩阵 X ∈ R^{n×d}, 初始 W_0, α_0, 进度率 c, 容差 ε, 阈值 ω\n# 输出: 估计的 DAG 邻接矩阵 W_hat\n\ndef notears(X, lambda1=0.1, max_iter=100, h_tol=1e-8, w_threshold=0.3):\n    d = X.shape[1]\n    W = np.zeros((d, d))          # 初始化权重矩阵\n    alpha = 0.0                    # 拉格朗日乘子\n    rho = 1.0                      # 惩罚参数\n\n    for t in range(max_iter):\n        # Step (a): 求解原始子问题 — 最小化增广拉格朗日函数\n        # L^ρ(W, α) = F(W) + (ρ/2)|h(W)|² + α·h(W)\n        W_new = minimize(augmented_lagrangian, W, args=(X, alpha, rho))\n\n        # 确保 h(W) 充分下降: h(W_new) &lt; c * h(W_old)\n        while h(W_new) &gt;= c * h(W):\n            rho *= 10              # 增大惩罚参数\n            W_new = minimize(augmented_lagrangian, W, args=(X, alpha, rho))\n\n        # Step (b): 对偶上升 — 更新拉格朗日乘子\n        alpha = alpha + rho * h(W_new)\n\n        # Step (c): 收敛判断\n        W = W_new\n        if h(W) &lt; h_tol:\n            break\n\n    # Step 3: 阈值化 — 去除小权重边\n    W_hat = W * (np.abs(W) &gt; w_threshold)\n    return W_hat\n\ndef h(W):\n    &quot;&quot;&quot;无环性约束: h(W) = tr(e^{W∘W}) - d&quot;&quot;&quot;\n    return np.trace(scipy.linalg.expm(W * W)) - W.shape[0]\n\ndef F(W, X):\n    &quot;&quot;&quot;最小二乘评分函数&quot;&quot;&quot;\n    n = X.shape[0]\n    R = X - X @ W\n    return 0.5 / n * np.sum(R ** 2)\n</code></pre>\n<h5>方法细节深入解析</h5>\n<p><strong>1. 无环性的连续刻画 — 从离散到连续的关键突破</strong></p>\n<p>NOTEARS 的核心贡献是发现了一个光滑函数 <span class=\"kb-math kb-math-inline\">h: \\mathbb{R}^{d \\times d} \\to \\mathbb{R}</span>，其零水平集恰好对应所有 DAG。推导过程分两步：</p>\n<p><strong>第一步：二值邻接矩阵的情形。</strong> 对于二值矩阵 <span class=\"kb-math kb-math-inline\">B \\in \\{0,1\\}^{d \\times d}</span>，矩阵幂 <span class=\"kb-math kb-math-inline\">B^k</span> 的迹 <span class=\"kb-math kb-math-inline\">\\text{tr}(B^k)</span> 恰好等于图中长度为 <span class=\"kb-math kb-math-inline\">k</span> 的闭合游走数量。因此，图无环当且仅当所有长度的闭合游走数为零：</p>\n<div class=\"kb-math kb-math-display\">\\text{tr}(B^k) = 0, \\quad \\forall k = 1, 2, \\ldots, d</div>\n<p>利用矩阵指数的定义 <span class=\"kb-math kb-math-inline\">e^B = \\sum_{k=0}^{\\infty} \\frac{B^k}{k!}</span>，可以将上述无穷多个条件压缩为一个等式：</p>\n<div class=\"kb-math kb-math-display\">h(B) = \\text{tr}(e^B) - d = 0</div>\n<div class=\"key-point\">💡 <strong>直觉</strong>：矩阵指数 <span class=\"kb-math kb-math-inline\">e^B</span> 对各阶闭合游走数按 <span class=\"kb-math kb-math-inline\">1/k!</span> 重新加权求和。如果图中没有环，则所有 <span class=\"kb-math kb-math-inline\">\\text{tr}(B^k) = 0</span>（<span class=\"kb-math kb-math-inline\">k \\geq 1</span>），因此 <span class=\"kb-math kb-math-inline\">\\text{tr}(e^B) = \\text{tr}(I) = d</span>，即 <span class=\"kb-math kb-math-inline\">h(B) = 0</span>。反之，任何环都会使 <span class=\"kb-math kb-math-inline\">h(B) &gt; 0</span>。</div>\n<p><strong>第二步：扩展到实值加权矩阵。</strong> 上述刻画对二值矩阵成立，但对含负权重的实值矩阵 <span class=\"kb-math kb-math-inline\">W</span> 不直接适用（负权重可能导致闭合游走贡献相消）。解决方案是使用 Hadamard 积 <span class=\"kb-math kb-math-inline\">W \\circ W</span>（逐元素平方），确保所有权重非负：</p>\n<div class=\"kb-math kb-math-display\">h(W) = \\text{tr}(e^{W \\circ W}) - d = 0</div>\n<div class=\"warn-box\">⚠️ <strong>关键性质</strong>：<span class=\"kb-math kb-math-inline\">h(W) \\geq 0</span> 对所有 <span class=\"kb-math kb-math-inline\">W</span> 成立，且 <span class=\"kb-math kb-math-inline\">h(W) = 0</span> 当且仅当 <span class=\"kb-math kb-math-inline\">W</span> 对应的图是 DAG。这意味着 DAG 集合恰好是 <span class=\"kb-math kb-math-inline\">h</span> 的全局最小值点集。</div>\n<p>其梯度具有简洁的解析形式：</p>\n<div class=\"kb-math kb-math-display\">\\nabla h(W) = (e^{W \\circ W})^T \\circ 2W</div>\n<p>计算复杂度为 <span class=\"kb-math kb-math-inline\">O(d^3)</span>，与矩阵指数运算相同，可利用成熟的数值库（如 <code>scipy.linalg.expm</code>）高效计算。</p>\n<p><strong>2. 评分函数与优化目标</strong></p>\n<p>对于线性结构方程模型（SEM）<span class=\"kb-math kb-math-inline\">X = W^T X + z</span>，其中 <span class=\"kb-math kb-math-inline\">z</span> 是噪声向量，NOTEARS 采用最小二乘评分函数：</p>\n<div class=\"kb-math kb-math-display\">F(W) = \\frac{1}{2n} \\|X - XW\\|_F^2</div>\n<p>加入 <span class=\"kb-math kb-math-inline\">\\ell_1</span> 正则化以促进稀疏性后，完整的等式约束优化问题（ECP）为：</p>\n<div class=\"kb-math kb-math-display\">\\min_{W \\in \\mathbb{R}^{d \\times d}} \\frac{1}{2n} \\|X - XW\\|_F^2 + \\lambda \\|W\\|_1 \\quad \\text{subject to} \\quad h(W) = 0</div>\n<div class=\"key-point\">💡 <strong>与无向图模型的类比</strong>：无向图的结构学习可通过 log-det 规划（凸优化）高效求解，这一连续化思路催生了 Graphical Lasso 等一系列突破。NOTEARS 将类似的连续化思想引入有向图，但由于无环约束的非凸性，问题本质上是非凸的。</div>\n<p><strong>3. 增广拉格朗日求解框架</strong></p>\n<p>NOTEARS 使用增广拉格朗日方法将等式约束问题转化为一系列无约束子问题。增广拉格朗日函数定义为：</p>\n<div class=\"kb-math kb-math-display\">L^{\\rho}(W, \\alpha) = F(W) + \\frac{\\rho}{2} |h(W)|^2 + \\alpha \\cdot h(W)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\alpha</span> 是拉格朗日乘子，<span class=\"kb-math kb-math-inline\">\\rho &gt; 0</span> 是二次惩罚参数。算法交替执行：</p>\n<ul>\n<li><strong>原始最小化</strong>：固定 <span class=\"kb-math kb-math-inline\">\\alpha</span>，用 L-BFGS 求解 <span class=\"kb-math kb-math-inline\">\\min_W L^{\\rho}(W, \\alpha)</span></li>\n<li><strong>对偶上升</strong>：更新 <span class=\"kb-math kb-math-inline\">\\alpha \\leftarrow \\alpha + \\rho \\cdot h(W)</span></li>\n<li><strong>进度控制</strong>：要求每步 <span class=\"kb-math kb-math-inline\">h(W_{t+1}) &lt; c \\cdot h(W_t)</span>，若不满足则增大 <span class=\"kb-math kb-math-inline\">\\rho</span></li>\n</ul>\n<p>增广拉格朗日方法的优势在于：不需要将惩罚参数 <span class=\"kb-math kb-math-inline\">\\rho</span> 推到无穷大即可获得约束问题的良好近似解。实验中通常只需不到 10 步外层迭代即可收敛。</p>\n<p><strong>4. 与传统方法的关键区别</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>传统方法（GES/PC）</th>\n<th>NOTEARS</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>搜索空间</td>\n<td>离散 DAG 空间</td>\n<td>连续实数矩阵空间 <span class=\"kb-math kb-math-inline\">\\mathbb{R}^{d \\times d}</span></td>\n</tr>\n<tr>\n<td>无环约束</td>\n<td>局部启发式检查</td>\n<td>全局光滑等式约束 <span class=\"kb-math kb-math-inline\">h(W)=0</span></td>\n</tr>\n<tr>\n<td>优化方法</td>\n<td>贪心搜索/条件独立检验</td>\n<td>L-BFGS + 增广拉格朗日</td>\n</tr>\n<tr>\n<td>结构假设</td>\n<td>部分方法需有界树宽/入度</td>\n<td>无需任何结构假设</td>\n</tr>\n<tr>\n<td>输出</td>\n<td>CPDAG（等价类）</td>\n<td>加权邻接矩阵（含权重）</td>\n</tr>\n<tr>\n<td>实现复杂度</td>\n<td>需要图搜索专用代码</td>\n<td>~50 行 Python，调用标准优化库</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 <strong>核心优势</strong>：NOTEARS 将 DAG 学习从\"图搜索问题\"转变为\"数值优化问题\"，使得整个机器学习和数值优化社区的工具和理论都可以直接应用。</div>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "定义倾向得分 <span class=\"kb-math kb-math-inline\">e(x) = \\Pr(Z=1 \\mid X=x)</span>：给定观测协变量 <span class=\"kb-math kb-math-inline\">X</span> 时个体接受处理的条件概率",
        "<strong>平衡性定理（Theorem 1）</strong>：条件于倾向得分后，处理分配 <span class=\"kb-math kb-math-inline\">Z</span> 与协变量 <span class=\"kb-math kb-math-inline\">X</span> 独立，即 <span class=\"kb-math kb-math-inline\">X \\perp Z \\mid e(X)</span>",
        "<strong>强可忽略性传递（Theorem 2）</strong>：若处理分配在给定 <span class=\"kb-math kb-math-inline\">X</span> 下是强可忽略的，则在给定任何平衡得分 <span class=\"kb-math kb-math-inline\">b(X)</span> 下也是强可忽略的",
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>因果结构学习</strong>：基于NOTEARS的连续优化方法，通过DAG约束 <span class=\"kb-math kb-math-inline\">h(\\mathbf{A}) = \\text{tr}(e^{\\mathbf{A} \\circ \\mathbf{A}}) - M = 0</span> 从数据中发现因果图",
      ...
      "detail": "<p><img alt=\"CAPE 框架总览\" src=\"https://arxiv.org/abs/2410.16197\" />\n<em>图：CAPE 三步框架示意 — (a) 从观测数据学习因果DAG；(b) 在双曲空间中嵌入因果结构；(c) 转换为旋转位置编码注入Transformer。（来源：论文 Figure 1）</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># CAPE 核心流程伪代码\n# === Step I: 因果结构学习 ===\n# 输入: 观测数据 X ∈ R^{N×M}, 特征集 V = {v1,...,vM}\nA = initialize_adjacency(M)  # 邻接矩阵\nfor iteration in range(max_iter):\n    # 最小化结构方程残差 + DAG约束\n    loss_structure = sum(||x_j - f_j(x_pa(j))||^2 for j in V)\n    loss_dag = tr(exp(A ⊙ A)) - M  # NOTEARS DAG约束 h(A)=0\n    loss = loss_structure + lambda_dag * loss_dag + lambda_sparse * ||A||_1\n    A = augmented_lagrangian_step(A, loss)\n\n# === Step II: 双曲空间嵌入 ===\n# 在 Hyperboloid H^d 上优化位置编码\nP = initialize_on_hyperboloid(M, d)  # {p_v1,...,p_vM} ∈ H^d\nfor iteration in range(max_iter):\n    # 对比学习: 拉近因果相关对, 推远无关对\n    L_con = contrastive_loss(P, A, k_hop=2)\n    # PageRank正则: 通用节点靠近原点\n    pi = compute_pagerank(A)\n    L_reg = sum(pi[j] * d_hyperbolic(P[j], origin) for j in V)\n    L_H = L_con + lambda_g * L_reg\n    # Riemannian SGD 更新\n    P = rsgd_update(P, L_H, learning_rate=eta)\n\n# 映射到 Poincaré 球: H^d → B^d\nE = diffeomorphism_to_poincare(P)  # {e_v1,...,e_vM} ∈ B^d\n\n# === Step III: 旋转位置编码 ===\n# 将 Poincaré 球嵌入转为旋转矩阵\nfor each feature pair (vm, vn):\n    R_q = rotation_matrix(e_vm)  # 查询旋转\n    R_k = rotation_matrix(e_vn)  # 键旋转\n    q_vm = R_q @ W_q @ embedding(vm)\n    k_vn = R_k @ W_k @ embedding(vn)\n    attention(vm, vn) = softmax(q_vm^T @ k_vn / sqrt(d))\n</code></pre>\n<h5>动机与背景</h5>\n<p>传统Transformer的位置编码（如正弦编码、RoPE）假设输入token具有天然的序列顺序（如文本中的词序、图像中的像素位置）。然而，在许多科学和医学领域（如基因组学、蛋白质组学），特征（如基因、蛋白质）之间<strong>没有固有的序列顺序</strong>，但存在复杂的<strong>因果关系</strong>。例如，转录因子调控下游基因表达，形成有向无环图（DAG）结构。</p>\n<p>现有方法要么忽略位置编码（丢失结构信息），要么使用随机/可训练的位置编码（无法捕获因果语义），导致Transformer在这类数据上的表现受限。CAPE的核心洞察是：<strong>因果关系本身就是一种\"位置\"信息</strong>——它定义了特征之间的相对关系和层次结构。</p>\n<h5>核心机制详解</h5>\n<p><strong>Step I：因果结构学习（Causal Structure Learning）</strong></p>\n<p>CAPE采用基于NOTEARS框架的连续优化方法从观测数据中发现因果DAG。核心思想是将离散的DAG搜索问题转化为连续优化问题：</p>\n<div class=\"kb-math kb-math-display\">\\min_{\\mathbf{A}} \\sum_{j=1}^{M} \\mathcal{L}(v_j, \\mathbf{A}_{\\cdot j}) + \\lambda_s \\|\\mathbf{A}\\|_1 \\quad \\text{s.t.} \\quad h(\\mathbf{A}) = 0</div>\n<p>其中DAG约束 <span class=\"kb-math kb-math-inline\">h(\\mathbf{A}) = \\text{tr}(e^{\\mathbf{A} \\circ \\mathbf{A}}) - M</span> 确保学到的图无环。结构方程模型 <span class=\"kb-math kb-math-inline\">v_j = f_j(\\mathbf{A}_{\\cdot j} \\circ \\mathbf{v}_{\\setminus j}) + \\epsilon_j</span> 允许非线性因果关系，<span class=\"kb-math kb-math-inline\">f_j</span> 由MLP参数化。通过增广拉格朗日方法求解此约束优化问题。</p>\n<div class=\"key-point\">💡 关键：DAG约束的巧妙之处在于 <span class=\"kb-math kb-math-inline\">\\text{tr}(e^{\\mathbf{A} \\circ \\mathbf{A}}) = \\sum_{k=0}^{\\infty} \\text{tr}((\\mathbf{A} \\circ \\mathbf{A})^k)/k!</span>，当且仅当图无环时等于 <span class=\"kb-math kb-math-inline\">M</span>（因为无环图的邻接矩阵幂次迹为零）。</div>\n<p><strong>Step II：双曲空间嵌入</strong></p>\n<p>学到因果DAG后，CAPE将其嵌入双曲空间（具体为Hyperboloid模型 <span class=\"kb-math kb-math-inline\">\\mathbb{H}^d</span>）。选择双曲空间的原因是：</p>\n<ol>\n<li><strong>树状结构的天然表示</strong>：双曲空间可视为离散树的连续类比，DAG的层次结构自然适配</li>\n<li><strong>指数增长的容量</strong>：双曲空间的体积随半径指数增长，能高效表示宽泛的层次关系</li>\n</ol>\n<p>嵌入需满足两个关键性质：</p>\n<ul>\n<li><strong>因果强度</strong>：<span class=\"kb-math kb-math-inline\">\\sigma(v_m, v_n) \\propto 1/d_l(p_{v_m}, p_{v_n})</span> — 因果关系越强的特征对，双曲距离越近</li>\n<li><strong>因果特异性</strong>：<span class=\"kb-math kb-math-inline\">\\ell(v_m) \\propto d_l(p_{v_m}, p_o)</span> — 越特异（影响范围小）的特征离原点越远</li>\n</ul>\n<p>通过对比学习损失 <span class=\"kb-math kb-math-inline\">\\mathcal{L}_{\\text{con}}</span> 实现因果强度编码，通过PageRank正则化 <span class=\"kb-math kb-math-inline\">\\Omega</span> 实现因果特异性编码：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_H = \\frac{1}{M}\\sum_{j=1}^{M} \\mathcal{L}_{\\text{con}}(p_{v_j}) + \\lambda_g \\Omega(p_{v_j})</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\Omega(p_{v_m}) = \\pi_{v_m} \\cdot d_l(p_{v_m}, p_o)</span>，<span class=\"kb-math kb-math-inline\">\\pi_{v_m}</span> 是PageRank值（因果通用性越高的节点PageRank越大），迫使通用节点靠近原点。</p>\n<div class=\"warn-box\">⚠️ 注意：优化在黎曼流形上进行（RSGD），需要将欧几里得梯度转换为黎曼梯度，再通过指数映射投影回流形。</div>\n<p><strong>Step III：旋转位置编码</strong></p>\n<p>优化后的Hyperboloid嵌入通过微分同胚映射到Poincaré球 <span class=\"kb-math kb-math-inline\">\\mathbb{B}^d</span>，然后转换为旋转形式。对于查询和键：</p>\n<div class=\"kb-math kb-math-display\">q^i_{v_m} = I_q(v^i_m, e_{v_m}), \\quad k^i_{v_n} = I_k(v^i_n, e_{v_n})</div>\n<p>注入函数 <span class=\"kb-math kb-math-inline\">I_q, I_k</span> 将Poincaré球坐标转化为旋转角度，使得注意力分数的内积自然编码了位置差异：</p>\n<div class=\"kb-math kb-math-display\">\\langle q^i_{v_m}, k^i_{v_n} \\rangle = \\mathcal{A}(v^i_m, v^i_n, \\gamma(e_{v_m}, e_{v_n}))</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\gamma(e_{v_m}, e_{v_n})</span> 是由两个位置编码的差异决定的旋转角度函数。</p>\n<h5>理论性质</h5>\n<p>论文证明了三个重要理论保证：</p>\n<ol>\n<li><strong>因果距离衰减</strong>（Prop. 4.1）：注意力分数 <span class=\"kb-math kb-math-inline\">\\mathcal{A}</span> 被上下界 <span class=\"kb-math kb-math-inline\">\\mathcal{A}^+, \\mathcal{A}^-</span> 约束，随因果距离 <span class=\"kb-math kb-math-inline\">d_p(e_{v_m}, e_{v_n}) \\to +\\infty</span> 两界收敛，注意力变化范围缩小</li>\n<li><strong>因果通用性衰减</strong>（Prop. 4.2）：因果通用性 <span class=\"kb-math kb-math-inline\">\\psi_{v_m} \\to 1</span> 时，上界单调递减、下界单调递增，注意力趋于均匀分布</li>\n<li><strong>鲁棒性</strong>（Prop. 4.3）：对位置编码的高斯扰动，注意力分数保持可区分性、无偏性和渐近收敛性</li>\n</ol>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>适用场景</th>\n<th>编码内容</th>\n<th>因果感知</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>正弦位置编码</td>\n<td>序列数据</td>\n<td>绝对位置</td>\n<td>❌</td>\n</tr>\n<tr>\n<td>RoPE</td>\n<td>序列数据</td>\n<td>相对位置</td>\n<td>❌</td>\n</tr>\n<tr>\n<td>可训练相对编码</td>\n<td>非序列数据</td>\n<td>学习到的相对关系</td>\n<td>❌</td>\n</tr>\n<tr>\n<td><strong>CAPE</strong></td>\n<td><strong>非序列因果数据</strong></td>\n<td><strong>因果强度+特异性</strong></td>\n<td><strong>✅</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>CAPE的独特优势在于：(1) 不依赖预定义的特征顺序；(2) 编码了因果语义而非任意位置；(3) 旋转形式兼容线性注意力；(4) 具有理论保证的注意力行为。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "重参数化技巧：<span class=\"kb-math kb-math-inline\">w = \\mu + \\log(1+\\exp(\\rho)) \\circ \\epsilon</span>，<span class=\"kb-math kb-math-inline\">\\epsilon \\sim \\mathcal{N}(0, I)</span>，使梯度可通过标准反向传播计算",
        "损失函数分解为复杂度代价（KL散度）和数据似然代价：<span class=\"kb-math kb-math-inline\">\\mathcal{F}(\\mathcal{D}, \\theta) = \\text{KL}[q(w|\\theta) \\| P(w)] - \\mathbb{E}_{q}[\\log P(\\mathcal{D}|w)]</span>",
        "Scale Mixture 先验：<span class=\"kb-math kb-math-inline\">P(w) = \\pi \\mathcal{N}(0, \\sigma_1^2) + (1-\\pi)\\mathcal{N}(0, \\sigma_2^2)</span>，鼓励权重稀疏分布",
        "Minibatch KL 重加权策略：<span class=\"kb-math kb-math-inline\">\\pi_i = \\frac{2^{M-i}}{2^M - 1}</span>，使训练早期更受先验约束",
        "信噪比剪枝：利用 <span class=\"kb-math kb-math-inline\">|\\mu_i|/\\sigma_i</span> 排序权重，移除95%权重后性能几乎不降",
        "Thompson Sampling 应用：从后验采样权重实现探索-利用平衡，适用于上下文赌博机问题"
      ],
      "detail": "<p><img alt=\"Bayes by Backprop 框架示意\" src=\"https://arxiv.org/html/1505.05424v1/extracted/figures/fig1.png\" />\n<em>图：Bayes by Backprop 通过学习权重分布参数 <span class=\"kb-math kb-math-inline\">\\theta = (\\mu, \\rho)</span> 替代传统点估计，每次前向传播从分布中采样权重</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Bayes by Backprop 核心算法\n# 输入: 变分参数 θ = (μ, ρ), 训练数据 D\n\ndef bayes_by_backprop(D, θ, num_samples=1):\n    &quot;&quot;&quot;\n    θ = (μ, ρ) 为变分后验参数\n    σ = log(1 + exp(ρ))  # softplus 确保正值\n    &quot;&quot;&quot;\n    f_total = 0\n    for _ in range(num_samples):\n        # 1. 采样噪声\n        ε ~ N(0, I)\n        # 2. 重参数化得到权重样本\n        w = μ + log(1 + exp(ρ)) ⊙ ε\n        # 3. 计算变分自由能\n        f = log q(w|θ) - log P(w) - log P(D|w)\n        #     复杂度代价        数据似然代价\n        f_total += f\n\n    f_total /= num_samples\n\n    # 4. 计算梯度（通过标准反向传播）\n    Δμ = ∂f/∂w + ∂f/∂μ           # = ∂f/∂w · 1 + ∂(log q)/∂μ\n    Δρ = ∂f/∂w · ε/(1+exp(-ρ)) + ∂f/∂ρ  # 链式法则\n\n    # 5. 更新参数\n    μ ← μ - α · Δμ\n    ρ ← ρ - α · Δρ\n</code></pre>\n<h5>动机与背景</h5>\n<p>传统神经网络训练通过最大似然估计（MLE）或最大后验估计（MAP）获得权重的<strong>点估计</strong>。这种方法存在三个根本缺陷：</p>\n<ol>\n<li><strong>过拟合</strong>：点估计容易过度拟合训练数据，尤其在数据稀少区域</li>\n<li><strong>无法量化不确定性</strong>：模型无法表达\"我不确定\"，在分布外区域仍给出高置信度预测</li>\n<li><strong>探索不足</strong>：在强化学习/决策场景中，点估计无法自然实现探索</li>\n</ol>\n<p>贝叶斯方法通过维护权重的完整后验分布 <span class=\"kb-math kb-math-inline\">P(w|\\mathcal{D})</span> 来解决这些问题。然而，精确贝叶斯推断在神经网络中是不可计算的（后验无解析形式），因此需要近似方法。</p>\n<h5>核心机制：变分推断框架</h5>\n<p><strong>变分自由能目标</strong></p>\n<p>Bayes by Backprop 的核心思想是用一个参数化的简单分布 <span class=\"kb-math kb-math-inline\">q(w|\\theta)</span> 来近似真实后验 <span class=\"kb-math kb-math-inline\">P(w|\\mathcal{D})</span>。优化目标为最小化变分自由能（即 ELBO 的负数）：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{F}(\\mathcal{D}, \\theta) = \\text{KL}[q(w|\\theta) \\| P(w)] - \\mathbb{E}_{q(w|\\theta)}[\\log P(\\mathcal{D}|w)]</div>\n<p>其中：\n- <strong>复杂度代价</strong> <span class=\"kb-math kb-math-inline\">\\text{KL}[q(w|\\theta) \\| P(w)]</span>：衡量学到的后验与先验的偏离程度，起正则化作用\n- <strong>数据似然代价</strong> <span class=\"kb-math kb-math-inline\">-\\mathbb{E}_{q}[\\log P(\\mathcal{D}|w)]</span>：衡量模型对数据的拟合能力</p>\n<div class=\"key-point\">💡 关键直觉：这个目标自动平衡了\"拟合数据\"和\"保持简单\"——正是奥卡姆剃刀原则的数学化表达。</div>\n<p><strong>重参数化技巧</strong></p>\n<p>直接对 <span class=\"kb-math kb-math-inline\">\\theta</span> 求 <span class=\"kb-math kb-math-inline\">\\mathbb{E}_{q(w|\\theta)}[f(w, \\theta)]</span> 的梯度是困难的，因为期望的分布本身依赖于 <span class=\"kb-math kb-math-inline\">\\theta</span>。重参数化技巧将随机性从参数中分离：</p>\n<div class=\"kb-math kb-math-display\">w = t(\\theta, \\epsilon) = \\mu + \\log(1 + \\exp(\\rho)) \\circ \\epsilon, \\quad \\epsilon \\sim \\mathcal{N}(0, I)</div>\n<p>这样梯度变为：</p>\n<div class=\"kb-math kb-math-display\">\\frac{\\partial}{\\partial \\theta} \\mathbb{E}_{q(w|\\theta)}[f(w, \\theta)] = \\mathbb{E}_{\\mathcal{N}(\\epsilon|0,I)}\\left[\\frac{\\partial f(w, \\theta)}{\\partial w}\\frac{\\partial w}{\\partial \\theta} + \\frac{\\partial f(w, \\theta)}{\\partial \\theta}\\right]</div>\n<div class=\"warn-box\">⚠️ 注意：使用 <span class=\"kb-math kb-math-inline\">\\sigma = \\log(1 + \\exp(\\rho))</span>（softplus函数）而非直接参数化 <span class=\"kb-math kb-math-inline\">\\sigma</span>，是为了确保标准差始终为正，同时避免 <span class=\"kb-math kb-math-inline\">\\exp</span> 的数值溢出问题。</div>\n<p><strong>Scale Mixture 先验</strong></p>\n<p>论文提出使用两个高斯分布的混合作为先验：</p>\n<div class=\"kb-math kb-math-display\">P(w) = \\pi \\mathcal{N}(w|0, \\sigma_1^2) + (1-\\pi)\\mathcal{N}(w|0, \\sigma_2^2)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\sigma_1</span> 较大（如 <span class=\"kb-math kb-math-inline\">\\exp(-1)</span> 到 <span class=\"kb-math kb-math-inline\">\\exp(0)</span>），<span class=\"kb-math kb-math-inline\">\\sigma_2</span> 很小（如 <span class=\"kb-math kb-math-inline\">\\exp(-6)</span> 到 <span class=\"kb-math kb-math-inline\">\\exp(-8)</span>）。这种设计类似于 spike-and-slab 先验：\n- 宽高斯允许重要权重取较大值\n- 窄高斯将不重要权重压向零附近</p>\n<p>这比单一高斯先验更灵活，能学到更稀疏的网络结构。</p>\n<p><strong>Minibatch KL 重加权</strong></p>\n<p>在使用 minibatch 训练时，KL 项需要合理分配到各个 batch。论文提出非均匀加权：</p>\n<div class=\"kb-math kb-math-display\">\\pi_i = \\frac{2^{M-i}}{2^M - 1}, \\quad i = 1, \\ldots, M</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">M</span> 是总 batch 数，<span class=\"kb-math kb-math-inline\">i</span> 是当前 batch 索引。这使得训练初期 KL 权重更大（更受先验约束），后期更多关注数据拟合。</p>\n<div class=\"key-point\">💡 直觉：类似于\"先学规则，再学例外\"——早期让先验引导网络结构，后期让数据精调细节。</div>\n<h5>训练与推理流程</h5>\n<p><strong>训练阶段</strong>：\n1. 对每个 minibatch，从 <span class=\"kb-math kb-math-inline\">\\mathcal{N}(0, I)</span> 采样 <span class=\"kb-math kb-math-inline\">\\epsilon</span>\n2. 通过重参数化计算权重 <span class=\"kb-math kb-math-inline\">w = \\mu + \\text{softplus}(\\rho) \\circ \\epsilon</span>\n3. 前向传播计算损失（复杂度代价 + 数据似然代价）\n4. 反向传播计算 <span class=\"kb-math kb-math-inline\">\\nabla_\\mu</span> 和 <span class=\"kb-math kb-math-inline\">\\nabla_\\rho</span> 的梯度\n5. 更新 <span class=\"kb-math kb-math-inline\">\\mu</span> 和 <span class=\"kb-math kb-math-inline\">\\rho</span></p>\n<p><strong>推理阶段</strong>：\n- 可以采样多组权重进行贝叶斯模型平均\n- 或直接使用均值权重 <span class=\"kb-math kb-math-inline\">\\mu</span> 作为点预测\n- 不确定性通过多次采样的预测方差估计</p>\n<h5>与传统方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>权重表示</th>\n<th>正则化</th>\n<th>不确定性</th>\n<th>探索能力</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>SGD (MLE)</td>\n<td>点估计</td>\n<td>无</td>\n<td>无</td>\n<td>无</td>\n</tr>\n<tr>\n<td>L2 正则化 (MAP)</td>\n<td>点估计</td>\n<td>高斯先验</td>\n<td>无</td>\n<td>无</td>\n</tr>\n<tr>\n<td>Dropout</td>\n<td>点估计+随机掩码</td>\n<td>隐式</td>\n<td>近似</td>\n<td>有限</td>\n</tr>\n<tr>\n<td><strong>Bayes by Backprop</strong></td>\n<td><strong>概率分布</strong></td>\n<td><strong>KL散度</strong></td>\n<td><strong>显式</strong></td>\n<td><strong>Thompson采样</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>关键优势：\n1. <strong>vs Dropout</strong>：BBB 显式学习每个权重的不确定性，而 dropout 只是二值掩码的近似\n2. <strong>vs 变分 Dropout (Graves 2011)</strong>：BBB 使用更好的梯度估计器（重参数化 vs REINFORCE），方差更低\n3. <strong>vs 集成方法</strong>：BBB 是隐式无限集成（对所有可能权重配置积分），且可通过信噪比剪枝压缩</p>\n<h5>实验亮点</h5>\n<p><strong>MNIST 分类</strong>：两层 1200 ReLU 单元网络，BBB 达到 <strong>1.32%</strong> 测试错误率，优于 dropout 的 1.34%。</p>\n<p><strong>权重剪枝</strong>：按信噪比 <span class=\"kb-math kb-math-inline\">|\\mu|/\\sigma</span> 排序后移除 95% 的权重（从 2.4M 降至 120K），错误率仅从 1.24% 升至 1.29%，展示了极强的压缩能力。</p>\n<p><strong>回归不确定性</strong>：在无数据区域，BBB 的置信区间自然发散，而标准网络错误地给出零方差预测。</p>\n<p><strong>Thompson Sampling</strong>：在蘑菇分类赌博机任务中，BBB 通过从后验采样权重实现探索，累积遗憾显著低于 <span class=\"kb-math kb-math-inline\">\\epsilon</span>-greedy 和 UCB 等基线方法。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "回归网络输出两个值：预测均值 <span class=\"kb-math kb-math-inline\">\\mu_\\theta(\\mathbf{x})</span> 和预测方差 <span class=\"kb-math kb-math-inline\">\\sigma^2_\\theta(\\mathbf{x})</span>，通过最小化负对数似然（NLL）联合优化",
      ...
      "detail": "<p><img alt=\"Deep Ensembles 方法示意图\" src=\"https://arxiv.org/html/1612.01474v3/extracted/figures/ensemble_diagram.png\" />\n<em>图：Deep Ensembles 训练与预测流程示意——M 个独立网络各自输出预测分布，集成后得到混合分布</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Deep Ensembles 训练与预测伪代码 (Algorithm 1)\nM = 5  # 集成网络数量\nepsilon = 0.01 * input_range  # 对抗扰动幅度\n\n# === 训练阶段 ===\nfor m in range(M):\n    theta_m = random_init()  # 随机初始化参数\n    for epoch in range(num_epochs):\n        for x_batch, y_batch in shuffle(dataset):\n            # 计算原始损失\n            loss_orig = NLL(theta_m, x_batch, y_batch)\n            # 生成对抗样本 (FGSM)\n            x_adv = x_batch + epsilon * sign(grad(loss_orig, x_batch))\n            # 对抗训练损失\n            loss_adv = NLL(theta_m, x_adv, y_batch)\n            # 联合优化\n            optimize(theta_m, loss_orig + loss_adv)\n\n# === 预测阶段（回归） ===\ndef predict(x):\n    mu_list, sigma2_list = [], []\n    for m in range(M):\n        mu_m, sigma2_m = network_m(x)  # 每个网络输出均值和方差\n        mu_list.append(mu_m)\n        sigma2_list.append(sigma2_m)\n    # 混合分布的均值和方差\n    mu_star = mean(mu_list)\n    sigma2_star = mean([s + m**2 for s, m in zip(sigma2_list, mu_list)]) - mu_star**2\n    return mu_star, sigma2_star\n</code></pre>\n<h5>动机与背景</h5>\n<p>深度学习模型通常只输出点估计，缺乏对预测不确定性的量化。在安全关键应用（自动驾驶、医疗诊断）中，模型需要\"知道自己不知道什么\"。传统的不确定性估计方法主要依赖贝叶斯神经网络（BNN），但 BNN 面临以下困难：</p>\n<ol>\n<li><strong>计算开销大</strong>：精确后验推断不可行，变分推断（VI）需要额外参数和复杂实现</li>\n<li><strong>先验选择困难</strong>：权重空间的先验难以解释，对结果影响大</li>\n<li><strong>扩展性差</strong>：MCMC 方法难以应用于大规模网络</li>\n</ol>\n<p>MC-Dropout 虽然简化了实现，但其理论基础（作为变分推断的近似）存在争议，且性能受限于 dropout 率的选择。</p>\n<p>Deep Ensembles 提出了一种<strong>非贝叶斯</strong>替代方案：利用神经网络损失函数的多模态性质，通过不同随机初始化训练多个网络，自然地探索参数空间中的不同模式，从而捕获模型不确定性。</p>\n<h5>核心机制一：Proper Scoring Rule 训练</h5>\n<p>论文的第一个关键洞察是：训练准则本身应当鼓励校准的不确定性估计。<strong>Proper scoring rule</strong> 是满足以下性质的评分函数：当且仅当预测分布等于真实分布时，期望得分最大化。</p>\n<p>对于回归问题，网络输出预测均值 <span class=\"kb-math kb-math-inline\">\\mu_\\theta(\\mathbf{x})</span> 和方差 <span class=\"kb-math kb-math-inline\">\\sigma^2_\\theta(\\mathbf{x})</span>（通过 softplus 保证正性），训练目标为最小化负对数似然：</p>\n<div class=\"kb-math kb-math-display\">-\\log p_\\theta(y_n|\\mathbf{x}_n) = \\frac{\\log \\sigma^2_\\theta(\\mathbf{x})}{2} + \\frac{(y - \\mu_\\theta(\\mathbf{x}))^2}{2\\sigma^2_\\theta(\\mathbf{x})} + \\text{constant}</div>\n<div class=\"key-point\">💡 关键：与传统 MSE 不同，NLL 损失让网络<strong>自适应地学习每个输入的预测方差</strong>。当模型对某个预测不确定时，它可以增大 <span class=\"kb-math kb-math-inline\">\\sigma^2</span> 来降低惩罚——但 <span class=\"kb-math kb-math-inline\">\\log \\sigma^2</span> 项又防止方差无限增大。这形成了一个自然的校准机制。</div>\n<p>对于分类问题，softmax 交叉熵损失本身就是 proper scoring rule（由 Gibbs 不等式保证）。</p>\n<h5>核心机制二：对抗训练平滑预测分布</h5>\n<p>论文的第二个创新是将对抗训练重新解释为<strong>预测分布平滑</strong>的手段。使用 FGSM（Fast Gradient Sign Method）生成对抗样本：</p>\n<div class=\"kb-math kb-math-display\">\\mathbf{x}&#x27; = \\mathbf{x} + \\epsilon \\cdot \\text{sign}(\\nabla_\\mathbf{x} \\ell(\\theta, \\mathbf{x}, y))</div>\n<p>训练时同时最小化原始样本和对抗样本上的损失：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L} = \\ell(\\theta_m, \\mathbf{x}, y) + \\ell(\\theta_m, \\mathbf{x}&#x27;, y)</div>\n<div class=\"key-point\">💡 关键：对抗训练的直觉是——它迫使网络在训练样本的 ε-邻域内保持平滑的预测分布。理想情况下应沿所有 <span class=\"kb-math kb-math-inline\">2^D</span> 个方向平滑，但计算上不可行；FGSM 选择损失增长最快的方向，是最高效的平滑策略。</div>\n<h5>核心机制三：集成组合</h5>\n<p>M 个独立训练的网络组成均匀加权混合模型：</p>\n<div class=\"kb-math kb-math-display\">p(y|\\mathbf{x}) = \\frac{1}{M}\\sum_{m=1}^{M} p_{\\theta_m}(y|\\mathbf{x})</div>\n<p>对于回归任务，每个网络输出高斯分布 <span class=\"kb-math kb-math-inline\">\\mathcal{N}(\\mu_{\\theta_m}(\\mathbf{x}), \\sigma^2_{\\theta_m}(\\mathbf{x}))</span>，集成预测近似为单个高斯分布：</p>\n<div class=\"kb-math kb-math-display\">\\mu_*(\\mathbf{x}) = \\frac{1}{M}\\sum_m \\mu_{\\theta_m}(\\mathbf{x})</div>\n<div class=\"kb-math kb-math-display\">\\sigma^2_*(\\mathbf{x}) = \\frac{1}{M}\\sum_m \\left(\\sigma^2_{\\theta_m}(\\mathbf{x}) + \\mu^2_{\\theta_m}(\\mathbf{x})\\right) - \\mu^2_*(\\mathbf{x})</div>\n<div class=\"warn-box\">⚠️ 注意：最终方差 <span class=\"kb-math kb-math-inline\">\\sigma^2_*</span> 可以分解为两部分：\n- <strong>随机不确定性（Aleatoric）</strong>：<span class=\"kb-math kb-math-inline\">\\frac{1}{M}\\sum_m \\sigma^2_{\\theta_m}(\\mathbf{x})</span>，即各网络预测方差的均值，反映数据本身的噪声\n- <strong>认知不确定性（Epistemic）</strong>：<span class=\"kb-math kb-math-inline\">\\frac{1}{M}\\sum_m \\mu^2_{\\theta_m}(\\mathbf{x}) - \\mu^2_*(\\mathbf{x})</span>，即各网络预测均值的方差，反映模型对预测的分歧</div>\n<p>这种分解使得 Deep Ensembles 能够区分\"数据本身就有噪声\"和\"模型不确定该怎么预测\"两种不同类型的不确定性。</p>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>MC-Dropout</th>\n<th>变分推断 (VI)</th>\n<th>Deep Ensembles</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>理论基础</td>\n<td>近似变分推断</td>\n<td>贝叶斯推断</td>\n<td>非贝叶斯（频率学派）</td>\n</tr>\n<tr>\n<td>实现复杂度</td>\n<td>低</td>\n<td>高</td>\n<td>低</td>\n</tr>\n<tr>\n<td>额外参数</td>\n<td>无</td>\n<td>2×</td>\n<td>M× 模型</td>\n</tr>\n<tr>\n<td>并行性</td>\n<td>差（顺序采样）</td>\n<td>差</td>\n<td>优（完全并行）</td>\n</tr>\n<tr>\n<td>超参数</td>\n<td>dropout 率</td>\n<td>先验、变分族</td>\n<td>M, ε</td>\n</tr>\n<tr>\n<td>多模态探索</td>\n<td>单模态附近</td>\n<td>单模态附近</td>\n<td>多模态</td>\n</tr>\n</tbody>\n</table></div>\n<p>Deep Ensembles 的核心优势在于：(1) 实现极其简单，只需对标准训练流程做最小修改；(2) 天然适合分布式计算；(3) 通过不同随机初始化自然探索损失函数的多个局部最优，比单模态近似方法更好地捕获模型不确定性。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "<strong>Temperature Scaling 方法</strong>：在已训练模型的 logits 上除以标量 <span class=\"kb-math kb-math-inline\">T</span>，再通过 softmax 得到校准后的概率；<span class=\"kb-math kb-math-inline\">T</span> 在验证集上最小化 NLL 求解",
      ...
      "detail": "<h5>示意图</h5>\n<p><img alt=\"校准对比：LeNet vs ResNet\" src=\"https://ar5iv.labs.arxiv.org/html/1706.04599/assets/x1.png\" />\n<em>图 1：5 层 LeNet（左）与 110 层 ResNet（右）在 CIFAR-100 上的置信度直方图（上）和可靠性图（下）。LeNet 的置信度分布较均匀且接近对角线（校准良好），而 ResNet 的置信度集中在高置信区间且严重偏离对角线（过度自信）。</em></p>\n<p><img alt=\"Miscalibration 影响因素\" src=\"https://ar5iv.labs.arxiv.org/html/1706.04599/assets/x3.png\" />\n<em>图 2：网络深度（最左）、宽度（中左）、Batch Normalization（中右）、权重衰减（最右）对 ECE 和测试误差的影响。增加深度/宽度、使用 BN 均降低测试误差但恶化校准；权重衰减不足同样导致 ECE 升高。</em></p>\n<p><img alt=\"校准前后对比\" src=\"https://ar5iv.labs.arxiv.org/html/1706.04599/assets/x5.png\" />\n<em>图 4：CIFAR-100 上校准前（最左）与各方法校准后的可靠性图。Temperature Scaling（最右）将预测概率拉回对角线，ECE 从 14.80% 降至 1.60%。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Temperature Scaling 校准流程\n# 输入：已训练模型 f(x)，验证集 D_val = {(x_i, y_i)}\n\n# Step 1: 提取 logits\nlogits = [f(x_i) for x_i in D_val]  # z_i ∈ R^K\n\n# Step 2: 在验证集上优化温度参数 T\nT = nn.Parameter(torch.ones(1) * 1.5)  # 初始化 T &gt; 0\noptimizer = optim.LBFGS([T], lr=0.01)\n\nfor _ in range(max_iter):\n    def closure():\n        scaled_logits = logits / T          # 温度缩放\n        loss = cross_entropy(scaled_logits, labels)  # NLL 损失\n        loss.backward()\n        return loss\n    optimizer.step(closure)\n\n# Step 3: 推理时使用校准后的概率\ndef calibrated_predict(x):\n    z = f(x)                    # 原始 logits\n    q = softmax(z / T)          # 校准后概率\n    confidence = max(q)         # 校准后置信度\n    prediction = argmax(q)      # 预测类别（与 argmax(z) 相同）\n    return prediction, confidence\n</code></pre>\n<h5>方法细节</h5>\n<p><strong>动机与背景：为什么需要校准？</strong></p>\n<p>在安全关键应用（自动驾驶、医疗诊断）中，模型不仅需要给出正确预测，还需要对预测的不确定性给出可靠估计。理想情况下，当模型声称\"90% 的置信度\"时，其预测应确实有 90% 的概率是正确的。这一性质称为<strong>完美校准（perfect calibration）</strong>，形式化定义为：</p>\n<div class=\"kb-math kb-math-display\">\\mathbb{P}(\\hat{Y} = Y \\mid \\hat{p} = p) = p, \\quad \\forall p \\in [0, 1]</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\hat{p}</span> 是模型输出的置信度（softmax 最大值），<span class=\"kb-math kb-math-inline\">\\hat{Y}</span> 是预测类别。然而，作者发现现代深度网络严重违反这一性质——它们系统性地<strong>过度自信</strong>：即使预测错误，输出的置信度仍然很高。</p>\n<div class=\"key-point\">💡 关键：早期的浅层网络（如 LeNet）校准性能反而较好，miscalibration 是随着网络变深变大而出现的\"现代病\"。</div>\n<p><strong>Miscalibration 的根源分析</strong></p>\n<p>论文通过控制变量实验揭示了四个关键因素：</p>\n<ol>\n<li>\n<p><strong>模型容量（深度与宽度）</strong>：增加网络层数或每层通道数可以降低分类错误率，但 ECE 同步恶化。更大的模型有更强的拟合能力，容易在训练集上将 NLL 压到极低，导致输出概率趋向 0/1 极端值。</p>\n</li>\n<li>\n<p><strong>Batch Normalization</strong>：BN 显著提升准确率，但也加剧了过度自信。这可能与 BN 改变了损失曲面的几何结构有关，使优化更容易到达 NLL 极低的区域。</p>\n</li>\n<li>\n<p><strong>权重衰减（Weight Decay）</strong>：减小权重衰减系数会降低测试误差，但大幅恶化校准。正则化不足使模型更容易过拟合训练集的 NLL。</p>\n</li>\n<li>\n<p><strong>NLL 过拟合</strong>：这是最直接的原因。作者观察到，在训练后期测试误差已经收敛，但测试 NLL 持续上升（过拟合），这意味着模型在提升准确率的同时，输出概率的质量在下降。</p>\n</li>\n</ol>\n<div class=\"warn-box\">⚠️ 注意：NLL 过拟合不等于分类过拟合。模型的测试准确率可能仍在改善，但概率估计的质量已经恶化——这是一种\"隐性过拟合\"。</div>\n<p><strong>校准度量：Expected Calibration Error (ECE)</strong></p>\n<p>由于完美校准的定义涉及连续概率值，实际中通过分箱近似来计算。将预测样本按置信度分入 <span class=\"kb-math kb-math-inline\">M</span> 个等宽区间 <span class=\"kb-math kb-math-inline\">B_1, \\ldots, B_M</span>，ECE 定义为：</p>\n<div class=\"kb-math kb-math-display\">\\text{ECE} = \\sum_{m=1}^{M} \\frac{|B_m|}{n} \\left| \\text{acc}(B_m) - \\text{conf}(B_m) \\right|</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\text{acc}(B_m)</span> 是第 <span class=\"kb-math kb-math-inline\">m</span> 个 bin 中样本的实际准确率，<span class=\"kb-math kb-math-inline\">\\text{conf}(B_m)</span> 是该 bin 中样本的平均置信度。ECE 越低，校准越好。对应的可视化工具是 <strong>reliability diagram</strong>：横轴为置信度区间，纵轴为实际准确率，完美校准对应对角线。</p>\n<p><strong>Temperature Scaling 的核心机制</strong></p>\n<p>Temperature Scaling 是 Platt Scaling 的极简特例。对于 <span class=\"kb-math kb-math-inline\">K</span> 分类问题，给定模型输出的 logit 向量 <span class=\"kb-math kb-math-inline\">\\mathbf{z}_i \\in \\mathbb{R}^K</span>，校准后的置信度为：</p>\n<div class=\"kb-math kb-math-display\">\\hat{q}_i = \\max_k \\, \\sigma_{\\text{SM}}\\left(\\frac{\\mathbf{z}_i}{T}\\right)_k</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\sigma_{\\text{SM}}</span> 是 softmax 函数，<span class=\"kb-math kb-math-inline\">T &gt; 0</span> 是温度参数。<span class=\"kb-math kb-math-inline\">T</span> 通过在验证集上最小化负对数似然（NLL）求解：</p>\n<div class=\"kb-math kb-math-display\">T^* = \\arg\\min_T \\, -\\sum_{i=1}^{n} \\log \\sigma_{\\text{SM}}\\left(\\frac{\\mathbf{z}_i}{T}\\right)_{y_i}</div>\n<p>这是一个关于单一标量 <span class=\"kb-math kb-math-inline\">T</span> 的凸优化问题，可以用 LBFGS 等方法高效求解。</p>\n<div class=\"key-point\">💡 关键：当 <span class=\"kb-math kb-math-inline\">T &gt; 1</span> 时，softmax 输出被\"软化\"（概率分布更均匀，置信度降低）；当 <span class=\"kb-math kb-math-inline\">T &lt; 1</span> 时，分布被\"锐化\"（置信度升高）。由于现代网络普遍过度自信，最优 <span class=\"kb-math kb-math-inline\">T</span> 通常大于 1。</div>\n<p><strong>为什么 Temperature Scaling 不改变准确率？</strong></p>\n<p>因为对所有 logit 除以同一个正数 <span class=\"kb-math kb-math-inline\">T</span> 不改变它们的大小排序：</p>\n<div class=\"kb-math kb-math-display\">\\arg\\max_k \\, \\sigma_{\\text{SM}}\\left(\\frac{\\mathbf{z}_i}{T}\\right)_k = \\arg\\max_k \\, z_{i,k}</div>\n<p>这意味着模型的 top-1 预测类别完全不变，Temperature Scaling 只是重新分配了各类别的概率值，使其更好地反映真实的不确定性。</p>\n<p><strong>与其他校准方法的对比</strong></p>\n<p>论文系统对比了以下方法：</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>类型</th>\n<th>参数量</th>\n<th>是否改变准确率</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Histogram Binning</td>\n<td>非参数</td>\n<td><span class=\"kb-math kb-math-inline\">M</span> 个 bin 边界</td>\n<td>是</td>\n</tr>\n<tr>\n<td>Isotonic Regression</td>\n<td>非参数</td>\n<td>单调映射</td>\n<td>是</td>\n</tr>\n<tr>\n<td>BBQ</td>\n<td>非参数</td>\n<td>贝叶斯 bin 组合</td>\n<td>是</td>\n</tr>\n<tr>\n<td>Platt Scaling</td>\n<td>参数化</td>\n<td><span class=\"kb-math kb-math-inline\">2K</span> (Matrix) / <span class=\"kb-math kb-math-inline\">2K</span> (Vector)</td>\n<td>可能</td>\n</tr>\n<tr>\n<td><strong>Temperature Scaling</strong></td>\n<td><strong>参数化</strong></td>\n<td><strong>1</strong></td>\n<td><strong>否</strong></td>\n</tr>\n</tbody>\n</table></div>\n<p>Platt Scaling 的一般形式为 <span class=\"kb-math kb-math-inline\">\\hat{q}_i = \\sigma_{\\text{SM}}(\\mathbf{W}\\mathbf{z}_i + \\mathbf{b})</span>，其中 Matrix Scaling 使 <span class=\"kb-math kb-math-inline\">\\mathbf{W}</span> 为对角矩阵（<span class=\"kb-math kb-math-inline\">K</span> 个参数），Vector Scaling 使 <span class=\"kb-math kb-math-inline\">\\mathbf{W} = \\text{diag}(\\mathbf{w})</span>（<span class=\"kb-math kb-math-inline\">K</span> 个参数加偏置）。Temperature Scaling 是其最简形式：<span class=\"kb-math kb-math-inline\">\\mathbf{W} = \\frac{1}{T}\\mathbf{I}</span>，<span class=\"kb-math kb-math-inline\">\\mathbf{b} = \\mathbf{0}</span>，仅 1 个参数。</p>\n<p>实验表明，在 CIFAR-10/100、ImageNet、SVHN 以及 Birds、Cars 等细粒度数据集上，Temperature Scaling 的 ECE 始终最低或接近最低，且由于参数极少，几乎不存在过拟合验证集的风险。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        ...
        "核心机制：<strong>非一致性度量（nonconformity measure）</strong> \\<span class=\"kb-math kb-math-inline\">A(B, z)\\</span> 量化样本相对于数据集的\"异常程度\"",
        "通过 <strong>p 值排名</strong> 构造预测区域：\\<span class=\"kb-math kb-math-inline\">\\Gamma_\\varepsilon = \\{z : p_z &gt; \\varepsilon\\}\\</span>",
        "<strong>有效性定理</strong>：对任意可交换序列，\\<span class=\"kb-math kb-math-inline\">\\Pr\\{z_n \\notin \\Gamma_\\varepsilon\\} \\leq \\varepsilon\\</span>",
        "可包装任意底层算法（最近邻、SVM、线性回归、神经网络等）为保形预测器",
        "<strong>最优性</strong>：在满足不变性、有效性、嵌套性三个条件的所有预测器中，保形预测器产生最小预测区域",
        "分类问题输出标签子集，回归问题输出预测区间",
        "关键概念：<strong>置信度（confidence）</strong> = \\<span class=\"kb-math kb-math-inline\">1-\\varepsilon\\</span>，<strong>可信度（credibility）</strong> = 最大使 \\<span class=\"kb-math kb-math-inline\">\\Gamma_\\varepsilon = \\emptyset\\</span> 的 \\<span class=\"kb-math kb-math-inline\">\\varepsilon\\</span>"
      ],
      "detail": "<h5>核心框架示意</h5>\n<p>Conformal Prediction 的核心思想可以概括为以下流程：</p>\n<pre><code>训练数据 {z₁,...,zₙ₋₁} + 候选新样本 zₙ\n         ↓\n计算所有样本的非一致性分数 αᵢ = A({z₁,...,zₙ}\\{zᵢ}, zᵢ)\n         ↓\n计算 p 值: p_z = #{i: αᵢ ≥ αₙ} / n\n         ↓\n构造预测区域: Γε = {z : p_z &gt; ε}\n         ↓\n输出具有 (1-ε) 覆盖率保证的预测区域\n</code></pre>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Conformal Prediction Algorithm (Transductive)\ndef conformal_predict(z_train, x_new, A, epsilon, Y_space):\n    &quot;&quot;&quot;\n    z_train: 训练样本 [(x1,y1), ..., (x_{n-1}, y_{n-1})]\n    x_new: 新对象的特征\n    A: 非一致性度量函数 A(bag, example) -&gt; score\n    epsilon: 显著性水平\n    Y_space: 标签空间（分类为有限集，回归为实数区间网格）\n    &quot;&quot;&quot;\n    prediction_region = []\n\n    for y in Y_space:\n        # Step 1: 假设新样本标签为 y\n        z_n = (x_new, y)\n        bag = z_train + [z_n]  # 所有 n 个样本\n\n        # Step 2: 计算所有非一致性分数\n        alphas = []\n        for i in range(len(bag)):\n            bag_without_i = bag[:i] + bag[i+1:]  # 去掉第 i 个\n            alpha_i = A(bag_without_i, bag[i])\n            alphas.append(alpha_i)\n\n        # Step 3: 计算 p 值\n        alpha_n = alphas[-1]  # 新样本的分数\n        p_y = sum(1 for a in alphas if a &gt;= alpha_n) / len(bag)\n\n        # Step 4: 判断是否纳入预测区域\n        if p_y &gt; epsilon:\n            prediction_region.append(y)\n\n    return prediction_region\n</code></pre>\n<h5>动机与背景</h5>\n<p>传统统计预测区间（如 Fisher 的正态预测区间）依赖强分布假设（独立性 + 正态性）。当这些假设不成立时，覆盖率保证失效。Conformal Prediction 的核心动机是：</p>\n<div class=\"key-point\">💡 <strong>关键洞察</strong>：能否在仅假设数据可交换（而非 i.i.d. 或正态）的条件下，为任意预测算法提供有限样本的覆盖率保证？</div>\n<p><strong>可交换性</strong>是比 i.i.d. 更弱的假设：随机变量序列 \\<span class=\"kb-math kb-math-inline\">z_1, \\ldots, z_n\\</span> 是可交换的，当且仅当对任意排列 \\<span class=\"kb-math kb-math-inline\">\\pi\\</span>，\\<span class=\"kb-math kb-math-inline\">(z_{\\pi(1)}, \\ldots, z_{\\pi(n)})\\</span> 与 \\<span class=\"kb-math kb-math-inline\">(z_1, \\ldots, z_n)\\</span> 同分布。i.i.d. 序列必然可交换，但可交换序列不必独立。</p>\n<h5>核心机制详解</h5>\n<p><strong>1. 非一致性度量 (Nonconformity Measure)</strong></p>\n<p>非一致性度量 \\<span class=\"kb-math kb-math-inline\">A(B, z)\\</span> 是一个函数，输入为一个样本袋 \\<span class=\"kb-math kb-math-inline\">B\\</span> 和一个样本 \\<span class=\"kb-math kb-math-inline\">z\\</span>，输出一个实数，衡量 \\<span class=\"kb-math kb-math-inline\">z\\</span> 相对于 \\<span class=\"kb-math kb-math-inline\">B\\</span> 的\"不一致程度\"。常见选择包括：</p>\n<ul>\n<li><strong>最近邻距离</strong>：\\<span class=\"kb-math kb-math-inline\">A(B, z) = \\min_{z&#x27; \\in B} d(z, z&#x27;)\\</span></li>\n<li><strong>残差</strong>（回归）：\\<span class=\"kb-math kb-math-inline\">A(B, (x,y)) = |y - \\hat{f}_B(x)|\\</span>，其中 \\<span class=\"kb-math kb-math-inline\">\\hat{f}_B\\</span> 是基于 \\<span class=\"kb-math kb-math-inline\">B\\</span> 训练的模型</li>\n<li><strong>到类均值距离</strong>（分类）：\\<span class=\"kb-math kb-math-inline\">A(B, (x,y)) = |\\bar{x}_{B,y} - x|\\</span></li>\n<li><strong>SVM 分离带</strong>：基于支持向量机分离超平面的位置给出分数</li>\n</ul>\n<div class=\"warn-box\">⚠️ 注意：非一致性度量的选择决定了预测区域的<strong>效率</strong>（大小），但不影响<strong>有效性</strong>（覆盖率保证）。任何非一致性度量都能保证覆盖率。</div>\n<p><strong>2. p 值计算与有效性保证</strong></p>\n<p>给定 \\<span class=\"kb-math kb-math-inline\">n\\</span> 个样本（包括假设的新样本），计算非一致性分数 \\<span class=\"kb-math kb-math-inline\">\\alpha_1, \\ldots, \\alpha_n\\</span>，新样本的 p 值为：</p>\n<div class=\"kb-math kb-math-display\">p_y = \\frac{\\#\\{i = 1, \\ldots, n \\mid \\alpha_i \\geq \\alpha_n\\}}{n}</div>\n<p><strong>有效性定理</strong>：若 \\<span class=\"kb-math kb-math-inline\">z_1, \\ldots, z_n\\</span> 可交换，则对任意 \\<span class=\"kb-math kb-math-inline\">\\varepsilon \\in (0,1)\\</span>：</p>\n<div class=\"kb-math kb-math-display\">\\Pr\\{p_{y_n} \\leq \\varepsilon\\} \\leq \\varepsilon</div>\n<p>等价地：</p>\n<div class=\"kb-math kb-math-display\">\\Pr\\{z_n \\in \\Gamma_\\varepsilon(z_1, \\ldots, z_{n-1})\\} \\geq 1 - \\varepsilon</div>\n<p><strong>证明直觉</strong>：可交换性意味着 \\<span class=\"kb-math kb-math-inline\">(\\alpha_1, \\ldots, \\alpha_n)\\</span> 也是可交换的。因此 \\<span class=\"kb-math kb-math-inline\">\\alpha_n\\</span> 在所有分数中排名最高的概率至多为 \\<span class=\"kb-math kb-math-inline\">1/n\\</span>，排名在前 \\<span class=\"kb-math kb-math-inline\">k\\</span> 的概率至多为 \\<span class=\"kb-math kb-math-inline\">k/n\\</span>。当 \\<span class=\"kb-math kb-math-inline\">p_y \\leq \\varepsilon\\</span> 时，意味着 \\<span class=\"kb-math kb-math-inline\">\\alpha_n\\</span> 的排名在前 \\<span class=\"kb-math kb-math-inline\">\\lfloor n\\varepsilon \\rfloor\\</span> 位，概率不超过 \\<span class=\"kb-math kb-math-inline\">\\varepsilon\\</span>。</p>\n<p><strong>3. 预测区域的构造</strong></p>\n<ul>\n<li><strong>分类问题</strong>：对标签空间 \\<span class=\"kb-math kb-math-inline\">\\mathcal{Y}\\</span> 中的每个标签 \\<span class=\"kb-math kb-math-inline\">y\\</span> 计算 \\<span class=\"kb-math kb-math-inline\">p_y\\</span>，预测区域为 \\<span class=\"kb-math kb-math-inline\">\\Gamma_\\varepsilon = \\{y : p_y &gt; \\varepsilon\\}\\</span></li>\n<li><strong>回归问题</strong>：对实数轴上的候选值计算 \\<span class=\"kb-math kb-math-inline\">p_y\\</span>，预测区域通常为一个区间</li>\n</ul>\n<p>对于分类，自然的报告方式是：\n- <strong>置信度</strong>：最大的 \\<span class=\"kb-math kb-math-inline\">1-\\varepsilon\\</span> 使得 \\<span class=\"kb-math kb-math-inline\">\\Gamma_\\varepsilon\\</span> 为单一标签\n- <strong>可信度</strong>：最大的 \\<span class=\"kb-math kb-math-inline\">\\varepsilon\\</span> 使得 \\<span class=\"kb-math kb-math-inline\">\\Gamma_\\varepsilon = \\emptyset\\</span>（低可信度表示新样本对该方法来说是异常的）</p>\n<p><strong>4. 最优性定理</strong></p>\n<p>设 \\<span class=\"kb-math kb-math-inline\">\\gamma\\</span> 是满足以下三个条件的任意区域预测器：\n1. <strong>不变性</strong>：预测不依赖训练样本的排列顺序\n2. <strong>有效性</strong>：\\<span class=\"kb-math kb-math-inline\">\\Pr\\{z_n \\in \\gamma_\\varepsilon\\} \\geq 1-\\varepsilon\\</span> 对所有可交换分布成立\n3. <strong>嵌套性</strong>：\\<span class=\"kb-math kb-math-inline\">\\varepsilon_1 \\geq \\varepsilon_2 \\Rightarrow \\gamma_{\\varepsilon_1} \\subseteq \\gamma_{\\varepsilon_2}\\</span></p>\n<p>则存在非一致性度量 \\<span class=\"kb-math kb-math-inline\">A\\</span> 使得保形预测器 \\<span class=\"kb-math kb-math-inline\">\\gamma^A\\</span> 满足 \\<span class=\"kb-math kb-math-inline\">\\gamma^A_\\varepsilon(B) \\subseteq \\gamma_\\varepsilon(B)\\</span> 对所有 \\<span class=\"kb-math kb-math-inline\">B\\</span> 和 \\<span class=\"kb-math kb-math-inline\">\\varepsilon\\</span> 成立。</p>\n<div class=\"key-point\">💡 <strong>直觉</strong>：保形预测器在满足有效性的所有预测器中产生最紧的预测区域——它是最优的。</div>\n<h5>与传统方法的对比</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th>Fisher 预测区间</th>\n<th>Conformal Prediction</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>分布假设</td>\n<td>正态 + 独立</td>\n<td>仅可交换性</td>\n</tr>\n<tr>\n<td>有效性</td>\n<td>渐近/精确（需假设成立）</td>\n<td>有限样本精确</td>\n</tr>\n<tr>\n<td>适用模型</td>\n<td>线性模型</td>\n<td>任意模型</td>\n</tr>\n<tr>\n<td>计算代价</td>\n<td>低</td>\n<td>较高（需遍历标签空间）</td>\n</tr>\n<tr>\n<td>预测区域形状</td>\n<td>固定（区间）</td>\n<td>自适应（可为任意集合）</td>\n</tr>\n</tbody>\n</table></div>\n<h5>实际应用示例</h5>\n<p>论文通过 Edgar Anderson 的鸢尾花数据集展示了三种非一致性度量的效果：</p>\n<ol>\n<li><strong>最近邻</strong>：\\<span class=\"kb-math kb-math-inline\">A(B,(x,y)) = \\min_{(x&#x27;,y&#x27;) \\in B, y&#x27;=y} |x-x&#x27;|\\</span></li>\n<li>\n<p>96% 置信度预测 versicolor，可信度 32%</p>\n</li>\n<li>\n<p><strong>到类均值距离</strong>：\\<span class=\"kb-math kb-math-inline\">A(B,(x,y)) = |\\bar{x}_{B,y} - x|\\</span></p>\n</li>\n<li>\n<p>96% 置信度预测 versicolor，可信度仅 8%</p>\n</li>\n<li>\n<p><strong>SVM 分离带</strong>：基于最优分离区间的位置</p>\n</li>\n<li>92% 置信度预测 versicolor，可信度 100%</li>\n</ol>\n<p>三种方法的有效性保证相同，但效率（预测区域大小）和可信度不同，体现了非一致性度量选择的重要性。</p>",
      "quiz": {
    ...
    },
    {
      ...
      "keyPoints": [
        "<strong>三种非一致性分数对比</strong>：Naive Coordinate（坐标空间 <span class=\"kb-math kb-math-inline\">L^\\infty</span>）、Standard Geodesic（测地距离 <span class=\"kb-math kb-math-inline\">d_{\\text{geo}}</span>）、Adaptive Geodesic（<span class=\"kb-math kb-math-inline\">d_{\\text{geo}}/\\hat\\sigma</span>）",
        "<strong>测地距离分数</strong>：利用流形内蕴距离替代欧氏残差，消除坐标图畸变导致的面积浪费",
        "<strong>局部难度估计器 <span class=\"kb-math kb-math-inline\">\\hat\\sigma(x)</span></strong>：通过 5-fold 交叉验证在训练集上训练 KNN 回归器，预测测地残差大小",
        "<strong>预测区域形式</strong>：以 <span class=\"kb-math kb-math-inline\">\\hat{y}(x)</span> 为中心、半径 <span class=\"kb-math kb-math-inline\">\\hat{q} \\cdot \\hat\\sigma(x)</span> 的测地球（球面帽），半径随局部难度自适应缩放",
        "<strong>分割保形框架</strong>：Train/Calibration/Test 三分，保证有限样本边际覆盖 <span class=\"kb-math kb-math-inline\">\\geq 1-\\alpha</span>",
        "<strong>实验验证</strong>：合成 <span class=\"kb-math kb-math-inline\">S^2</span> 球面（vMF 分布，50× 异质性）+ IGRF-14 地磁场真实数据",
        "<strong>关键结果</strong>：条件覆盖标准差降低 19%（合成）/71%（IGRF-14），最坏情况覆盖率提升 3–17 个百分点"
      ],
      "detail": "<h5>框架总览</h5>\n<p><img alt=\"Adaptive Geodesic CP 框架示意\" src=\"https://arxiv.org/html/2602.16015v1/x1.png\" />\n<em>图：三种预测区域对比。左：Naive Coordinate 在坐标空间构建 <span class=\"kb-math kb-math-inline\">L^\\infty</span> 矩形再投影（极点处面积膨胀）；中：Standard Geodesic 产生固定半径球面帽；右：Adaptive Geodesic 根据局部难度调整帽的半径。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Algorithm 1: Adaptive Geodesic Conformal Prediction on Manifolds\n# Input: D_train, D_cal, x_new, manifold M, significance α\n\n# Step 1: Train base predictor\ny_hat = KNN_regressor(k=20).fit(D_train)  # extrinsic mean → project to M\n\n# Step 2: Train difficulty estimator via 5-fold CV\nfor fold in 5_fold_split(D_train):\n    residuals_fold = [d_geo(y_hat_fold(x_i), y_i) for (x_i, y_i) in held_out]\nsigma_hat = KNN_regressor(k=20).fit(X_train, residuals_all_folds)\n\n# Step 3: Compute nonconformity scores on calibration set\nscores = [d_geo(y_hat(x_i), y_i) / sigma_hat(x_i) for (x_i, y_i) in D_cal]\n\n# Step 4: Compute conformal quantile\nq_hat = quantile(scores, level=ceil((1-α)(|D_cal|+1)) / |D_cal|)\n\n# Step 5: Construct prediction region for new point\nC(x_new) = {y ∈ M : d_geo(y_hat(x_new), y) ≤ q_hat * sigma_hat(x_new)}\n# This is a geodesic ball (spherical cap on S²) with adaptive radius\n</code></pre>\n<h5>动机与背景</h5>\n<p>传统保形预测（Conformal Prediction）假设数据位于欧氏空间，使用 <span class=\"kb-math kb-math-inline\">L^p</span> 范数构建预测区间/区域。当响应变量天然位于黎曼流形（如球面 <span class=\"kb-math kb-math-inline\">S^2</span> 上的方向数据、地磁场向量、旋转矩阵等）时，直接在坐标空间操作会引入两个问题：</p>\n<ol>\n<li><strong>坐标图畸变</strong>：球坐标 <span class=\"kb-math kb-math-inline\">(\\theta, \\phi)</span> 中的 <span class=\"kb-math kb-math-inline\">L^\\infty</span> 矩形投影到球面后，在极点附近面积急剧膨胀，导致不必要的覆盖浪费（实验中 Naive 方法面积多出 26%）。</li>\n<li><strong>条件覆盖不均匀</strong>：固定阈值的非一致性分数无法适应预测难度的空间异质性，导致\"容易\"区域过度覆盖而\"困难\"区域欠覆盖。</li>\n</ol>\n<div class=\"key-point\">💡 关键洞察：测地距离是流形上唯一与坐标选择无关的度量，用它作为非一致性分数可以保证预测区域面积的位置无关性。</div>\n<h5>核心机制详解</h5>\n<p><strong>1. 测地距离非一致性分数</strong></p>\n<p>在球面 <span class=\"kb-math kb-math-inline\">S^2</span> 上，两点 <span class=\"kb-math kb-math-inline\">p, q</span> 的测地距离为：</p>\n<div class=\"kb-math kb-math-display\">d_{\\text{geo}}(p, q) = \\arccos(\\langle p, q \\rangle)</div>\n<p>标准测地分数定义为：</p>\n<div class=\"kb-math kb-math-display\">s(x, y) = d_{\\text{geo}}(\\hat{y}(x), y)</div>\n<p>产生的预测区域是以 <span class=\"kb-math kb-math-inline\">\\hat{y}(x)</span> 为中心、半径 <span class=\"kb-math kb-math-inline\">\\hat{q}</span> 的测地球（球面帽）：</p>\n<div class=\"kb-math kb-math-display\">C(x) = \\{y \\in S^2 : d_{\\text{geo}}(\\hat{y}(x), y) \\leq \\hat{q}\\}</div>\n<p>球面帽面积为 <span class=\"kb-math kb-math-inline\">A = 2\\pi(1 - \\cos\\hat{q})</span>，仅依赖半径 <span class=\"kb-math kb-math-inline\">\\hat{q}</span>，与中心位置无关——这正是测地分数消除坐标畸变的几何原因。</p>\n<p><strong>2. 局部难度估计器</strong></p>\n<p>为实现条件覆盖自适应，引入难度函数 <span class=\"kb-math kb-math-inline\">\\hat\\sigma: \\mathcal{X} \\to \\mathbb{R}_{&gt;0}</span>，将分数归一化：</p>\n<div class=\"kb-math kb-math-display\">s_{\\text{adaptive}}(x, y) = \\frac{d_{\\text{geo}}(\\hat{y}(x), y)}{\\hat\\sigma(x)}</div>\n<p><span class=\"kb-math kb-math-inline\">\\hat\\sigma(x)</span> 的训练采用 5-fold 交叉验证策略：\n- 将训练集分为 5 折\n- 每折用其余 4 折训练临时预测器，计算该折样本的测地残差\n- 以所有折的 <span class=\"kb-math kb-math-inline\">(x_i, \\text{residual}_i)</span> 对训练 KNN 回归器</p>\n<div class=\"warn-box\">⚠️ 注意：<span class=\"kb-math kb-math-inline\">\\hat\\sigma</span> 必须仅在训练集上估计（通过 CV），不能使用校准集数据，否则会破坏交换性假设导致覆盖保证失效。</div>\n<p>归一化后的预测区域半径变为 <span class=\"kb-math kb-math-inline\">\\hat{q} \\cdot \\hat\\sigma(x)</span>：难度高的区域（<span class=\"kb-math kb-math-inline\">\\hat\\sigma</span> 大）获得更大的帽，难度低的区域获得更小的帽，从而均衡条件覆盖。</p>\n<p><strong>3. 覆盖保证</strong></p>\n<p>在交换性假设下，分割保形预测保证：</p>\n<div class=\"kb-math kb-math-display\">\\Pr[Y_{n+1} \\in C(X_{n+1})] \\geq 1 - \\alpha</div>\n<p>该保证对任意基础预测器 <span class=\"kb-math kb-math-inline\">\\hat{y}</span> 和难度估计器 <span class=\"kb-math kb-math-inline\">\\hat\\sigma</span> 均成立（有限样本、分布无关）。自适应分数不改变边际覆盖保证，但通过均衡化显著改善条件覆盖。</p>\n<p><strong>4. 与坐标方法的对比</strong></p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>特性</th>\n<th style=\"text-align: center;\">Naive Coordinate</th>\n<th style=\"text-align: center;\">Standard Geodesic</th>\n<th style=\"text-align: center;\">Adaptive Geodesic</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>分数函数</td>\n<td style=\"text-align: center;\"><span class=\"kb-math kb-math-inline\">\\|y - \\hat{y}\\|_\\infty</span> (坐标)</td>\n<td style=\"text-align: center;\"><span class=\"kb-math kb-math-inline\">d_{\\text{geo}}(\\hat{y}, y)</span></td>\n<td style=\"text-align: center;\"><span class=\"kb-math kb-math-inline\">d_{\\text{geo}}(\\hat{y}, y)/\\hat\\sigma(x)</span></td>\n</tr>\n<tr>\n<td>区域形状</td>\n<td style=\"text-align: center;\">坐标矩形→不规则球面块</td>\n<td style=\"text-align: center;\">固定半径球面帽</td>\n<td style=\"text-align: center;\">变半径球面帽</td>\n</tr>\n<tr>\n<td>面积一致性</td>\n<td style=\"text-align: center;\">✗（极点膨胀）</td>\n<td style=\"text-align: center;\">✓</td>\n<td style=\"text-align: center;\">✓（且自适应）</td>\n</tr>\n<tr>\n<td>条件覆盖</td>\n<td style=\"text-align: center;\">最差</td>\n<td style=\"text-align: center;\">中等</td>\n<td style=\"text-align: center;\">最优</td>\n</tr>\n</tbody>\n</table></div>\n<h5>实验结果</h5>\n<p><strong>Case 1: 合成球面数据</strong>（<span class=\"kb-math kb-math-inline\">n=1200</span>, vMF 分布 <span class=\"kb-math kb-math-inline\">\\kappa \\in [3, 150]</span>, 50× 异质性, 300 trials, <span class=\"kb-math kb-math-inline\">\\alpha=0.10</span>）</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th style=\"text-align: center;\">平均面积 (sr)</th>\n<th style=\"text-align: center;\">条件覆盖 Std</th>\n<th style=\"text-align: center;\">最坏 Bin 覆盖</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Adaptive Geodesic</td>\n<td style=\"text-align: center;\"><strong>1.865</strong></td>\n<td style=\"text-align: center;\"><strong>0.042</strong></td>\n<td style=\"text-align: center;\"><strong>0.839</strong></td>\n</tr>\n<tr>\n<td>Standard Geodesic</td>\n<td style=\"text-align: center;\">1.885</td>\n<td style=\"text-align: center;\">0.052</td>\n<td style=\"text-align: center;\">0.814</td>\n</tr>\n<tr>\n<td>Naive Coordinate</td>\n<td style=\"text-align: center;\">2.376</td>\n<td style=\"text-align: center;\">0.067</td>\n<td style=\"text-align: center;\">0.784</td>\n</tr>\n</tbody>\n</table></div>\n<p><strong>Case 2: IGRF-14 地磁场预测</strong>（<span class=\"kb-math kb-math-inline\">n=3000</span>, 100 trials, <span class=\"kb-math kb-math-inline\">r(\\hat\\sigma, \\text{residual})=0.516</span>）</p>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th style=\"text-align: center;\">平均面积 (sr)</th>\n<th style=\"text-align: center;\">条件覆盖 Std</th>\n<th style=\"text-align: center;\">最坏 Bin 覆盖</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Adaptive Geodesic</td>\n<td style=\"text-align: center;\"><strong>0.038</strong></td>\n<td style=\"text-align: center;\"><strong>0.031</strong></td>\n<td style=\"text-align: center;\"><strong>0.855</strong></td>\n</tr>\n<tr>\n<td>Standard Geodesic</td>\n<td style=\"text-align: center;\">0.039</td>\n<td style=\"text-align: center;\">0.107</td>\n<td style=\"text-align: center;\">0.689</td>\n</tr>\n<tr>\n<td>Naive Coordinate</td>\n<td style=\"text-align: center;\">0.046</td>\n<td style=\"text-align: center;\">0.060</td>\n<td style=\"text-align: center;\">0.805</td>\n</tr>\n</tbody>\n</table></div>\n<div class=\"key-point\">💡 关键发现：在 IGRF-14 数据上，自适应方法将条件覆盖标准差从 0.107 降至 0.031（降低 71%），最坏情况覆盖从 0.689 提升至 0.855（提升 16.6 个百分点）。Wilcoxon 检验 <span class=\"kb-math kb-math-inline\">p &lt; 4 \\times 10^{-18}</span>。</div>\n<h5>局限性与未来方向</h5>\n<ul>\n<li><strong>交换性假设</strong>：要求校准数据与测试数据可交换，时间序列等非平稳场景需扩展至非交换保形预测</li>\n<li><strong>各向同性约束</strong>：当前仅生成球面帽（各向同性），无法捕捉方向依赖的预测误差结构</li>\n<li><strong>流形限制</strong>：仅在 <span class=\"kb-math kb-math-inline\">S^2</span> 上验证，推广到一般黎曼流形需要高效的测地距离计算</li>\n<li><strong>难度估计器质量</strong>：当 <span class=\"kb-math kb-math-inline\">r(\\hat\\sigma, \\text{residual}) &lt; 0.15</span> 时，建议退化为 Standard Geodesic</li>\n</ul>",
      "quiz": {
