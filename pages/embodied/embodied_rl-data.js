/**
 * embodied_rl-data.js — 由 pipeline/build.py 于 2026-06-16 17:00:12 自动生成。
...
window.PAGE_CONFIG = {
  ...
  "algos": [
    ...
    },
    {
      ...
      "summary": "TRPO 提出用 KL 散度信任域约束策略更新，在最大化策略梯度替代目标的同时限制新旧策略距离，解决大步更新导致策略性能崩溃的问题。它把自然策略梯度、保守策略迭代和深度神经网络策略优化连接起来，成为后续 PPO 等算法的直接基础。",
      "keyPoints": [
        "<strong>单调改进下界</strong>：从 <span class=\"kb-math kb-math-inline\">L_{\\pi_{\\text{old}}}(\\pi)</span> 的局部替代目标出发，用 KL 距离惩罚给出策略改进下界",
        "<strong>信任域约束</strong>：将理论上的最大 KL 约束近似为采样状态上的平均 KL 约束 <span class=\"kb-math kb-math-inline\">\\bar{D}_{\\mathrm{KL}}(\\pi_{\\text{old}}\\|\\pi_\\theta)\\le \\delta</span>",
        "<strong>重要性采样替代目标</strong>：用旧策略采集轨迹，通过 <span class=\"kb-math kb-math-inline\">\\frac{\\pi_\\theta(a|s)}{\\pi_{\\theta_{\\text{old}}}(a|s)}A_{\\theta_{\\text{old}}}(s,a)</span> 估计新策略收益",
        "<strong>共轭梯度求解</strong>：用 Fisher 信息矩阵的 Hessian-vector product 近似自然梯度方向，不显式构造大矩阵",
        "<strong>线搜索防崩溃</strong>：沿近似方向回溯搜索，确保替代目标提升且 KL 约束满足",
        "<strong>两种采样方案</strong>：Single Path 可直接用于真实系统；Vine 需要仿真器可重置到中间状态，以更低方差估计优势",
        "<strong>实验覆盖广</strong>：在 MuJoCo 机器人运动和 Atari 图像输入任务中验证了稳定性与较少超参数调节需求"
      ],
      "detail": "<h5>方法示意图</h5>\n<p><img alt=\"TRPO single-path 采样示意\" src=\"https://ar5iv.labs.arxiv.org/html/1502.05477/assets/x1.png\" />\n<img alt=\"TRPO vine 采样示意\" src=\"https://ar5iv.labs.arxiv.org/html/1502.05477/assets/x2.png\" /></p>\n<p><em>图：TRPO 论文 Figure 1 的两种采样方式。Single Path 直接沿旧策略生成轨迹；Vine 从主干轨迹的若干状态分支 rollout，用更多局部动作评估降低方差。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Trust Region Policy Optimization\ntheta = initialize_policy()\n\nwhile not converged:\n    trajectories = rollout(policy=pi(theta_old))\n    advantages = estimate_advantage(trajectories)\n\n    # 重要性采样替代目标\n    def surrogate(theta):\n        ratio = pi(theta, a, s) / pi(theta_old, a, s)\n        return mean(ratio * advantages)\n\n    # 平均 KL 约束的二阶近似：0.5 * step.T @ F @ step &lt;= delta\n    g = grad(surrogate(theta_old))\n    step_dir = conjugate_gradient(Fisher_vector_product, g)\n    step_size = sqrt(2 * delta / (step_dir.T @ F @ step_dir))\n\n    # 回溯线搜索：同时检查 surrogate 改进和 KL 约束\n    for scale in [1.0, 0.5, 0.25, 0.125]:\n        theta_new = theta_old + scale * step_size * step_dir\n        if surrogate(theta_new) &gt; surrogate(theta_old) and mean_kl(theta_old, theta_new) &lt;= delta:\n            theta_old = theta_new\n            break\n</code></pre>\n<h5>动机与背景</h5>\n<p>标准策略梯度只告诉我们“朝哪个方向提高期望回报”，但没有给出“走多远才安全”。在深度策略网络中，参数空间的一小步可能让动作分布发生大变化；如果新策略把概率质量移动到优势估计不可靠的动作上，性能会突然下降。TRPO 的核心问题就是：如何在利用梯度样本效率的同时，为每次策略更新设置一个可计算的安全边界。</p>\n<p>论文从保守策略迭代出发，把新策略真实性能 <span class=\"kb-math kb-math-inline\">\\eta(\\tilde{\\pi})</span> 下界写成局部替代目标减去策略距离惩罚：</p>\n<div class=\"kb-math kb-math-display\">\\eta(\\tilde{\\pi}) \\ge L_{\\pi}(\\tilde{\\pi}) - C D_{\\mathrm{KL}}^{\\max}(\\pi,\\tilde{\\pi})</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">L_\\pi(\\tilde{\\pi})</span> 使用旧策略访问分布和旧策略优势函数来近似新策略收益。这个式子的直觉是：只要新策略在旧策略附近，状态分布变化带来的误差可由 KL 距离控制；因此最大化替代目标并限制 KL，就能避免过大的策略漂移。</p>\n<p>实际算法不能对所有状态施加最大 KL 约束，所以 TRPO 将其近似为样本上的平均 KL：</p>\n<div class=\"kb-math kb-math-display\">\\max_\\theta L_{\\theta_{\\text{old}}}(\\theta)\n\\quad \\text{s.t.} \\quad\n\\bar{D}_{\\mathrm{KL}}^{\\rho_{\\theta_{\\text{old}}}}(\\theta_{\\text{old}},\\theta)\\le \\delta</div>\n<p>替代目标进一步用旧策略采样得到：</p>\n<div class=\"kb-math kb-math-display\">L_{\\theta_{\\text{old}}}(\\theta)\n=\n\\mathbb{E}_{s,a\\sim\\pi_{\\theta_{\\text{old}}}}\n\\left[\n\\frac{\\pi_\\theta(a|s)}{\\pi_{\\theta_{\\text{old}}}(a|s)}\nA_{\\theta_{\\text{old}}}(s,a)\n\\right]</div>\n<p>求解时，TRPO 对目标做一阶近似、对 KL 做二阶近似，得到接近自然梯度的方向 <span class=\"kb-math kb-math-inline\">s\\approx F^{-1}g</span>。这里 <span class=\"kb-math kb-math-inline\">F</span> 是由平均 KL 的 Hessian 给出的 Fisher 信息矩阵。由于神经网络参数很多，论文使用共轭梯度，只需要计算 Fisher-vector product，而不显式保存 <span class=\"kb-math kb-math-inline\">F</span>。</p>\n<p>最后的线搜索是工程稳定性的关键。二阶近似只在局部成立，如果直接走满理论步长，非线性网络可能仍然违反 KL 约束或降低替代目标。因此 TRPO 会逐步缩短步长，直到真实 mini-batch 估计下的替代目标提升且平均 KL 小于 <span class=\"kb-math kb-math-inline\">\\delta</span>。这也是它比普通自然梯度更稳的原因。</p>\n<div class=\"key-point\">💡 关键：TRPO 不是简单“加 KL 正则”，而是把 KL 放进硬约束，并在每次更新后显式检查。这个设计用计算开销换来了策略更新幅度的可控性。</div>\n<h5>与传统策略梯度的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>更新约束</th>\n<th>样本复用</th>\n<th>主要风险</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Vanilla Policy Gradient</td>\n<td>学习率隐式控制</td>\n<td>低</td>\n<td>学习率敏感，容易震荡</td>\n</tr>\n<tr>\n<td>Natural Policy Gradient</td>\n<td>Fisher 度量下缩放梯度</td>\n<td>低</td>\n<td>步长仍需手调</td>\n</tr>\n<tr>\n<td>TRPO</td>\n<td>平均 KL 信任域硬约束</td>\n<td>中</td>\n<td>实现复杂，共轭梯度和线搜索开销较高</td>\n</tr>\n</tbody>\n</table></div>\n<p>TRPO 的优势在于把“步长”从参数空间转移到策略分布空间。对于机器人控制，动作分布变化比参数范数更接近真实行为变化，因此 KL 信任域比普通学习率更有意义。</p>",
      "quiz": {
        "q": "TRPO 中 KL 散度约束的主要作用是什么？",
        "options": [
          "减少策略网络参数量",
          "限制新旧策略分布差异，避免单次更新过大",
          "替代优势函数估计",
    ...
    },
    {
      ...
      "summary": "PPO 用裁剪后的概率比替代 TRPO 的复杂信任域约束，使策略更新既能多轮 mini-batch 复用数据，又能抑制新旧策略差异过大。它保留了 TRPO 稳定更新的直觉，但实现成本接近普通策略梯度。",
      "keyPoints": [
        "<strong>裁剪替代目标</strong>：用 <span class=\"kb-math kb-math-inline\">\\min(r_tA_t,\\mathrm{clip}(r_t,1-\\epsilon,1+\\epsilon)A_t)</span> 限制单样本策略改进幅度",
        "<strong>概率比控制</strong>：核心变量 <span class=\"kb-math kb-math-inline\">r_t(\\theta)=\\frac{\\pi_\\theta(a_t|s_t)}{\\pi_{\\theta_{\\text{old}}}(a_t|s_t)}</span>，直接刻画新旧策略对同一动作的概率变化",
        "<strong>多 epoch mini-batch 更新</strong>：同一批 rollout 数据可反复优化多轮，相比普通 on-policy 策略梯度样本效率更高",
        "<strong>两种近端方案</strong>：论文讨论 KL penalty 与 clipped surrogate，实践中 clipped PPO 更常用",
        "<strong>Actor-Critic 训练</strong>：策略损失、价值函数损失和 entropy bonus 通常联合优化",
        "<strong>GAE 优势估计</strong>：常与 Generalized Advantage Estimation 搭配，平衡方差与偏差",
        "<strong>基准广泛</strong>：在 MuJoCo 连续控制和 Atari 任务上取得优于或接近 TRPO 的效果，同时实现更简单、运行更快"
      ],
      "detail": "<h5>方法示意图</h5>\n<p><img alt=\"PPO 论文实验图直链\" src=\"https://ar5iv.labs.arxiv.org/html/1707.06347/assets/x1.png\" /></p>\n<p><em>图：ar5iv 提供的 PPO 论文图像资源。PPO 原文主要通过公式定义裁剪目标，未提供类似 TRPO 的框架示意图；因此方法理解重点在下方目标函数与伪代码。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># PPO-Clip\nfor iteration in range(num_iterations):\n    trajectories = rollout(pi_theta_old, horizon=T)\n    advantages = compute_gae(trajectories)\n    returns = advantages + values_old\n\n    for epoch in range(K):\n        for minibatch in split(trajectories):\n            ratio = pi_theta(a | s) / pi_theta_old(a | s)\n            unclipped = ratio * advantages\n            clipped = clip(ratio, 1 - eps, 1 + eps) * advantages\n            policy_loss = -mean(min(unclipped, clipped))\n\n            value_loss = mean((V_theta(s) - returns) ** 2)\n            entropy_bonus = mean(entropy(pi_theta(. | s)))\n            loss = policy_loss + c1 * value_loss - c2 * entropy_bonus\n            optimizer.step(loss)\n\n    theta_old = theta\n</code></pre>\n<h5>动机与背景</h5>\n<p>TRPO 的稳定性来自 KL 信任域，但它需要 Fisher-vector product、共轭梯度和线搜索，工程实现复杂，也不容易和包含共享网络、离散动作、循环结构的策略一起使用。PPO 的目标是保留“不要让新策略离旧策略太远”这一原则，同时把优化问题改成普通一阶优化器可以直接处理的损失函数。</p>\n<p>PPO 的核心变量是新旧策略概率比：</p>\n<div class=\"kb-math kb-math-display\">r_t(\\theta)=\\frac{\\pi_\\theta(a_t|s_t)}{\\pi_{\\theta_{\\text{old}}}(a_t|s_t)}</div>\n<p>如果 <span class=\"kb-math kb-math-inline\">A_t&gt;0</span>，说明旧策略采到的动作比平均更好，优化会倾向提高该动作概率；如果 <span class=\"kb-math kb-math-inline\">A_t&lt;0</span>，则倾向降低该动作概率。问题在于，普通替代目标 <span class=\"kb-math kb-math-inline\">r_t(\\theta)A_t</span> 可能把概率推得过远，导致和 TRPO 中同样的过大策略更新问题。</p>\n<p>PPO-Clip 将目标改为：</p>\n<div class=\"kb-math kb-math-display\">L^{\\mathrm{CLIP}}(\\theta)\n=\n\\mathbb{E}_t\n\\left[\n\\min\\left(\nr_t(\\theta)A_t,\n\\mathrm{clip}(r_t(\\theta),1-\\epsilon,1+\\epsilon)A_t\n\\right)\n\\right]</div>\n<p>这个 <span class=\"kb-math kb-math-inline\">\\min</span> 的设计非常关键。当 <span class=\"kb-math kb-math-inline\">A_t&gt;0</span> 时，如果 <span class=\"kb-math kb-math-inline\">r_t</span> 已经超过 <span class=\"kb-math kb-math-inline\">1+\\epsilon</span>，继续增大动作概率不会带来更多目标收益；当 <span class=\"kb-math kb-math-inline\">A_t&lt;0</span> 时，如果 <span class=\"kb-math kb-math-inline\">r_t</span> 已经低于 <span class=\"kb-math kb-math-inline\">1-\\epsilon</span>，继续压低概率也不会得到额外收益。裁剪不是硬性禁止策略变化，而是让“越界方向”的梯度消失，从而降低破坏性更新的诱因。</p>\n<p>训练流程上，PPO 仍是 on-policy：先用当前策略采样 rollout，再在这批数据上更新若干 epoch，然后丢弃旧数据。与 vanilla policy gradient 每条数据只用一次不同，PPO 的裁剪目标允许有限复用同一批样本，使得样本效率和 wall-clock 效率都更好。</p>\n<p>实际实现通常把策略损失和价值函数损失合并：</p>\n<div class=\"kb-math kb-math-display\">L_t(\\theta)=\n\\mathbb{E}_t\\left[\nL_t^{\\mathrm{CLIP}}(\\theta)\n-c_1(V_\\theta(s_t)-V_t^{\\mathrm{target}})^2\n+c_2\\mathcal{H}(\\pi_\\theta(\\cdot|s_t))\n\\right]</div>\n<p>价值函数负责降低优势估计方差，entropy bonus 防止策略过早坍缩。PPO 的简洁性也让它成为 RLHF、机器人控制和游戏智能体中非常常用的默认策略优化器。</p>\n<div class=\"warn-box\">⚠️ 注意：PPO 的 clip 并不等价于严格 KL 约束。实际训练中仍常监控 approximate KL，若 KL 过大则提前停止 epoch 或调小学习率。</div>\n<h5>与 TRPO 的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>TRPO</th>\n<th>PPO</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>更新限制</td>\n<td>显式平均 KL 约束</td>\n<td>裁剪概率比的软限制</td>\n</tr>\n<tr>\n<td>优化器</td>\n<td>共轭梯度 + 线搜索</td>\n<td>Adam/SGD 一阶优化</td>\n</tr>\n<tr>\n<td>实现复杂度</td>\n<td>高</td>\n<td>低</td>\n</tr>\n<tr>\n<td>数据复用</td>\n<td>有限</td>\n<td>多 epoch mini-batch</td>\n</tr>\n<tr>\n<td>理论保证</td>\n<td>更接近单调改进下界</td>\n<td>更偏工程近似</td>\n</tr>\n</tbody>\n</table></div>\n<p>PPO 的贡献不是提出一个全新的 RL 目标，而是把 TRPO 的信任域思想压缩成一个可微、可 mini-batch 优化的目标函数。这种折中让它在实践中成为最通用的 on-policy 强化学习算法之一。</p>",
      "quiz": {
        "q": "PPO-Clip 中 clipped surrogate 的主要效果是什么？",
        "options": [
          "让策略完全不再改变",
    ...
    },
    {
      ...
      "summary": "LFI-DR 将域随机化从手工设定参数范围推进到后验推断：用真实系统观测和仿真 rollout 的差异，通过似然无关推理估计物理参数分布，再从该后验中随机化训练策略。给定 `paper_url` 实际指向核物理论文而非该算法，以下解读基于 YAML 元信息、Domain Randomization 背景和 sim-to-real 中 likelihood-free / simulation-based inference 的通用方法组织。",
      "keyPoints": [
        "<strong>依据限制</strong>：<code>https://arxiv.org/abs/2602.05678</code> 标题为 “Three-body Effect in Short-range Correlations”，与 LFI-DR 元信息不匹配，未能获取匹配论文原文或原图",
        "<strong>后验式域随机化</strong>：将物理参数 <span class=\"kb-math kb-math-inline\">\\phi</span> 视为随机变量，目标是估计 <span class=\"kb-math kb-math-inline\">p(\\phi|x_{\\mathrm{real}})</span>，而不是固定均匀随机化范围",
        "<strong>似然无关推理</strong>：当 <span class=\"kb-math kb-math-inline\">p(x|\\phi)</span> 无法解析时，用仿真器采样 <span class=\"kb-math kb-math-inline\">\\phi\\sim p(\\phi)</span>、<span class=\"kb-math kb-math-inline\">x_{\\mathrm{sim}}\\sim \\mathrm{Sim}(\\phi)</span>，再用距离/分类器/密度比学习近似后验",
        "<strong>参数闭环更新</strong>：真实轨迹进入推断器，推断器输出后验，策略在后验覆盖的仿真环境中训练，再部署收集新真实轨迹",
        "<strong>降低无效随机化</strong>：相比宽范围 Domain Randomization，后验集中在真实系统可能参数上，提高样本效率并减少过度保守",
        "<strong>适用对象</strong>：摩擦、质量、阻尼、执行器延迟、接触刚度、传感器噪声等难以直接测量但可在仿真器中参数化的因素",
        "<strong>风险点</strong>：后验质量依赖 summary statistics、仿真器结构误差和真实数据覆盖；若仿真器缺少关键物理项，后验会自信但错误"
      ],
      "detail": "<h5>概念示意图</h5>\n<p><img alt=\"LFI-DR 概念流程图\" src=\"https://placehold.co/1200x480/png?text=LFI-DR+Posterior+Domain+Randomization\" /></p>\n<p><em>图：给定 URL 未提供匹配论文原图，上图为结构占位。LFI-DR 的核心流程可概括为：真实轨迹 → 似然无关参数后验 → 后验域随机化 → 策略训练 → 真实部署反馈。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># LFI-DR conceptual pipeline\nprior = p_phi()                         # 物理参数先验：质量、摩擦、延迟等\nposterior = prior\n\nfor round in range(num_rounds):\n    # 1. 用当前后验随机化仿真，生成参数-轨迹样本\n    sim_data = []\n    for k in range(num_simulations):\n        phi = sample(posterior)\n        traj = simulator.rollout(policy=current_policy, physics=phi)\n        sim_data.append((phi, summary(traj)))\n\n    # 2. 通过 likelihood-free inference 拟合 p(phi | summary(real))\n    inference_model.fit(sim_data)\n    z_real = summary(real_robot_rollouts())\n    posterior = inference_model.condition(z_real)\n\n    # 3. 用后验分布进行域随机化训练\n    for update in range(policy_updates):\n        phi = sample(posterior)\n        batch = simulator.rollout(policy=current_policy, physics=phi)\n        current_policy = rl_update(current_policy, batch)\n</code></pre>\n<h5>动机与背景</h5>\n<p>经典 Domain Randomization 的核心假设是：只要随机化范围足够宽，真实世界就会落在仿真训练分布里。但这个假设在接触丰富的机器人任务中代价很高。摩擦、软接触、执行器延迟等参数若随机得过窄，策略无法迁移；随机得过宽，训练分布包含大量不可能的物理世界，策略会变得保守且学习效率低。</p>\n<p>LFI-DR 的动机是把“随机化范围怎么选”变成统计推断问题。设真实观测为 <span class=\"kb-math kb-math-inline\">x_{\\mathrm{real}}</span>，仿真物理参数为 <span class=\"kb-math kb-math-inline\">\\phi</span>，理想目标是：</p>\n<div class=\"kb-math kb-math-display\">p(\\phi|x_{\\mathrm{real}})\\propto p(x_{\\mathrm{real}}|\\phi)p(\\phi)</div>\n<p>难点在于 <span class=\"kb-math kb-math-inline\">p(x|\\phi)</span> 通常不可写出：仿真器可以前向生成轨迹，却不会返回轨迹的解析似然。似然无关推理（Likelihood-Free Inference，也常称 simulation-based inference）正适合这种“能采样、不能写似然”的场景。</p>\n<p>一种简单形式是 ABC（Approximate Bayesian Computation）：从先验采样 <span class=\"kb-math kb-math-inline\">\\phi</span>，仿真得到 <span class=\"kb-math kb-math-inline\">x_{\\mathrm{sim}}</span>，如果摘要统计距离 <span class=\"kb-math kb-math-inline\">d(S(x_{\\mathrm{sim}}),S(x_{\\mathrm{real}}))&lt;\\epsilon</span>，就接受该 <span class=\"kb-math kb-math-inline\">\\phi</span>。更现代的做法会训练条件密度估计器、神经后验估计器或分类器密度比模型，使推断器输出连续后验而不是只接受/拒绝样本。</p>\n<p>后验式随机化的关键收益在于把训练分布从人工大盒子变成数据约束分布：</p>\n<div class=\"kb-math kb-math-display\">\\phi \\sim q(\\phi|x_{\\mathrm{real}}), \\qquad\n\\pi^*=\\arg\\max_\\pi \\mathbb{E}_{\\phi\\sim q}\\left[J_{\\mathrm{sim}}(\\pi;\\phi)\\right]</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">q</span> 是 LFI 得到的近似后验。若真实系统信息足够，<span class=\"kb-math kb-math-inline\">q</span> 会比原始先验更集中，策略不必在大量无关物理配置上浪费能力；若信息不足，后验仍保留不确定性，训练仍具备鲁棒性。</p>\n<div class=\"key-point\">💡 关键：LFI-DR 不是“用真实数据拟合一个单点仿真参数”，而是拟合参数后验。后验的不确定性本身就是域随机化分布。</div>\n<h5>与传统 Domain Randomization 的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>传统 DR</th>\n<th>LFI-DR</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>参数分布</td>\n<td>人工设定，多为均匀分布</td>\n<td>由真实观测推断后验</td>\n</tr>\n<tr>\n<td>真实数据使用</td>\n<td>可为零样本，也可只用于验证</td>\n<td>直接用于更新随机化分布</td>\n</tr>\n<tr>\n<td>主要风险</td>\n<td>范围过宽/过窄</td>\n<td>后验受摘要统计和仿真偏差影响</td>\n</tr>\n<tr>\n<td>策略训练</td>\n<td>在固定随机化分布上训练</td>\n<td>可随真实部署数据迭代收缩或修正</td>\n</tr>\n</tbody>\n</table></div>\n<p>在具身强化学习中，这类方法特别适合“可观测轨迹很少但仿真可大量采样”的设置。它把真实系统辨识和策略鲁棒训练合并为一个闭环：先推断真实世界可能在哪些物理参数区域，再把策略训练集中到这些区域。</p>\n<h5>依据限制说明</h5>\n<p>由于清单中的 <code>paper_url</code> 与算法名称、机构和动机不匹配，无法确认 LFI-DR 是否已有公开论文、原始公式或实验设置。本文中的公式和伪代码是基于 YAML 元信息和 sim-to-real 中 LFI/SBI + Domain Randomization 的标准范式抽象，不能替代原文细节。</p>",
      "quiz": {
        "q": "LFI-DR 相比普通域随机化最核心的变化是什么？",
        "options": [
          "完全取消仿真训练",
    ...
    },
    {
      ...
      "summary": "HDMI 可理解为面向人形机器人交互技能的互联网视频模仿学习框架：从视频中抽取人体、物体和接触关系，将其 retarget 到人形机器人，再通过仿真模仿强化学习获得可执行策略。给定 `paper_url` 实际指向理论物理论文而非该算法，以下解读基于 YAML 元信息和视频模仿/人形控制的公开通用范式完成。",
      "keyPoints": [
        "<strong>依据限制</strong>：<code>https://arxiv.org/abs/2602.12345</code> 标题为 “Lazarides-Shafi axion models as Dijkgraaf-Witten theories”，与 HDMI 元信息不匹配，未能获取匹配论文原文或原图",
        "<strong>互联网视频作为示范源</strong>：从大规模公开视频中挖掘坐、推、拿、开门、搬运等全身交互行为",
        "<strong>三类中间表示</strong>：人体姿态/根轨迹、物体 6D 轨迹、接触事件或手脚接触标签",
        "<strong>人形 retargeting</strong>：把人类骨架动作映射到机器人关节，约束足底接触、质心稳定、关节限位和自碰撞",
        "<strong>交互模仿奖励</strong>：同时奖励身体姿态跟踪、末端执行器轨迹、物体状态变化和接触时序一致性",
        "<strong>仿真到真实迁移</strong>：结合动力学随机化、扰动注入和低层 PD/力控接口，让策略在真实人形机器人上更稳",
        "<strong>区别于纯运动模仿</strong>：HDMI 的重点不是复现人体动作本身，而是复现“身体动作如何改变外部物体”"
      ],
      "detail": "<h5>概念示意图</h5>\n<p><img alt=\"HDMI 概念流程图\" src=\"https://placehold.co/1200x480/png?text=HDMI+Video-to-Humanoid+Interaction+Imitation\" /></p>\n<p><em>图：给定 URL 未提供匹配论文原图，上图为结构占位。HDMI 的核心流程可抽象为：互联网视频 → 3D 人体/物体/接触重建 → 人形 retargeting → 仿真模仿 RL → 真实交互。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># HDMI conceptual pipeline\nvideos = crawl_interaction_videos(keywords=[&quot;sit&quot;, &quot;push&quot;, &quot;open&quot;, &quot;carry&quot;])\ndemos = []\n\nfor video in videos:\n    human_pose = estimate_3d_human_motion(video)\n    object_traj = estimate_object_pose(video)\n    contacts = infer_contact_events(human_pose, object_traj)\n    robot_motion = retarget_to_humanoid(\n        human_pose,\n        constraints=[feet_contact, joint_limits, balance, contacts],\n    )\n    demos.append((robot_motion, object_traj, contacts))\n\npolicy = initialize_humanoid_policy()\nfor update in range(num_updates):\n    demo = sample(demos)\n    rollout = simulate(policy, randomized_physics=True)\n    reward = (\n        w_pose * pose_tracking(rollout, demo.robot_motion)\n        + w_ee * end_effector_tracking(rollout, demo)\n        + w_obj * object_state_match(rollout, demo.object_traj)\n        + w_contact * contact_timing_match(rollout, demo.contacts)\n        - w_energy * action_penalty(rollout)\n    )\n    policy = rl_update(policy, rollout, reward)\n</code></pre>\n<h5>动机与背景</h5>\n<p>人形机器人要完成真实交互任务，难点不只是“走得像人”，而是要让全身运动、接触力和物体状态变化协调起来。传统 motion imitation 数据多来自动作捕捉，质量高但规模小、场景单一；互联网视频规模大，但没有机器人关节标签、力信息和精确 3D 状态。HDMI 的动机就是把大规模视频中的交互先验转化为可被机器人策略学习的中间表示。</p>\n<p>一个合理的 HDMI 管线会先把视频转成结构化示范：</p>\n<div class=\"kb-math kb-math-display\">v \\rightarrow \\{q^{\\mathrm{human}}_{1:T},\\; o_{1:T},\\; c_{1:T}\\}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">q^{\\mathrm{human}}</span> 是人体姿态，<span class=\"kb-math kb-math-inline\">o_t</span> 是物体状态，<span class=\"kb-math kb-math-inline\">c_t</span> 是接触事件。相比只跟踪人体姿态，接触和物体状态是交互任务的核心，因为机器人最终要改变的是外部世界，而不是在空中复现一段动作。</p>\n<p>retargeting 阶段需要解决身体结构差异。人类动作不能直接投到人形机器人关节上，否则会产生足底滑动、质心越界、关节超限或手部无法接触物体等问题。通常会优化：</p>\n<div class=\"kb-math kb-math-display\">\\min_{q^{\\mathrm{robot}}_{1:T}}\n\\sum_t\n\\left[\n\\|f_{\\mathrm{ee}}(q_t)-x^{\\mathrm{human}}_{\\mathrm{ee},t}\\|^2\n+ \\lambda_c \\mathcal{L}_{\\mathrm{contact}}\n+ \\lambda_b \\mathcal{L}_{\\mathrm{balance}}\n+ \\lambda_j \\mathcal{L}_{\\mathrm{joint}}\n\\right]</div>\n<p>模仿强化学习阶段再把 retargeted motion 转成闭环策略。闭环策略比直接播放轨迹更重要，因为真实交互中的物体位置、摩擦、接触时机都会有偏差。奖励通常由姿态跟踪、末端位置、物体目标和接触一致性组成：</p>\n<div class=\"kb-math kb-math-display\">r_t =\nw_p r^{\\mathrm{pose}}_t\n+w_e r^{\\mathrm{ee}}_t\n+w_o r^{\\mathrm{object}}_t\n+w_c r^{\\mathrm{contact}}_t\n-w_a\\|a_t\\|^2</div>\n<div class=\"key-point\">💡 关键：HDMI 类方法的价值在于把“视频里的人做了什么”拆成机器人可优化的状态、接触和目标，而不是直接让机器人逐帧模仿像素或人体骨架。</div>\n<h5>与纯视频模仿的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>纯视频动作模仿</th>\n<th>HDMI 式交互模仿</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>学习目标</td>\n<td>人体姿态相似</td>\n<td>物体变化和接触成功</td>\n</tr>\n<tr>\n<td>数据表示</td>\n<td>视频帧或人体关键点</td>\n<td>人体姿态 + 物体轨迹 + 接触标签</td>\n</tr>\n<tr>\n<td>控制对象</td>\n<td>多为身体运动</td>\n<td>全身运动与环境交互</td>\n</tr>\n<tr>\n<td>sim-to-real 重点</td>\n<td>姿态稳定</td>\n<td>接触鲁棒性、物体动力学、延迟</td>\n</tr>\n</tbody>\n</table></div>\n<p>真正落地时，HDMI 还需要处理视频中的遮挡、相机运动、物体尺度不确定和不可观测接触力。可行做法通常是引入多候选重建、用物理仿真筛掉不可行轨迹，并在 RL 训练中加入随机扰动让策略学会从偏差中恢复。</p>\n<h5>依据限制说明</h5>\n<p>由于清单中的 <code>paper_url</code> 与 HDMI 不匹配，无法确认原论文中的模型结构、实验机器人或具体指标。本文将 HDMI 解释为“互联网视频到人形机器人交互技能”的算法族抽象，保留 YAML 中的动机，不声称包含原文未公开的实验结论。</p>",
      "quiz": {
        "q": "HDMI 类人形交互模仿方法相比只模仿人体姿态，最需要额外建模什么？",
        "options": [
          "视频压缩码率",
          "物体轨迹与接触事件",
          "图像背景颜色",
    ...
    },
    {
      ...
      "summary": "LIDE 可理解为将任务规划/运动规划约束注入扩散策略的双臂接触控制方法：扩散模型负责生成多模态动作轨迹，规划器在采样过程中提供可达性、碰撞、接触和目标约束引导。给定 `paper_url` 实际指向文学大模型论文而非该算法，以下内容基于 YAML 元信息和 diffusion policy + planning guidance 的通用技术路线整理。",
      "keyPoints": [
        "<strong>依据限制</strong>：<code>https://arxiv.org/abs/2602.15678</code> 标题为 “Revisiting Northrop Frye's Four Myths Theory with Large Language Models”，与 LIDE 元信息不匹配",
        "<strong>扩散轨迹生成</strong>：用条件扩散模型表示 <span class=\"kb-math kb-math-inline\">p(\\tau|o,g)</span>，生成双臂动作或末端轨迹，天然支持多峰解",
        "<strong>规划引导采样</strong>：在 denoising 每一步加入来自规划代价的梯度或重打分，如碰撞、可达性、同步抓取、接触顺序",
        "<strong>双臂接触建模</strong>：显式考虑两臂协同、闭链约束、接触切换和物体稳定性",
        "<strong>仿真训练与迁移</strong>：在随机化动力学和接触参数下训练，减少真实部署时摩擦/刚度误差影响",
        "<strong>与纯 Diffusion Policy 区别</strong>：不是只拟合示范分布，而是在采样时用规划器把生成结果推向物理可行且任务可达的区域",
        "<strong>典型任务</strong>：双臂搬运、插拔、开合、推拉、协同装配等长接触序列任务"
      ],
      "detail": "<h5>概念示意图</h5>\n<p><img alt=\"LIDE 概念流程图\" src=\"https://placehold.co/1200x480/png?text=LIDE+Planning-Guided+Diffusion+for+Bimanual+Contact\" /></p>\n<p><em>图：给定 URL 未提供匹配论文原图，上图为结构占位。LIDE 的抽象流程是：观测/目标条件 → 扩散轨迹采样 → 规划代价引导 → 低层控制执行 → 接触反馈修正。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># LIDE conceptual planning-guided diffusion\ndiffusion = train_diffusion_policy(demos, condition=[&quot;observation&quot;, &quot;goal&quot;])\nplanner = build_planner(costs=[&quot;collision&quot;, &quot;reachability&quot;, &quot;contact&quot;, &quot;goal&quot;])\n\ndef sample_action_sequence(obs, goal):\n    x = gaussian_noise(shape=(horizon, action_dim))\n    for k in reversed(range(num_diffusion_steps)):\n        score = diffusion.score(x, obs, goal, step=k)\n        plan_grad = grad(planner.cost(x, obs, goal), x)\n        x = denoise_step(x, score - guidance_scale * plan_grad, step=k)\n        x = project_to_constraints(x, planner.hard_constraints)\n    return x\n\nfor control_step in range(T):\n    action_seq = sample_action_sequence(current_obs, task_goal)\n    execute(action_seq[0])\n    current_obs = observe()\n</code></pre>\n<h5>动机与背景</h5>\n<p>双臂接触任务的困难来自两个方面：一是动作解高度多模态，例如同一个物体可以从不同侧抓取、不同顺序插入；二是物理约束很硬，轻微碰撞、闭链误差或接触时机错误都会让任务失败。传统规划器能处理约束，但在高维连续动作和复杂接触中搜索困难；纯行为克隆或扩散策略能学习示范分布，却可能生成看似合理但不可达或碰撞的动作。</p>\n<p>LIDE 式方法的核心思想是把两者结合。扩散模型学习示范轨迹分布：</p>\n<div class=\"kb-math kb-math-display\">p_\\theta(\\tau|o,g)</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\tau</span> 可以是双臂末端轨迹、关节轨迹或低层动作序列，<span class=\"kb-math kb-math-inline\">o</span> 是当前观测，<span class=\"kb-math kb-math-inline\">g</span> 是任务目标。扩散模型通过从噪声逐步 denoise 生成轨迹，因此每一步都可以被外部代价函数引导。</p>\n<p>规划器定义可微或可近似求梯度的代价：</p>\n<div class=\"kb-math kb-math-display\">C(\\tau)=\n\\lambda_{\\mathrm{goal}}C_{\\mathrm{goal}}\n+\\lambda_{\\mathrm{col}}C_{\\mathrm{collision}}\n+\\lambda_{\\mathrm{reach}}C_{\\mathrm{reach}}\n+\\lambda_{\\mathrm{contact}}C_{\\mathrm{contact}}\n+\\lambda_{\\mathrm{sync}}C_{\\mathrm{bimanual}}</div>\n<p>采样时不只是沿扩散模型 score <span class=\"kb-math kb-math-inline\">\\nabla_\\tau \\log p_\\theta(\\tau|o,g)</span> 走，而是加入规划引导：</p>\n<div class=\"kb-math kb-math-display\">\\tilde{s}(\\tau)\n=\n\\nabla_\\tau \\log p_\\theta(\\tau|o,g)\n- \\eta \\nabla_\\tau C(\\tau)</div>\n<p>这样生成轨迹既保持示范分布的自然性，又被推向满足任务约束的低代价区域。对于不可微约束，也可以采用候选轨迹重采样、MPC 打分或投影步骤来实现规划引导。</p>\n<p>在双臂任务中，规划代价应特别关注闭链与同步。比如搬运刚性物体时，两只手的相对位姿必须维持物体几何；插入任务中，一只手固定、另一只手施加位移时接触力方向必须稳定。这些约束很难仅靠示范模仿自动学到，规划引导能在推理时持续纠偏。</p>\n<div class=\"key-point\">💡 关键：扩散模型解决“生成多种可行候选”，规划器解决“筛掉物理不可行候选”。LIDE 的价值在于把规划代价放进采样过程，而不是采样后才简单选择。</div>\n<h5>与传统规划和 Diffusion Policy 的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>优点</th>\n<th>局限</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>传统运动规划</td>\n<td>约束清晰，可解释</td>\n<td>高维接触搜索成本高，示范先验弱</td>\n</tr>\n<tr>\n<td>Diffusion Policy</td>\n<td>多模态、动作自然</td>\n<td>可能生成不可达/碰撞轨迹</td>\n</tr>\n<tr>\n<td>LIDE 式规划引导扩散</td>\n<td>多模态生成 + 约束引导</td>\n<td>依赖规划代价设计和实时采样效率</td>\n</tr>\n</tbody>\n</table></div>\n<p>sim-to-real 阶段，LIDE 需要把接触参数、物体质量、摩擦和控制延迟纳入随机化。否则扩散轨迹在仿真中可行，真实接触中仍可能因为力学偏差失败。一个实用实现通常会在 MPC 执行层加入短视野反馈，让每次只执行轨迹前几步并持续重采样。</p>\n<h5>依据限制说明</h5>\n<p>由于清单中的 <code>paper_url</code> 与 LIDE 不匹配，无法确认原论文的命名、公式、实验或机器人平台。本文只依据 YAML 中“规划引导扩散解决双臂接触任务”的元信息，给出符合该技术路线的精读式结构化说明。</p>",
      "quiz": {
        "q": "LIDE 式规划引导扩散中，规划器最主要在采样阶段提供什么？",
        "options": [
          "随机噪声初始化",
    ...
    },
    {
      ...
      "summary": "BCQ 提出在离线/固定批数据中只从与数据集相似的动作集合里选择动作，用条件 VAE 生成候选动作、扰动网络做小范围修正、双 Q 网络估值，从而缓解 DDPG 等 off-policy 方法在数据集外动作上的外推误差。",
      "keyPoints": [
        "<strong>外推误差定义</strong>：离线数据未覆盖的 <span class=\"kb-math kb-math-inline\">(s,a)</span> 会被函数逼近器错误估值，最大化 Q 值会持续选择这些错误高估动作",
        "<strong>批量约束策略</strong>：策略应诱导与 batch 中状态-动作访问分布相近的行为，避免查询远离数据支撑的动作",
        "<strong>条件 VAE 行为模型</strong>：学习 <span class=\"kb-math kb-math-inline\">G_\\omega(s)</span> 生成数据集中给定状态下可能出现的动作",
        "<strong>扰动模型</strong>：<span class=\"kb-math kb-math-inline\">\\xi_\\phi(s,a,\\Phi)</span> 只允许在 <span class=\"kb-math kb-math-inline\">[-\\Phi,\\Phi]</span> 小范围调整生成动作，兼顾约束和改进",
        "<strong>候选动作最大化</strong>：对每个状态采样 <span class=\"kb-math kb-math-inline\">n</span> 个 VAE 动作，经扰动后选择 Q 值最高的动作",
        "<strong>Clipped Double Q 变体</strong>：训练两个 Q 网络，用加权 min/max target 减少不确定未来状态的过估计",
        "<strong>连续控制离线 RL</strong>：在 MuJoCo 的固定 batch、并行学习、专家模仿和不完美示范设置中显著优于 DDPG/BC 等基线"
      ],
      "detail": "<h5>方法示意图</h5>\n<p><img alt=\"BCQ 外推误差实验图\" src=\"https://ar5iv.labs.arxiv.org/html/1812.02900/assets/x1.png\" />\n<img alt=\"BCQ 结果图\" src=\"https://ar5iv.labs.arxiv.org/html/1812.02900/assets/x2.png\" /></p>\n<p><em>图：BCQ 论文中的外推误差现象与 BCQ 对比结果。标准 off-policy 深度 RL 在固定数据上会高估未见动作并性能崩溃；BCQ 通过 batch constraint 稳定价值估计。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Batch-Constrained deep Q-learning\nfor gradient_step in range(num_steps):\n    s, a, r, s_next = sample(D)\n\n    # 1. 训练条件 VAE 重构数据动作\n    z = encoder(s, a)\n    a_recon = decoder(s, z)\n    loss_vae = mse(a_recon, a) + beta * kl(q(z | s, a), Normal(0, 1))\n\n    # 2. 在下一状态生成候选动作并小范围扰动\n    candidates = []\n    for i in range(n):\n        a_i = decoder(s_next, clipped_normal_noise())\n        a_tilde = a_i + perturb_target(s_next, a_i, Phi)\n        candidates.append(clip_action(a_tilde))\n\n    # 3. 用双 Q 的加权保守 target 更新 critic\n    q_target = max_over_candidates(\n        lambda_ * min(Q1_target, Q2_target) + (1 - lambda_) * max(Q1_target, Q2_target)\n    )\n    y = r + gamma * q_target\n    loss_q = mse(Q1(s, a), y) + mse(Q2(s, a), y)\n\n    # 4. 扰动网络只学习在 VAE 动作附近提升 Q\n    a_gen = decoder(s, clipped_normal_noise())\n    loss_perturb = -Q1(s, a_gen + perturb(s, a_gen, Phi))\n</code></pre>\n<h5>动机与背景</h5>\n<p>离线 RL 的核心矛盾是：Q-learning 的 Bellman target 需要在下一状态上取最大动作，但离线数据只覆盖行为策略访问过的动作。对于数据集外动作，神经 Q 函数只能外推，估值可能毫无依据。一旦某个 OOD 动作被误高估，策略改进会更倾向选择它，而离线设置又无法通过真实交互纠正这个错误。</p>\n<p>BCQ 把这个问题称为 extrapolation error，并提出一个直接原则：策略应该只在 batch 支撑内或其邻域内改进。对于表格情形，批量约束 Q-learning 的更新是：</p>\n<div class=\"kb-math kb-math-display\">Q(s,a)\\leftarrow (1-\\alpha)Q(s,a)\n+\\alpha\\left(r+\\gamma\\max_{a&#x27;\\ \\mathrm{s.t.}\\ (s&#x27;,a&#x27;)\\in\\mathcal{B}}Q(s&#x27;,a&#x27;)\\right)</div>\n<p>也就是最大化只在数据集中出现过的动作上进行。连续动作空间不能枚举“出现过的动作”，因此 BCQ 用条件 VAE 近似行为数据的动作分布 <span class=\"kb-math kb-math-inline\">P_\\mathcal{B}(a|s)</span>。</p>\n<p>VAE 由编码器 <span class=\"kb-math kb-math-inline\">E_{\\omega_1}(s,a)</span> 和解码器 <span class=\"kb-math kb-math-inline\">D_{\\omega_2}(s,z)</span> 组成，训练目标为：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\mathrm{VAE}}\n=\n\\|D_{\\omega_2}(s,z)-a\\|^2\n+\\lambda D_{\\mathrm{KL}}(\\mathcal{N}(\\mu,\\sigma)\\|\\mathcal{N}(0,I))</div>\n<p>推理时，从 VAE 采样多个候选动作。为了避免纯生成模型只能模仿、不能改进，BCQ 引入扰动模型：</p>\n<div class=\"kb-math kb-math-display\">\\pi(s)=\n\\arg\\max_{a_i+\\xi_\\phi(s,a_i,\\Phi)}\nQ_\\theta(s,a_i+\\xi_\\phi(s,a_i,\\Phi)),\n\\quad a_i\\sim G_\\omega(s)</div>\n<p><span class=\"kb-math kb-math-inline\">\\Phi</span> 控制策略能离开数据动作多远。若 <span class=\"kb-math kb-math-inline\">\\Phi</span> 太大，BCQ 退化得更像 DDPG，重新暴露在 OOD 外推风险下；若太小，则更像行为克隆，改进能力有限。论文的消融显示增大 <span class=\"kb-math kb-math-inline\">\\Phi</span> 会带来更不稳定的价值估计。</p>\n<p>BCQ 还使用两个 Q 网络构造 target：</p>\n<div class=\"kb-math kb-math-display\">y=r+\\gamma\\max_i\\left[\n\\lambda\\min_{j=1,2}Q_{\\theta&#x27;_j}(s&#x27;,\\tilde{a}_i)\n+(1-\\lambda)\\max_{j=1,2}Q_{\\theta&#x27;_j}(s&#x27;,\\tilde{a}_i)\n\\right]</div>\n<p>这里 <span class=\"kb-math kb-math-inline\">\\lambda=0.75</span> 时偏向保守的 min，但保留一部分 max 以避免过度低估。这个设计同时服务于“不要高估未知动作”和“不要把所有动作都压得过低”。</p>\n<div class=\"key-point\">💡 关键：BCQ 的策略不是直接输出一个动作，而是先从数据分布生成候选，再在候选邻域内做 Q 最大化。生成模型是约束，Q 网络是改进信号。</div>\n<h5>与 DDPG/行为克隆的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>动作来源</th>\n<th>是否能超越数据</th>\n<th>OOD 风险</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>行为克隆</td>\n<td>直接拟合数据动作</td>\n<td>弱</td>\n<td>低</td>\n</tr>\n<tr>\n<td>DDPG 离线训练</td>\n<td>Actor 任意输出</td>\n<td>理论上强</td>\n<td>高</td>\n</tr>\n<tr>\n<td>BCQ</td>\n<td>VAE 数据动作 + 小扰动</td>\n<td>中到强</td>\n<td>低到中</td>\n</tr>\n</tbody>\n</table></div>\n<p>BCQ 的贡献在于明确指出“离线 off-policy”不同于带 replay buffer 的在线 off-policy。在线 DDPG 可以探索并纠正外推误差；离线 DDPG 不能。因此离线算法必须把策略改进限制在可被数据支持的区域。</p>",
      "quiz": {
        "q": "BCQ 使用条件 VAE 的主要目的是什么？",
        "options": [
          "压缩状态向量以减少内存",
    ...
    },
    {
      ...
      "summary": "CQL 在标准 Bellman 误差上加入保守 Q 值正则，主动压低策略可能选择的分布外动作 Q 值、抬高数据动作相对优势，使离线策略优化基于价值下界而不是乐观外推。它不需要显式学习行为策略约束，就能在复杂离线数据集上缓解过估计和分布偏移。",
      "keyPoints": [
        "<strong>保守价值估计</strong>：学习一个使当前策略期望价值成为真实价值下界的 Q 函数",
        "<strong>CQL 正则项</strong>：最常用形式为 <span class=\"kb-math kb-math-inline\">\\log\\sum_a \\exp Q(s,a)-\\mathbb{E}_{a\\sim\\hat{\\pi}_\\beta}Q(s,a)</span>，压低非数据动作、相对抬高数据动作",
        "<strong>兼容 Q-learning 和 Actor-Critic</strong>：可接入 DQN/QR-DQN，也可接入 SAC 类连续控制 actor-critic",
        "<strong>无需行为策略模型</strong>：相比 BCQ/BEAR 等显式约束方法，CQL 不必额外拟合 <span class=\"kb-math kb-math-inline\">\\hat{\\pi}_\\beta(a|s)</span>",
        "<strong>gap-expanding 性质</strong>：扩大数据内动作与分布外动作之间的 Q 值差距，降低策略被 OOD 高估动作吸引的风险",
        "<strong>安全改进分析</strong>：理论上给出保守下界和高置信策略改进关系",
        "<strong>D4RL 与 Atari 验证</strong>：在 MuJoCo、AntMaze、Adroit、Kitchen 和 Atari 离线设置中优于大量基线，复杂多模态数据集上优势更明显"
      ],
      "detail": "<h5>方法示意图</h5>\n<p><img alt=\"CQL 论文图像资源\" src=\"https://ar5iv.labs.arxiv.org/html/2006.04779/assets/x1.png\" /></p>\n<p><em>图：CQL 论文的 ar5iv 图像资源。CQL 的方法核心主要由保守 Q 目标函数定义，而不是单一架构图；下方公式展示其关键机制。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Conservative Q-Learning, actor-critic style\ninitialize Q_theta, policy pi_phi\n\nfor step in range(num_steps):\n    s, a, r, s_next = sample(offline_dataset)\n\n    # SAC/actor-critic Bellman target\n    a_next = pi_phi.sample(s_next)\n    target = r + gamma * (Q_target(s_next, a_next) - alpha_ent * log_prob(a_next))\n    bellman_loss = mse(Q_theta(s, a), target)\n\n    # CQL(H) regularizer: penalize high Q on broad action samples\n    sampled_actions = sample_actions_from([pi_phi, uniform, current_policy])\n    cql_penalty = logsumexp(Q_theta(s, sampled_actions)) - Q_theta(s, dataset_action=a)\n\n    critic_loss = bellman_loss + alpha_cql * cql_penalty\n    theta = gradient_step(theta, critic_loss)\n\n    # actor can remain SAC-style; no extra behavior constraint is required\n    actor_loss = -mean(Q_theta(s, pi_phi.sample(s)) - alpha_ent * log_prob(pi_phi.sample(s)))\n    phi = gradient_step(phi, actor_loss)\n</code></pre>\n<h5>动机与背景</h5>\n<p>离线 RL 中，标准 off-policy actor-critic 会在 Bellman backup 和策略改进中查询当前策略动作。但当前策略逐渐偏离行为数据后，Q 网络会在数据集外动作上外推。若这些动作被错误高估，策略优化会进一步偏向它们，形成自我强化的过估计循环。BCQ 通过限制动作生成来解决，CQL 则从另一个方向出发：让 Q 函数本身对非数据动作保持悲观。</p>\n<p>最基础的保守 off-policy evaluation 目标是在 Bellman 误差外增加最小化 Q 值的项：</p>\n<div class=\"kb-math kb-math-display\">\\hat{Q}^{k+1}\\leftarrow\n\\arg\\min_Q\n\\alpha\\mathbb{E}_{s\\sim\\mathcal{D},a\\sim\\mu(a|s)}[Q(s,a)]\n+\\frac{1}{2}\\mathbb{E}_{s,a,s&#x27;\\sim\\mathcal{D}}\n\\left[\n\\left(Q(s,a)-\\hat{\\mathcal{B}}^\\pi\\hat{Q}^k(s,a)\\right)^2\n\\right]</div>\n<p>如果只压低 <span class=\"kb-math kb-math-inline\">\\mu</span> 下的 Q 值，可能过度悲观。CQL 因此加入数据分布上的 Q 值最大化项，得到更紧的期望价值下界：</p>\n<div class=\"kb-math kb-math-display\">\\alpha\\left(\n\\mathbb{E}_{s\\sim\\mathcal{D},a\\sim\\mu(a|s)}[Q(s,a)]\n-\n\\mathbb{E}_{s\\sim\\mathcal{D},a\\sim\\hat{\\pi}_\\beta(a|s)}[Q(s,a)]\n\\right)</div>\n<p>实际 CQL(<span class=\"kb-math kb-math-inline\">\\mathcal{H}</span>) 将 <span class=\"kb-math kb-math-inline\">\\mu</span> 取为 Boltzmann/均匀先验诱导的 softmax，形成常见的 log-sum-exp 正则：</p>\n<div class=\"kb-math kb-math-display\">\\min_Q\n\\alpha\n\\mathbb{E}_{s\\sim\\mathcal{D}}\n\\left[\n\\log\\sum_a \\exp(Q(s,a))\n-\\mathbb{E}_{a\\sim\\hat{\\pi}_\\beta(a|s)}Q(s,a)\n\\right]\n+\\frac{1}{2}\\mathbb{E}_{\\mathcal{D}}\n\\left[\n\\left(Q-\\hat{\\mathcal{B}}^{\\pi_k}\\hat{Q}^k\\right)^2\n\\right]</div>\n<p>这项的直觉很直接：<span class=\"kb-math kb-math-inline\">\\log\\sum\\exp</span> 会关注当前 Q 值高的动作，其中很多可能是 OOD 动作；减去数据动作的 Q 值，相当于要求“不要让非数据动作比数据动作高太多”。因此 CQL 不必显式把 actor 限制在行为策略附近，Q 函数的保守性会间接让 actor 避开 OOD 高估动作。</p>\n<p>CQL 的理论分析强调两个性质。第一，学到的 Q 值在当前策略期望下是保守下界，即 <span class=\"kb-math kb-math-inline\">\\hat{V}^\\pi(s)\\le V^\\pi(s)</span>。第二，CQL backup 具有 gap-expanding 行为，会扩大数据内动作与 OOD 动作之间的价值差距。这解释了为什么 CQL 在混合质量数据、稀疏奖励 AntMaze 和人类示范 Adroit/Kitchen 等任务上比只做策略约束更稳。</p>\n<div class=\"warn-box\">⚠️ 注意：CQL 的 <span class=\"kb-math kb-math-inline\">\\alpha</span> 控制悲观程度。太小无法压住 OOD 过估计，太大则可能把有用的泛化也压掉；论文实践中还使用 Lagrange 版本自适应调节保守强度。</div>\n<h5>与 BCQ 的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>BCQ</th>\n<th>CQL</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>核心思想</td>\n<td>限制 actor 只能选数据附近动作</td>\n<td>让 critic 对 OOD 动作悲观</td>\n</tr>\n<tr>\n<td>是否拟合行为分布</td>\n<td>是，条件 VAE</td>\n<td>不必须</td>\n</tr>\n<tr>\n<td>约束位置</td>\n<td>策略/动作生成</td>\n<td>Q 函数训练目标</td>\n</tr>\n<tr>\n<td>泛化能力</td>\n<td>受生成模型支撑影响</td>\n<td>可允许 actor 自由优化，但由 Q 值抑制 OOD</td>\n</tr>\n</tbody>\n</table></div>\n<p>CQL 的工程吸引力在于它可以作为一个 critic loss 插件加到现有 SAC 或 DQN 代码中。它牺牲一些乐观探索能力，换取离线设置下更可信的价值估计。</p>",
      "quiz": {
        "q": "CQL 正则项中 logsumexp(Q(s,a)) - E_data[Q(s,a)] 的核心作用是什么？",
        "options": [
          "鼓励所有动作 Q 值都变大",
    ...
    },
    {
      ...
      "summary": "Unifloral 在论文 “A Clean Slate for Offline Reinforcement Learning” 中提出统一的离线 RL 评估协议和统一算法空间，用显式在线调参预算衡量方法性能，并把多种离线 RL 设计拆成可组合超参数。它的核心贡献不是单一损失函数，而是让离线 RL 的算法比较、消融和新方法搜索变得更可复现。",
      "keyPoints": [
        "<strong>官方对应关系</strong>：NeurIPS 2025 oral 条目对应 OpenReview <code>8P3QNSckMp</code> 与 arXiv <code>2504.11453</code>，题名为 “A Clean Slate for Offline Reinforcement Learning”",
        "<strong>问题重定义</strong>：指出许多离线 RL 论文隐含使用大量在线评估调参，导致“离线”算法比较不公平",
        "<strong>在线调参预算量化</strong>：用预部署评估 episode 数 <span class=\"kb-math kb-math-inline\">N</span> 表示可用在线交互预算，并模拟 bandit 式 policy selection",
        "<strong>统一算法空间</strong>：把模型设计、critic objective、actor objective、dynamics modeling 四类设计做成统一超参数空间",
        "<strong>单文件重实现</strong>：提供简洁一致的 model-free/model-based 离线 RL 实现，减少 boilerplate 差异",
        "<strong>Unifloral 作为元算法</strong>：不是固定一组超参数，而是一个覆盖多种既有方法和新组合的配置空间",
        "<strong>派生新方法</strong>：用统一空间提出 TD3-AWR（model-free）和 MoBRAC（model-based），展示组合式研究流程"
      ],
      "detail": "<h5>方法示意图</h5>\n<p><img alt=\"Unifloral 离线 RL 变体分类图\" src=\"https://ar5iv.labs.arxiv.org/html/2504.11453/assets/x1.png\" /></p>\n<p><em>图：Unifloral 论文中的离线 RL 变体形式化图。论文区分零样本部署、预部署 policy selection、部署后选择和在线微调等设置，强调必须报告在线调参预算。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Unifloral evaluation protocol + unified search space\nmethod_space = {\n    &quot;model_design&quot;: sample_arch_optimizer_and_ensemble(),\n    &quot;critic_objective&quot;: sample_value_target_bc_entropy_diversity(),\n    &quot;actor_objective&quot;: sample_q_bc_entropy_awr_weights(),\n    &quot;dynamics_modeling&quot;: sample_model_based_options(),\n}\n\npolicies = []\nfor i in range(P):\n    config = sample(method_space)\n    policy = train_offline(config, dataset=D)\n    scores = evaluate_policy(policy, episodes=R)  # collected once for analysis\n    policies.append((policy, scores))\n\nfor online_budget_N in budgets:\n    selected = simulate_ucb_policy_selection(policies, budget=online_budget_N)\n    report(best_arm_performance(selected))\n</code></pre>\n<h5>动机与背景</h5>\n<p>离线 RL 名义上不允许训练时与环境交互，但很多论文会在目标任务上训练大量超参数配置，再用在线评估挑选最好的结果。这相当于把“在线调参”藏在实验流程里，而不同方法的超参数空间大小、调参次数和报告方式并不一致。Unifloral 的第一项贡献就是把这个隐含成本显式化：一个离线 RL 方法不仅包含算法，还包含固定超参数范围和允许的在线 policy selection 预算。</p>\n<p>论文形式化了多种离线 RL 变体。最严格的是训练一个策略后直接部署；更常见的是训练多个策略，通过 <span class=\"kb-math kb-math-inline\">N</span> 次目标环境评估在部署前选择；还有部署后选择和在线 fine-tuning。Unifloral 认为若使用了预部署评估，就应报告对应预算，而不是只报告无限调参后的最好结果。</p>\n<p>评估协议可以抽象为：先从算法的固定超参数范围采样 <span class=\"kb-math kb-math-inline\">P</span> 个配置，离线训练得到策略集合；再用一个 UCB bandit 在有限在线 episode 预算下模拟策略选择：</p>\n<div class=\"kb-math kb-math-display\">\\pi_{\\mathrm{deploy}}\n=\n\\mathrm{BanditSelect}(\\{\\pi_i\\}_{i=1}^P, N)</div>\n<p>这样同一算法可以画出随在线调参预算变化的性能曲线。若一个方法只有在大量在线选择下表现好，而小预算下不稳，就能被直接看出来。</p>\n<p>Unifloral 的第二项贡献是统一算法空间。它把既有方法拆成四类组件：模型设计、critic 目标、actor 目标、动力学建模。比如 critic 可以选择 IQL 式 value target 或 TD3 式 target policy smoothing，可以加入 BC/entropy/diversity 项；actor 可以组合 Q 最大化、行为克隆、熵和 AWR 权重：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\mathrm{actor}}\n=\n\\beta_q\\mathcal{L}_q\n+\\beta_{\\mathrm{BC}}\\mathcal{L}_{\\mathrm{BC}}\n-\\beta_{\\mathcal{H}}\\mathcal{H}(\\pi(\\cdot|s))</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\mathcal{L}_{\\mathrm{BC}}</span> 可进一步变为 AWR：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{\\mathrm{BC}}\n=\nd(a_t,\\hat{a}_t)\n\\cdot\n\\min(A_{\\max}, \\exp(\\eta(Q(s_t,a_t)-V(s_t))))</div>\n<p>统一空间的意义是研究者可以在同一个实现里组合 IQL、TD3-BC、ReBRAC、EDAC、MOPO 等思想，而不是每次比较一套代码库。论文用这个框架提出 TD3-AWR：把 ReBRAC/TD3-BC 的 actor Q 优化与 IQL 的优势加权行为克隆结合；又提出 MoBRAC：把 MOPO 式模型 rollout 与 ReBRAC 式 policy optimizer 结合。</p>\n<div class=\"key-point\">💡 关键：Unifloral 不是“又一个离线 RL 单点算法”，而是把算法、超参数范围和在线评估预算绑定成可比较对象。</div>\n<h5>与传统离线 RL 论文的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>常规报告方式</th>\n<th>Unifloral</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>超参数</td>\n<td>常按任务调优，预算不透明</td>\n<td>固定范围，显式采样</td>\n</tr>\n<tr>\n<td>在线评估</td>\n<td>往往只报告最佳配置</td>\n<td>报告随预算变化的选择性能</td>\n</tr>\n<tr>\n<td>实现</td>\n<td>多代码库混合比较</td>\n<td>单文件、组件化一致实现</td>\n</tr>\n<tr>\n<td>方法开发</td>\n<td>增加新算法整体</td>\n<td>在统一空间内组合和消融</td>\n</tr>\n</tbody>\n</table></div>\n<p>这使 Unifloral 更像一个离线 RL “实验协议 + 元算法空间”。对于具身任务，尤其是机器人离线数据昂贵、真实评估成本高的场景，这种预算意识比单纯追求最高表格分数更接近真实部署。</p>",
      "quiz": {
        "q": "Unifloral 评估协议为什么要显式量化在线调参预算？",
        "options": [
          "因为离线 RL 不需要任何数据集",
    ...
    },
    {
      ...
      "summary": "CPQL 将 Peng's Q(\\(\\lambda\\)) 多步算子嵌入 CQL 的保守价值估计，用离线轨迹的多步信息缓解单步 Bellman backup 的过度悲观，同时保持对分布外动作的保守约束。它的固定点对应行为策略与学习策略的混合策略价值，因此天然带有隐式行为正则。",
      "keyPoints": [
        "<strong>官方论文</strong>：OpenReview 条目为 ICLR 2026 poster；arXiv 可访问版本为 <code>2605.14779</code>",
        "<strong>多步离线算子</strong>：使用 Peng's Q(<span class=\"kb-math kb-math-inline\">\\lambda</span>) operator，而不是单步 Bellman operator",
        "<strong>固定点解释</strong>：PQL 在离线设置下收敛到 <span class=\"kb-math kb-math-inline\">\\lambda\\hat{\\pi}_\\beta+(1-\\lambda)\\pi</span> 的混合策略价值",
        "<strong>保守项继承 CQL</strong>：critic loss 保留 CQL 风格 log-sum-exp 减数据动作 Q 的正则",
        "<strong>缓解过度悲观</strong>：多步轨迹信息使 Q 估计比 CQL 更温和，减少 conservative value estimation 的过低估值",
        "<strong>理论保证</strong>：给出混合策略价值下界、至少不低于行为策略、缩小次优间隙等结果",
        "<strong>离线到在线</strong>：CPQL 预训练 Q 函数后接在线 PQL，可缓解 online fine-tuning 开始阶段的性能掉落"
      ],
      "detail": "<h5>方法示意图</h5>\n<p><img alt=\"CPQL 方法示意图\" src=\"https://arxiv.org/html/2605.14779v1/x1.png\" /></p>\n<p><em>图：CPQL 论文 arXiv HTML 中的主图资源。核心思想是用 PQL 多步目标替换 CQL 的单步 Bellman 目标，同时保留保守 Q 正则。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Conservative Peng's Q(lambda)\ninitialize Q1, Q2, policy pi, target_Q1, target_Q2\n\nfor gradient_step in range(num_steps):\n    traj = sample_partial_trajectories(D, length=n)\n\n    # 从尾到头构造 Peng's Q(lambda) 多步目标\n    for i in reversed(range(n)):\n        for j in [1, 2]:\n            q_hat[j][i] = (\n                r[i]\n                + gamma * target_Qj(s[i+1], pi(s[i+1]))\n                + gamma * lambda_ * (q_hat[j][i+1] - target_Qj(s[i+1], pi(s[i+1])))\n            )\n\n    y = min(q_hat[1][0], q_hat[2][0]) - gamma**n * alpha_td * log_prob(pi, s[n])\n\n    # CQL-style conservative critic loss\n    conservative = logsumexp(Qj(s, all_actions)) - mean(Qj(s, dataset_actions))\n    critic_loss = 0.5 * mse(Qj(s0, a0), y) + alpha_cql * conservative\n    update(Q1, Q2, critic_loss)\n\n    # SAC-style actor update\n    actor_objective = mean(min(Q1(s, pi(s)), Q2(s, pi(s))) - alpha_pol * log_prob(pi, s))\n    update(pi, -actor_objective)\n    soft_update_targets()\n</code></pre>\n<h5>动机与背景</h5>\n<p>CQL 解决了离线 RL 的 OOD 动作过估计问题，但保守正则也可能带来过度悲观：Q 值被压得太低时，策略改进信号变弱，尤其在高质量数据或需要轨迹拼接的任务中会损失性能。另一方面，很多离线数据以完整轨迹存在，单步 Bellman backup 没有充分利用多步回报信息。CPQL 的问题意识是：能否用多步算子提供更丰富的价值传播，同时仍保持 CQL 的保守性？</p>\n<p>Peng's Q(<span class=\"kb-math kb-math-inline\">\\lambda</span>) operator 定义为：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{T}^{\\pi_\\beta,\\pi}_\\lambda Q\n=\n(1-\\lambda)\\sum_{n=1}^{\\infty}\\lambda^{n-1}\n\\mathcal{T}^{\\pi_\\beta,\\pi}_n Q</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\mathcal{T}^{\\pi_\\beta,\\pi}_n Q=(\\mathcal{T}^{\\pi_\\beta})^{n-1}\\mathcal{T}^{\\pi}Q</span>。直觉上，前 <span class=\"kb-math kb-math-inline\">n-1</span> 步沿行为策略轨迹传播，最后一步使用学习策略评估。这非常适合离线数据，因为真实轨迹来自行为策略，而目标仍希望朝学习策略改进。</p>\n<p>CPQL 将 CQL 的单步 Bellman 目标替换为 PQL 目标：</p>\n<div class=\"kb-math kb-math-display\">\\widehat{Q}_{k+1}\n\\in\n\\arg\\min_Q\n\\left\\{\n\\frac{1}{2}\\mathbb{E}_{\\mathcal{D}}\n\\left[\n\\left(Q(s,a)-\\mathcal{T}_{\\lambda}^{\\hat{\\pi}_\\beta,\\pi_k}\\widehat{Q}_k(s,a)\\right)^2\n\\right]\n+\\alpha\n\\left(\n\\mathbb{E}_{s\\sim\\mathcal{D},a\\sim\\pi_k}Q(s,a)\n-\\mathbb{E}_{s,a\\sim\\mathcal{D}}Q(s,a)\n\\right)\n\\right\\}</div>\n<p>这个公式说明 CPQL 不是丢掉 CQL，而是把 CQL 的 target 从单步换成多步。保守项继续压制学习策略在 OOD 动作上的高 Q，PQL target 则利用离线轨迹的长时序信息，避免所有改进都依赖单步 bootstrapping。</p>\n<p>论文给出的关键固定点解释是：PQL operator 在离线设置下对应混合策略 <span class=\"kb-math kb-math-inline\">\\lambda\\hat{\\pi}_\\beta+(1-\\lambda)\\pi</span> 的价值。这样 <span class=\"kb-math kb-math-inline\">\\lambda</span> 控制行为策略锚定强度：<span class=\"kb-math kb-math-inline\">\\lambda=0</span> 退回单步 CQL 风格；<span class=\"kb-math kb-math-inline\">\\lambda</span> 较大时价值更接近行为策略，分布偏移更小，但过大也会限制最优性。</p>\n<p>实际算法从离线数据中采样长度为 <span class=\"kb-math kb-math-inline\">n</span> 的 partial trajectories，从尾到头递推：</p>\n<div class=\"kb-math kb-math-display\">\\widehat{Q}_{\\theta_j}^{i}\n=\nr_i+\\gamma Q_{\\theta_j^-}(s_{i+1},\\pi_\\phi(s_{i+1}))\n+\\gamma\\lambda\n\\left(\n\\widehat{Q}_{\\theta_j}^{i+1}\n-Q_{\\theta_j^-}(s_{i+1},\\pi_\\phi(s_{i+1}))\n\\right)</div>\n<p>然后用双 Q 的最小值构造 target，并加上 entropy 项。actor 更新与 SAC 类似，最大化 <span class=\"kb-math kb-math-inline\">\\min_jQ_{\\theta_j}(s,a)-\\alpha_{\\mathrm{pol}}\\log\\pi(a|s)</span>。因此 CPQL 可视为“CQL critic 正则 + PQL 多步 target + SAC actor”的组合。</p>\n<div class=\"key-point\">💡 关键：CPQL 的 <span class=\"kb-math kb-math-inline\">\\lambda</span> 不是普通 TD(<span class=\"kb-math kb-math-inline\">\\lambda</span>) 的工程技巧，而是控制行为策略与学习策略混合固定点的保守性旋钮。</div>\n<h5>与 CQL 的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>维度</th>\n<th>CQL</th>\n<th>CPQL</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>Backup</td>\n<td>单步 Bellman target</td>\n<td>Peng's Q(<span class=\"kb-math kb-math-inline\">\\lambda</span>) 多步 target</td>\n</tr>\n<tr>\n<td>保守性来源</td>\n<td>Q 正则压低 OOD 动作</td>\n<td>Q 正则 + 行为策略混合固定点</td>\n</tr>\n<tr>\n<td>轨迹利用</td>\n<td>主要用单步 transition</td>\n<td>使用 partial trajectory 多步信息</td>\n</tr>\n<tr>\n<td>主要改进</td>\n<td>防过估计</td>\n<td>防过估计同时缓解过度悲观</td>\n</tr>\n</tbody>\n</table></div>\n<p>CPQL 的代价是多步 target 会增加一些计算和实现复杂度，并且在低质量数据上过长轨迹可能传播不理想行为。论文因此将轨迹长度、<span class=\"kb-math kb-math-inline\">\\alpha</span> 和 <span class=\"kb-math kb-math-inline\">\\lambda</span> 作为关键超参数，并报告 CPQL 可在 <span class=\"kb-math kb-math-inline\">\\lambda=0</span> 时复现单步 TD 学习。</p>",
      "quiz": {
        "q": "CPQL 中 Peng's Q(λ) 算子的核心作用是什么？",
        "options": [
          "完全移除 CQL 的保守正则项",
    ...
    },
    {
      ...
      "summary": "RND 提出用一个固定随机目标网络和一个可训练预测网络之间的预测误差作为内在奖励，解决稀疏奖励环境中“状态是否新颖”难以稳定估计的问题。它把探索信号从复杂的动力学预测改成简单的特征蒸馏误差，因此在 Montezuma's Revenge 等硬探索 Atari 任务上显著提升样本效率。",
      "keyPoints": [
        "<strong>双网络新颖性估计</strong>：固定随机目标网络 <span class=\"kb-math kb-math-inline\">f</span> 产生观测特征，可训练预测网络 <span class=\"kb-math kb-math-inline\">\\hat f_\\theta</span> 学习预测该特征",
        "<strong>内在奖励</strong>：使用 <span class=\"kb-math kb-math-inline\">\\|\\hat f_\\theta(o_t)-f(o_t)\\|_2^2</span> 衡量预测误差，误差越大说明状态越少见",
        "<strong>与 PPO 结合</strong>：论文主要把 RND 奖励加入 PPO 训练流程，策略同时最大化外在奖励和内在奖励",
        "<strong>奖励归一化</strong>：对观测、内在奖励和外在奖励做运行统计归一化，减少尺度漂移",
        "<strong>双 Value 头</strong>：分别估计外在回报和内在回报，因为两类奖励的折扣因子和动态范围不同",
        "<strong>抗“噪声电视”问题</strong>：目标网络固定且随机，预测误差只随访问频率下降，不直接追逐环境随机性",
        "<strong>硬探索验证</strong>：重点实验在 Montezuma's Revenge、Gravitar、Pitfall 等 Atari 稀疏奖励任务上展开"
      ],
      "detail": "<h5>整体机制示意图</h5>\n<p><img alt=\"RND 随机网络蒸馏机制\" src=\"https://ar5iv.labs.arxiv.org/html/1810.12894/assets/x1.png\" /></p>\n<p><em>图：RND 使用固定随机目标网络提供不可学习的特征目标，预测网络只在访问过的状态上逐渐降低误差，剩余误差被解释为探索奖励。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Random Network Distillation + PPO 伪代码\ninitialize fixed random target network f\ninitialize predictor network f_hat_theta\ninitialize policy pi_phi and two value heads V_ext, V_int\ninitialize replay/statistics for observation and reward normalization\n\nfor iteration in range(num_updates):\n    trajectories = []\n    for t in range(rollout_length):\n        o_t = normalize_obs(env.obs)\n        a_t = pi_phi.sample(o_t)\n        o_next, r_ext, done = env.step(a_t)\n\n        # RND intrinsic reward\n        target = stop_gradient(f(normalize_obs(o_next)))\n        pred = f_hat_theta(normalize_obs(o_next))\n        r_int = mean_squared_error(pred, target)\n\n        trajectories.append(o_t, a_t, r_ext, r_int, done)\n\n    # 分别估计外在与内在优势，再合成为 PPO 优化信号\n    adv_ext = GAE(trajectories.r_ext, V_ext, gamma_ext)\n    adv_int = GAE(normalize(trajectories.r_int), V_int, gamma_int)\n    adv = adv_ext + beta * adv_int\n\n    update pi_phi with PPO clipped objective using adv\n    update V_ext and V_int with value regression losses\n    update f_hat_theta to minimize ||f_hat_theta(o) - f(o)||^2\n</code></pre>\n<h5>动机与背景</h5>\n<p>硬探索任务的困难不在于局部控制，而在于智能体很长时间都收不到外在奖励。例如 Montezuma's Revenge 中，随机策略很难偶然完成“取钥匙、避开敌人、打开门”等长序列动作。传统基于计数的探索在高维图像观测上难以定义“同一个状态”，而基于动力学预测的 curiosity 方法又容易被不可控噪声吸引。</p>\n<p>RND 的关键简化是把新颖性估计转化为一个监督学习问题。目标网络 <span class=\"kb-math kb-math-inline\">f</span> 在初始化后冻结，预测网络 <span class=\"kb-math kb-math-inline\">\\hat f_\\theta</span> 只在智能体实际访问过的观测上训练。因此，某个观测被访问越多，预测网络越会记住它，误差越低；新观测还没有被训练覆盖，误差自然更高。</p>\n<p>核心内在奖励写作：</p>\n<div class=\"kb-math kb-math-display\">r_t^{\\text{int}} = \\left\\|\\hat f_\\theta(o_t) - f(o_t)\\right\\|_2^2</div>\n<p>这里的 <span class=\"kb-math kb-math-inline\">f(o_t)</span> 不是语义标签，而是随机投影后的特征。随机目标看似粗糙，但它有两个好处：一是固定不变，避免目标随训练漂移；二是足够高维时能把不同观测映射到可区分特征，让预测误差成为访问频率的近似代理。</p>\n<h5>训练流程与奖励组合</h5>\n<p>RND 不是替代 RL 算法，而是提供额外奖励。论文中策略优化仍使用 PPO，轨迹采样后同时得到环境奖励 <span class=\"kb-math kb-math-inline\">r_t^{\\text{ext}}</span> 与内在奖励 <span class=\"kb-math kb-math-inline\">r_t^{\\text{int}}</span>，总优化信号通常写成：</p>\n<div class=\"kb-math kb-math-display\">r_t = r_t^{\\text{ext}} + \\beta r_t^{\\text{int}}</div>\n<p>由于外在奖励可能极稀疏、内在奖励会随着学习不断衰减，RND 使用运行均值和方差做归一化，并为两类奖励维护不同的 value function。外在 value 关注任务目标，内在 value 关注未来探索收益；把二者混到同一个 critic 中会让尺度和折扣选择互相干扰。</p>\n<p>RND 与 ICM 的区别在于新颖性来源。ICM 预测动作导致的下一个特征状态，容易把环境中不可预测但与控制无关的噪声当成奖励；RND 预测的是固定随机函数输出，误差下降只依赖训练覆盖，而不是环境动力学是否随机。因此 RND 在随机背景、随机敌人等场景中更不容易被“预测不了的噪声”劫持。</p>\n<div class=\"key-point\">💡 关键：RND 的探索奖励不是“这个状态是否重要”，而是“这个状态我是否还不会预测”。重要性仍由外在任务奖励最终筛选，RND 只负责把智能体推向未访问区域。</div>\n<h5>与传统方法的区别</h5>\n<div class=\"table-wrap\"><table>\n<thead>\n<tr>\n<th>方法</th>\n<th>新颖性信号</th>\n<th>主要风险</th>\n<th>RND 的改进</th>\n</tr>\n</thead>\n<tbody>\n<tr>\n<td>计数探索</td>\n<td>离散状态访问次数</td>\n<td>图像状态难以计数</td>\n<td>用神经预测误差近似访问频率</td>\n</tr>\n<tr>\n<td>动力学 curiosity</td>\n<td>下一状态预测误差</td>\n<td>被不可控随机性吸引</td>\n<td>固定随机目标不建模环境噪声</td>\n</tr>\n<tr>\n<td>手工奖励塑形</td>\n<td>人工中间奖励</td>\n<td>任务依赖强且可能改写目标</td>\n<td>通用内在奖励，少量额外网络即可接入</td>\n</tr>\n</tbody>\n</table></div>",
      "quiz": {
        "q": "RND 中内在奖励为什么会随状态访问次数增加而下降？",
        "options": [
          "因为 PPO 会自动降低所有奖励的尺度",
    ...
    },
    {
      ...
      "summary": "LaGEA 提出把 VLM 对失败轨迹的结构化语言反思转成时间局部化的 dense reward shaping，解决机器人操作中稀疏奖励无法指出“何时、为何失败”的问题。它通过关键帧、图文反馈对齐、delta potential 和自适应塑形权重，让语言反馈在早期探索中强介入、在策略变好后逐渐退出。",
      "keyPoints": [
        "<strong>结构化 VLM 反思</strong>：每个 rollout 后由 Qwen-2.5-VL-3B 生成 schema-constrained JSON 反馈，避免自由文本漂移",
        "<strong>关键帧时间接地</strong>：从目标相似度轨迹中选取接近目标、变化快或转折明显的帧，把反馈权重扩散到局部时间窗口",
        "<strong>反馈-视觉对齐</strong>：用图像、目标、指令和反馈投影器把状态图像与语言反馈嵌入到共享空间",
        "<strong>Goal-delta reward</strong>：根据当前状态与目标图像/指令的 potential 变化奖励真实进展",
        "<strong>Feedback-delta reward</strong>：根据状态与 VLM 诊断反馈的一致性变化奖励纠错方向",
        "<strong>自适应塑形系数</strong>：只在失败轨迹上强化塑形，并随成功率 EMA 和进展信号提升而衰减",
        "<strong>在线 RL 接入</strong>：最终奖励输入 SAC critic，保留环境 sparse reward，同时添加 bounded VLM shaping",
        "<strong>资料限制</strong>：清单中的 <code>paper_url</code> 指向 arXiv:2602.03001，但该链接实际是非 LaGEA 的 batch-size/GNS 论文；本文正文基于公开 LaGEA 论文 arXiv:2509.23155 与其 arXiv HTML 页面，YAML 元信息按清单原样保留"
      ],
      "detail": "<h5>整体架构示意图</h5>\n<p><img alt=\"LaGEA 框架总览\" src=\"https://arxiv.org/html/2509.23155v2/x1.png\" /></p>\n<p><em>图：LaGEA 在每次 rollout 后抽取关键帧并查询 VLM，随后把结构化反馈与视觉状态对齐，最终把目标进展和反馈一致性转成 step-wise shaping reward。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># LaGEA 训练循环伪代码\ninitialize SAC policy, critic, replay buffer\ninitialize visual encoder, text encoder, feedback/image projectors\n\nfor episode in range(num_episodes):\n    traj = rollout(policy, env)\n    keyframes, weights = select_keyframes(traj.obs, goal_image)\n\n    feedback_json = VLM_reflect(\n        task_instruction=instruction,\n        frames=keyframes,\n        error_taxonomy=taxonomy,\n        recent_history=history,\n    )\n    f = encode_feedback(feedback_json)\n\n    for transition (s_t, a_t, r_env, s_next) in traj:\n        phi_goal_t = goal_potential(s_t, goal_image, instruction)\n        phi_goal_next = goal_potential(s_next, goal_image, instruction)\n        r_goal = gamma * phi_goal_next - phi_goal_t\n\n        phi_fb_t = feedback_potential(s_t, f)\n        phi_fb_next = feedback_potential(s_next, f)\n        r_feedback = weight_t * (gamma * phi_fb_next - phi_fb_t)\n\n        confidence = instruction_feedback_agreement(instruction, f)\n        r_vlm = mix(confidence, r_goal, r_feedback)\n        alpha = failure_aware_schedule(success_ema, progress)\n        r_total = r_env + alpha * r_vlm\n        replay.add(s_t, a_t, r_total, s_next)\n\n    update projectors with calibration + contrastive losses\n    update SAC actor and critic from replay\n</code></pre>\n<h5>动机与背景</h5>\n<p>VLM 可以判断图像是否接近语言目标，但直接把 VLM 分数作为 reward 往往不稳定：同一失败可能被不同视角解释成不同原因，单帧评分也无法告诉 RL 哪一步造成了失败。机器人操作任务又常是长程、稀疏奖励，只有 episode 末端的成功/失败信号会导致大量无效探索。</p>\n<p>LaGEA 的设计目标是把“自然语言反思”变成 RL 可用的局部学习信号。它不让 VLM 直接控制动作，而是在每个 episode 后生成可审计的结构化反馈，例如失败阶段、约束违反、可恢复建议等；再用关键帧选择把这段反馈绑定到轨迹中真正关键的时间片。</p>\n<p>关键帧选择以目标相似度轨迹为基础。设图像嵌入为 <span class=\"kb-math kb-math-inline\">e_t</span>，目标嵌入为 <span class=\"kb-math kb-math-inline\">g</span>，则可以得到接近度 <span class=\"kb-math kb-math-inline\">c_t=\\cos(e_t,g)</span>，并结合一阶变化和局部转折得到 saliency <span class=\"kb-math kb-math-inline\">u_t</span>。选出的关键帧再通过三角核扩散成逐步权重 <span class=\"kb-math kb-math-inline\">w_t</span>，避免把同一段反馈平均撒到所有 transition 上。</p>\n<h5>Delta Potential 奖励</h5>\n<p>LaGEA 的奖励塑形借鉴 potential-based shaping：不直接奖励某个状态的高分，而奖励 potential 的变化：</p>\n<div class=\"kb-math kb-math-display\">r_t^{\\Delta\\Phi}=\\gamma \\Phi(s_{t+1})-\\Phi(s_t)</div>\n<p>这能缓解“站在看起来接近目标的位置反复刷分”的问题。Goal potential 由当前状态与目标图像、任务指令的相似度组成；Feedback potential 则衡量当前状态与 VLM 反馈 embedding 的一致性，并由关键帧权重 <span class=\"kb-math kb-math-inline\">w_t</span> 控制强度。</p>\n<p>两个 delta 奖励通过置信度混合：</p>\n<div class=\"kb-math kb-math-display\">r_t^{\\text{vlm}}=\\lambda_t r_t^{\\text{goal}} + (1-\\lambda_t) r_t^{\\text{feedback}}</div>\n<p>其中 <span class=\"kb-math kb-math-inline\">\\lambda_t</span> 可由指令和反馈的一致性估计得到。若 VLM 反馈与任务指令高度一致，反馈项权重大；若反馈不可靠，系统更多依赖目标图像/指令的进展信号。</p>\n<h5>动态塑形与 SAC 训练</h5>\n<p>LaGEA 的最终奖励为：</p>\n<div class=\"kb-math kb-math-display\">r_t = r_t^{\\text{env}} + \\alpha_t r_t^{\\text{vlm}}</div>\n<p><span class=\"kb-math kb-math-inline\">\\alpha_t</span> 不是常数，而是 failure-aware schedule：失败时塑形较强，成功率 EMA 上升后逐步减弱。这样做的直觉是，语言反馈适合帮助早期探索和失败恢复，但当策略已经学会任务后，继续过度依赖 VLM 可能造成 reward hacking 或偏离真实环境目标。</p>\n<p>与 RND 这类通用新颖性奖励相比，LaGEA 的内在信号更“任务语义化”：RND 奖励没见过的状态，LaGEA 奖励“按反馈看起来在纠错的状态变化”。因此它更适合目标明确但中间奖励稀缺的机器人操作任务；代价是需要 VLM、视觉语言嵌入与反馈模板，系统复杂度高于纯预测误差方法。</p>\n<div class=\"key-point\">💡 关键：LaGEA 的核心不是“让 VLM 打分”，而是把 VLM 反思先结构化、再时间接地、最后只以 potential difference 的形式进入 RL 奖励。</div>",
      "quiz": {
        "q": "LaGEA 为什么使用 delta potential 而不是直接把状态-目标相似度作为奖励？",
        "options": [
          "为了让 VLM 可以直接输出动作",
    ...
    },
    {
      ...
      "summary": "MRBT 提出用行为树同时组织奖励塑形和动作掩码，解决组合任务中 LLM 生成奖励缺乏反应性、模块性和形式化验证的问题。它让 LLM 填充 MRBT 模板，再用 SMT 求解器验证子任务逻辑，最后把可验证的 symbolic reward/mask 接入神经 RL 训练。",
      "keyPoints": [
        "<strong>Masking Reward Behavior Tree</strong>：行为树叶节点是带 action mask 的 masking behavior reward machine，tick 后输出奖励和动作掩码",
        "<strong>组合任务模板</strong>：面向顺序 object-interaction subtasks，模板包含子任务完成条件、接近目标对象条件和对应动作 mask",
        "<strong>反应性回退</strong>：BT 的 Sequence/Fallback 结构允许前置子任务失效后回到对应子树，而不是继续奖励后续子任务",
        "<strong>动作掩码与奖励联合设计</strong>：同一符号结构既限制无效动作，又输出 shaping reward，减少奖励和动作约束互相矛盾",
        "<strong>SMT 形式化验证</strong>：把 completion correctness、non-triviality、proximity correctness、composition persistence 等规格交给 Z3 检查",
        "<strong>LLM 自动生成闭环</strong>：LLM 依据模板和任务空间生成 MRBT；若 SMT 不满足规格，则把错误反馈重新提示 LLM 修正",
        "<strong>Neurosymbolic RL</strong>：训练时神经策略负责感知与控制，MRBT 负责基于符号标签提供 reward/mask 指导",
        "<strong>资料限制</strong>：清单中的 <code>paper_url</code> 指向 arXiv:2602.04567，但该链接实际是短视频推荐数据集论文；本文正文基于公开 MRBT 论文 arXiv:2605.05795，YAML 元信息按清单原样保留"
      ],
      "detail": "<h5>MRBT 模板示意图</h5>\n<p><img alt=\"MRBT 行为树模板\" src=\"https://arxiv.org/html/2605.05795v2/x2.png\" /></p>\n<p><em>图：MRBT 把多个子任务组织成行为树模板，每个叶节点维护简单 reward-machine 状态、奖励函数和动作掩码，内部节点负责反应式执行顺序。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># LLM + SMT + MRBT 的自动奖励/掩码生成流程\ninput: task_space, environment_predicates, MRBT_template\n\nwhile True:\n    mrbt_spec = LLM_generate(\n        template=MRBT_template,\n        task_space=task_space,\n        predicates=environment_predicates,\n    )\n\n    logic_formulas = extract_logic(mrbt_spec)\n    smt_result = Z3_verify(\n        formulas=logic_formulas,\n        specs=[\n            &quot;completion correctness&quot;,\n            &quot;completion non-triviality&quot;,\n            &quot;object proximity correctness&quot;,\n            &quot;object proximity non-triviality&quot;,\n            &quot;composition persistence&quot;,\n        ],\n        transition_model=symbolic_env_model,\n    )\n\n    if smt_result.sat:\n        break\n    else:\n        MRBT_template = add_counterexample_feedback(MRBT_template, smt_result)\n\nfor episode in training:\n    task = sample(task_space)\n    state = env.reset(task)\n    mrbt_state = mrbt.initial_state()\n\n    while not done:\n        labels = labeling_function(state, task)\n        ticked_leaves = mrbt.tick(labels, mrbt_state)\n        reward = sum(leaf.reward for leaf in ticked_leaves)\n        action_mask = ticked_leaves[-1].action_mask\n        action = policy.sample(obs, mask=action_mask)\n        next_state, env_reward, done = env.step(action)\n        update_policy(obs, action, env_reward + reward, next_state)\n</code></pre>\n<h5>动机与背景</h5>\n<p>许多具身任务本质上是组合任务：先找到某个物体，再操作门，再到达目标区域。直接用 LLM 生成一段奖励代码可以降低人工设计成本，但如果没有反应性，当前置子任务失效时奖励逻辑可能仍然鼓励后续动作；如果没有模块性，换一个物体颜色或房间布局就要重写；如果没有验证，生成代码看似合理却可能在边界状态下给出错误奖励。</p>\n<p>行为树适合解决反应性和模块性。BT 每一步从根节点 tick，Sequence 保证顺序，Fallback 支持条件失败后的回退。MRBT 在 BT 叶节点中嵌入 masking behavior reward machine：每个叶节点不仅返回 Success/Running/Failure，还维护状态转移、奖励函数和可用动作集合。</p>\n<p>论文把一个 MRBT 形式化为共享逻辑公式集合 <span class=\"kb-math kb-math-inline\">\\mathcal{F}</span> 和动作空间 <span class=\"kb-math kb-math-inline\">\\mathcal{A}</span> 上的一组叶节点。给定当前标签 <span class=\"kb-math kb-math-inline\">l_t</span>，BT tick 得到叶序列 <span class=\"kb-math kb-math-inline\">\\mathcal{B}_t</span>，只更新被 tick 的叶节点：</p>\n<div class=\"kb-math kb-math-display\">x_{t+1}^b =\n\\begin{cases}\n\\delta^b(x_t^b,l_t), &amp; b \\in \\mathcal{B}_t \\\\\nx_t^b, &amp; b \\notin \\mathcal{B}_t\n\\end{cases}</div>\n<p>奖励和动作掩码由被 tick 的叶节点给出：</p>\n<div class=\"kb-math kb-math-display\">r_t = \\sum_{b \\in \\mathcal{B}_t} r^b(x_t^b,l_t), \\qquad m_t = m^{b_{\\text{last}}}(x_t^{b_{\\text{last}}},l_t)</div>\n<h5>LLM 生成与 SMT 验证</h5>\n<p>MRBT 的自动化流程并不盲信 LLM。LLM 接收任务空间、环境谓词和 MRBT 模板，生成完成条件、接近条件、奖励函数和动作掩码。随后系统把这些公式放入 Z3，在符号环境转移模型约束下检查若干规格：完成条件必须真的对应子任务完成，接近条件不能平凡为真/假，组合后不能因前序子任务回退而仍保持错误的后续奖励。</p>\n<p>这种验证方式比只用专家演示测试更强。演示只能覆盖有限轨迹，SMT 可以主动寻找反例；若发现 unsat 或违反规格，系统把反例作为反馈重新提示 LLM 修正。最终得到的 MRBT 再用于 RL 训练，神经策略仍学习感知到动作的映射，但 symbolic MRBT 提供更密集、更可信的训练信号。</p>\n<p>与 LaGEA 的 VLM 时间接地奖励相比，MRBT 更强调离散逻辑正确性。LaGEA 适合视觉连续控制中的失败诊断，MRBT 适合能抽取谓词和任务结构的组合任务。它的主要限制也来自这里：需要环境能提供或学习出稳定谓词，并且 MRBT 模板当前主要覆盖顺序 object-interaction 子任务。</p>\n<div class=\"key-point\">💡 关键：MRBT 的价值不只是“LLM 写奖励”，而是让 LLM 的奖励/掩码设计落在行为树模板内，并接受 SMT 反例驱动的形式化检查。</div>",
      "quiz": {
        "q": "MRBT 相比普通 LLM 生成奖励函数的核心优势是什么？",
        "options": [
          "完全不需要环境状态谓词",
    ...
    },
    {
      ...
      "summary": "VSIMR 将 VAE 的状态新颖性奖励与 LLM 基于任务描述生成的语义引导奖励结合起来，解决极端稀疏奖励环境中单一好奇心信号方向性不足的问题。它在 A2C 训练中把环境奖励、VAE KL 内在奖励和 LLM 评分奖励相加，使探索既覆盖新状态，也更偏向任务相关状态。",
      "keyPoints": [
        "<strong>VAE 新颖性奖励</strong>：用 VAE 编码状态并以 KL divergence 衡量状态对潜变量分布的“惊讶度”",
        "<strong>LLM 语义奖励</strong>：把 MiniGrid 状态和任务 mission 格式化成 prompt，请 LLaMA 3.2 评估新状态对目标的帮助程度",
        "<strong>三路奖励融合</strong>：A2C 接收 <span class=\"kb-math kb-math-inline\">r^{env}+\\beta r^{VAE}+\\alpha r^{LLM}</span> 作为训练回报",
        "<strong>Prompt 缓存</strong>：维护 prompt-answer 数据集，避免对重复状态反复调用 LLM",
        "<strong>周期性 VAE 训练</strong>：收集最近状态后每 <span class=\"kb-math kb-math-inline\">N</span> 步训练 VAE，使新颖性估计随访问覆盖更新",
        "<strong>A2C 主干</strong>：Actor-Critic 仍负责策略学习，VSIMR/LLM 只改变奖励信号",
        "<strong>实验环境</strong>：MiniGrid DoorKey-8x8-v0，论文把最大步数从默认 640 增加 40% 到 896 以观察稀疏奖励学习",
        "<strong>结果观察</strong>：聚合结果中 LLM+VAE 比单独 VAE 更快、更稳定，但不同随机运行仍有明显方差"
      ],
      "detail": "<h5>方法流程示意图</h5>\n<p><img alt=\"VSIMR + LLM 内在动机流程\" src=\"https://arxiv.org/html/2508.18420v1/RL_VAE_AGENTpng.png\" /></p>\n<p><em>图：智能体执行动作得到新状态后，VAE 计算状态新颖性奖励，LLM 根据状态文本和任务目标给出语义进展奖励，二者与环境奖励融合后训练 A2C。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># A2C with VSIMR and LLM intrinsic reward\ninitialize actor_critic\ninitialize VAE encoder/decoder\ninitialize D_vae = []          # states for VAE training\ninitialize D_prompt = {}       # prompt cache\n\nfor episode in range(num_episodes):\n    s = env.reset()\n    for t in range(T):\n        a = actor_critic.policy.sample(s)\n        s_next, r_env, done = env.step(a)\n\n        # VSIMR: variational state novelty\n        mu, logvar = VAE.encode(s_next)\n        r_vae = KL(q(z | s_next) || p(z))\n        D_vae.append(s_next)\n\n        # LLM reward: task-aware semantic guidance\n        prompt = build_prompt(mission=env.mission, state=s_next)\n        if prompt not in D_prompt:\n            D_prompt[prompt] = LLM_score(prompt, scale=&quot;0-10&quot;)\n        r_llm = normalize(D_prompt[prompt])\n\n        r_total = r_env + beta * r_vae + alpha * r_llm\n        actor_critic.store(s, a, r_total, s_next, done)\n\n        if t % N == 0:\n            actor_critic.update()\n            VAE.train(D_vae)\n            D_vae.clear()\n\n        s = s_next\n        if done:\n            break\n</code></pre>\n<h5>动机与背景</h5>\n<p>在 DoorKey 这类 MiniGrid 任务里，智能体必须找到钥匙、开门并到达目标，但大多数中间步骤没有外在奖励。仅靠环境奖励训练 A2C，早期几乎没有可学习信号。RND 等预测误差方法能鼓励访问新状态，但它们不理解“钥匙”“门”“目标”之间的任务语义，可能把探索预算花在新颖但无关的区域。</p>\n<p>VSIMR 的变分状态奖励来自 VAE。VAE 将状态 <span class=\"kb-math kb-math-inline\">s</span> 编码为潜变量分布 <span class=\"kb-math kb-math-inline\">q_\\phi(z|s)</span>，并通过重构损失和 KL 项学习状态结构。对当前状态的内在奖励可以取：</p>\n<div class=\"kb-math kb-math-display\">r_t^{VAE} = D_{KL}\\big(q_\\phi(z|s_t)\\,\\|\\,p(z)\\big)</div>\n<p>直觉上，如果一个状态在当前 VAE 表征中不常见或信息量大，它的后验会偏离先验更多，KL 项更高，因此应鼓励智能体访问。这与 RND 的“预测误差高说明不熟悉”相似，但 VSIMR 使用概率潜变量而不是固定随机网络特征。</p>\n<h5>LLM 奖励与融合</h5>\n<p>论文进一步加入 LLM 奖励。系统把环境 mission、当前可见对象和状态描述写入 prompt，要求 LLM 在 0 到 10 的尺度上判断“这个新状态是否帮助智能体完成最终目标”。这样得到的 <span class=\"kb-math kb-math-inline\">r_t^{LLM}</span> 为探索提供任务方向：看见钥匙、接近门、拿到关键物体等状态可获得比无关移动更高的语义分数。</p>\n<p>总奖励写成：</p>\n<div class=\"kb-math kb-math-display\">r_t = r_t^{env} + \\beta r_t^{VAE} + \\alpha r_t^{LLM}</div>\n<p><span class=\"kb-math kb-math-inline\">\\beta</span> 控制新颖性探索强度，<span class=\"kb-math kb-math-inline\">\\alpha</span> 控制语言引导强度。若 <span class=\"kb-math kb-math-inline\">\\alpha</span> 太大，LLM 的粗糙评分可能覆盖真实任务奖励；若 <span class=\"kb-math kb-math-inline\">\\beta</span> 太大，智能体可能持续追逐新状态而不收敛。因此 VSIMR 的关键并不是“更多奖励项”，而是让状态新颖性和任务语义互补。</p>\n<h5>训练流程与缓存机制</h5>\n<p>每一步环境交互后，A2C 保存带融合奖励的 transition。VAE 不是每步都立即更新，而是在累计一批状态后周期性训练，以降低噪声和计算开销。LLM 调用同样昂贵，因此实现中保存 prompt-answer 对：若同一个状态描述已经问过，就直接复用结果。</p>\n<p>与 RND 相比，VSIMR 的优势是奖励更懂任务；与纯 LLM reward 相比，VAE 新颖性保留了自主探索能力。当 LLM 评分不够细或错误时，VAE 仍能推动智能体覆盖未知区域；当 VAE 只会鼓励无方向探索时，LLM 语义信号又能把探索拉回目标路径。</p>\n<div class=\"warn-box\">⚠️ 注意：论文实验显示 LLM+VAE 的聚合表现更好，但不同 run 的差异仍然大。这说明该方向有效，但 prompt、奖励权重和 VAE 更新频率仍是敏感超参数。</div>",
      "quiz": {
        "q": "VSIMR 中同时使用 VAE 奖励和 LLM 奖励的主要目的是什么？",
        "options": [
    ...
    },
    {
      ...
      "summary": "MBPO 提出只在学习到的动力学模型中生成短步长 rollout，并把这些模型样本与真实样本混合训练 SAC，解决模型式 RL 中“样本便宜但误差会随 rollout 长度累积”的核心权衡。它用模型 ensemble 和短 horizon 让模型生成数据足够有用，同时避免长程模型偏差破坏策略优化。",
      "keyPoints": [
        "<strong>短模型 rollout</strong>：从真实 replay buffer 的状态出发，只展开 <span class=\"kb-math kb-math-inline\">k</span> 步模型轨迹，通常 <span class=\"kb-math kb-math-inline\">k</span> 很小",
        "<strong>模型 ensemble</strong>：训练多个 probabilistic dynamics models，降低单模型过拟合并估计不确定性",
        "<strong>Dyna 风格混合数据</strong>：真实环境样本进入 <span class=\"kb-math kb-math-inline\">\\mathcal{D}_{env}</span>，模型生成样本进入 <span class=\"kb-math kb-math-inline\">\\mathcal{D}_{model}</span>，SAC 从混合 buffer 更新",
        "<strong>理论分析</strong>：论文分析模型误差、策略分布偏移和 rollout 长度对单调改进界的影响",
        "<strong>模型使用调度</strong>：训练早期使用更短 rollout，随模型变准逐渐增加 rollout horizon",
        "<strong>低模型偏差优先</strong>：MBPO 不追求完全用模型规划，而是让模型只负责补充局部 transition",
        "<strong>基准验证</strong>：在 MuJoCo 连续控制任务上以远少于无模型 SAC 的真实交互达到强性能"
      ],
      "detail": "<h5>模型 rollout 示意图</h5>\n<p><img alt=\"MBPO 模型使用与 rollout 分析\" src=\"https://ar5iv.labs.arxiv.org/html/1906.08253/assets/x1.png\" /></p>\n<p><em>图：MBPO 论文围绕“何时信任模型”分析 rollout 长度与模型误差的权衡，核心结论是短 rollout 能提供样本效率而不过度放大模型偏差。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Model-Based Policy Optimization\ninitialize SAC policy pi, Q-functions\ninitialize environment replay buffer D_env\ninitialize model replay buffer D_model\ninitialize probabilistic dynamics ensemble {p_theta_i}\n\nfor epoch in range(num_epochs):\n    # 真实环境交互\n    for step in range(env_steps_per_epoch):\n        a = pi.sample(s)\n        s_next, r, done = env.step(a)\n        D_env.add(s, a, r, s_next, done)\n        s = reset_if_done(s_next, done)\n\n    # 训练动力学模型 ensemble\n    for model_update in range(model_train_steps):\n        batch = D_env.sample()\n        update each p_theta_i to maximize log p(s_next, r | s, a)\n\n    # 从真实状态启动短模型 rollout\n    for rollout in range(num_model_rollouts):\n        s_model = D_env.sample_state()\n        for h in range(k):  # k is short\n            a_model = pi.sample(s_model)\n            model = random_choice(ensemble)\n            s_next_model, r_model = model.predict(s_model, a_model)\n            D_model.add(s_model, a_model, r_model, s_next_model)\n            s_model = s_next_model\n\n    # 用真实 + 模型数据训练 SAC\n    for grad_step in range(policy_updates):\n        batch = mix_sample(D_env, D_model)\n        update_SAC(pi, Q, batch)\n</code></pre>\n<h5>动机与背景</h5>\n<p>基于模型的强化学习有一个诱人的优势：一旦学到环境动力学，模型内部生成 transition 的成本远低于真实交互。但神经动力学模型不可避免有误差，长 rollout 会让误差逐步累积，最终把策略训练到真实环境中不存在或不可靠的状态上。</p>\n<p>MBPO 的核心判断是：模型最可信的是局部一步或短步转移，而不是长程预测。因此它不让模型承担完整规划任务，而是从真实 replay buffer 中的状态出发，生成短 horizon 的合成样本。真实状态作为起点限制了分布偏移，短 horizon 限制了复合误差。</p>\n<p>动力学模型通常预测状态差分和奖励：</p>\n<div class=\"kb-math kb-math-display\">p_\\theta(s_{t+1}, r_t \\mid s_t, a_t)</div>\n<p>并使用负对数似然训练。采用 ensemble 后，每个模型 <span class=\"kb-math kb-math-inline\">p_{\\theta_i}</span> 在 bootstrap 数据或不同初始化下学习，生成 rollout 时随机选择一个模型。这样既能提升鲁棒性，也能在实践中减少单个模型错误被策略利用的风险。</p>\n<h5>偏差-方差权衡</h5>\n<p>MBPO 的理论分析可简化理解为：模型 rollout 越长，能生成的数据越多、策略更新越接近 on-policy，但模型偏差项会按 horizon 累积；rollout 越短，模型偏差小，但补充样本有限。短 rollout 是二者之间的实用平衡。</p>\n<p>策略训练仍使用 SAC 的最大熵目标：</p>\n<div class=\"kb-math kb-math-display\">J(\\pi)=\\mathbb{E}\\left[\\sum_t r_t + \\alpha \\mathcal{H}(\\pi(\\cdot|s_t))\\right]</div>\n<p>不同的是，SAC 的 replay batch 现在可同时包含真实 transition 和模型 transition。模型样本扩大了数据量，使 Q 函数和策略能更频繁更新；真实样本持续校正模型和策略，防止偏差失控。</p>\n<h5>与传统模型式方法的区别</h5>\n<p>早期 Dyna 方法同样混合真实与模型样本，但在高维连续控制中，模型误差和策略分布偏移会更严重。PETS 等方法强调模型预测控制（MPC），每一步在模型中规划动作；MBPO 则保持一个 amortized policy，用模型主要提升 off-policy actor-critic 的样本效率。</p>\n<p>与纯无模型 SAC 相比，MBPO 增加了模型训练成本，但显著减少真实环境步数。与长 horizon 模型规划相比，它牺牲了一部分模型利用率，换来更稳的策略优化。这也是 MBPO 影响后续 Dreamer 系列的关键思想：世界模型很有用，但必须限制和管理模型误差进入策略学习的方式。</p>\n<div class=\"key-point\">💡 关键：MBPO 的“模型”不是为了完全替代环境，而是为了在可信的短局部范围内制造额外训练样本。</div>",
      "quiz": {
        "q": "MBPO 为什么偏好从真实 replay buffer 状态启动短步长模型 rollout？",
        "options": [
    ...
    },
    {
      ...
      "summary": "DreamerV1 提出在学习到的 RSSM 隐空间中“想象”长程轨迹，并通过价值梯度训练 actor-critic，解决像素控制中直接在图像空间规划成本高、误差大的问题。它把世界模型学习、隐空间 rollout 和策略优化分离成稳定的三阶段循环，在 DeepMind Control Suite 视觉任务上取得高样本效率。",
      "keyPoints": [
        "<strong>RSSM 世界模型</strong>：由确定性循环状态 <span class=\"kb-math kb-math-inline\">h_t</span> 和随机状态 <span class=\"kb-math kb-math-inline\">z_t</span> 组成，兼顾历史记忆与不确定性",
        "<strong>像素到隐空间</strong>：encoder 把图像观测压缩到 latent state，decoder/reward head 学习重构观测和奖励",
        "<strong>Latent Imagination</strong>：actor 不在真实环境或像素空间中 rollout，而是在 RSSM dynamics 的隐空间中展开轨迹",
        "<strong>Analytic Value Gradient</strong>：通过可微分的想象轨迹，把价值估计梯度反传到 actor",
        "<strong>Actor-Critic in Model</strong>：critic 学习 imagined trajectory 的 <span class=\"kb-math kb-math-inline\">\\lambda</span>-return，actor 最大化想象回报",
        "<strong>Experience Replay</strong>：真实环境交互只用于训练世界模型，策略可在模型内进行大量更新",
        "<strong>视觉连续控制验证</strong>：在 20 个 DeepMind Control Suite image-based tasks 上超过 PlaNet、A3C、D4PG 等基线"
      ],
      "detail": "<h5>Dreamer 总体架构</h5>\n<p><img alt=\"DreamerV1 隐空间想象训练\" src=\"https://ar5iv.labs.arxiv.org/html/1912.01603/assets/x1.png\" /></p>\n<p><em>图：Dreamer 先从真实经验学习紧凑世界模型，再在 latent dynamics 中想象未来轨迹，并通过 critic 的价值梯度训练 actor。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># DreamerV1 training loop\ninitialize RSSM world model p_phi\ninitialize actor pi_theta and value model v_psi\ninitialize replay buffer D\n\nfor each environment step:\n    h_t, z_t = infer_state(world_model, history)\n    a_t = pi_theta.sample(h_t, z_t)\n    o_next, r, done = env.step(a_t)\n    D.add(o_t, a_t, r, done)\n\n    # 1. World model learning from real sequences\n    batch = D.sample_sequences()\n    states = RSSM.observe(batch.obs, batch.actions)\n    L_model = reconstruction_loss(batch.obs)\n            + reward_prediction_loss(batch.rewards)\n            + KL(q(z_t | h_t, o_t) || p(z_t | h_t))\n    update(world_model, L_model)\n\n    # 2. Latent imagination\n    start_states = detach(states)\n    imagined = []\n    s = start_states\n    for tau in range(H):\n        a = pi_theta.sample(s)\n        s = RSSM.imagine_step(s, a)\n        r_hat = reward_head(s)\n        imagined.append(s, a, r_hat)\n\n    # 3. Actor-Critic learning in imagination\n    returns = lambda_returns(imagined.rewards, v_psi(imagined.states))\n    update(v_psi, mse(v_psi(imagined.states), stop_gradient(returns)))\n    update(pi_theta, -mean(returns))  # gradients flow through imagined dynamics\n</code></pre>\n<h5>动机与背景</h5>\n<p>PlaNet 已经证明可以学习 latent dynamics 并用 MPC 在模型中规划，但每一步都进行在线规划计算昂贵，且规划 horizon、候选动作数量等超参数影响很大。纯无模型方法虽然推理快，却需要大量真实交互。DreamerV1 的目标是在二者之间取平衡：用世界模型提供样本效率，用 actor 网络摊销规划结果。</p>\n<p>Dreamer 的世界模型是 Recurrent State-Space Model。模型状态 <span class=\"kb-math kb-math-inline\">s_t=(h_t,z_t)</span>，其中 <span class=\"kb-math kb-math-inline\">h_t</span> 是确定性 RNN 隐状态，<span class=\"kb-math kb-math-inline\">z_t</span> 是随机 latent：</p>\n<div class=\"kb-math kb-math-display\">h_t=f_\\phi(h_{t-1}, z_{t-1}, a_{t-1})</div>\n<div class=\"kb-math kb-math-display\">z_t \\sim q_\\phi(z_t \\mid h_t, o_t), \\qquad \\hat z_t \\sim p_\\phi(\\hat z_t \\mid h_t)</div>\n<p>训练时，后验 <span class=\"kb-math kb-math-inline\">q_\\phi</span> 看见当前观测，先验 <span class=\"kb-math kb-math-inline\">p_\\phi</span> 只根据历史预测。KL 项让先验学会在没有未来观测时也能产生合理 latent，这正是想象 rollout 所需的动力学能力。</p>\n<h5>隐空间想象与价值梯度</h5>\n<p>世界模型训练好后，Dreamer 从真实序列的 posterior state 出发，在模型内递推：</p>\n<div class=\"kb-math kb-math-display\">s_{t+1} \\sim p_\\phi(s_{t+1}\\mid s_t,a_t), \\qquad a_t \\sim \\pi_\\theta(a_t\\mid s_t)</div>\n<p>奖励由 reward head 预测，critic 给出 bootstrap value。想象轨迹上的 <span class=\"kb-math kb-math-inline\">\\lambda</span>-return 为：</p>\n<div class=\"kb-math kb-math-display\">V_t^\\lambda = \\hat r_t + \\gamma\\big((1-\\lambda)v_\\psi(s_{t+1})+\\lambda V_{t+1}^\\lambda\\big)</div>\n<p>critic 拟合 <span class=\"kb-math kb-math-inline\">V_t^\\lambda</span>，actor 最大化这些 imagined returns。与 REINFORCE 式采样梯度不同，Dreamer 的模型和 reward head 可微，因此 actor 可以接收穿过 dynamics 的 analytic gradient，更有效地学习长程行为。</p>\n<h5>与 MBPO 的区别</h5>\n<p>MBPO 在真实状态附近用短模型 rollout 生成显式 transition，再交给 SAC；Dreamer 则把整个策略学习搬到 latent space，避免生成像素级未来图像作为训练数据。Dreamer 的 rollout horizon 可以比 MBPO 更长，因为它在紧凑 latent 中预测，并通过 value bootstrap 降低远期误差影响。</p>\n<p>不过 DreamerV1 仍依赖连续 latent 的表达能力。后续 DreamerV2 发现，在 Atari 等离散、多模态视觉环境中，离散 latent 更适合表达突变事件和多峰未来，这成为 V2 的主要改进方向。</p>\n<div class=\"key-point\">💡 关键：DreamerV1 的“梦”不是生成漂亮图像，而是在可微隐空间中生成足够准确的奖励和价值轨迹，让 actor 能从想象未来中学习。</div>",
      "quiz": {
        "q": "DreamerV1 为什么在隐空间而不是像素空间中训练 actor？",
        "options": [
    ...
    },
    {
      ...
      "summary": "DreamerV2 将 Dreamer 的连续随机 latent 改为多组离散 categorical latent，并配合 KL balancing、straight-through estimator 和 latent imagination actor-critic，解决 Atari 这类离散视觉环境中世界模型表达能力不足的问题。它首次让一个单 GPU 世界模型智能体在 55 个 Atari 游戏上达到人类级水平，并仍然完全在学习到的隐空间中训练行为。",
      "keyPoints": [
        "<strong>离散 RSSM latent</strong>：随机状态由多组 categorical 变量组成，比高斯 latent 更适合表示 Atari 中物体、事件和离散模式",
        "<strong>Straight-through estimator</strong>：前向采样 one-hot 离散变量，反向用连续概率近似传递梯度",
        "<strong>KL balancing</strong>：把 representation learning 和 dynamics learning 的梯度权重分开，避免先验或后验一方主导",
        "<strong>Actor-Critic 想象训练</strong>：策略和价值函数仍只在世界模型 latent rollout 中训练，不依赖真实环境中的策略梯度",
        "<strong>No image reconstruction for acting</strong>：行为学习使用 latent state、reward 和 value，不需要在想象过程中生成像素",
        "<strong>Atari 55 任务验证</strong>：在 200M frames 设置下超过 Rainbow 和 IQN 等强单 GPU agent",
        "<strong>连续控制兼容</strong>：同一思想也可用于 humanoid stand/walk 等连续动作视觉控制任务"
      ],
      "detail": "<h5>DreamerV2 方法示意图</h5>\n<p><img alt=\"DreamerV2 离散世界模型\" src=\"https://ar5iv.labs.arxiv.org/html/2010.02193/assets/x1.png\" /></p>\n<p><em>图：DreamerV2 延续“世界模型学习 + latent imagination + actor-critic”的框架，但将世界模型中的随机表示替换为离散 latent，以提高对 Atari 环境的建模能力。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># DreamerV2 training loop\ninitialize discrete RSSM world model\ninitialize actor pi_theta and critic v_psi\ninitialize replay buffer D\n\nfor step in range(training_steps):\n    # collect real experience\n    s_t = RSSM.infer(o_t, history)\n    a_t = pi_theta.sample(s_t)\n    o_next, r, done = env.step(a_t)\n    D.add(o_t, a_t, r, done)\n\n    # world model update\n    batch = D.sample_sequences()\n    posterior = RSSM.observe(batch.obs, batch.actions)   # q(z_t | h_t, o_t)\n    prior = RSSM.imagine_prior(batch.actions)            # p(z_t | h_t)\n    z = straight_through_sample(posterior.categorical_probs)\n\n    L_pred = -log p(o_t | h_t, z_t) - log p(r_t | h_t, z_t) - log p(done_t | h_t, z_t)\n    L_kl = kl_balance(KL(q || p), alpha)\n    update(world_model, L_pred + beta * L_kl)\n\n    # behavior learning in imagination\n    start = detach(posterior.states)\n    imagined = rollout_discrete_RSSM(start, pi_theta, horizon=H)\n    returns = lambda_returns(imagined.rewards, v_psi(imagined.states))\n    update critic to predict stop_gradient(returns)\n    update actor to maximize imagined returns with entropy regularization\n</code></pre>\n<h5>动机与背景</h5>\n<p>DreamerV1 在连续控制图像任务上表现强，但 Atari 带来不同挑战：屏幕中对象和事件是离散的，奖励可能由突然事件触发，未来分布常是多模态的。用连续高斯 latent 表示这些结构时，模型容易把多个可能状态平均到一起，导致奖励预测和长程想象不够稳定。</p>\n<p>DreamerV2 的主要变化是离散随机状态。典型表示可以理解为 <span class=\"kb-math kb-math-inline\">N</span> 组 categorical 变量，每组有 <span class=\"kb-math kb-math-inline\">K</span> 个类别：</p>\n<div class=\"kb-math kb-math-display\">z_t = \\{z_t^{(1)},\\ldots,z_t^{(N)}\\}, \\qquad z_t^{(i)} \\in \\{1,\\ldots,K\\}</div>\n<p>世界模型仍由 deterministic hidden state <span class=\"kb-math kb-math-inline\">h_t</span> 和 stochastic state <span class=\"kb-math kb-math-inline\">z_t</span> 构成。后验 <span class=\"kb-math kb-math-inline\">q_\\phi(z_t|h_t,o_t)</span> 看见观测，先验 <span class=\"kb-math kb-math-inline\">p_\\phi(z_t|h_t)</span> 只从历史预测。离散 latent 让模型能组合出大量离散状态码，更自然地表达“钥匙是否出现”“敌人在哪个格子”“子弹是否发射”等事件。</p>\n<h5>KL Balancing 与离散梯度</h5>\n<p>世界模型训练包含预测损失和 KL 正则：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{model} =\n\\mathcal{L}_{image}+\\mathcal{L}_{reward}+\\mathcal{L}_{discount}\n\\beta\\,D_{KL}\\big(q_\\phi(z_t|h_t,o_t)\\,\\|\\,p_\\phi(z_t|h_t)\\big)</div>\n<p>若直接优化 KL，模型可能出现两类问题：后验为了重构图像携带太多信息，导致先验跟不上；或先验压力过强，导致后验不愿编码细节。KL balancing 通过 stop-gradient 把表示学习和动力学学习分开加权，使后验和先验以更稳定的节奏互相靠近。</p>\n<p>离散变量不可直接反传采样梯度，DreamerV2 使用 straight-through estimator：前向用 one-hot 样本参与模型计算，反向把梯度近似传给 softmax 概率。这让模型保留离散表示的组合能力，同时仍能端到端训练。</p>\n<h5>想象训练与 Atari 适配</h5>\n<p>行为学习阶段与 DreamerV1 一样，不在真实环境中做 on-policy 策略梯度，而是在 RSSM prior 中 rollout。actor 选择动作，RSSM 预测下一个 latent，reward head 给出奖励，critic 估计 bootstrap value。Actor 最大化 imagined <span class=\"kb-math kb-math-inline\">\\lambda</span>-return：</p>\n<div class=\"kb-math kb-math-display\">V_t^\\lambda = \\hat r_t + \\gamma \\hat c_t\\left((1-\\lambda)v_\\psi(s_{t+1})+\\lambda V_{t+1}^\\lambda\\right)</div>\n<p>Atari 的动作是离散的，DreamerV2 的 actor 输出 categorical action distribution，并使用熵正则保持探索。重要的是，策略学习完全依赖世界模型的 latent 预测，因此模型若不能表示关键离散事件，actor 就会学到错误行为；这正是离散 latent 带来提升的原因。</p>\n<p>与 MBPO 相比，DreamerV2 不把模型样本写回 replay buffer 给 SAC，而是直接在模型内部训练 actor-critic。与 DreamerV1 相比，它保持整体训练范式不变，但把世界模型的表征从连续改为离散，并加入一组稳定训练技巧，使方法能扩展到 Atari 这样更复杂的离散视觉任务。</p>\n<div class=\"key-point\">💡 关键：DreamerV2 的突破不是更长的规划，而是让世界模型的 latent 代码更像环境中的离散事实，从而让想象轨迹对策略训练足够可信。</div>",
      "quiz": {
        "q": "DreamerV2 相比 DreamerV1 的核心表征改动是什么？",
        "options": [
    ...
    },
    {
      ...
      "summary": "RWML 提出用强化学习训练 LLM agent 的动作条件世界模型，让模型根据交互历史和候选动作预测真实下一状态，并用 embedding 空间中的 sim-to-real gap 奖励优化，解决 SFT 预测下一状态过度追求字面复现而忽略语义等价的问题。该方法完全自监督，可在没有专家轨迹、强模型标注或任务成功奖励的情况下提升 ALFWorld 与 \\(\\tau^2\\) Bench 中的代理表现。",
      "keyPoints": [
        "<strong>动作条件世界模型</strong>：LLM 输入历史 <span class=\"kb-math kb-math-inline\">h_t</span> 和动作 <span class=\"kb-math kb-math-inline\">a_t</span>，先生成 reasoning tokens，再预测 simulated next state <span class=\"kb-math kb-math-inline\">\\hat s_{t+1}</span>",
        "<strong>Sim-to-real gap reward</strong>：比较 <span class=\"kb-math kb-math-inline\">\\hat s_{t+1}</span> 与真实环境返回 <span class=\"kb-math kb-math-inline\">s_{t+1}</span> 的 embedding 距离，而非要求 token 完全一致",
        "<strong>二值奖励更稳健</strong>：论文使用阈值化的相似度奖励，降低连续 reward 被 hacking 的风险",
        "<strong>GRPO 优化</strong>：用 group-relative policy optimization 最大化世界模型奖励，并加入 KL 正则约束",
        "<strong>自采样数据</strong>：目标模型自己与环境交互收集 rollout，再转换成 <span class=\"kb-math kb-math-inline\">(h_t,a_t,s_{t+1})</span> 训练三元组",
        "<strong>难例子采样</strong>：先用少量数据训练 next-state predictor，过滤过于容易的样本，让 RL 更关注非平凡世界知识",
        "<strong>组合训练收益</strong>：RWML 单独提升 base agent；与 task-success reward RL 结合时超过直接 policy RL",
        "<strong>评测环境</strong>：ALFWorld 文本具身任务和 <span class=\"kb-math kb-math-inline\">\\tau^2</span> Bench 工具调用任务，均需要预测动作后果和环境动态"
      ],
      "detail": "<h5>RWML 流程示意图</h5>\n<p><img alt=\"RWML 训练流程\" src=\"https://arxiv.org/html/2602.05842v2/x1.png\" /></p>\n<p><em>图：RWML 先由目标模型在环境中收集交互，把轨迹转成动作条件下一状态预测样本，再用 embedding 相似度构成奖励，通过 GRPO 训练模型的世界模拟能力。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># Reinforcement World Model Learning for LLM agents\ninput: base LLM policy pi_theta, environments E\n\n# 1. Collect self-supervised transition data\nD = []\nfor task in training_tasks:\n    for rollout_id in range(M):\n        history = env.reset(task)\n        while not done:\n            action = pi_theta.generate_action(history)\n            next_state = env.step(action)\n            D.append((history, action, next_state))\n            history = history + [action, next_state]\n\n# 2. Subsample hard world-model examples\nwm_sft = train_next_state_predictor(D[:10%])\nD_hard = []\nfor sample in D[10%:]:\n    rewards = evaluate_multiple_predictions(wm_sft, sample)\n    if mean(rewards) &lt; easy_threshold or random() &lt; keep_easy_prob:\n        D_hard.append(sample)\n\n# 3. RL train world model with GRPO\nfor batch in sample_batches(D_hard):\n    completions = pi_theta.generate_world_predictions(batch.history, batch.action, n=G)\n    rewards = []\n    for y_hat, y_real in zip(completions, batch.next_state):\n        sim = cosine(embed(y_hat), embed(y_real))\n        rewards.append(1 if sim &gt;= tau else 0)\n    advantages = group_relative_advantage(rewards)\n    update pi_theta with GRPO objective + KL_to_reference\n\n# 4. Optional: continue with task-success policy RL\npi_theta = policy_RL(pi_theta, task_success_reward)\n</code></pre>\n<h5>动机与背景</h5>\n<p>LLM agent 在 ALFWorld 或工具调用环境中不仅要知道“下一步说什么”，还要预判动作会如何改变世界。例如执行 <code>open fridge</code> 后会看到哪些物品，调用某个客服工具后系统会返回什么字段。若模型不能预测动作后果，规划就会退化成短视试错。</p>\n<p>直接用 SFT 训练 next-state prediction 有一个缺陷：文本状态存在大量语义等价表述，SFT 会惩罚所有非字面匹配的输出。模型可能学会复述训练集格式，而不是学会“动作导致状态变化”的语义规律，严重时还会出现 model collapse。</p>\n<p>RWML 把世界模型学习写成 RL 目标。模型生成：</p>\n<div class=\"kb-math kb-math-display\">\\hat s_{t+1} \\sim \\pi_\\theta(\\cdot \\mid h_t, a_t)</div>\n<p>然后用 embedding 模型比较预测状态和真实状态：</p>\n<div class=\"kb-math kb-math-display\">d(\\hat s_{t+1}, s_{t+1}) = 1 - \\cos\\big(E(\\hat s_{t+1}), E(s_{t+1})\\big)</div>\n<p>奖励可阈值化为：</p>\n<div class=\"kb-math kb-math-display\">r^{WM} = \\mathbb{1}\\left[\\cos(E(\\hat s_{t+1}),E(s_{t+1})) \\ge \\tau\\right]</div>\n<p>这样，语义正确但措辞不同的预测仍可得到奖励，模型更关注状态转移是否对，而不是 token 是否逐字一致。</p>\n<h5>GRPO 世界模型训练</h5>\n<p>RWML 使用 GRPO。对同一输入采样一组候选下一状态，计算组内相对优势，再更新当前模型：</p>\n<div class=\"kb-math kb-math-display\">\\mathcal{L}_{GRPO} =\n-\\mathbb{E}\\left[\n\\min\\left(\\rho_t A_t,\\operatorname{clip}(\\rho_t,1-\\epsilon,1+\\epsilon)A_t\\right)\n-\\beta D_{KL}(\\pi_\\theta \\| \\pi_{ref})\n\\right]</div>\n<p>其中优势 <span class=\"kb-math kb-math-inline\">A_t</span> 来自同组候选的世界模型奖励。KL 项防止模型为了相似度奖励而偏离原始语言能力。论文还发现二值奖励比连续相似度更不容易被 hacking，因为模型不能通过投机地生成 embedding 友好但人类不可读的文本来获得细粒度奖励。</p>\n<h5>与传统世界模型的区别</h5>\n<p>Dreamer/MBPO 学习的是连续或离散环境的状态转移模型，训练信号通常是图像、奖励或 latent 重构误差；RWML 学习的是文本状态空间中的世界模型，核心挑战从像素预测变成语义状态等价。它不直接训练动作策略，而是先提高 LLM 对“动作后果”的内部模拟能力，再与 task-success policy RL 组合。</p>\n<p>这种方法尤其适合长期任务：单步任务成功奖励太稀疏，专家轨迹又昂贵；但环境交互自然提供了大量 <span class=\"kb-math kb-math-inline\">(history, action, next_state)</span> 三元组。RWML 利用这些自监督转移，把 agent 后训练的一部分目标从“完成任务”拆成“先学会世界如何变化”。</p>\n<div class=\"key-point\">💡 关键：RWML 的奖励不是任务成功，而是“我模拟的下一状态是否语义上接近真实下一状态”。这让世界模型学习成为可规模化的自监督 RL 问题。</div>",
      "quiz": {
        "q": "RWML 为什么不用普通 SFT 的 token-level next-state prediction 作为主要训练目标？",
        "options": [
          "因为环境没有真实下一状态",
    ...
    },
    {
      ...
      "summary": "HY-Embodied-0.5 提出面向真实具身智能体的视觉语言基础模型族，通过原生分辨率视觉编码器、Mixture-of-Transformers、视觉 latent tokens、大规模具身数据和迭代后训练，解决通用 VLM 在空间感知、时间理解和具身推理上不足的问题。其 MoT-2B 变体面向边缘部署，进一步扩展成 VLA 后在真实双臂机器人任务中展示了可迁移控制能力。",
      "keyPoints": [
        "<strong>HY-ViT 2.0</strong>：轻量原生分辨率视觉编码器，支持任意比例输入，并通过蒸馏和重构监督保留细粒度视觉信息",
        "<strong>Mixture-of-Transformers</strong>：为视觉 token 和文本 token 使用部分非共享 QKV/FFN 与不同 attention 机制，提高视觉建模能力且减少语言能力退化",
        "<strong>视觉 latent tokens</strong>：在视觉序列末尾追加专用 latent tokens，桥接视觉 full attention 和语言 causal attention",
        "<strong>100M+ 训练样本</strong>：数据覆盖基础感知、空间感知、具身感知、推理与规划，并包含真实机器人数据和高质量 reasoning 数据",
        "<strong>迭代后训练</strong>：冷启动数据、rejection sampling SFT 和 RL 交替提升 thinking 能力",
        "<strong>大到小在线蒸馏</strong>：把大模型能力迁移到小模型，提升 2B 边缘变体的具身推理表现",
        "<strong>22 项评测</strong>：覆盖视觉感知、空间推理和具身理解，MoT-2B 在同规模模型中表现领先，MoE-A32B 对比前沿 VLM 也有竞争力",
        "<strong>真实机器人 VLA</strong>：在 MoT-2B 上扩展 Action Expert，先用 5K 小时 UMI 数据微调，再用 300-700 条真实演示适配具体任务"
      ],
      "detail": "<h5>MoT 架构示意图</h5>\n<p><img alt=\"HY-Embodied-0.5 MoT 架构\" src=\"https://arxiv.org/html/2604.07430v1/x2.png\" /></p>\n<p><em>图：HY-Embodied-0.5 的 Mixture-of-Transformers 为视觉和文本 token 分配不同计算路径，并通过视觉 latent tokens 强化跨模态连接。</em></p>\n<h5>算法伪代码</h5>\n<pre><code class=\"language-python\"># HY-Embodied-0.5 训练与部署流程抽象\n\n# 1. Perception-centric pretraining\ntrain HY_ViT_2_0 with native-resolution images\ndistill visual encoder from stronger internal vision model\nadd reconstruction/discrete-code supervision to preserve visual detail\n\n# 2. Multimodal foundation model training\ninitialize LLM backbone\ninsert Mixture-of-Transformers layers:\n    text tokens -&gt; original QKV/FFN + causal attention\n    visual tokens -&gt; vision-specific QKV/FFN + visual/full attention\nappend visual latent tokens after visual sequence\ntrain on 100M+ perception, spatial, embodied, reasoning/planning samples\n\n# 3. Iterative embodied post-training\nfor round in iterative_training:\n    train on cold-start reasoning data\n    sample candidate responses / reasoning traces\n    apply rejection sampling SFT\n    apply RL to improve thinking and embodied reasoning\n\n# 4. Large-to-small on-policy distillation\nteacher = HY_Embodied_large\nstudent = HY_Embodied_MoT_2B\ncollect on-policy teacher/student trajectories\ndistill reasoning and embodied responses into student\n\n# 5. VLA robot control adaptation\nextend MoT-2B with Action Expert\nfinetune on 5K hours UMI data\nSFT on 300-700 real-robot demonstrations per task\ndeploy on dual-arm robot with head/wrist cameras\n</code></pre>\n<h5>动机与背景</h5>\n<p>通用 VLM 通常擅长图文问答和常识推理，但真实具身任务要求更细的能力：识别物体可操作部位、理解三维空间关系、跟踪时间变化、预测动作后果，并最终服务于机器人控制。单纯扩大通用 VLM 规模并不一定提升这些能力，因为训练数据和架构计算路径没有专门面向具身视觉。</p>\n<p>HY-Embodied-0.5 的第一层改进是视觉侧。HY-ViT 2.0 面向原生分辨率输入，避免强行缩放造成小物体、距离和局部接触区域丢失。论文还训练更大视觉模型产生离散视觉表示，用于监督视觉 token，从而在接入语言模型时保留更多可 grounding 的细节。</p>\n<h5>Mixture-of-Transformers 与视觉 latent tokens</h5>\n<p>标准 VLM 常把视觉 token 投影到 LLM embedding 后与文本 token 混合处理，视觉训练过重时可能损伤语言能力。HY-Embodied-0.5 使用 MoT：复制或分离部分 QKV/FFN 参数，让视觉 token 走更适合视觉建模的路径，文本 token 保持原语言路径。</p>\n<p>可抽象为：</p>\n<div class=\"kb-math kb-math-display\">y_i =\n\\begin{cases}\n\\operatorname{Block}_{vision}(x_i), &amp; x_i \\in \\mathcal{V} \\\\\n\\operatorname{Block}_{text}(x_i), &amp; x_i \\in \\mathcal{T}\n\\end{cases}</div>\n<p>视觉 latent tokens 则类似跨模态寄存器：它们从视觉 full attention 中聚合关键空间区域，再被语言 causal attention 读取。论文的注意力可视化显示，这些 token 会关注物体部位、空间关系和动作相关区域，因此能把“看见什么”更稳定地转成“如何推理/操作”。</p>\n<h5>后训练与蒸馏</h5>\n<p>HY-Embodied-0.5 的后训练不是一次性 SFT。它使用少量冷启动数据建立推理格式，再通过 iterative RL 和 rejection sampling SFT 提升 thinking 能力。大模型探索出的高质量推理和答案，再通过 large-to-small on-policy distillation 迁移给 MoT-2B，使小模型在边缘部署预算下仍保留较强具身能力。</p>\n<p>这个流程与 Dreamer/RWML 的“世界模型”概念不同：HY-Embodied-0.5 不是学习一个显式环境动力学模型再在其中 rollout，而是把空间、时间、计划和动作相关知识压进 VLM/VLA 基础模型。它更像具身 agent 的感知-推理底座，可被下游控制头或 Action Expert 接入。</p>\n<h5>机器人控制部署</h5>\n<p>论文在 MoT-2B 基础上扩展 Action Expert 得到 VLA 模型。训练先用 5K 小时 UMI 数据进行通用操作微调，再用每个真实任务 300-700 条演示做 SFT。评测在双臂 Xtrainer、头部和腕部相机设置下进行，任务包括 Precision Plug-in Packing、Tableware Stacking 和 Mug Hanging。</p>\n<p>这种部署流程体现了 HY-Embodied-0.5 的定位：大规模视觉语言预训练提供空间和任务理解，具身后训练提升推理与规划，少量真实机器人数据完成 embodiment-specific 适配。与纯 RL 世界模型相比，它更依赖监督/后训练与蒸馏；与通用 VLM 相比，它在架构和数据上明确为物理世界 grounding 优化。</p>\n<div class=\"key-point\">💡 关键：HY-Embodied-0.5 的核心贡献是把 VLM 改造成具身基础模型，而不是只在通用 VLM 外面接一个机器人控制头。</div>",
      "quiz": {
